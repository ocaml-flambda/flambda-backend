...
let fatal_if_not_irc_debug () =
  if not irc_debug then
    fatal "IRC_VERBOSE is set but debugging mode is disabled"

let irc_verbose : bool =
  match Sys.getenv_opt "IRC_VERBOSE" with
  | Some "1" -> fatal_if_not_irc_debug (); true
  | Some _ | None -> false
...
let log_body_and_terminator
  : indent:int -> Cfg.basic Cfg.instruction list -> Cfg.terminator Cfg.instruction -> unit
  = fun ~indent body term ->
    if irc_debug && irc_verbose then begin
...
module RegisterStamp = struct
  ...
  module PS = Hashtbl.Make (struct
      ...
      let equal (left : t) (right : t) : bool =
        Int.equal (fst left) (fst right)
        && Int.equal (snd left) (snd right)
  ...
  module PairSet = struct
    ...
    (* CR xclerc for xclec:  the caller is likely to call mem and add,
       so build the pair there *)
...
module Split_mode = struct
  ...
  let env =
    ...
    in
    lazy begin match Sys.getenv_opt "IRC_SPLIT" with
      | None ->
        fatal "the IRC_SPLIT environment variable is not set (possible values: %s)"
          (available_modes ())
      | Some id ->
        ...
        | "naive" -> Naive
        | _ ->
          fatal "unknown split mode %S (possible values: %s)"
            id (available_modes ())
    end
    ...
    let available_heuristics () =
      String.concat ", "
        (all
         ...
         |> List.map ~f:(Printf.sprintf "%S"))
    in
    lazy begin match Sys.getenv_opt "IRC_SPILLING_HEURISTICS" with
    | None ->
      fatal "the IRC_SPILLING_HEURISTICS environment variable is not set (possible values: %s)"
        (available_heuristics ())
    | Some id ->
      match String.lowercase_ascii id with
      | "set_choose" | "set-choose" -> Set_choose
      | "flat_uses" | "flat-uses" -> Flat_uses
      (* CR xclerc for xclerc: | "hierarchical_uses" | "hierarchical-uses" -> Hierarchical_uses *)
      | _ ->
        fatal "unknown heuristics %S (possible values: %s)"
          id (available_heuristics ())
    end
