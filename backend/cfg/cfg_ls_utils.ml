...
let iter_cfg_order :
 ...
 fun cfg_with_layout order ~f ->
  ...
  match order with
  | Layout ->
    Flambda_backend_utils.Doubly_linked_list.iter (Cfg_with_layout.layout cfg_with_layout) ~f:(fun label ->
        let block = Cfg.get_block_exn cfg label in
        f block)
...
let iter_instructions_order :
 ...
 fun cfg_with_layout order ~instruction ~terminator ->
  iter_cfg_order cfg_with_layout order ~f:(fun block ->
      ...
      in
      Flambda_backend_utils.Doubly_linked_list.iter block.body ~f:(fun instr ->
          instruction
