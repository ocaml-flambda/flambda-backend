...
let[@inline] apply_regs
  : Reg.t Reg.Tbl.t -> Reg.t array -> Reg.t array
  = fun subst arr ->
...
let[@inline] apply_instr
  : Reg.t Reg.Tbl.t -> Instruction.t -> unit
  = fun subst instr ->
...
let rec naive_split_body
  ...
  = fun state liveness subst acc body split_points new_regs ->
    ...
    | hd_body :: tl_body, hd_split_points :: tl_split_points ->
      if hd_body.Cfg.id = hd_split_points then begin
        let live = Cfg_dataflow.Instr.Tbl.find liveness hd_body.Cfg.id in
        if irc_debug then begin
...
let naive_split_cfg
  : State.t -> Cfg_with_layout.t -> Instruction.id list -> liveness -> Reg.t list
    ...
    Cfg.fold_blocks
      ...
      ~f:(fun label block ((split_points, new_regs) as acc) ->
          ...
          | hd :: _ ->
            ...
            if List.exists block.body ~f:(fun instr -> instr.Cfg.id = hd) then begin
              let { body; split_points; new_regs; } =
                naive_split_body state liveness subst [] block.body split_points new_regs
              in
              block.body <- body;
