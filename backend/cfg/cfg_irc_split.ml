...
let is_naive_split_point : Instruction.t -> bool =
  fun instr ->
    (* CR xclerc for xclerc: we may want to split [heuristically] in more situations. *)
...
let[@inline] naive_split_instr :
  ...
  fun state liveness new_regs subst cell  ->
    ...
    if is_naive_split_point instr then begin
