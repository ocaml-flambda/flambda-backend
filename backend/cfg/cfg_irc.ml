...
let build
  : State.t -> Cfg_with_layout.t -> liveness -> unit
  = fun state cfg_with_layout liveness ->
    ...
    let add_edges_live
        ...
        ~(def : Reg.t array)
        ~(move_src : Reg.t)
        ~(destroyed : Reg.t array) : unit =
      let live = Cfg_dataflow.Instr.Tbl.find liveness id in
      if Array.length def > 0 then begin
        Reg.Set.iter
          (fun reg1 ->
             if reg1 != move_src then begin
               Array.iter def ~f:(fun reg2 ->
                   State.add_edge state reg1 reg2)
             end)
          live.before;
      end;
      if Array.length destroyed > 0 then begin
        Reg.Set.iter
          (fun reg1 ->
             Array.iter destroyed ~f:(fun reg2 ->
                 State.add_edge state reg1 reg2))
          live.across;
      end;
    in
    iter_instructions
      ...
      ~instruction:(fun (instr : Instruction.t) ->
          ...
          && same_reg_class instr.arg.(0) instr.res.(0) then begin
            State.add_move_list state instr.arg.(0) instr;
            if not (same_reg instr.arg.(0) instr.res.(0)) then
              State.add_move_list state instr.res.(0) instr;
            State.add_work_list_moves state instr;
            let move_src =
              if same_reg instr.arg.(0) instr.res.(0) then
                Reg.dummy
              else
                instr.arg.(0)
            in
    ...
    Cfg.iter_blocks (Cfg_with_layout.cfg cfg_with_layout) ~f:(fun _label block ->
        if block.is_trap_handler then begin
          let first_id =
    ...
    Cfg_regalloc_utils.Stats.update_cfg_with_layout
      cfg_with_layout
      Cfg_regalloc_utils.Stats.build
      (after -. before)
  ...
  = fun state ->
    ...
    List.iter initial ~f:(fun reg ->
        ...
        if irc_debug then begin
          log ~indent:2 "- %a has degree=%s (k=%d)"
            Printmach.reg reg
            (Degree.to_string deg)
            (k reg)
        end;
        if deg >= k reg then begin
          if irc_debug then log ~indent:3 "~> spill_work_list";
          State.add_spill_work_list state reg
        end else if State.is_move_related state reg then begin
...
let simplify
  ...
  = fun state ->
    ...
    State.iter_adjacent state reg ~f:(fun adj ->
        State.decr_degree state adj)

let ok
  : State.t -> Reg.t -> Reg.t -> bool
  = fun state t r ->
...
let all_adjacent_are_ok
  : State.t -> Reg.t -> Reg.t -> bool
  = fun state u v ->
    State.for_all_adjacent state v ~f:(fun t ->
        ok state t u)
...
let conservative
  ...
  = fun state reg1 reg2 ->
    ...
    let f (reg : Reg.t) : unit=
      if not (Reg.Tbl.mem seen reg) then begin
        Reg.Tbl.replace seen reg ();
        let k = k reg in
        if reg.Reg.degree >= k then begin
          incr i;
...
let combine
  : State.t -> Reg.t -> Reg.t -> unit
  = fun state u v ->
    if irc_debug then log ~indent:2 "combine u=%a v=%a" Printmach.reg u Printmach.reg v;
    if State.mem_freeze_work_list state v then begin
      State.remove_freeze_work_list state v
    end else begin
      State.remove_spill_work_list state v
    end;
    State.add_coalesced_nodes state v;
    State.add_alias state v u;
    (* note: See book errata (https://www.cs.princeton.edu/~appel/modern/ml/errata98.html) *)
    ...
    if (State.mem_freeze_work_list state u) && (u.Reg.degree >= k u) then begin
...
let add_work_list
  : State.t -> Reg.t -> unit
  = fun state reg ->
    (* note: the test that `reg` is not precolored is redundant since
       precolored registers have an infinite degree. *)
    if (reg.Reg.degree < k reg) && not (State.is_move_related state reg) then begin
...
let freeze_moves
  ...
  = fun state u ->
    ...
    State.iter_node_moves state u ~f:(fun m ->
        ...
        let v =
          if same_reg alias_y (State.find_alias state u) then
            State.find_alias state x
          else
            alias_y
        in
        State.remove_active_moves state m;
        State.add_frozen_moves state m;
        if v.Reg.degree < (k v) && State.is_empty_node_moves state v then begin
          State.remove_freeze_work_list state v;
...
let select_spilling_register_using_heuristics
  ...
  = fun state ->
    ...
    | Set_choose ->
      (* This is the "heuristics" from the IRC paper: pick any candidate,
         just try to avoid any of the temporaries introduces for spilling. *)
      ...
      begin match Reg.Set.choose_opt (Reg.Set.diff spill_work_list introduced_temporaries) with
        | Some reg -> reg
        | None ->
          begin match Reg.Set.choose_opt spill_work_list with
            | Some reg -> reg
            | None -> fatal "spill_work_list is empty"
          end
      end
    | Flat_uses ->
      (* note: this assumes that `Reg.spill_cost` has been updated as needed
         (only when `rewrite` is called); the value computed here can however
         not be similarly cached because it depends on the degree (which does
         not need a call to `rewrite` to change). *)
      let weighted_cost (reg : Reg.t) =
        ((float reg.Reg.spill_cost) /. (float reg.Reg.degree))
        (* note: while this magic constant is questionable, it is key to not
           favor the introduced temporaries which, by construct, have very few
           occurrences. *)
        +. (if State.mem_introduced_temporaries state reg
            then 10_000.
            else 0.)
      in
      begin match State.spill_work_list state with
        | [] -> fatal "spill_work_list is empty"
        | [one] -> one
        | hd :: tl ->
          List.fold_left tl ~init:(hd, weighted_cost hd) ~f:(fun ((_curr_reg, curr_min_cost) as acc) reg ->
              let reg_cost = weighted_cost reg in
              if reg_cost < curr_min_cost then
                reg, reg_cost
              else
                acc)
          |> fst
...
let select_spill
  ...
  = fun state ->
    ...
    let reg =
      select_spilling_register_using_heuristics state
    in
    if irc_debug then begin
      log ~indent:2 "chose %a using heuristics %S"
        Printmach.reg reg
...
let assign_colors
  : State.t -> Cfg_with_layout.t -> unit
  = fun state cfg_with_layout ->
    ...
    State.iter_and_clear_select_stack state ~f:(fun n ->
        ...
        let reg_num_avail =
          Array.unsafe_get Proc.num_available_registers reg_class in
        let reg_first_avail =
          Array.unsafe_get Proc.first_available_register reg_class in
        let ok_colors = Array.make reg_num_avail true in
        let counter = ref reg_num_avail in
        let rec mark_adjacent_colors_and_get_first_available (adj : Reg.t list) : int =
          match adj with
          | [] ->
            let first_avail = ref 0 in
            while !first_avail < reg_num_avail && not (Array.unsafe_get ok_colors !first_avail) do
              incr first_avail
            done;
            !first_avail
          | hd :: tl ->
            let alias = State.find_alias state hd in
            if State.is_precolored_or_colored state alias then begin
              match alias.Reg.irc_color with
              | None -> assert false
              | Some color ->
                if irc_debug then log ~indent:3 "color %d is not available" color;
                if Array.unsafe_get ok_colors (color - reg_first_avail) then begin
                  Array.unsafe_set ok_colors (color - reg_first_avail) false;
                  decr counter;
                  if !counter > 0 then
                    mark_adjacent_colors_and_get_first_available tl
                  else
                    reg_num_avail
                end else
                  mark_adjacent_colors_and_get_first_available tl
        ...
        in
        let first_avail =
          mark_adjacent_colors_and_get_first_available
            (State.adj_list state n)
        in
        if first_avail = reg_num_avail then begin
    ...
    Cfg_regalloc_utils.Stats.update_cfg_with_layout
      cfg_with_layout
      Cfg_regalloc_utils.Stats.assign
      (after -. before)

let rewrite
  : State.t -> Cfg_with_layout.t -> Reg.t list -> reset:bool -> unit
  = fun state cfg_with_layout spilled_nodes ~reset ->
    if irc_debug then log ~indent:1 "rewrite";
    let spilled_map : Reg.t Reg.Tbl.t =
      List.fold_left spilled_nodes ~init:(Reg.Tbl.create 17) ~f:(fun spilled_map reg ->
          if irc_debug then begin
            assert (reg.Reg.irc_work_list = Reg.Spilled);
          end;
          let spilled = Reg.create reg.Reg.typ in
          spilled.spill <- true; (* for printing *)
    ...
    let make_new_temporary ~(move : Move.t) (reg : Reg.t) : Reg.t =
      let res = make_temporary ~same_class_and_base_name_as:reg ~name_prefix:"temp" in
      new_temporaries := res :: !new_temporaries;
      if irc_debug then begin
        log ~indent:2 "adding temporary %a (to %s %a)"
          Printmach.reg res
          (Move.to_string move)
          Printmach.reg reg
      end;
    ...
    let[@inline] array_contains_spilled (arr : Reg.t array) : bool =
      ...
      let i = ref 0 in
      while (!i < len) && ((Array.unsafe_get arr !i).Reg.irc_work_list <> Reg.Spilled) do
        incr i
      done;
      !i < len
    in
    let[@inline] instruction_contains_spilled (instr : Instruction.t) : bool =
      array_contains_spilled instr.arg
      || array_contains_spilled instr.res
    in
    let rec instruction_list_contains_spilled (l : Instruction.t list) : bool =
      match l with
      | [] -> false
      | hd :: tl ->
        instruction_contains_spilled hd
        || instruction_list_contains_spilled tl
    in
    let rewrite_instruction
        ~(direction : [`fetch | `store])
        ~(sharing : (Reg.t * [`fetch | `store]) Reg.Tbl.t)
        (acc : Instruction.t list)
        (instr : _ Cfg.instruction)
      : Instruction.t list =
      let res = ref acc in
      let f (reg : Reg.t) : Reg.t =
        if reg.Reg.irc_work_list = Reg.Spilled then begin
          let spilled =
            match Reg.Tbl.find_opt spilled_map reg with
            | None -> assert false
            | Some r -> r
          in
          let move = match direction with `fetch -> Move.Fetch | `store -> Move.Store in
          let add_instr, temp =
    ...
    let rec rewrite_body
      ...
      | [] ->
        let acc =
          rewrite_instruction ~direction:`fetch ~sharing:(Reg.Tbl.create 8) acc terminator
        in
        List.rev acc
      | hd :: tl ->
        let sharing = Reg.Tbl.create 8 in
        let acc =
          rewrite_instruction ~direction:`fetch ~sharing acc hd
        in
        let acc = hd :: acc in
        let acc =
          rewrite_instruction ~direction:`store ~sharing acc hd
        in
    ...
    Cfg.iter_blocks (Cfg_with_layout.cfg cfg_with_layout) ~f:(fun label block ->
        ...
        in
        if body_needs_rewrite then begin
          if irc_debug then begin
...
(* CR xclerc for xclerc: could probably be lower; the compiler
   distribution seems to be fine with 4 *)
let max_rounds = 50

let rec main
  : round:int -> State.t -> Cfg_with_layout.t -> liveness
  = fun ~round state cfg_with_layout ->
    ...
    let work_lists_desc state (name, f) =
      Printf.sprintf
        "%s:%s"
        name
        (if f state then "{}" else "...")
    in
    let work_lists_desc state =
      String.concat " " (List.map ~f:(work_lists_desc state) [
    ...
    let log_work_list_desc prefix =
      if irc_debug then begin
        log ~indent:1 "%s -- %s" prefix (work_lists_desc state)
      end
    in
    ...
    if irc_debug then begin
      ...
      log ~indent:1 "(%d pairs in adj_set)" (RegisterStamp.PairSet.cardinal adj_set);
    ...
    while !continue do
      if not (State.is_empty_simplify_work_list state) then begin
        simplify state;
      end else if not (State.is_empty_work_list_moves state) then begin
        coalesce state;
      end else if not (State.is_empty_freeze_work_list state) then begin
        freeze state;
      end else if not (State.is_empty_spill_work_list state) then begin
        if not !spill_cost_is_up_to_date then begin
...
let run
  ...
  = fun cfg_with_layout ->
    ...
    if irc_debug then log ~indent:0 "run (%S)" (Cfg_with_layout.cfg cfg_with_layout).fun_name;
    let before = cpu_time () in
    Reg.reinit ();
    if irc_debug && irc_invariants then begin
      log ~indent:0 "precondition";
      precondition cfg_with_layout;
    end;
    if irc_debug then begin
      Array.iteri all_precolored_regs ~f:(fun i reg ->
          log ~indent:0 "precolored[%d] = %a (class %d)"
            i
            Printmach.reg reg
            reg.Reg.clas);
    end;
    ...
    Cfg_regalloc_utils.Stats.update_cfg_with_layout
      cfg_with_layout
      Cfg_regalloc_utils.Stats.before_main
      (after -. before);
    Cfg_regalloc_utils.Stats.update_cfg_with_layout
      cfg_with_layout
    ...
    Cfg_regalloc_utils.Stats.update_cfg_with_layout
      cfg_with_layout
      Cfg_regalloc_utils.Stats.main
      (after -. before);
    let before = cpu_time () in
    (* note: slots need to be updated before prologue removal *)
    if irc_debug then begin
      Array.iteri (State.num_stack_slots state) ~f:(fun reg_class num_stack_slots ->
          log ~indent:1 "stack_slots[%d]=%d" reg_class num_stack_slots)
    end;
    update_stack_slots cfg_with_layout ~num_stack_slots:(State.num_stack_slots state);
    ...
    Cfg_regalloc_utils.Stats.update_cfg_with_layout
      cfg_with_layout
      Cfg_regalloc_utils.Stats.after_main
      (after -. before);
    if irc_debug && irc_invariants then begin
