...
let gc_for_benchmarks () =
   if run_gc_for_benchmarks then Gc.full_major ()
...
module Stats = struct
  ...
  let select_spill = {
    name = "select_spill";
    default = 0.;
    update = (fun line value -> line.select_spill <- line.select_spill +. value);
  ...
  let () = at_exit (fun () ->
      ...
      | Some directory ->
        ...
        let chan = open_out (Filename.concat directory (Digest.to_hex digest)) in
        output_string chan args;
        output_string chan "\n";
        let col_names =
          List.map columns ~f:(function Column { name; _ } -> name) in
        output_string chan (String.concat "," ("function" :: col_names));
        output_string chan "\n";
        Hashtbl.iter
          (fun name line ->
             let values =
               List.map columns ~f:(function Column { to_string; _ } -> to_string line)
             in
...
let destroyed_at_basic
  : Cfg.basic -> Reg.t array
  = fun basic ->
    let at_oper = Proc.destroyed_at_oper in
    let default = at_oper (Iop Imove) in
    match basic with
    | Op op ->
      at_oper begin match op with
        | Move -> Iop Imove
        | Spill -> Iop Ispill
        | Reload -> Iop Ireload
        ...
        | Probe { name; handler_code_sym; } -> Iop (Iprobe { name; handler_code_sym; })
        ...
        | Name_for_debugger { ident; which_parameter; provenance; is_assignment; } ->
          Iop (Iname_for_debugger { ident; which_parameter; provenance; is_assignment; })
      end
    | Call c ->
      begin match c with
...
let destroyed_at_terminator
  ...
  = fun term ->
    ...
    | Tailcall x ->
      begin match x with
        | Self { destination; } ->
          let func = ignore destination; "dummy" in
          at_oper (Mach.Iop (Itailcall_imm { func; }))
        | Func Indirect ->
          at_oper (Mach.Iop Itailcall_ind)
...
let collect_cfg_infos
  : Cfg_with_layout.t -> cfg_infos
  = fun cfg_with_layout ->
...
let liveness_analysis
  : Cfg_with_layout.t -> liveness
  = fun cfg_with_layout ->
    let before = cpu_time () in
    let cfg = Cfg_with_layout.cfg cfg_with_layout in
    let init = { Cfg_liveness.before = Reg.Set.empty; across = Reg.Set.empty; } in
    match Cfg_liveness.Liveness.run cfg ~init ~map:Cfg_liveness.Liveness.Instr () with
    | Result.Ok liveness ->
      let after = cpu_time () in
      Stats.update_cfg_with_layout
        cfg_with_layout
        Stats.liveness
        (after -. before);
      liveness
  ...
  let make_instr
    ...
    = fun move ~id ~copy:instr ~from ~to_ ->
      { desc = Op (op_of_move move);
        ...
        dbg = instr.dbg;
        fdo = instr.fdo;
        live = instr.live; (* note: recomputed anyway *)
...
let same_reg
  : Reg.t -> Reg.t -> bool
  = fun reg1 reg2 ->
    Int.equal reg1.stamp reg2.stamp

let same_reg_class
  : Reg.t -> Reg.t -> bool
  = fun reg1 reg2 ->
    Int.equal reg1.clas reg2.clas
...
let make_temporary
  ...
  = fun ~same_class_and_base_name_as:reg ~name_prefix ->
    ...
    if not (Reg.anonymous reg) then begin
      let name =
        Reg.Raw_name.to_string reg.Reg.raw_name
...
let simplify_cfg
  ...
  = fun cfg_with_layout ->
    let cfg = Cfg_with_layout.cfg cfg_with_layout in
    Cfg.iter_blocks cfg ~f:(fun _label block ->
        block.body <- List.filter block.body ~f:(fun instr ->
...
let precondition
  : Cfg_with_layout.t -> unit
  = fun cfg_with_layout ->
    (* note: the `live` field is set, because we want to call the `Deadcode`
       pass before `Cfgize`. *)
    let desc_is_neither_spill_or_reload (id : Instruction.id) (desc : Cfg.basic) : unit =
      match desc with
      | Op op ->
        begin match op with
          | Move -> ()
    ...
    let register_must_not_be_on_stack (id : Instruction.id) (reg : Reg.t) : unit =
      ...
      | Reg _ -> () (* some register are precolored, e.g. to enforce constraints *)
    ...
    in
    let registers_must_not_be_on_stack (id : Instruction.id) (regs : Reg.t array) : unit =
    ...
    (* CR xclerc for xclerc: the check below should not be in this function, since
       it is IRC-specific *)
    let register_must_be_on_unknown_list (id : Instruction.id) (reg : Reg.t) : unit =
      ...
      | Colored
      | Select_stack ->
        fatal "instruction %d has a register (%a) already in a work list (%S)"
          id
          Printmach.reg reg
          (Reg.string_of_irc_work_list reg.Reg.irc_work_list)
    in
    let register_must_be_on_unknown_list (id : Instruction.id) (regs : Reg.t array) : unit =
    ...
    let fun_num_stack_slots = (Cfg_with_layout.cfg cfg_with_layout).fun_num_stack_slots in
    Array.iteri fun_num_stack_slots ~f:(fun reg_class num_slots ->
        if num_slots <> 0 then begin
          fatal "register class %d has %d slots(s)" reg_class num_slots
        end)
...
let postcondition
  ...
  = fun cfg_with_layout ->
    ...
    in
    let registers_must_not_be_unknown (id : Instruction.id) (regs : Reg.t array) : unit =
    ...
    let num_stack_locals (regs : Reg.t array) : int =
      Array.fold_left regs ~init:0 ~f:(fun acc reg ->
          match reg.Reg.loc with
          | Unknown | Reg _ | Stack (Incoming _ | Outgoing _ | Domainstate _) -> acc
          | Stack (Local _) -> succ acc)
    in
    let arch_constraints (id : Instruction.id) (desc : Cfg.basic) (arg : Reg.t array) (res : Reg.t array) : unit =
      match Config.architecture with (* CR xclerc for xclerc: what about cross-compilation? *)
      | "amd64" | "arm64" ->
        ...
        begin match desc with
          | Op (Spill | Reload) ->
            ...
            if num_locals > 1 then begin
              fatal "instruction %d is a move and refers to %d spilling slots" id num_locals
            end
          | _ ->
            if num_locals > 0 then begin
    ...
    in
    let register_classes_must_be_consistent (id : Instruction.id) (reg : Reg.t) : unit =
      match reg.Reg.loc with
      | Reg phys_reg ->
        let phys_reg = Proc.phys_reg phys_reg in
        if not (same_reg_class reg phys_reg) then begin
          fatal "instruction %d assigned %a to %a but they are in different classes"
            id
            Printmach.reg reg
            Printmach.reg phys_reg
        end
      | Stack _ | Unknown -> ()
    in
    let register_classes_must_be_consistent (id : Instruction.id) (regs : Reg.t array) : unit =
    ...
    Array.iter2 max_stack_slots fun_num_stack_slots ~f:(fun max_slot num_slots ->
        if succ max_slot <> num_slots then begin
          fatal "register class %d has a max slot of %d, but the number of slots is %d"
            !reg_class
            max_slot
            num_slots
        end;
      ...
      ~annotate_block:(fun label ->
          let block =
            Cfg.get_block_exn
              ...
              label
          in
          Printf.sprintf "label:%d stack_offset:%d"
            label
            block.stack_offset)
      ~annotate_succ:(Printf.sprintf "%d->%d")
      str

let update_stack_slots
  : Cfg_with_layout.t -> num_stack_slots:int array -> unit
...
let remove_prologue_if_not_required
  : Cfg_with_layout.t -> unit
  = fun cfg_with_layout ->
    let cfg = Cfg_with_layout.cfg cfg_with_layout in
    ...
    in
    if not prologue_required then begin
...
let update_live_fields
  : Cfg_with_layout.t -> liveness -> unit
  = fun cfg_with_layout liveness ->
    (* CR xclerc for xclerc: partial duplicate of `Asmgen.recompute_liveness_on_cfg` *)
    let before = cpu_time () in
    let with_liveness (instr : _ Cfg.instruction) =
      match Cfg_dataflow.Instr.Tbl.find_opt liveness instr.id with
      | None ->
        fatal "Missing liveness information for instruction %d" instr.id
