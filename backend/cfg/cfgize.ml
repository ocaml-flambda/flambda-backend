...
let basic_or_terminator_of_operation :
 ...
 fun state op ->
  ...
  | Icall_ind { original_position } ->
    With_next_label
      (fun label_after ->
        Call { op = Indirect; label_after; original_position })
  | Icall_imm { func; original_position } ->
    With_next_label
      (fun label_after ->
        Call { op = Direct func; label_after; original_position })
  | Itailcall_ind { original_position } ->
    Terminator (Tailcall_func { op = Indirect; original_position })
  | Itailcall_imm { func; original_position } ->
    Terminator
      (if String.equal (State.get_fun_name state) func.sym_name
      then
        Tailcall_self
          { op = { destination = State.get_tailrec_label state };
            original_position
          }
      else Tailcall_func { op = Direct func; original_position })
