...
let basic_or_terminator_of_operation :
 ...
 fun state op ->
  ...
  | Icall_ind { tail }->
    With_next_label (fun label_after -> Call { op = Indirect; label_after; tail })
  | Icall_imm { func; tail } ->
    With_next_label (fun label_after -> Call { op = Direct func; label_after; tail })
  ...
  | Itailcall_imm { func; tail } ->
    Terminator
      ...
      then Tailcall_self { op = { destination = State.get_tailrec_label state }; tail }
