...
let[@inline] reset state ~new_temporaries =
  ...
  let unknown_list_instr (l : Instruction.t list) : unit =
    List.iter l ~f:(fun (instr : Instruction.t) -> instr.irc_work_list <- Unknown_list)
  in
  List.iter (Reg.all_registers ()) ~f:(fun reg ->
      reg.Reg.irc_color <- None;
      reg.Reg.irc_alias <- None;
  ...
  Array.iter all_precolored_regs ~f:(fun reg ->
      ...
      begin match reg.Reg.loc, reg.Reg.irc_color with
  ...
  List.iter state.initial ~f:(fun reg ->
      reg.Reg.irc_work_list <- Initial);
  ...
  state.introduced_temporaries <-
    List.fold_left new_temporaries ~init:state.introduced_temporaries ~f:(fun acc reg ->
        Reg.Set.add reg acc)
...
let[@inline] remove_reg (reg : Reg.t) (l : Reg.t list) : Reg.t list =
  let rec filter reg acc = function
    | [] -> acc
    | hd :: tl ->
      if same_reg reg hd then
        filter reg acc tl
      else
        filter reg (hd :: acc) tl
  in
  filter reg [] l
...
let[@inline] spilled_nodes state =
  state.spilled_nodes
...
let[@inline] coalesced_nodes state =
  state.coalesced_nodes
...
let[@inline] remove_instr (instr : Instruction.t) (l : Instruction.t list) : Instruction.t list =
  let rec filter instr acc = function
    | [] -> acc
    | hd :: tl ->
      if hd.Cfg.id = instr.Cfg.id then
        filter instr acc tl
      else
        filter instr (hd :: acc) tl
  in
  filter instr [] l
...
let[@inline] adj_list _state reg =
  reg.Reg.interf
...
let[@inline] add_edge state u v =
  ...
  in
  if not (same_reg u v)
  && (is_interesting_reg u)
  && (is_interesting_reg v)
  && (same_reg_class u v)
  && not (RegisterStamp.PairSet.mem state.adj_set u.Reg.stamp v.Reg.stamp) then begin
    RegisterStamp.PairSet.add state.adj_set u.Reg.stamp v.Reg.stamp;
    let add_adj_list x y =
      x.Reg.interf <- y :: x.Reg.interf
    in
    let incr_degree x =
      let deg = x.Reg.degree in
      if irc_debug && deg = Degree.infinite then
        fatal "trying to increment the degree of a precolored node";
      x.Reg.degree <- succ deg
    in
    if not (is_precolored state u) then begin
      add_adj_list u v;
      incr_degree u
    end;
    if not (is_precolored state v) then begin
      add_adj_list v u;
      incr_degree v
    end;
  end
...
let[@inline] is_empty_node_moves state reg =
  ...
  | None -> true
  | Some move_list ->
    not (Instruction.Set.exists move_list ~f:(fun instr ->
...
let[@inline] add_move_list state reg instr =
  match Reg.Tbl.find_opt state.move_list reg with
  | None -> Reg.Tbl.replace state.move_list reg (Instruction.Set.singleton instr)
  | Some existing ->
...
let[@inline] rec find_alias state reg =
  if reg.Reg.irc_work_list = Reg.Coalesced then
...
let[@inline] add_alias _state v u =
  if irc_debug && not (same_reg_class v u) then begin
    fatal "trying to create an alias between %a and %a but they are in different classes"
      Printmach.reg v
      Printmach.reg u
  end;
...
let[@inline] num_stack_slots state =
  state.num_stack_slots
...
let[@inline] add_introduced_temporaries_list state regs =
  state.introduced_temporaries <-
    List.fold_left regs ~init:state.introduced_temporaries ~f:(fun introduced reg ->
      Reg.Set.add reg introduced)

let[@inline] mem_introduced_temporaries state reg =
  Reg.Set.mem reg state.introduced_temporaries

let[@inline] introduced_temporaries state =
  state.introduced_temporaries
  ...
  List.iter sets ~f:(fun (name1, set1) ->
      List.iter sets ~f:(fun (name2, set2) ->
          if name1 < name2 then begin
            if not (is_disjoint set1 set2) then begin
              fatal "sets %s and %s are not disjoint" name1 name2
            end
          end))
...
let[@inline] check_set_and_field_consistency_reg (work_list, set, field_value) =
  Reg.Set.iter
    (fun reg ->
       if reg.Reg.irc_work_list <> field_value then begin
         fatal "register %a is in %s but its field equals %S"
           Printmach.reg reg
           work_list
...
let[@inline] check_set_and_field_consistency_instr (work_list, set, field_value) =
  Instruction.Set.iter set ~f:(fun instr ->
      if instr.Cfg.irc_work_list <> field_value then begin
        fatal "instruction %d is in %s but its field equals %S"
          instr.Cfg.id
          work_list
...
let[@inline] check_inter_has_no_duplicates (reg : Reg.t) : unit =
  ...
  if List.length l <> Reg.Set.cardinal s then begin
    fatal "interf list for %a is not a set"
      Printmach.reg reg
  end
...
let[@inline] invariant state =
  if irc_debug && irc_invariants then begin
    ...
    List.iter ~f:check_set_and_field_consistency_reg [
      ...
      "simplify_work_list", Reg.Set.of_list state.simplify_work_list, Reg.Simplify;
    ...
    List.iter ~f:check_set_and_field_consistency_instr [
      "coalesced_moves", Instruction.Set.of_list state.coalesced_moves, Cfg.Coalesced;
      "constrained_moves", Instruction.Set.of_list state.constrained_moves, Cfg.Constrained;
      "frozen_moves", Instruction.Set.of_list state.frozen_moves, Cfg.Frozen;
      "work_list_moves", Instruction.Set.of_list state.work_list_moves, Cfg.Work_list;
    ...
    let work_lists_or_precolored =
      Reg.Set.union
        (Reg.set_of_array all_precolored_regs)
        work_lists
    in
    Reg.Set.iter
      (fun u ->
         let degree = u.Reg.degree in
         if degree = Degree.infinite then
           fatal "invariant: infinite degree for %a" Printmach.reg u
         else begin
           let adj_list =
             Reg.Set.of_list (adj_list state u)
           in
           let cardinal = Reg.Set.cardinal (Reg.Set.inter adj_list work_lists_or_precolored) in
           if not (Int.equal degree cardinal) then begin
             List.iter u.Reg.interf ~f:(fun r ->
                 log ~indent:0 "%a <- interf[%a]"
                   Printmach.reg r
                   Printmach.reg u);
             Reg.Set.iter
               (fun r ->
                  log ~indent:0 "%a <- adj_list[%a]"
                    Printmach.reg r
                    Printmach.reg u)
               adj_list;
             Reg.Set.iter
               (fun r ->
                  log ~indent:0 "%a <- work_lists_or_precolored[%a]"
                    Printmach.reg r
                    Printmach.reg u)
               (Reg.Set.inter adj_list work_lists_or_precolored);
             fatal "invariant expected degree for %a to be %d but got %d\n (#adj_list=%d, #work_lists_or_precolored=%d)"
               Printmach.reg u
               cardinal
               degree
