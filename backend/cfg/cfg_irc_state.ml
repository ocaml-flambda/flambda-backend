...
module RegWorkList = WorkList.Make (struct
  type t = Reg.t
...
let[@inline] reset state ~new_temporaries =
  ...
  state.initial <- new_temporaries @ state.colored_nodes @ (RegWorkList.to_list state.coalesced_nodes);
...
let[@inline] spilled_nodes state =
  RegWorkList.to_list state.spilled_nodes
...
let[@inline] add_constrained_moves state instr =
  ...
  state.constrained_moves <- InstructionWorkList.add state.constrained_moves instr
...
let[@inline] is_empty_node_moves state reg =
  ...
  | Some move_list ->
    not
      (Instruction.Set.exists (fun instr ->
...
let[@inline] iter_node_moves state reg ~f =
  ...
  | None -> ()
  | Some move_list ->
    Instruction.Set.iter (fun instr ->
        match instr.irc_work_list with Active | Work_list -> f instr | _ -> ()) move_list
  ...
  Instruction.Set.iter (fun instr ->
      ...
      then
        fatal "instruction %d is in %s but its field equals %S" instr.Cfg.id
          work_list
          (Cfg.string_of_irc_work_list instr.Cfg.irc_work_list)) set
    ...
    List.iter ~f:check_set_and_field_consistency_reg
      [ "precolored", Reg.set_of_array all_precolored_regs, Reg.Precolored;
        ...
        ( "simplify_work_list",
          ...
          Reg.Simplify );
        "freeze_work_list", RegWorkList.to_set state.freeze_work_list, Reg.Freeze;
        ...
        "coalesced_nodes", RegWorkList.to_set state.coalesced_nodes, Reg.Coalesced;
    ...
    List.iter ~f:check_set_and_field_consistency_instr
      [ ( "coalesced_moves",
        ...
        ( "constrained_moves",
          ...
          Cfg.Constrained );
        "frozen_moves", InstructionWorkList.to_set state.frozen_moves, Cfg.Frozen;
        ( "work_list_moves",
          InstructionWorkList.to_set state.work_list_moves,
          Cfg.Work_list );
        "active_moves", InstructionWorkList.to_set state.active_moves, Cfg.Active
      ];
