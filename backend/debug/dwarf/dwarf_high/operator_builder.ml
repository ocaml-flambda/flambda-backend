...
let add_unsigned_const i : O.t list =
  if Targetint_extra.compare i Targetint_extra.zero < 0
  then
    Misc.fatal_error
...
let conditional ?(at_join = [O.DW_op_nop]) ~if_zero ~if_nonzero () =
  ...
  if nonzero_branch_size > max_branch_size
  then Misc.fatal_error "Dwarf_operator.conditional: nonzero branch too long";
  let nonzero_branch_size =
    Numbers_extra.Int16.of_int64_exn nonzero_branch_size
  in
  let if_zero =
  ...
  if zero_branch_size > max_branch_size
  then Misc.fatal_error "Dwarf_operator.conditional: zero branch too long";
