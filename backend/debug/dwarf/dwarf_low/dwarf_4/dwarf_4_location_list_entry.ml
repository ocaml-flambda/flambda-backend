...
module Location_list_entry = struct
  ...
  let create ~start_of_code_symbol ~first_address_when_in_scope
      ...
      ~first_address_when_not_in_scope_offset ~single_location_description =
  ...
  let ending_value t =
    let offset_upper =
      ...
      | None ->
        (* It might seem as if this should be "-1", but actually not. DWARF-4
           spec p.30 (point 2): "...the first address past the end of the
    ...
    Dwarf_value.code_address_from_label_symbol_diff ~comment:"ending address"
      ~upper:t.ending_address_label ~lower:t.start_of_code_symbol ~offset_upper
      ()
...
module Base_address_selection_entry = struct
  ...
  let to_dwarf_values t =
    [ Dwarf_value.absolute_address ~comment:"largest representable addr. offset"
        Targetint_extra.minus_one;
      (* all "1"s *)
...
let create_location_list_entry ~start_of_code_symbol
    ~first_address_when_in_scope ~first_address_when_in_scope_offset
...
let size = function
  | Location_list_entry entry -> Location_list_entry.size entry
