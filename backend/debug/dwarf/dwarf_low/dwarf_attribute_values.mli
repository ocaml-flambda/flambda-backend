...
module Value : sig
  ...
  val flag_true :
    ?comment:string -> unit -> Dwarf_attributes.Form.flag_present t

  val bool : ?comment:string -> bool -> Dwarf_attributes.Form.data1 t

  val int8 :
    ?comment:string -> Numbers_extra.Int8.t -> Dwarf_attributes.Form.data1 t

  val int16 :
    ?comment:string -> Numbers_extra.Int16.t -> Dwarf_attributes.Form.data2 t

  val int32 : ?comment:string -> Int32.t -> Dwarf_attributes.Form.data4 t

  val int64 : ?comment:string -> Int64.t -> Dwarf_attributes.Form.data8 t

  val uleb128 :
    ?comment:string -> Numbers_extra.Uint64.t -> Dwarf_attributes.Form.udata t
  ...
  val offset_into_debug_line : Asm_label.t -> Dwarf_attributes.Form.sec_offset t

  val offset_into_debug_line_from_symbol :
    Asm_symbol.t -> Dwarf_attributes.Form.sec_offset t

  val offset_into_debug_info :
    ?comment:string -> Asm_label.t -> Dwarf_attributes.Form.ref_addr t

  val offset_into_debug_info_from_symbol :
    ?comment:string -> Asm_symbol.t -> Dwarf_attributes.Form.ref_addr t

  (** Not for use for DWARF >= version 5. *)
  val offset_into_debug_loc : Asm_label.t -> Dwarf_attributes.Form.sec_offset t

  (** Not for use for DWARF >= version 5. *)
  val offset_into_debug_ranges :
    Asm_label.t -> Dwarf_attributes.Form.sec_offset t

  (** Not for use for DWARF < version 5. *)
  val offset_into_debug_addr : Asm_label.t -> Dwarf_attributes.Form.sec_offset t
  ...
  val single_location_description :
    Single_location_description.t -> Dwarf_attributes.Form.exprloc t

  val composite_location_description :
    Composite_location_description.t -> Dwarf_attributes.Form.exprloc t

  val encoding_attribute : Encoding_attribute.t -> Dwarf_attributes.Form.data1 t

  (** Not for use for DWARF < version 5. *)
  val loclistx :
    index:Numbers_extra.Uint64.t -> Dwarf_attributes.Form.loclistx t

  (** Not for use for DWARF < version 5. *)
  val rnglistx :
    index:Numbers_extra.Uint64.t -> Dwarf_attributes.Form.rnglistx t
...
module Attribute_value : sig
  ...
  val create :
    'form Dwarf_attributes.Attribute_specification.t -> 'form Value.t -> t
