...
let bind_list name args fn =
  let rec aux bound_args = function
    | [] -> fn bound_args
    | arg :: args -> bind name arg (fun bound_arg -> aux (bound_arg :: bound_args) args)
  in
  aux [] (List.rev args)
;;
...
let block_header ?(scannable_prefix = Scan_all) tag sz =
  let hdr =
    Nativeint.add (Nativeint.shift_left (Nativeint.of_int sz) 10) (Nativeint.of_int tag)
  in
  match scannable_prefix with
  | Scan_all -> hdr
  | Scan_prefix scannable_prefix -> Mixed_block_support.make_header hdr ~scannable_prefix
;;
...
let closure_info ~(arity : arity) ~startenv ~is_last =
  closure_info' ~arity:(arity.function_kind, arity.params_layout) ~startenv ~is_last
;;
...
let int_const dbg n =
  ...
  else Cconst_natint (Nativeint.add (Nativeint.shift_left (Nativeint.of_int n) 1) 1n, dbg)
;;
...
let rec add_int c1 c2 dbg =
  ...
  | Cop (Caddi, [ c1; Cconst_int (n1, _) ], _), c2 -> add_const (add_int c1 c2 dbg) n1 dbg
  | c1, Cop (Caddi, [ c2; Cconst_int (n2, _) ], _) -> add_const (add_int c1 c2 dbg) n2 dbg
...
let rec sub_int c1 c2 dbg =
  ...
  | Cop (Caddi, [ c1; Cconst_int (n1, _) ], _), c2 -> add_const (sub_int c1 c2 dbg) n1 dbg
...
let rec lsl_int c1 c2 dbg =
  ...
  | Cop (Caddi, [ c1; Cconst_int (n1, _) ], _), Cconst_int (n2, _)
    when Misc.no_overflow_lsl n1 n2 -> add_const (lsl_int c1 c2 dbg) (n1 lsl n2) dbg
...
let rec mul_int c1 c2 dbg =
  match c1, c2 with
  | c, Cconst_int (0, _) | Cconst_int (0, _), c -> Csequence (c, Cconst_int (0, dbg))
  | c, Cconst_int (1, _) | Cconst_int (1, _), c -> c
  | c, Cconst_int (-1, _) | Cconst_int (-1, _), c -> sub_int (Cconst_int (0, dbg)) c dbg
...
let asr_int c1 c2 dbg =
  ...
  | Cconst_int (n, _) when n > 0 ->
    (match ignore_low_bit_int c1 with
     ...
     | Cop (Clsl, [ c; Cconst_int (1, _) ], _) when n = 1 && guaranteed_to_be_small_int c
       -> c
...
let tag_int i dbg =
  ...
  | Cop (Casr, [ c; Cconst_int (n, _) ], _) when n > 0 ->
    Cop (Cor, [ asr_int c (Cconst_int (n - 1, dbg)) dbg; Cconst_int (1, dbg) ], dbg)
...
let untag_int i dbg =
  ...
  | Cop (Cor, [ Cop (Casr, [ c; Cconst_int (n, _) ], _); Cconst_int (1, _) ], _)
    when n > 0 && n < (size_int * 8) - 1 -> Cop (Casr, [ c; Cconst_int (n + 1, dbg) ], dbg)
  | Cop (Cor, [ Cop (Clsr, [ c; Cconst_int (n, _) ], _); Cconst_int (1, _) ], _)
    when n > 0 && n < (size_int * 8) - 1 -> Cop (Clsr, [ c; Cconst_int (n + 1, dbg) ], dbg)
  ...
  | Cop (Caddi, [ Cop (Clsl, [ c; Cconst_int (1, _) ], _); Cconst_int (1, _) ], dbg') ->
    (match c with
     | Cop (Ccmpi cmp, [ c1; c2 ], dbg'') ->
       tag_int (Cop (Ccmpi (negate_integer_comparison cmp), [ c1; c2 ], dbg'')) dbg'
     | Cop (Ccmpa cmp, [ c1; c2 ], dbg'') ->
       tag_int (Cop (Ccmpa (negate_integer_comparison cmp), [ c1; c2 ], dbg'')) dbg'
     | Cop (Ccmpf cmp, [ c1; c2 ], dbg'') ->
       tag_int (Cop (Ccmpf (negate_float_comparison cmp), [ c1; c2 ], dbg'')) dbg'
     | _ ->
       (* 0 -> 3, 1 -> 1 *)
       Cop
         (Csubi, [ Cconst_int (3, dbg); Cop (Clsl, [ c; Cconst_int (1, dbg) ], dbg) ], dbg))
...
let divimm_parameters d =
  Nativeint.(
    ...
    let rec loop p (q1, r1) (q2, r2) =
      let p = p + 1 in
      let q1 = shift_left q1 1
      and r1 = shift_left r1 1 in
      let q1, r1 = if ucompare r1 nc >= 0 then succ q1, sub r1 nc else q1, r1 in
      let q2 = shift_left q2 1
      and r2 = shift_left r2 1 in
...
let rec div_int c1 c2 is_safe dbg =
  match c1, c2 with
  | c1, Cconst_int (0, _) -> Csequence (c1, raise_symbol dbg "caml_exn_Division_by_zero")
  ...
  | c1, Cconst_int (n, _) when n <> min_int ->
    ...
    then sub_int (Cconst_int (0, dbg)) (div_int c1 (Cconst_int (-n, dbg)) is_safe dbg) dbg
    else (
      ...
      bind "dividend" c1 (fun c1 ->
        let t =
          Cop (Cmulhi { signed = true }, [ c1; natint_const_untagged dbg m ], dbg)
        in
        let t = if m < 0n then Cop (Caddi, [ t; c1 ], dbg) else t in
        let t = if p > 0 then Cop (Casr, [ t; Cconst_int (p, dbg) ], dbg) else t in
        add_int t (lsr_int c1 (Cconst_int (Nativeint.size - 1, dbg)) dbg) dbg))
  | c1, c2 when !Clflags.unsafe || is_safe = Lambda.Unsafe -> Cop (Cdivi, [ c1; c2 ], dbg)
  | c1, c2 ->
...
let test_bool dbg cmm =
  ...
  | Cconst_int (n, dbg) -> if n = 1 then Cconst_int (0, dbg) else Cconst_int (1, dbg)
  ...
  map_tail ~kind:Any (function
    | Cop (Calloc _, [ Cconst_natint (hdr, _); _ops; c ], _)
      when Nativeint.equal hdr boxedfloat32_header
           || Nativeint.equal hdr boxedfloat32_local_header -> c
...
let unbox_float dbg =
  map_tail ~kind:Any (function
    | Cop (Calloc _, [ Cconst_natint (hdr, _); c ], _)
      when Nativeint.equal hdr float_header || Nativeint.equal hdr float_local_header -> c
    | Cconst_symbol (s, _dbg) as cmm ->
      (match Cmmgen_state.structured_constant_of_sym s.sym_name with
  ...
  map_tail ~kind:Any (function
    | Cop (Calloc _, [ Cconst_natint (hdr, _); c ], _)
      when Nativeint.equal hdr boxedvec128_header
           || Nativeint.equal hdr boxedvec128_local_header -> c
    | Cconst_symbol (s, _dbg) as cmm ->
      (match Cmmgen_state.structured_constant_of_sym s.sym_name with
...
let box_complex dbg c_re c_im =
  Cop (Calloc Lambda.alloc_heap, [ alloc_floatarray_header 2 dbg; c_re; c_im ], dbg)
;;
...
let complex_im c dbg =
  Cop
    (mk_load_immut Double, [ Cop (Cadda, [ c; Cconst_int (size_float, dbg) ], dbg) ], dbg)
;;
...
let get_field_gen mut ptr n dbg = get_field_gen_given_memory_chunk Word_val mut ptr n dbg
...
let get_tag ptr dbg =
  ...
  then
    ...
    Cop
      ( (if Config.runtime5 then mk_load_immut Byte_unsigned else mk_load_mut Byte_unsigned)
      , [ Cop (Cadda, [ ptr; Cconst_int (tag_offset, dbg) ], dbg) ]
      , dbg )
;;

let get_size ptr dbg = Cop (Clsr, [ get_header_masked ptr dbg; Cconst_int (10, dbg) ], dbg)
...
let array_indexing ?typ log2size ptr ofs dbg =
  ...
  | Cconst_int (n, _) ->
    let i = n asr 1 in
    if i = 0 then ptr else Cop (add, [ ptr; Cconst_int (i lsl log2size, dbg) ], dbg)
  | Cop (Caddi, [ Cop (Clsl, [ c; Cconst_int (1, _) ], _); Cconst_int (1, _) ], dbg') ->
    Cop (add, [ ptr; lsl_const c log2size dbg ], dbg')
  | Cop (Caddi, [ c; Cconst_int (n, _) ], dbg') when log2size = 0 ->
    Cop
      (add, [ Cop (add, [ ptr; untag_int c dbg ], dbg); Cconst_int (n asr 1, dbg) ], dbg')
...
let custom_ops_unboxed_int32_array =
  Cconst_symbol (Cmm.global_symbol "caml_unboxed_int32_array_ops", Debuginfo.none)
;;
...
let custom_ops_unboxed_int64_array =
  Cconst_symbol (Cmm.global_symbol "caml_unboxed_int64_array_ops", Debuginfo.none)
;;

let custom_ops_unboxed_nativeint_array =
  Cconst_symbol (Cmm.global_symbol "caml_unboxed_nativeint_array_ops", Debuginfo.none)
;;
...
let rec low_32 dbg = function
  ...
  | Cop (Cand, [ x; Cconst_natint (0xFFFFFFFFn, _) ], _) -> low_32 dbg x
...
let sign_extend_32 dbg e =
  ...
  | Cop
      ( Cload
          { memory_chunk = Thirtytwo_unsigned | Thirtytwo_signed; mutability; is_atomic }
      , args
      , dbg ) ->
    Cop (Cload { memory_chunk = Thirtytwo_signed; mutability; is_atomic }, args, dbg)
  | e ->
    Cop (Casr, [ Cop (Clsl, [ e; Cconst_int (32, dbg) ], dbg); Cconst_int (32, dbg) ], dbg)
;;
...
let unboxed_int32_mixed_field_ref arr index dbg =
  bind "arr" arr (fun arr ->
    bind "index" index (fun index ->
      let index = mul_int index (int ~dbg 2) dbg in
      unboxed_int32_array_ref arr index dbg))
;;
...
let unboxed_int32_mixed_field_set arr ~index ~new_value dbg =
  bind "arr" arr (fun arr ->
    bind "index" index (fun index ->
      bind "new_value" new_value (fun new_value ->
        let index = mul_int index (int ~dbg 2) dbg in
        unboxed_int32_array_set arr ~index ~new_value dbg)))
;;
...
let get_field_computed imm_or_ptr mutability ~block ~index dbg =
  ...
  Cop (Cload { memory_chunk; mutability; is_atomic = false }, [ field_address ], dbg)
;;
...
let string_length exp dbg =
  bind "str" exp (fun str ->
    ...
    Clet
      ...
      , Cop
          ( Csubi
          , [ Cop (Clsl, [ get_size str dbg; Cconst_int (log2_size_addr, dbg) ], dbg)
            ; Cconst_int (1, dbg)
            ]
          , dbg )
      , Cop
...
let bigstring_get_alignment ba idx align dbg =
  bind
    ...
    (fun ba_data ->
      Cop (Cand, [ Cconst_int (align - 1, dbg); Cop (Caddi, [ ba_data; idx ], dbg) ], dbg))
;;
...
module Extended_machtype = struct
  ...
  let to_machtype t = Array.map Extended_machtype_component.to_machtype_component t
  ...
  let rec of_layout (layout : Lambda.layout) =
    ...
    | Pbottom -> Misc.fatal_error "No unique Extended_machtype for layout [Pbottom]"
...
let machtype_of_layout_changing_tagged_int_to_val layout =
  layout |> Extended_machtype.of_layout |> Extended_machtype.change_tagged_int_to_val
;;
...
let machtype_identifier t =
  let char_of_component (component : machtype_component) =
    ...
    | Vec128 -> 'X'
    | Float32 -> 'S'
    | Addr -> Misc.fatal_error "[Addr] is forbidden inside arity for generic functions"
  in
...
let make_alloc_generic ?(scannable_prefix = Scan_all) ~mode set_fn dbg tag wordsize args =
  ...
  then (
    ...
    let rec fill_fields idx = function
      ...
      | e1 :: el ->
        Csequence
          (set_fn idx (Cvar id) (int_const dbg idx) e1 dbg, fill_fields (idx + 1) el)
    in
...
let make_mixed_alloc ~mode dbg tag shape args =
  ...
  let set_fn idx arr ofs newval dbg =
    ...
    then addr_array_init arr ofs newval dbg
      ...
      | Bits32 -> unboxed_int32_mixed_field_set arr ~index:ofs ~new_value:newval dbg
      | Bits64 | Word ->
        unboxed_int64_or_nativeint_array_set arr ~index:ofs ~new_value:newval dbg)
  in
  let size =
  ...
  then
    Misc.fatal_error
      "Unable to compile mixed blocks on a platform where a float is not the same width \
       as a value.";
  make_alloc_generic
    ~scannable_prefix:(Scan_prefix value_prefix_len)
    ~mode
    set_fn
    dbg
    tag
...
let curry_function_sym_name function_kind arity result =
  ...
  | Lambda.Tupled ->
    ...
    Compilenv.need_curry_fun function_kind (List.map (fun _ -> [| Val |]) arity) result;
    tuplify_function_name (List.length arity) result
;;

let curry_function_sym function_kind arity result =
  { sym_name = curry_function_sym_name function_kind arity result; sym_global = Global }
;;
...
let check_64_bit_target func =
  if size_int <> 8
  then Misc.fatal_errorf "Cmm helpers function %s can only be used on 64-bit targets" func
;;
...
let rec low_63 dbg e =
  ...
  | Cop (Casr, [ Cop (Clsl, [ x; Cconst_int (1, _) ], _); Cconst_int (1, _) ], _) ->
    low_63 dbg x
  | Cop (Cand, [ x; Cconst_natint (0x7FFF_FFFF_FFFF_FFFFn, _) ], _) -> low_63 dbg x
...
let sign_extend_63 dbg e =
  ...
  Cop (Casr, [ Cop (Clsl, [ e; Cconst_int (1, dbg) ], dbg); Cconst_int (1, dbg) ], dbg)
;;
...
let zero_extend_32 dbg e =
  ...
  | Cop
      ( Cload
          { memory_chunk = Thirtytwo_signed | Thirtytwo_unsigned; mutability; is_atomic }
      , args
      , dbg ) ->
    Cop (Cload { memory_chunk = Thirtytwo_unsigned; mutability; is_atomic }, args, dbg)
...
let alloc_matches_boxed_int bi ~hdr ~ops =
  ...
  | Pint32, Cconst_natint (hdr, _dbg), Cconst_symbol (sym, _) ->
    (Nativeint.equal hdr boxedint32_header || Nativeint.equal hdr boxedint32_local_header)
    && String.equal sym.sym_name caml_int32_ops
  | Pint64, Cconst_natint (hdr, _dbg), Cconst_symbol (sym, _) ->
    (Nativeint.equal hdr boxedint64_header || Nativeint.equal hdr boxedint64_local_header)
...
let unbox_int dbg bi =
  let default arg =
    let memory_chunk = if bi = Primitive.Pint32 then Thirtytwo_signed else Word_int in
    Cop
  ...
  map_tail ~kind:Any (function
    | Cop
        (Calloc _, [ hdr; ops; Cop (Clsl, [ contents; Cconst_int (32, _) ], _dbg') ], _dbg)
      when bi = Primitive.Pint32 && big_endian && alloc_matches_boxed_int bi ~hdr ~ops ->
      (* Force sign-extension of low 32 bits *)
      sign_extend_32 dbg contents
    | Cop (Calloc _, [ hdr; ops; contents ], _dbg)
      when bi = Primitive.Pint32
           && (not big_endian)
           && alloc_matches_boxed_int bi ~hdr ~ops ->
      (* Force sign-extension of low 32 bits *)
      sign_extend_32 dbg contents
    | Cop (Calloc _, [ hdr; ops; contents ], _dbg)
      when alloc_matches_boxed_int bi ~hdr ~ops -> contents
    | Cconst_symbol (s, _dbg) as cmm ->
      (match Cmmgen_state.structured_constant_of_sym s.sym_name, bi with
       | Some (Const_nativeint n), Primitive.Pnativeint -> natint_const_untagged dbg n
...
let unaligned_set_16 ptr idx newval dbg =
  if Arch.allow_unaligned_access
  then Cop (Cstore (Sixteen_unsigned, Assignment), [ add_int ptr idx dbg; newval ], dbg)
  else (
    let cconst_int i = Cconst_int (i, dbg) in
    let v1 =
  ...
  then Cop (mk_load_mut Thirtytwo_unsigned, [ add_int ptr idx dbg ], dbg)
    ...
    let v4 =
      Cop
        ...
        , dbg )
    in
    let b1, b2, b3, b4 = if Arch.big_endian then v1, v2, v3, v4 else v4, v3, v2, v1 in
    Cop
      ( Cor
      , [ Cop
            (Cor, [ lsl_int b1 (cconst_int 24) dbg; lsl_int b2 (cconst_int 16) dbg ], dbg)
...
let unaligned_set_32 ptr idx newval dbg =
  if Arch.allow_unaligned_access
  then Cop (Cstore (Thirtytwo_unsigned, Assignment), [ add_int ptr idx dbg; newval ], dbg)
  else (
    let cconst_int i = Cconst_int (i, dbg) in
    let v1 =
...
let unaligned_set_64 ptr idx newval dbg =
  ...
  then Cop (Cstore (Word_int, Assignment), [ add_int ptr idx dbg; newval ], dbg)
    ...
    let cconst_int i = Cconst_int (i, dbg) in
    let v1 =
      Cop (Cand, [ Cop (Clsr, [ newval; cconst_int (8 * 7) ], dbg); cconst_int 0xFF ], dbg)
    in
    let v2 =
      Cop (Cand, [ Cop (Clsr, [ newval; cconst_int (8 * 6) ], dbg); cconst_int 0xFF ], dbg)
    in
    let v3 =
      Cop (Cand, [ Cop (Clsr, [ newval; cconst_int (8 * 5) ], dbg); cconst_int 0xFF ], dbg)
    in
    let v4 =
      Cop (Cand, [ Cop (Clsr, [ newval; cconst_int (8 * 4) ], dbg); cconst_int 0xFF ], dbg)
    in
    let v5 =
      Cop (Cand, [ Cop (Clsr, [ newval; cconst_int (8 * 3) ], dbg); cconst_int 0xFF ], dbg)
    in
    let v6 =
      Cop (Cand, [ Cop (Clsr, [ newval; cconst_int (8 * 2) ], dbg); cconst_int 0xFF ], dbg)
    in
    let v7 =
      Cop (Cand, [ Cop (Clsr, [ newval; cconst_int 8 ], dbg); cconst_int 0xFF ], dbg)
    in
...
let unaligned_set_128 ptr idx newval dbg =
  assert (size_vec128 = 16);
  Cop (Cstore (Onetwentyeight_unaligned, Assignment), [ add_int ptr idx dbg; newval ], dbg)
;;
...
let aligned_set_128 ptr idx newval dbg =
  assert (size_vec128 = 16);
  Cop (Cstore (Onetwentyeight_aligned, Assignment), [ add_int ptr idx dbg; newval ], dbg)
;;
...
let make_switch arg cases actions dbg kind =
  let extract_uconstant = function
    ...
    | Cconst_int (n, _), _dbg when n land 1 = 1 -> Some (Cint (Nativeint.of_int n))
    ...
    then (
      ...
      | Cint v0, Cint v1 ->
        ...
        let check i = function
          ...
          | _ -> false
        in
        if Misc.Stdlib.Array.for_alli (fun i idx -> check i const_actions.(idx)) cases
        then Some (v0, slope)
        else None
      | _, _ -> None)
    else None
  ...
  let make_table_lookup ~cases ~const_actions arg dbg =
    ...
    Cmmgen_state.add_constant
      table
      (Const_table
         (Local, Array.to_list (Array.map (fun act -> const_actions.(act)) cases)));
...
module SArgBlocks = struct
  ...
  let make_if value_kind cond ifso ifnot =
    Cifthenelse
      (cond, Debuginfo.none, ifso, Debuginfo.none, ifnot, Debuginfo.none, value_kind)
  ;;
...
let transl_switch_clambda loc value_kind arg index cases =
  ...
  | inters ->
    bind "switcher" arg (fun a ->
      SwitcherBlocks.zyva loc value_kind (0, n_index - 1) a (Array.of_list inters) store)
;;
...
let call_caml_apply extended_ty extended_args_type mut clos args pos mode dbg =
  ...
  let really_call_caml_apply clos args =
    let cargs =
      (Cconst_symbol (apply_function_sym extended_args_type extended_ty mode, dbg) :: args)
      @ [ clos ]
    in
...
let rec might_split_call_caml_apply ?old_region result arity mut clos args pos mode dbg =
  ...
  | (arity, args), Some (arity', args') ->
    ...
    (match old_region, mode with
     | None, Lambda.Alloc_heap when Config.stack_allocation ->
       ...
       bind "region" (Cop (Cbeginregion, [], dbg ())) (fun region -> body (Some region))
     | _ -> body old_region)
;;
...
let send kind met obj args args_type result akind dbg =
  ...
  bind "obj" obj (fun obj ->
    ...
    | Self, _, _ -> bind "met" (lookup_label obj met dbg) (call_met obj args args_type)
                                   ...
                                   , [ tag
                                     ; Cop
                                         ...
                                         , [ Cop
                                               ( Cadda
                                               , [ meths
                                                 ; lsl_const (Cvar mi) log2_size_addr dbg
                                                 ]
                                               , dbg )
                                           ]
                                         , dbg )
...
let apply_function_body arity result (mode : Lambda.alloc_mode) =
  ...
  let code =
    ...
    | Some reg -> Clet (VP.create reg, Cop (Cbeginregion, [], dbg ()), app_fun clos args)
  in
...
let send_function (arity, result, mode) =
  ...
  let clos =
    ...
    and obj = Cvar obj
    and tag = Cvar tag in
    let meths = V.create_local "meths"
    and cached = V.create_local "cached" in
  ...
  let fun_args =
    [ obj, typ_val; tag, typ_int; cache, typ_addr ] @ List.combine (List.tl args) arity
  in
...
let tuplify_function arity return =
  ...
  let rec access_components i =
    if i >= arity
    then []
    else get_field_gen Asttypes.Mutable (Cvar arg) i (dbg ()) :: access_components (i + 1)
  in
  ...
  let _, l =
    List.fold_left_map
      (fun (non_scanned_pos, scanned_pos) c ->
        match (c : machtype_component) with
        | Int -> (non_scanned_pos + 1, scanned_pos), load Word_int non_scanned_pos
        | Float ->
          (non_scanned_pos + ints_per_float, scanned_pos), load Double non_scanned_pos
        | Float32 ->
          (* Float32 slots still take up a full word *)
  ...
  | arg_type :: args_type ->
    ...
    Clet
      ...
      , make_curry_apply
          ...
          narity
          args_type
          (read_from_closure_given_machtype arg_type (Cvar clos) arg_pos (dbg ()) :: args)
          newclos
          (n - 1) )
;;
...
let final_curry_function nlocal arity result =
  ...
  Cfunction
    { fun_name
    ; fun_args = [ VP.create last_arg, List.hd args_type; VP.create last_clos, typ_val ]
    ; fun_body =
        make_curry_apply
          result
          narity
...
let intermediate_curry_functions ~nlocal ~arity result =
  let name1 = curry_function_sym_name (Lambda.Curried { nlocal }) arity result in
  let narity = List.length arity in
  let dbg = placeholder_dbg in
  let rec loop accumulated_args remaining_args num =
    ...
    | arg_type :: remaining_args ->
      ...
      let clos = V.create_local "clos" in
      let args =
        List.init (Array.length arg_type) (fun i -> V.create_local "arg", arg_type.(i))
      in
      ...
      (Cfunction
         ...
         ; fun_body =
             Cop
               ...
               , [ alloc_closure_header
                     ...
                     (dbg ())
                 ; Cconst_symbol
                     (global_symbol (name1 ^ "_" ^ Int.to_string (num + 1)), dbg ())
                 ; Cconst_natint
                     ( pack_closure_info
       ...
       (if has_nary
        then (
          let direct_args =
            List.mapi
              (fun i ty -> V.create_local (Printf.sprintf "arg%d" (i + num + 2)), ty)
              remaining_args
          in
          let fun_args =
            List.map (fun (arg, ty) -> VP.create arg, ty) (direct_args @ [ clos, typ_val ])
          in
...
let curry_function (kind, arity, return) =
  ...
  | Lambda.Curried { nlocal } -> intermediate_curry_functions ~nlocal ~arity return
;;
...
let bbswap bi arg dbg =
  ...
  if (bi = Primitive.Pint64 && size_int = 4) || not (Proc.operation_supported op)
  then (
    let prim, tyarg =
      match (bi : Primitive.boxed_integer) with
...
let setfield_computed ptr init arg1 arg2 arg3 dbg =
  ...
  | Caml_modify_local -> return_unit dbg (addr_array_set_local arg1 arg2 arg3 dbg)
  | Caml_initialize -> return_unit dbg (addr_array_initialize arg1 arg2 arg3 dbg)
...
let emit_boxed_int32_constant_fields n cont =
  ...
  Csymbol_address (global_symbol caml_int32_ops) :: Cint32 n :: Cint32 0n :: cont
;;
...
let emit_string_constant symb s cont =
  emit_block symb (string_header (String.length s)) (emit_string_constant_fields s cont)
;;
...
let emit_nativeint_constant symb n cont =
  emit_block symb boxedintnat_header (emit_boxed_nativeint_constant_fields n cont)
;;
...
let make_symbol ?compilation_unit name =
  ...
  in
  Symbol.for_name compilation_unit name |> Symbol.linkage_name |> Linkage_name.to_string
;;
...
let entry_point namelist =
  ...
  let body =
    let dbg = dbg () in
    let incr_i =
      Cassign (VP.var id, Cop (Caddi, [ Cvar (VP.var id); Cconst_int (1, dbg) ], dbg))
    in
    ...
    Clet_mut
      ...
      , ccatch
          ...
          , create_loop
              (Csequence
                 (exit_if_last_iteration, Csequence (call (Cvar (VP.var id)), incr_i)))
              dbg
          , Ctuple []
          , dbg
          , Any
...
let frame_table namelist =
  ...
  in
  Cdata
    ((Cdefine_symbol (global_symbol "caml_frametable") :: List.map mksym namelist)
     @ [ cint_zero ])
;;
...
let predef_exception i name =
  let name_sym = { sym_name = Compilenv.new_const_symbol (); sym_global = Local } in
...
let emit_gc_roots_table ~symbols cont =
  ...
  Cdata
    ((Cdefine_symbol table_symbol :: List.map (fun s -> Csymbol_address s) symbols)
     @ [ Cint 0n ])
  :: cont
;;
...
let letin v ~defining_expr ~body =
  match body with
  | Cvar v' when Backend_var.same (Backend_var.With_provenance.var v) v' -> defining_expr
  | Cvar _
  | Cconst_int _
  | Cconst_natint _
  | Cconst_float32 _
  ...
  | Ccatch _
  | Cexit _
  | Ctrywith _ -> Clet (v, defining_expr, body)
;;
...
let trap_return arg trap_actions = Cmm.Cexit (Cmm.Return_lbl, [ arg ], trap_actions)
...
let lsl_int_caml_raw ~dbg arg1 arg2 = incr_int (lsl_int (decr_int arg1 dbg) arg2 dbg) dbg
...
let probe ~dbg ~name ~handler_code_linkage_name ~enabled_at_init ~args =
  Cop
    ( Cprobe { name; handler_code_sym = handler_code_linkage_name; enabled_at_init }
    , args
    , dbg )
;;
...
let indirect_call ~dbg ty pos alloc_mode f args_type args =
  might_split_call_caml_apply ty args_type Asttypes.Mutable f args pos alloc_mode dbg
;;
...
let bigarray_load ~dbg ~elt_kind ~elt_size ~elt_chunk ~bigarray ~index =
  ...
  let addr = array_indexing ~typ:Addr (Misc.log2 elt_size) ba_data_p index dbg in
...
let bigarray_store
  ...
  | Pbigarray_complex32 | Pbigarray_complex64 ->
    ...
    return_unit
      dbg
      (sequence
         (store ~dbg elt_chunk Assignment ~addr ~new_value:(complex_re new_value dbg))
         (store
            ~dbg
            elt_chunk
            Assignment
...
let infix_field_address ~dbg ptr n =
  if n = 0 then ptr else Cmm.Cop (Cmm.Caddv, [ ptr; int ~dbg (n * Arch.size_addr) ], dbg)
;;
  ...
  let rec cmm_arith_size0 (e : Cmm.expression) =
    ...
    | Cop
        ( ( Caddi
          ...
          | Casr )
        , l
        , _ ) -> List.fold_left ( + ) 1 (List.map cmm_arith_size0 l)
    | _ -> 0
  in
  match e with
  | Cconst_int _
...
let make_unboxed_int32_array_payload dbg unboxed_int32_list =
  ...
  *)
  if Sys.big_endian
  then Misc.fatal_error "Big-endian platforms not yet supported for unboxed arrays";
...
let allocate_unboxed_int64_or_nativeint_array
  custom_ops
  ~elements
