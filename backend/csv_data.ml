...
module Make(C : Cell) = struct
  ...
  let empty_row row_name column_names = 
    row_name, List.fold_left (fun row column_name -> 
      Row_contents.add column_name (C.empty ()) row) Row_contents.empty column_names

  let add_row t row = t.rows <- row :: t.rows
  
  let add_empty_row t row_name = t.rows <- (empty_row row_name t.column_names) :: t.rows

  let update_row (row_name, row) column_name (update_func: C.t -> C.t) =
    if Row_contents.exists (fun (key: Row_contents.key) (_value: C.t) -> 
      String.equal key column_name) row then 
  ...
  let print_row oc (row_name, row_contents) column_names =
    ...
    List.iter (fun column_name -> 
      Stdlib.output_string oc ", "; 
      Stdlib.output_string oc (C.to_string (Row_contents.find column_name row_contents))) 
    column_names;
    Stdlib.output_string oc "\n"
