...
module Make (Target : Cfg_selectgen_target_intf.S) = struct
  ...
  let select_operation0 (op : Cmm.operation) (args : Cmm.expression list)
    ...
    | Capply _ -> (
      ...
      | Cconst_symbol (func, _dbg) :: rem ->
        ( Terminator (Call (OCaml { op = Direct func; returns = label_after })),
          rem )
      | _ ->
        Terminator (Call (OCaml { op = Indirect; returns = label_after })), args
      )
  ...
  and emit_expr_op env sub_cfg bound_name op args dbg : _ Or_never_returns.t =
    ...
    | Ok (simple_args, env) -> (
      ...
      match new_op with
      | Terminator
          (Call (OCaml { op = Indirect; returns = label_after }) as term) ->
        ...
        Ok rd
      | Terminator
          (Call (OCaml { op = Direct _; returns = label_after }) as term) ->
        let* r1 = emit_tuple env sub_cfg new_args in
        let rd = Reg.createv ty in
      ...
      | Terminator
          (Call
            (External { func_symbol; ty_res; ty_args; alloc; returns; effects }))
        ->
        ...
        let label_after = Cmm.new_label () in
        let orig_returns = Option.is_some returns in
        let returns, ty_res =
          ...
          then true, Cmm.typ_int
          else Option.is_some returns, ty_res
        in
        let loc_res =
          SU.insert_op_debug' env sub_cfg
            (Call
               (External
                  { func_symbol;
                    alloc;
                    (* XXX we are ignoring the old [returns] label here *)
                    returns = (if returns then Some label_after else None);
                    effects;
                    ty_res;
                    ty_args;
  ...
  and emit_tail_apply env sub_cfg ty op args dbg =
    ...
    | Ok (simple_args, env) -> (
      ...
      | Terminator
          (Call (OCaml { op = Direct func; returns = label_after }) as term) ->
        let** r1 = emit_tuple env sub_cfg new_args in
        let rd = Reg.createv ty in
        let loc_arg, stack_ofs_args = Proc.loc_arguments (Reg.typv r1) in
        let loc_res, stack_ofs_res = Proc.loc_results_call (Reg.typv rd) in
