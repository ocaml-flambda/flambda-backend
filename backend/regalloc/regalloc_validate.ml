...
module Description : sig
  ...
  let compare_terminators ~successor_ids ~id (old_instr : terminator)
    ...
    (* CR-someday azewierzejew: Avoid using polymorphic comparison. *)
      ...
      ()
    | ( Tailcall_self { op = { destination = l1 }; original_position = _ },
        Tailcall_self { op = { destination = l2 }; original_position = _ } ) ->
      compare_label l1 l2
    ...
    (* CR-someday azewierzejew: Avoid using polymorphic comparison. *)
      when Stdlib.compare call1 call2 = 0 ->
      ()
    | ( Call { op = call1; label_after = l1; original_position = _ },
        Call { op = call2; label_after = l2; original_position = _ } )
    (* CR-someday azewierzejew: Avoid using polymorphic comparison. *)
      when Stdlib.compare call1 call2 = 0 ->
      compare_label l1 l2
