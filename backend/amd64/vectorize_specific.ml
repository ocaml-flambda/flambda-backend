...
let memory_access : Arch.specific_operation -> Memory_access.t option =
 fun op ->
    ...
    let desc =
      Memory_access.Write
        { width_in_bits = W64;
          addressing_mode;
          init_or_assign =
            (if is_assignment then Assignment else Initialization)
        }
    in
  ...
  | Ifloatarithmem (float_width, _float_op, addressing_mode) ->
    ...
    in
    let is_mutable =
      (* CR-someday gyorsh: conservative, propagate mutability of Ifloatarithmem
         from selection to make it precise. *)
      true
    in
    let desc =
      Memory_access.Read
  ...
  | Ioffset_loc (_n, addressing_mode) ->
    let desc =
      Memory_access.Read_and_write
        { width_in_bits = W64; addressing_mode; is_atomic = false }
    in
    create desc
