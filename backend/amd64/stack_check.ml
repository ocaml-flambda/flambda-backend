...
let block_preproc_stack_check_result
  : Cfg.basic_block -> frame_size:int -> Emitaux.preproc_stack_check_result * int
  = fun block ~frame_size ->
    ...
    let max_frame_size, max_instr_id =
      DLL.fold_left block.body ~init:(block.terminator.stack_offset, block.terminator.id) ~f:(fun (max_stack_frame, max_instr_id) (instr : _ Cfg.instruction) ->
...
let build_cfg_infos
  ...
  = fun cfg ->
    let frame_required =
      Proc.frame_required ~fun_contains_calls:cfg.fun_contains_calls ~fun_num_stack_slots:cfg.fun_num_stack_slots
    in
    let frame_size =
      frame_size
        ~stack_offset:0
    ...
    let init =
      { max_frame_size = 0;
        ...
        max_instr_id = 0; }
    in
    Cfg.fold_blocks cfg ~init ~f:(fun label block { max_frame_size; blocks_needing_stack_checks; max_instr_id } ->
      ...
      in
      let max_frame_size = Int.max max_frame_size preproc_stack_check_result.max_frame_size in
      let blocks_needing_stack_checks =
        if block_needs_stack_checks then
          Label.Set.add label blocks_needing_stack_checks
        else
...
(* Populates `num_checks` with the number of blocks needing a stack check in the subtree
   whose root is the associated label, and returns that value. *)
let rec num_checks_tree
  : Cfg_dominators.dominator_tree -> blocks_needing_stack_checks:Label.Set.t -> num_checks:int Label.Tbl.t -> int
  = fun tree ~blocks_needing_stack_checks ~num_checks ->
  let num_for_root : int = if Label.Set.mem tree.label blocks_needing_stack_checks then 1 else 0 in
  let num_for_children : int =
    List.fold_left (fun acc child -> acc + num_checks_tree child ~blocks_needing_stack_checks ~num_checks) 0 tree.children
  in
  let res = num_for_root + num_for_children in
  Label.Tbl.replace num_checks tree.label res;
  res

(* Determines which block should have the stack check.
   `num_checks` contains for each label the number of blocks needing the check in the
   subtree; `to_cover` is the number of blocks needing the check at the entry point.
   We recursively choose the child whose `num_checks` is equal to `to_cover`, if it
   exists. There cannot be two such children since the `num_checks` value for the
   current node is know to be `>=` the sum of the values for the children.
*)
let rec find_stack_check_block
  : Cfg_dominators.dominator_tree -> to_cover:int -> num_checks:int Label.Tbl.t -> loop_infos:Cfg_loop_infos.t Lazy.t -> Label.t
  = fun tree ~to_cover ~num_checks ~loop_infos ->
    ...
    | [candidate] ->
      ...
      in
      if candidate_is_in_a_loop then
        tree.label
      else
        find_stack_check_block candidate ~to_cover ~num_checks ~loop_infos
    | _ -> assert false

(* CR-someday xclerc for xclerc: we may want to duplicate the check in some cases, rather
   than simply pushing it down. *)
let cfg
  ...
  = fun cfg_with_layout ->
    ...
    let { max_frame_size; blocks_needing_stack_checks; max_instr_id } =
      build_cfg_infos cfg
    in
    if not (Label.Set.is_empty blocks_needing_stack_checks) then begin
      (* CR-soon xclerc for xclerc: use the dominators and loop infos from Cfg_with_infos
         (at least on some paths). *)
      ...
      List.iter
        (fun (tree : Cfg_dominators.dominator_tree) ->
           ...
           if Label.equal tree.label cfg.entry_label then begin
             let num_checks = Label.Tbl.create (Label.Tbl.length cfg.blocks) in
             let num_checks_root = num_checks_tree tree ~blocks_needing_stack_checks ~num_checks in
             ...
             | to_cover ->
               ...
               let check : Cfg.basic Cfg.instruction =
                 { desc = Stack_check {
                   ...
                   ls_order = 0;
                   (* CR xclerc for xclerc: double check `available_before` and `available_across`. *)
                   available_before = None;
                   available_across = None
                 }
               in
