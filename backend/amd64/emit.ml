...
let emit_instr ~first ~fallthrough i =
  ...
  | Lop (Const_vec256 { word0; word1; word2; word3 }) -> (
    ...
    | false ->
      ...
      I.simd vmovapd_Y_Ym256 [| mem64_rip VEC256 (emit_label lbl); res i 0 |])
  | Lop (Const_vec512 _) ->
    (* CR-soon mslater: avx512 *)
...
let begin_assembly unix =
  ...
  then (
    ...
    D.extrn "caml_call_gc_avx" NEAR;
    D.extrn "caml_call_gc_avx512" NEAR;
    D.extrn "caml_c_call" NEAR;
    D.extrn "caml_allocN" NEAR;
    D.extrn "caml_allocN_avx" NEAR;
    D.extrn "caml_allocN_avx512" NEAR;
    D.extrn "caml_alloc1" NEAR;
    D.extrn "caml_alloc1_avx" NEAR;
    D.extrn "caml_alloc1_avx512" NEAR;
    D.extrn "caml_alloc2" NEAR;
    D.extrn "caml_alloc2_avx" NEAR;
    D.extrn "caml_alloc2_avx512" NEAR;
    D.extrn "caml_alloc3" NEAR;
    D.extrn "caml_alloc3_avx" NEAR;
    D.extrn "caml_alloc3_avx512" NEAR;
    D.extrn "caml_ml_array_bound_error" NEAR;
    D.extrn "caml_ml_array_align_error" NEAR;
  ...
  cfi_endproc ();
  D.label (emit_cmm_symbol call_gc_local_sym_avx);
  cfi_startproc ();
  I.jmp (rel_plt (Cmm.global_symbol "caml_call_gc_avx"));
  cfi_endproc ();
  D.label (emit_cmm_symbol call_gc_local_sym_avx512);
  cfi_startproc ();
