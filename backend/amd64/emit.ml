...
let emit_vec128_constant ({ word0; word1 } : Cmm.vec128_bits) lbl =
  ...
  ND.define_label lbl;
  ND.float64_from_bits word0;
  ND.float64_from_bits word1
...
let emit_vec256_constant ({ word0; word1; word2; word3 } : Cmm.vec256_bits) lbl
  ...
  ND.float64_from_bits word0;
  ND.float64_from_bits word1;
  ND.float64_from_bits word2;
  ND.float64_from_bits word3
...
let emit_vec512_constant
  ...
  ND.float64_from_bits word2;
  ND.float64_from_bits word3;
  ND.float64_from_bits word4;
  ND.float64_from_bits word5;
  ND.float64_from_bits word6;
  ND.float64_from_bits word7
...
let begin_assembly unix =
  ...
  then (
    ...
    ND.extrn (S.create ~already_encoded:true "caml_call_gc_avx");
    ND.extrn (S.create ~already_encoded:true "caml_call_gc_avx512");
    ND.extrn (S.create ~already_encoded:true "caml_c_call");
    ND.extrn (S.create ~already_encoded:true "caml_allocN");
    ND.extrn (S.create ~already_encoded:true "caml_allocN_avx");
    ND.extrn (S.create ~already_encoded:true "caml_allocN_avx512");
    ND.extrn (S.create ~already_encoded:true "caml_alloc1");
    ND.extrn (S.create ~already_encoded:true "caml_alloc1_avx");
    ND.extrn (S.create ~already_encoded:true "caml_alloc1_avx512");
    ND.extrn (S.create ~already_encoded:true "caml_alloc2");
    ND.extrn (S.create ~already_encoded:true "caml_alloc2_avx");
    ND.extrn (S.create ~already_encoded:true "caml_alloc2_avx512");
    ND.extrn (S.create ~already_encoded:true "caml_alloc3");
    ND.extrn (S.create ~already_encoded:true "caml_alloc3_avx");
    ND.extrn (S.create ~already_encoded:true "caml_alloc3_avx512");
    ND.extrn (S.create ~already_encoded:true "caml_ml_array_bound_error");
    ND.extrn (S.create ~already_encoded:true "caml_ml_array_align_error");
...
let end_assembly () =
  ...
  then (
    ND.switch_to_section Thirtytwo_byte_literals;
    ND.align ~fill_x86_bin_emitter:Zero ~bytes:32;
    List.iter (fun (cst, lbl) -> emit_vec256_constant cst lbl) !vec256_constants);
  if not (Misc.Stdlib.List.is_empty !vec512_constants)
  then (
    ND.switch_to_section Sixtyfour_byte_literals;
    ND.align ~fill_x86_bin_emitter:Zero ~bytes:64;
