...
(* CR gyorsh: Add [t] per analysis when at least one more analysis is implemented *)
type t =
  {
...
let set_value (t:t) s (v:value) =
  let f new_ old =
    if not (Option.is_none old) then
      Misc.fatal_errorf "Value of %s is already set" s;
    new_
  in
