  ...
  object (self : 'self)
    ...
    method mark_instr : Mach.instruction_desc -> unit =
      ...
      | Iop (Icall_ind _ | Icall_imm _ | Iextcall _ | Iprobe _) ->
        self#mark_call
      | Iop (Itailcall_ind _ | Itailcall_imm _) -> self#mark_tailcall
    ...
    method select_operation (op : Cmm.operation) (args : Cmm.expression list)
      ...
      let open Mach in
      match op, args with
      | Capply (_, _, original_position), Cconst_symbol (func, _dbg) :: rem ->
        Icall_imm { func; original_position }, rem
      | Capply (_, _, original_position), _ ->
        Icall_ind { original_position }, args
      ...
      | Cop ((Capply (ty, Rc_normal, _) as op), args, dbg) -> (
        ...
        | Some (simple_args, env) -> (
          ...
          | Icall_ind { original_position } ->
            ...
            then (
              let call = Mach.Iop (Itailcall_ind { original_position }) in
          ...
          | Icall_imm { func; original_position } ->
            ...
            if stack_ofs = 0 && Select_utils.trap_stack_is_empty env
            then (
              let call = Mach.Iop (Itailcall_imm { func; original_position }) in
            ...
            then (
              ...
              self#insert env
                (Mach.Ireturn (Select_utils.pop_all_traps env))
                loc_res [||])
          | _ -> Misc.fatal_error "Selection.emit_original_position"))
      | Csequence (e1, e2) -> (
        match self#emit_expr env e1 ~bound_name:None with
        | None -> ()
