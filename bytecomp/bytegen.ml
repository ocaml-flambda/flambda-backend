...
let preserve_tailcall_for_prim = function
  ...
  | Parray_element_size_in_bytes _ | Pidxmixedfield _
...
let comp_primitive stack_info p sz args =
  ...
  | Pidxmixedfield _ -> Misc.fatal_error "unimplemented"
  ...
  | Pmixedfield ([n], _, _sem) ->
      ...
      *)
      Kgetfield n
  | Pmixedfield (_, _, _sem) -> assert false
  | Psetmixedfield ([n], _shape, _init) ->
      (* See the comment in the [Pmixedfield] case. *)
      Ksetfield n
  | Psetmixedfield (_, _shape, _init) -> assert false
