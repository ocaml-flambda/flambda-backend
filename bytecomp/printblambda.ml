...
let primitive ppf = function
  ...
  | Intcomp cmp -> fprintf ppf "intcomp %a" Printlambda.integer_comparison cmp
  | Isout -> pp_print_string ppf "isout"
...
let rec blambda ppf = function
  ...
  | Staticraise ({ static_label = id }, args) ->
    fprintf ppf "@[<2>(staticraise %d@ %a)@]" id
      (pp_print_list ~pp_sep:pp_print_space blambda)
      args
  | Staticcatch { id = { static_label = id }; args; handler; body } ->
