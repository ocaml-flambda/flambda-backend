...
[%%expect{|
type _ t = Int : int t
val to_int : 'a t -> 'a -> int @@ global many = <fun>
|}];;

let w_bool : bool t = Obj.magic 0;;
let f_bool (x : bool) : int = let Int = w_bool in x;; (* fail *)
[%%expect{|
val w_bool : bool t @@ global many portable = Int
...
[%%expect{|
val w_buffer : Buffer.t t @@ global many portable = Int
val f_buffer : Buffer.t -> int @@ global many = <fun>
|}];;

let w_spec : Arg.spec t = Obj.magic 0;;
let f_spec (x : Arg.spec) : int = let Int = w_spec in x;; (* fail *)
[%%expect{|
val w_spec : Arg.spec t @@ global many portable = Int
...
[%%expect{|
module M : sig type u val w : u t val x : u end
val m_x : int @@ global many portable = 33
|}];;
...
[%%expect{|
module F :
  functor (X : sig type u = int val x : u end) ->
    sig val x : int @@ global many portable end
val fm_x : int @@ global many portable = 33
|}];;
...
module F' :
  functor (X : sig module M : sig type u = int val x : u end end) ->
    sig val x : int @@ global many portable end
val fm'_x : int @@ global many portable = 33
|}];;
...
module F : functor (M : S) -> sig val zero : M.t @@ global many portable end
|}];;
