...
let f ~x:_ ~y:_ ~z:_ = ();;
[%%expect{|
val f : x:'a -> y:'b -> z:'c -> unit @@ global many = <fun>
|}];;
...
let rec y = f ~y;;
[%%expect{|
val y : x:'a -> z:'b -> unit @@ global many = <fun>
|}];;
...
[%%expect{|
val g : omitted:'a -> given:(omitted:'a -> 'b) -> given:'c -> 'b @@ global
  many = <fun>
val f : omitted:'a -> given:int -> 'b @@ global many = <fun>
|}];;
...
val g :
  omitted_g:'a ->
  given:(omitted_f:unit -> 'b) -> omitted_f:'c -> given:'d -> 'b @@ global
  many = <fun>
