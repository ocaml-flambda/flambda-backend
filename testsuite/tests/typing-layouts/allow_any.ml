...
type ('a, 'b) arity_2 : immutable_data with ('b * 'a * 'b option) list  = { x : 'a }
[@@unsafe_allow_any_mode_crossing]

type ('a, 'b) bad_reexport_2 : immutable_data with ('a * 'b * 'a option) list = ('a, 'b) arity_2 = { x : 'a }
[@@unsafe_allow_any_mode_crossing]
(* CR aspsmith: this is wrong! this should be rejected *)
[%%expect{|
type ('a, 'b) arity_2
  : immutable_data
  with ('b * 'a * 'b option) list = {
  x : 'a;
} [@@unsafe_allow_any_mode_crossing]
type ('a, 'b) bad_reexport_2 =
  ('a, 'b) arity_2
  : immutable_data
  with ('a * 'b * 'a option) list = {
  x : 'a;
} [@@unsafe_allow_any_mode_crossing]
|}]
