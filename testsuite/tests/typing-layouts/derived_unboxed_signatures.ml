...
[%%expect{|
module type S = sig type 'a t = { i : int; } end
|}]
...
[%%expect {|
module type Shadowed = sig type 'a t end
|}]

(* ...so we can't reference the derived type... *)
module M(Shadowed : Shadowed) = struct
  type 'a t = 'a Shadowed.t#
end
[%%expect{|
Line 2, characters 17-28:
2 |   type 'a t = 'a Shadowed.t#
                     ^^^^^^^^^^^
Error: "Shadowed.t" has no unboxed version.
|}]
...
[%%expect{|
module M : functor (Shadowed : Shadowed) -> sig val x : r/2# end
|}]
...
struct
  let f : 'a t -> 'a M.t = fun x -> x
  let f : 'a t# -> 'a M.t# = fun x -> x
end
[%%expect{|
type 'a t = { i : int; j : int; }
module Check_constraints :
  functor (M : sig type 'a t = 'a t = { i : int; j : int; } end) ->
    sig val f : 'a t# -> 'a M.t# end
|}]
...
[%%expect{|
Lines 1-4, characters 16-23:
1 | ................sig
2 |   type 'a u = 'a t
3 |   and 'a t
4 | end with type t = float
Error: In this "with" constraint, the new definition of "t"
       does not match its original definition in the constrained signature:
       Type declarations do not match:
         type t = float
...
Error: In this "with" constraint, the new definition of "t"
       ...
       is not included in
         type t = float
       The type "float/1#" = "float/2#" is not equal to the type
         "float/1" = "float/2"
