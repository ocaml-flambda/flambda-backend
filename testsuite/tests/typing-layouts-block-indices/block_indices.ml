...
let () =
  ...
  let x = get_idx_mut r (.r.#f) in
  Printf.printf "%f\n" x;
  set_idx_mut r (.r.#f) 2.0;
  Printf.printf "%f\n" r.r.#f;
  print_newline ()
...
let print_t_b t =
  let #{ i = bi; a = #{ s; i }; s = bs } = get_idx_mut t (.b) in
...
let () =
  ...
  set_idx_mut t deeper_idx #{ s = "aaa"; i = #200L };
  print_t_b t;
  print_newline ();
  ()
...
let () =
  ...
  for i = 0 to 9 do
    ...
    let x = get_idx_mut a idx in
    Printf.printf "%f\n" (Float_u.to_float (Float32_u.to_float x))
  done;
  print_endline "\nWriting to a float32# array";
  for i = 0 to 9 do
    let idx : (_, float32#) idx_mut = (.(i)) in
    set_idx_mut a idx (Float32_u.of_float (Float_u.of_int (i + 10)))
  done;
  for i = 0 to 9 do
...
let () =
  ...
  let s = get_idx_mut a (.(3)) in
  print_endline s;
  let s = get_idx_mut a (.L(#3L)) in
  print_endline s;
  let s = get_idx_mut a (.l(#3l)) in
  print_endline s;
  let s = get_idx_mut a (.n(#3n)) in
  print_endline s;
  print_newline ()
...
let () =
  ...
  let #{ i; j } = get_idx_mut a (.(3)) in
  let i2 = get_idx_mut a (.(3).#i) in
  let j2 = get_idx_mut a (.(3).#j) in
  Printf.printf "%d %d %d %d\n" i i2 j j2;
  let #{ i; j } = get_idx_mut a (.L(#3L)) in
  let i2 = get_idx_mut a (.L(#3L).#i) in
  let j2 = get_idx_mut a (.L(#3L).#j) in
  Printf.printf "%d %d %d %d\n" i i2 j j2;
  let #{ i; j } = get_idx_mut a (.l(#3l)) in
  let i2 = get_idx_mut a (.l(#3l).#i) in
  let j2 = get_idx_mut a (.l(#3l).#j) in
  Printf.printf "%d %d %d %d\n" i i2 j j2;
  let #{ i; j } = get_idx_mut a (.n(#3n)) in
  let i2 = get_idx_mut a (.n(#3n).#i) in
  let j2 = get_idx_mut a (.n(#3n).#j) in
  Printf.printf "%d %d %d %d\n" i i2 j j2;
  print_newline ()
