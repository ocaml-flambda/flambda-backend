...
(* If copying this test for a new shape, you should only have to
   ...
   test directory. *)
type boxed_t = float
type unboxed_t = float#

let elem : boxed_t elem = float_elem
let words_wide : int = 1
let zero () : unboxed_t = #0.

let to_boxed a = Float_u.to_float a
let of_boxed a = Float_u.of_float a
...
module Float_u_array = Gen_u_array.Make (Float_u_array0)

module Float_u_array_boxed = Test_gen_u_array.Make_boxed (struct
    module M = Float_u_array
...
module _ = Test_gen_u_array.Test (Float_u_array_boxed)
