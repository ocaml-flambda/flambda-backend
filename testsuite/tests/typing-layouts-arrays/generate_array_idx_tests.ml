...
let test_makearray_dynamic ~uninit ~local ty =
  ...
  for_i_below_size ~debug_exprs (fun ~debug_exprs ->
    line "set a i (mk_value (i + 1));"
  );
  line "Gc.compact ();";
  for_i_below_size ~debug_exprs (fun ~debug_exprs ->
    seq_assert ~debug_exprs "eq (get a i) (mk_value (i + 1))"
  );
  line "(* Also read back those values with block indices *)";
  for_i_below_size ~debug_exprs (fun ~debug_exprs ->
    seq_assert ~debug_exprs "eq (get_idx_mut a (.(i))) (mk_value (i + 1))"
  );
  line "(* And set values with block indices *)";
  for_i_below_size ~debug_exprs (fun ~debug_exprs ->
  ...
  line "Gc.compact ();";
  line "(* 6. Array contents were unaffected by setting bad indices *)";
  for_i_below_size ~debug_exprs (fun ~debug_exprs ->
    seq_assert ~debug_exprs "eq (get a i) (mk_value i)"
  );
