...
module Ty : sig
  ...
  let assemble colon_or_eq fields vals =
      let labeled_fields =
        List.map2 fields vals ~f:(fun (s, _) x -> s ^ " " ^ colon_or_eq ^ " " ^ x)
      in
      "#{ " ^ String.concat ~sep:"; " labeled_fields ^ " }"

  let assemble_expr name fields vals  = "(" ^ assemble "=" fields vals ^ " : " ^ name ^ ")"

  let pat name fields i =
    assemble_expr name fields (List.map fields ~f:(fun (s, _) -> s ^ Int.to_string i))
      end

  module Gen_tuple = struct
    let assemble ~unboxed ~sep xs =
  ...
  let rec ty_code = function
    ...
    | Unboxed_tuple ts ->
      Gen_tuple.assemble ~unboxed:true ~sep:" * " (List.map ts ~f:ty_code)
    | Non_product { ty_code; _ } -> ty_code

  let rec value_code = function
    | Unboxed_record { name; fields } -> fun i ->
      Unboxed_record.assemble_expr name fields (List.map fields ~f:(fun (_, t) -> value_code t i))
    | Unboxed_tuple ts -> fun i ->
      Gen_tuple.assemble ~unboxed:true ~sep:", " (List.map ts ~f:(fun t -> value_code t i))
    | Non_product { value_code; _ } -> value_code

  let rec mk_value_code t : string =
    match t with
    | Unboxed_record { name; fields } ->
      Unboxed_record.assemble_expr name fields
        (List.map fields ~f:(fun (_, t) -> mk_value_code t))
    | Unboxed_tuple ts ->
      Gen_tuple.assemble ~unboxed:true ~sep:", " (List.map ts ~f:mk_value_code)
  ...
  let rec eq = function
    | Unboxed_record { name; fields } ->
      let body =
        ...
        |> String.concat ~sep:" && "
      in
      let pat = Unboxed_record.pat name fields in
      sprintf "(fun %s %s -> %s)" (pat 1) (pat 2) body
    | Unboxed_tuple ts ->
      let pat s =
        Gen_tuple.assemble ~unboxed:true ~sep:", " (List.mapi ts ~f:(fun i _ -> s ^ Int.to_string i))
      in
      let body =
  ...
  let unboxed_record name fields =
    ...
    add_decl ~name ~def:(Unboxed_record.assemble ":" fields xs);
    Unboxed_record { name; fields }
...
let test_makearray_dynamic ~uninit ~local ty =
  ...
  line "let a : %s = %s size %s in" ty_array_s makearray_dynamic (Ty.value_code ty 0);
 ...
 include stdlib_upstream_compatible;|};
  ...
  toplevel_unit_block (fun () ->
    ...
    List.iter always_ignored_types ~f:(fun ty ->
      line "let _ = (makearray_dynamic_uninit 1 : %s array) in" (Ty.ty_code ty)
    );
  );
