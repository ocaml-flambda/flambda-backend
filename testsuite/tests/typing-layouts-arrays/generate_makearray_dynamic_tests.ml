...
let test_makearray_dynamic ~uninit ~local ty =
  ...
  (if uninit then (
     ...
     line "let a : %s = %s size (Sys.opaque_identity %s) in"
       ty_array_s makearray_dynamic (ty.Ty.value_code 0);
  ...
  iter "bad_indices size" "i" ~debug_exprs (fun ~debug_exprs ->
    ...
    line "let raises =";
    with_indent (fun () ->
      line "match set a i (Sys.opaque_identity %s) with" (ty.Ty.value_code 0);
      line "| exception Invalid_argument _ -> true";
      line "| _ -> false"
    );
    line "in";
  ...
  iter "blit_offsets size" "ofs1" ~debug_exprs (fun ~debug_exprs ->
    iter "blit_offsets size" "ofs2" ~debug_exprs (fun ~debug_exprs ->
      ...
      iter lens "len" ~debug_exprs (fun ~debug_exprs ->
        ...
        line "(* Reset array *)";
        for_i_below_size ~debug_exprs (fun ~debug_exprs ->
          line "set a i (Sys.opaque_identity (mk_value i));"
        )
      );
    );
  );
    ...
    iter "sizes" "size2" ~debug_exprs (fun ~debug_exprs ->
      iter "blit_offsets size" "ofs1" ~debug_exprs (fun ~debug_exprs ->
        iter "blit_offsets size2" "ofs2" ~debug_exprs (fun ~debug_exprs ->
          ...
          iter lens "len" ~debug_exprs (fun ~debug_exprs ->
            (if to_local then
               line "let local_ a2 = makearray_dynamic_local size2 (Sys.opaque_identity %s) in"
                 (ty.Ty.value_code 0)
            else
              line "let a2 = makearray_dynamic size2 (Sys.opaque_identity %s )in"
                (ty.Ty.value_code 0));
...
type compilation_settings = Native of string | Bytecode

let main ~compilation_settings =
  let debug_exprs = [] in
  line {|(* TEST
 include stdlib_stable;
 include stdlib_upstream_compatible;|};
  begin match compilation_settings with
  | Bytecode ->
...
let () =
  let compilation_settings =
    match Sys.argv with
    | [| _; "native-o3" |] -> Native "[@@@ocaml.flambda_o3]"
    | [| _; "native-oclassic" |] -> Native "[@@@ocaml.flambda_oclassic]"
    | [| _; "byte" |] -> Bytecode
    | _ -> failwith (sprintf "Usage %s <byte|native-o3|native-oclassic>" Sys.argv.(0))
      in
  main ~compilation_settings
