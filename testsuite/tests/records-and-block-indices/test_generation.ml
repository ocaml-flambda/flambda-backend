...
let test_record_size ty ~local:_ =
  (* CR rtjoa: implement *)
  type_section ty
...
let main test ~bytecode =
 ...
 include stdlib_upstream_compatible;|};
  ...
  line {|*)|};
  (* CR rtjoa: update filename *)
  line "(** This is code generated by [generate_block_tests.ml]. *)";
  line "";
  line {|[@@@warning "-23"]|};
  ...
  begin
    ...
    | Array_idx_access { local } ->
      line "let test size =";
      with_indent (fun () ->
          List.iter types ~f:(test_array_idx_get_and_set ~local);
          line "()"
      );
      print_endline ";;";
      print_newline ();
    ...
    | Record_idx_access { local } ->
      toplevel_unit_block (fun () ->
          List.iter types ~f:(test_record_idx_get_and_set ~local)
      )
    | Record_idx_deepening ->
      toplevel_unit_block (fun () ->
    ...
    | Record_size { local } ->
      toplevel_unit_block (fun () ->
          List.iter types ~f:(test_record_size ~local)
      )
  end;
...
let tests =
  [ "array_idx_access", Array_idx_access { local = false };
    ...
    "record_size", Record_size { local = false };
    "record_size_local", Record_size { local = true }
  ]
