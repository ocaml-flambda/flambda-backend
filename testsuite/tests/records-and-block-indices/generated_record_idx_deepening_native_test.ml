...
let () =
  ...
  (* Deepening to (.b95.#a18) *)
  let idx : (t95, _) idx_mut = (.b95.#a18) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b95) *)
    let shallow : (t95, _) idx_mut = (.b95) in
    let deepened = (.idx_mut(shallow).#a18) in
    ...
    (* from (.b95.#a18) *)
    let shallow : (t95, _) idx_mut = (.b95.#a18) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 246;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b96.#a20) *)
  let idx : (t96, _) idx_mut = (.b96.#a20) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b96) *)
    let shallow : (t96, _) idx_mut = (.b96) in
    let deepened = (.idx_mut(shallow).#a20) in
    ...
    (* from (.b96.#a20) *)
    let shallow : (t96, _) idx_mut = (.b96.#a20) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 250;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 250 failed %d" i;
  );
  (* Deepening to (.b96.#b20) *)
  let idx : (t96, _) idx_mut = (.b96.#b20) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b96) *)
    let shallow : (t96, _) idx_mut = (.b96) in
    let deepened = (.idx_mut(shallow).#b20) in
    ...
    (* from (.b96.#b20) *)
    let shallow : (t96, _) idx_mut = (.b96.#b20) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 252;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b97.#a24) *)
  let idx : (t97, _) idx_mut = (.b97.#a24) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b97) *)
    let shallow : (t97, _) idx_mut = (.b97) in
    let deepened = (.idx_mut(shallow).#a24) in
    ...
    (* from (.b97.#a24) *)
    let shallow : (t97, _) idx_mut = (.b97.#a24) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 256;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 256 failed %d" i;
  );
  (* Deepening to (.b97.#b24) *)
  let idx : (t97, _) idx_mut = (.b97.#b24) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b97) *)
    let shallow : (t97, _) idx_mut = (.b97) in
    let deepened = (.idx_mut(shallow).#b24) in
    ...
    (* from (.b97.#b24) *)
    let shallow : (t97, _) idx_mut = (.b97.#b24) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 258;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b98.#a34) *)
  let idx : (t98, _) idx_mut = (.b98.#a34) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* Note: can't deepen (.b98) because it's a path to a flattened
       float, making its element type [float#] *)
    (* from (.b98.#a34) *)
    let shallow : (t98, _) idx_mut = (.b98.#a34) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 261;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b99.#a36) *)
  let idx : (t99, _) idx_mut = (.b99.#a36) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b99) *)
    let shallow : (t99, _) idx_mut = (.b99) in
    let deepened = (.idx_mut(shallow).#a36) in
    ...
    (* from (.b99.#a36) *)
    let shallow : (t99, _) idx_mut = (.b99.#a36) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 265;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 265 failed %d" i;
  );
  (* Deepening to (.b99.#b36) *)
  let idx : (t99, _) idx_mut = (.b99.#b36) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b99) *)
    let shallow : (t99, _) idx_mut = (.b99) in
    let deepened = (.idx_mut(shallow).#b36) in
    ...
    (* from (.b99.#b36) *)
    let shallow : (t99, _) idx_mut = (.b99.#b36) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 267;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b100.#a38) *)
  let idx : (t100, _) idx_mut = (.b100.#a38) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b100) *)
    let shallow : (t100, _) idx_mut = (.b100) in
    let deepened = (.idx_mut(shallow).#a38) in
    ...
    (* from (.b100.#a38) *)
    let shallow : (t100, _) idx_mut = (.b100.#a38) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 271;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 271 failed %d" i;
  );
  (* Deepening to (.b100.#b38) *)
  let idx : (t100, _) idx_mut = (.b100.#b38) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b100) *)
    let shallow : (t100, _) idx_mut = (.b100) in
    let deepened = (.idx_mut(shallow).#b38) in
    ...
    (* from (.b100.#b38) *)
    let shallow : (t100, _) idx_mut = (.b100.#b38) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 273;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b108.#a18) *)
  let idx : (t108, _) idx_mut = (.b108.#a18) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b108) *)
    let shallow : (t108, _) idx_mut = (.b108) in
    let deepened = (.idx_mut(shallow).#a18) in
    ...
    (* from (.b108.#a18) *)
    let shallow : (t108, _) idx_mut = (.b108.#a18) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 294;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b109.#a20) *)
  let idx : (t109, _) idx_mut = (.b109.#a20) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b109) *)
    let shallow : (t109, _) idx_mut = (.b109) in
    let deepened = (.idx_mut(shallow).#a20) in
    ...
    (* from (.b109.#a20) *)
    let shallow : (t109, _) idx_mut = (.b109.#a20) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 298;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 298 failed %d" i;
  );
  (* Deepening to (.b109.#b20) *)
  let idx : (t109, _) idx_mut = (.b109.#b20) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b109) *)
    let shallow : (t109, _) idx_mut = (.b109) in
    let deepened = (.idx_mut(shallow).#b20) in
    ...
    (* from (.b109.#b20) *)
    let shallow : (t109, _) idx_mut = (.b109.#b20) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 300;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b110.#a26) *)
  let idx : (t110, _) idx_mut = (.b110.#a26) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b110) *)
    let shallow : (t110, _) idx_mut = (.b110) in
    let deepened = (.idx_mut(shallow).#a26) in
    ...
    (* from (.b110.#a26) *)
    let shallow : (t110, _) idx_mut = (.b110.#a26) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 304;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 304 failed %d" i;
  );
  (* Deepening to (.b110.#b26) *)
  let idx : (t110, _) idx_mut = (.b110.#b26) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b110) *)
    let shallow : (t110, _) idx_mut = (.b110) in
    let deepened = (.idx_mut(shallow).#b26) in
    ...
    (* from (.b110.#b26) *)
    let shallow : (t110, _) idx_mut = (.b110.#b26) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 306;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b113.#a40) *)
  let idx : (t113, _) idx_mut = (.b113.#a40) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b113) *)
    let shallow : (t113, _) idx_mut = (.b113) in
    let deepened = (.idx_mut(shallow).#a40) in
    ...
    (* from (.b113.#a40) *)
    let shallow : (t113, _) idx_mut = (.b113.#a40) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 316;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b114.#a42) *)
  let idx : (t114, _) idx_mut = (.b114.#a42) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b114) *)
    let shallow : (t114, _) idx_mut = (.b114) in
    let deepened = (.idx_mut(shallow).#a42) in
    ...
    (* from (.b114.#a42) *)
    let shallow : (t114, _) idx_mut = (.b114.#a42) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 320;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 320 failed %d" i;
  );
  (* Deepening to (.b114.#b42) *)
  let idx : (t114, _) idx_mut = (.b114.#b42) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b114) *)
    let shallow : (t114, _) idx_mut = (.b114) in
    let deepened = (.idx_mut(shallow).#b42) in
    ...
    (* from (.b114.#b42) *)
    let shallow : (t114, _) idx_mut = (.b114.#b42) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 322;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b115.#a44) *)
  let idx : (t115, _) idx_mut = (.b115.#a44) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b115) *)
    let shallow : (t115, _) idx_mut = (.b115) in
    let deepened = (.idx_mut(shallow).#a44) in
    ...
    (* from (.b115.#a44) *)
    let shallow : (t115, _) idx_mut = (.b115.#a44) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 326;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 326 failed %d" i;
  );
  (* Deepening to (.b115.#b44) *)
  let idx : (t115, _) idx_mut = (.b115.#b44) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b115) *)
    let shallow : (t115, _) idx_mut = (.b115) in
    let deepened = (.idx_mut(shallow).#b44) in
    ...
    (* from (.b115.#b44) *)
    let shallow : (t115, _) idx_mut = (.b115.#b44) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 328;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a116.#a18) *)
  let idx : (t116, _) idx_mut = (.a116.#a18) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a116) *)
    let shallow : (t116, _) idx_mut = (.a116) in
    let deepened = (.idx_mut(shallow).#a18) in
    ...
    (* from (.a116.#a18) *)
    let shallow : (t116, _) idx_mut = (.a116.#a18) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 331;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a117.#a18) *)
  let idx : (t117, _) idx_mut = (.a117.#a18) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a117) *)
    let shallow : (t117, _) idx_mut = (.a117) in
    let deepened = (.idx_mut(shallow).#a18) in
    ...
    (* from (.a117.#a18) *)
    let shallow : (t117, _) idx_mut = (.a117.#a18) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 334;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a118.#a18) *)
  let idx : (t118, _) idx_mut = (.a118.#a18) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a118) *)
    let shallow : (t118, _) idx_mut = (.a118) in
    let deepened = (.idx_mut(shallow).#a18) in
    ...
    (* from (.a118.#a18) *)
    let shallow : (t118, _) idx_mut = (.a118.#a18) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 338;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a119.#a18) *)
  let idx : (t119, _) idx_mut = (.a119.#a18) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a119) *)
    let shallow : (t119, _) idx_mut = (.a119) in
    let deepened = (.idx_mut(shallow).#a18) in
    ...
    (* from (.a119.#a18) *)
    let shallow : (t119, _) idx_mut = (.a119.#a18) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 342;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a120.#a18) *)
  let idx : (t120, _) idx_mut = (.a120.#a18) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a120) *)
    let shallow : (t120, _) idx_mut = (.a120) in
    let deepened = (.idx_mut(shallow).#a18) in
    ...
    (* from (.a120.#a18) *)
    let shallow : (t120, _) idx_mut = (.a120.#a18) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 346;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a121.#a20) *)
  let idx : (t121, _) idx_mut = (.a121.#a20) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a121) *)
    let shallow : (t121, _) idx_mut = (.a121) in
    let deepened = (.idx_mut(shallow).#a20) in
    ...
    (* from (.a121.#a20) *)
    let shallow : (t121, _) idx_mut = (.a121.#a20) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 350;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 350 failed %d" i;
  );
  (* Deepening to (.a121.#b20) *)
  let idx : (t121, _) idx_mut = (.a121.#b20) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a121) *)
    let shallow : (t121, _) idx_mut = (.a121) in
    let deepened = (.idx_mut(shallow).#b20) in
    ...
    (* from (.a121.#b20) *)
    let shallow : (t121, _) idx_mut = (.a121.#b20) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 352;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a122.#a20) *)
  let idx : (t122, _) idx_mut = (.a122.#a20) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a122) *)
    let shallow : (t122, _) idx_mut = (.a122) in
    let deepened = (.idx_mut(shallow).#a20) in
    ...
    (* from (.a122.#a20) *)
    let shallow : (t122, _) idx_mut = (.a122.#a20) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 355;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 355 failed %d" i;
  );
  (* Deepening to (.a122.#b20) *)
  let idx : (t122, _) idx_mut = (.a122.#b20) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a122) *)
    let shallow : (t122, _) idx_mut = (.a122) in
    let deepened = (.idx_mut(shallow).#b20) in
    ...
    (* from (.a122.#b20) *)
    let shallow : (t122, _) idx_mut = (.a122.#b20) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 357;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a123.#a20) *)
  let idx : (t123, _) idx_mut = (.a123.#a20) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a123) *)
    let shallow : (t123, _) idx_mut = (.a123) in
    let deepened = (.idx_mut(shallow).#a20) in
    ...
    (* from (.a123.#a20) *)
    let shallow : (t123, _) idx_mut = (.a123.#a20) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 361;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 361 failed %d" i;
  );
  (* Deepening to (.a123.#b20) *)
  let idx : (t123, _) idx_mut = (.a123.#b20) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a123) *)
    let shallow : (t123, _) idx_mut = (.a123) in
    let deepened = (.idx_mut(shallow).#b20) in
    ...
    (* from (.a123.#b20) *)
    let shallow : (t123, _) idx_mut = (.a123.#b20) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 363;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a124.#a20) *)
  let idx : (t124, _) idx_mut = (.a124.#a20) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a124) *)
    let shallow : (t124, _) idx_mut = (.a124) in
    let deepened = (.idx_mut(shallow).#a20) in
    ...
    (* from (.a124.#a20) *)
    let shallow : (t124, _) idx_mut = (.a124.#a20) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 367;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 367 failed %d" i;
  );
  (* Deepening to (.a124.#b20) *)
  let idx : (t124, _) idx_mut = (.a124.#b20) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a124) *)
    let shallow : (t124, _) idx_mut = (.a124) in
    let deepened = (.idx_mut(shallow).#b20) in
    ...
    (* from (.a124.#b20) *)
    let shallow : (t124, _) idx_mut = (.a124.#b20) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 369;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a126.#a22) *)
  let idx : (t126, _) idx_mut = (.a126.#a22) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a126) *)
    let shallow : (t126, _) idx_mut = (.a126) in
    let deepened = (.idx_mut(shallow).#a22) in
    ...
    (* from (.a126.#a22) *)
    let shallow : (t126, _) idx_mut = (.a126.#a22) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 379;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 379 failed %d" i;
  );
  (* Deepening to (.a126.#b22) *)
  let idx : (t126, _) idx_mut = (.a126.#b22) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a126) *)
    let shallow : (t126, _) idx_mut = (.a126) in
    let deepened = (.idx_mut(shallow).#b22) in
    ...
    (* from (.a126.#b22) *)
    let shallow : (t126, _) idx_mut = (.a126.#b22) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 381;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a127.#a22) *)
  let idx : (t127, _) idx_mut = (.a127.#a22) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a127) *)
    let shallow : (t127, _) idx_mut = (.a127) in
    let deepened = (.idx_mut(shallow).#a22) in
    ...
    (* from (.a127.#a22) *)
    let shallow : (t127, _) idx_mut = (.a127.#a22) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 384;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 384 failed %d" i;
  );
  (* Deepening to (.a127.#b22) *)
  let idx : (t127, _) idx_mut = (.a127.#b22) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a127) *)
    let shallow : (t127, _) idx_mut = (.a127) in
    let deepened = (.idx_mut(shallow).#b22) in
    ...
    (* from (.a127.#b22) *)
    let shallow : (t127, _) idx_mut = (.a127.#b22) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 386;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a128.#a22) *)
  let idx : (t128, _) idx_mut = (.a128.#a22) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a128) *)
    let shallow : (t128, _) idx_mut = (.a128) in
    let deepened = (.idx_mut(shallow).#a22) in
    ...
    (* from (.a128.#a22) *)
    let shallow : (t128, _) idx_mut = (.a128.#a22) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 390;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 390 failed %d" i;
  );
  (* Deepening to (.a128.#b22) *)
  let idx : (t128, _) idx_mut = (.a128.#b22) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a128) *)
    let shallow : (t128, _) idx_mut = (.a128) in
    let deepened = (.idx_mut(shallow).#b22) in
    ...
    (* from (.a128.#b22) *)
    let shallow : (t128, _) idx_mut = (.a128.#b22) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 392;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a129.#a24) *)
  let idx : (t129, _) idx_mut = (.a129.#a24) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a129) *)
    let shallow : (t129, _) idx_mut = (.a129) in
    let deepened = (.idx_mut(shallow).#a24) in
    ...
    (* from (.a129.#a24) *)
    let shallow : (t129, _) idx_mut = (.a129.#a24) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 396;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 396 failed %d" i;
  );
  (* Deepening to (.a129.#b24) *)
  let idx : (t129, _) idx_mut = (.a129.#b24) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a129) *)
    let shallow : (t129, _) idx_mut = (.a129) in
    let deepened = (.idx_mut(shallow).#b24) in
    ...
    (* from (.a129.#b24) *)
    let shallow : (t129, _) idx_mut = (.a129.#b24) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 398;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a130.#a24) *)
  let idx : (t130, _) idx_mut = (.a130.#a24) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a130) *)
    let shallow : (t130, _) idx_mut = (.a130) in
    let deepened = (.idx_mut(shallow).#a24) in
    ...
    (* from (.a130.#a24) *)
    let shallow : (t130, _) idx_mut = (.a130.#a24) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 401;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 401 failed %d" i;
  );
  (* Deepening to (.a130.#b24) *)
  let idx : (t130, _) idx_mut = (.a130.#b24) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a130) *)
    let shallow : (t130, _) idx_mut = (.a130) in
    let deepened = (.idx_mut(shallow).#b24) in
    ...
    (* from (.a130.#b24) *)
    let shallow : (t130, _) idx_mut = (.a130.#b24) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 403;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a131.#a24) *)
  let idx : (t131, _) idx_mut = (.a131.#a24) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a131) *)
    let shallow : (t131, _) idx_mut = (.a131) in
    let deepened = (.idx_mut(shallow).#a24) in
    ...
    (* from (.a131.#a24) *)
    let shallow : (t131, _) idx_mut = (.a131.#a24) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 407;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 407 failed %d" i;
  );
  (* Deepening to (.a131.#b24) *)
  let idx : (t131, _) idx_mut = (.a131.#b24) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a131) *)
    let shallow : (t131, _) idx_mut = (.a131) in
    let deepened = (.idx_mut(shallow).#b24) in
    ...
    (* from (.a131.#b24) *)
    let shallow : (t131, _) idx_mut = (.a131.#b24) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 409;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a132.#a26) *)
  let idx : (t132, _) idx_mut = (.a132.#a26) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a132) *)
    let shallow : (t132, _) idx_mut = (.a132) in
    let deepened = (.idx_mut(shallow).#a26) in
    ...
    (* from (.a132.#a26) *)
    let shallow : (t132, _) idx_mut = (.a132.#a26) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 413;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 413 failed %d" i;
  );
  (* Deepening to (.a132.#b26) *)
  let idx : (t132, _) idx_mut = (.a132.#b26) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a132) *)
    let shallow : (t132, _) idx_mut = (.a132) in
    let deepened = (.idx_mut(shallow).#b26) in
    ...
    (* from (.a132.#b26) *)
    let shallow : (t132, _) idx_mut = (.a132.#b26) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 415;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a133.#a26) *)
  let idx : (t133, _) idx_mut = (.a133.#a26) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a133) *)
    let shallow : (t133, _) idx_mut = (.a133) in
    let deepened = (.idx_mut(shallow).#a26) in
    ...
    (* from (.a133.#a26) *)
    let shallow : (t133, _) idx_mut = (.a133.#a26) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 418;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 418 failed %d" i;
  );
  (* Deepening to (.a133.#b26) *)
  let idx : (t133, _) idx_mut = (.a133.#b26) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a133) *)
    let shallow : (t133, _) idx_mut = (.a133) in
    let deepened = (.idx_mut(shallow).#b26) in
    ...
    (* from (.a133.#b26) *)
    let shallow : (t133, _) idx_mut = (.a133.#b26) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 420;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a134.#a26) *)
  let idx : (t134, _) idx_mut = (.a134.#a26) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a134) *)
    let shallow : (t134, _) idx_mut = (.a134) in
    let deepened = (.idx_mut(shallow).#a26) in
    ...
    (* from (.a134.#a26) *)
    let shallow : (t134, _) idx_mut = (.a134.#a26) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 424;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 424 failed %d" i;
  );
  (* Deepening to (.a134.#b26) *)
  let idx : (t134, _) idx_mut = (.a134.#b26) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a134) *)
    let shallow : (t134, _) idx_mut = (.a134) in
    let deepened = (.idx_mut(shallow).#b26) in
    ...
    (* from (.a134.#b26) *)
    let shallow : (t134, _) idx_mut = (.a134.#b26) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 426;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a137.#a135) *)
  let idx : (t137, _) idx_mut = (.a137.#a135) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a137) *)
    let shallow : (t137, _) idx_mut = (.a137) in
    ...
    (* from (.a137.#a135) *)
    let shallow : (t137, _) idx_mut = (.a137.#a135) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 430;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 430 failed %d" i;
  );
  (* Deepening to (.a137.#b135) *)
  let idx : (t137, _) idx_mut = (.a137.#b135) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a137) *)
    let shallow : (t137, _) idx_mut = (.a137) in
    ...
    (* from (.a137.#b135) *)
    let shallow : (t137, _) idx_mut = (.a137.#b135) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 432;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a140.#a138) *)
  let idx : (t140, _) idx_mut = (.a140.#a138) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a140) *)
    let shallow : (t140, _) idx_mut = (.a140) in
    ...
    (* from (.a140.#a138) *)
    let shallow : (t140, _) idx_mut = (.a140.#a138) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 440;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 440 failed %d" i;
  );
  (* Deepening to (.a140.#b138) *)
  let idx : (t140, _) idx_mut = (.a140.#b138) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a140) *)
    let shallow : (t140, _) idx_mut = (.a140) in
    ...
    (* from (.a140.#b138) *)
    let shallow : (t140, _) idx_mut = (.a140.#b138) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 442;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a143.#a141) *)
  let idx : (t143, _) idx_mut = (.a143.#a141) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a143) *)
    let shallow : (t143, _) idx_mut = (.a143) in
    ...
    (* from (.a143.#a141) *)
    let shallow : (t143, _) idx_mut = (.a143.#a141) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 450;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 450 failed %d" i;
  );
  (* Deepening to (.a143.#b141) *)
  let idx : (t143, _) idx_mut = (.a143.#b141) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a143) *)
    let shallow : (t143, _) idx_mut = (.a143) in
    ...
    (* from (.a143.#b141) *)
    let shallow : (t143, _) idx_mut = (.a143.#b141) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 452;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b143.#a142) *)
  let idx : (t143, _) idx_mut = (.b143.#a142) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b143) *)
    let shallow : (t143, _) idx_mut = (.b143) in
    ...
    (* from (.b143.#a142) *)
    let shallow : (t143, _) idx_mut = (.b143.#a142) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 455;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 455 failed %d" i;
  );
  (* Deepening to (.b143.#b142) *)
  let idx : (t143, _) idx_mut = (.b143.#b142) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b143) *)
    let shallow : (t143, _) idx_mut = (.b143) in
    ...
    (* from (.b143.#b142) *)
    let shallow : (t143, _) idx_mut = (.b143.#b142) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 457;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a145.#a141) *)
  let idx : (t145, _) idx_mut = (.a145.#a141) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a145) *)
    let shallow : (t145, _) idx_mut = (.a145) in
    let deepened = (.idx_mut(shallow).#a141) in
    ...
    (* from (.a145.#a141) *)
    let shallow : (t145, _) idx_mut = (.a145.#a141) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 460;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 460 failed %d" i;
  );
  (* Deepening to (.a145.#b141) *)
  let idx : (t145, _) idx_mut = (.a145.#b141) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a145) *)
    let shallow : (t145, _) idx_mut = (.a145) in
    let deepened = (.idx_mut(shallow).#b141) in
    ...
    (* from (.a145.#b141) *)
    let shallow : (t145, _) idx_mut = (.a145.#b141) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 462;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a146.#a28) *)
  let idx : (t146, _) idx_mut = (.a146.#a28) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a146) *)
    let shallow : (t146, _) idx_mut = (.a146) in
    let deepened = (.idx_mut(shallow).#a28) in
    ...
    (* from (.a146.#a28) *)
    let shallow : (t146, _) idx_mut = (.a146.#a28) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 470;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a147.#a28) *)
  let idx : (t147, _) idx_mut = (.a147.#a28) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a147) *)
    let shallow : (t147, _) idx_mut = (.a147) in
    let deepened = (.idx_mut(shallow).#a28) in
    ...
    (* from (.a147.#a28) *)
    let shallow : (t147, _) idx_mut = (.a147.#a28) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 473;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a149.#a30) *)
  let idx : (t149, _) idx_mut = (.a149.#a30) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a149) *)
    let shallow : (t149, _) idx_mut = (.a149) in
    let deepened = (.idx_mut(shallow).#a30) in
    ...
    (* from (.a149.#a30) *)
    let shallow : (t149, _) idx_mut = (.a149.#a30) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 481;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 481 failed %d" i;
  );
  (* Deepening to (.a149.#b30) *)
  let idx : (t149, _) idx_mut = (.a149.#b30) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a149) *)
    let shallow : (t149, _) idx_mut = (.a149) in
    let deepened = (.idx_mut(shallow).#b30) in
    ...
    (* from (.a149.#b30) *)
    let shallow : (t149, _) idx_mut = (.a149.#b30) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 483;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a150.#a30) *)
  let idx : (t150, _) idx_mut = (.a150.#a30) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a150) *)
    let shallow : (t150, _) idx_mut = (.a150) in
    let deepened = (.idx_mut(shallow).#a30) in
    ...
    (* from (.a150.#a30) *)
    let shallow : (t150, _) idx_mut = (.a150.#a30) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 486;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 486 failed %d" i;
  );
  (* Deepening to (.a150.#b30) *)
  let idx : (t150, _) idx_mut = (.a150.#b30) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a150) *)
    let shallow : (t150, _) idx_mut = (.a150) in
    let deepened = (.idx_mut(shallow).#b30) in
    ...
    (* from (.a150.#b30) *)
    let shallow : (t150, _) idx_mut = (.a150.#b30) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 488;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a151.#a30) *)
  let idx : (t151, _) idx_mut = (.a151.#a30) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a151) *)
    let shallow : (t151, _) idx_mut = (.a151) in
    let deepened = (.idx_mut(shallow).#a30) in
    ...
    (* from (.a151.#a30) *)
    let shallow : (t151, _) idx_mut = (.a151.#a30) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 492;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 492 failed %d" i;
  );
  (* Deepening to (.a151.#b30) *)
  let idx : (t151, _) idx_mut = (.a151.#b30) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a151) *)
    let shallow : (t151, _) idx_mut = (.a151) in
    let deepened = (.idx_mut(shallow).#b30) in
    ...
    (* from (.a151.#b30) *)
    let shallow : (t151, _) idx_mut = (.a151.#b30) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 494;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a152.#a32) *)
  let idx : (t152, _) idx_mut = (.a152.#a32) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a152) *)
    let shallow : (t152, _) idx_mut = (.a152) in
    let deepened = (.idx_mut(shallow).#a32) in
    ...
    (* from (.a152.#a32) *)
    let shallow : (t152, _) idx_mut = (.a152.#a32) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 498;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 498 failed %d" i;
  );
  (* Deepening to (.a152.#b32) *)
  let idx : (t152, _) idx_mut = (.a152.#b32) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a152) *)
    let shallow : (t152, _) idx_mut = (.a152) in
    let deepened = (.idx_mut(shallow).#b32) in
    ...
    (* from (.a152.#b32) *)
    let shallow : (t152, _) idx_mut = (.a152.#b32) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 500;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a153.#a32) *)
  let idx : (t153, _) idx_mut = (.a153.#a32) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a153) *)
    let shallow : (t153, _) idx_mut = (.a153) in
    let deepened = (.idx_mut(shallow).#a32) in
    ...
    (* from (.a153.#a32) *)
    let shallow : (t153, _) idx_mut = (.a153.#a32) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 503;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 503 failed %d" i;
  );
  (* Deepening to (.a153.#b32) *)
  let idx : (t153, _) idx_mut = (.a153.#b32) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a153) *)
    let shallow : (t153, _) idx_mut = (.a153) in
    let deepened = (.idx_mut(shallow).#b32) in
    ...
    (* from (.a153.#b32) *)
    let shallow : (t153, _) idx_mut = (.a153.#b32) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 505;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a154.#a32) *)
  let idx : (t154, _) idx_mut = (.a154.#a32) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a154) *)
    let shallow : (t154, _) idx_mut = (.a154) in
    let deepened = (.idx_mut(shallow).#a32) in
    ...
    (* from (.a154.#a32) *)
    let shallow : (t154, _) idx_mut = (.a154.#a32) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 509;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 509 failed %d" i;
  );
  (* Deepening to (.a154.#b32) *)
  let idx : (t154, _) idx_mut = (.a154.#b32) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a154) *)
    let shallow : (t154, _) idx_mut = (.a154) in
    let deepened = (.idx_mut(shallow).#b32) in
    ...
    (* from (.a154.#b32) *)
    let shallow : (t154, _) idx_mut = (.a154.#b32) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 511;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a155.#a34) *)
  let idx : (t155, _) idx_mut = (.a155.#a34) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* Note: can't deepen (.a155) because it's a path to a flattened
       float, making its element type [float#] *)
    (* from (.a155.#a34) *)
    let shallow : (t155, _) idx_mut = (.a155.#a34) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 514;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a156.#a34) *)
  let idx : (t156, _) idx_mut = (.a156.#a34) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a156) *)
    let shallow : (t156, _) idx_mut = (.a156) in
    let deepened = (.idx_mut(shallow).#a34) in
    ...
    (* from (.a156.#a34) *)
    let shallow : (t156, _) idx_mut = (.a156.#a34) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 517;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a157.#a34) *)
  let idx : (t157, _) idx_mut = (.a157.#a34) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* Note: can't deepen (.a157) because it's a path to a flattened
       float, making its element type [float#] *)
    (* from (.a157.#a34) *)
    let shallow : (t157, _) idx_mut = (.a157.#a34) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 520;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a158.#a36) *)
  let idx : (t158, _) idx_mut = (.a158.#a36) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a158) *)
    let shallow : (t158, _) idx_mut = (.a158) in
    let deepened = (.idx_mut(shallow).#a36) in
    ...
    (* from (.a158.#a36) *)
    let shallow : (t158, _) idx_mut = (.a158.#a36) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 524;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 524 failed %d" i;
  );
  (* Deepening to (.a158.#b36) *)
  let idx : (t158, _) idx_mut = (.a158.#b36) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a158) *)
    let shallow : (t158, _) idx_mut = (.a158) in
    let deepened = (.idx_mut(shallow).#b36) in
    ...
    (* from (.a158.#b36) *)
    let shallow : (t158, _) idx_mut = (.a158.#b36) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 526;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a159.#a36) *)
  let idx : (t159, _) idx_mut = (.a159.#a36) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a159) *)
    let shallow : (t159, _) idx_mut = (.a159) in
    let deepened = (.idx_mut(shallow).#a36) in
    ...
    (* from (.a159.#a36) *)
    let shallow : (t159, _) idx_mut = (.a159.#a36) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 529;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 529 failed %d" i;
  );
  (* Deepening to (.a159.#b36) *)
  let idx : (t159, _) idx_mut = (.a159.#b36) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a159) *)
    let shallow : (t159, _) idx_mut = (.a159) in
    let deepened = (.idx_mut(shallow).#b36) in
    ...
    (* from (.a159.#b36) *)
    let shallow : (t159, _) idx_mut = (.a159.#b36) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 531;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a160.#a36) *)
  let idx : (t160, _) idx_mut = (.a160.#a36) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a160) *)
    let shallow : (t160, _) idx_mut = (.a160) in
    let deepened = (.idx_mut(shallow).#a36) in
    ...
    (* from (.a160.#a36) *)
    let shallow : (t160, _) idx_mut = (.a160.#a36) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 535;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 535 failed %d" i;
  );
  (* Deepening to (.a160.#b36) *)
  let idx : (t160, _) idx_mut = (.a160.#b36) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a160) *)
    let shallow : (t160, _) idx_mut = (.a160) in
    let deepened = (.idx_mut(shallow).#b36) in
    ...
    (* from (.a160.#b36) *)
    let shallow : (t160, _) idx_mut = (.a160.#b36) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 537;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a161.#a38) *)
  let idx : (t161, _) idx_mut = (.a161.#a38) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a161) *)
    let shallow : (t161, _) idx_mut = (.a161) in
    let deepened = (.idx_mut(shallow).#a38) in
    ...
    (* from (.a161.#a38) *)
    let shallow : (t161, _) idx_mut = (.a161.#a38) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 541;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 541 failed %d" i;
  );
  (* Deepening to (.a161.#b38) *)
  let idx : (t161, _) idx_mut = (.a161.#b38) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a161) *)
    let shallow : (t161, _) idx_mut = (.a161) in
    let deepened = (.idx_mut(shallow).#b38) in
    ...
    (* from (.a161.#b38) *)
    let shallow : (t161, _) idx_mut = (.a161.#b38) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 543;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a162.#a38) *)
  let idx : (t162, _) idx_mut = (.a162.#a38) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a162) *)
    let shallow : (t162, _) idx_mut = (.a162) in
    let deepened = (.idx_mut(shallow).#a38) in
    ...
    (* from (.a162.#a38) *)
    let shallow : (t162, _) idx_mut = (.a162.#a38) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 546;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 546 failed %d" i;
  );
  (* Deepening to (.a162.#b38) *)
  let idx : (t162, _) idx_mut = (.a162.#b38) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a162) *)
    let shallow : (t162, _) idx_mut = (.a162) in
    let deepened = (.idx_mut(shallow).#b38) in
    ...
    (* from (.a162.#b38) *)
    let shallow : (t162, _) idx_mut = (.a162.#b38) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 548;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a163.#a38) *)
  let idx : (t163, _) idx_mut = (.a163.#a38) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a163) *)
    let shallow : (t163, _) idx_mut = (.a163) in
    let deepened = (.idx_mut(shallow).#a38) in
    ...
    (* from (.a163.#a38) *)
    let shallow : (t163, _) idx_mut = (.a163.#a38) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 552;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 552 failed %d" i;
  );
  (* Deepening to (.a163.#b38) *)
  let idx : (t163, _) idx_mut = (.a163.#b38) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a163) *)
    let shallow : (t163, _) idx_mut = (.a163) in
    let deepened = (.idx_mut(shallow).#b38) in
    ...
    (* from (.a163.#b38) *)
    let shallow : (t163, _) idx_mut = (.a163.#b38) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 554;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a166.#a164) *)
  let idx : (t166, _) idx_mut = (.a166.#a164) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a166) *)
    let shallow : (t166, _) idx_mut = (.a166) in
    ...
    (* from (.a166.#a164) *)
    let shallow : (t166, _) idx_mut = (.a166.#a164) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 558;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 558 failed %d" i;
  );
  (* Deepening to (.a166.#b164) *)
  let idx : (t166, _) idx_mut = (.a166.#b164) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a166) *)
    let shallow : (t166, _) idx_mut = (.a166) in
    ...
    (* from (.a166.#b164) *)
    let shallow : (t166, _) idx_mut = (.a166.#b164) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 560;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a167.#a40) *)
  let idx : (t167, _) idx_mut = (.a167.#a40) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a167) *)
    let shallow : (t167, _) idx_mut = (.a167) in
    let deepened = (.idx_mut(shallow).#a40) in
    ...
    (* from (.a167.#a40) *)
    let shallow : (t167, _) idx_mut = (.a167.#a40) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 568;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a168.#a40) *)
  let idx : (t168, _) idx_mut = (.a168.#a40) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a168) *)
    let shallow : (t168, _) idx_mut = (.a168) in
    let deepened = (.idx_mut(shallow).#a40) in
    ...
    (* from (.a168.#a40) *)
    let shallow : (t168, _) idx_mut = (.a168.#a40) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 571;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a169.#a40) *)
  let idx : (t169, _) idx_mut = (.a169.#a40) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a169) *)
    let shallow : (t169, _) idx_mut = (.a169) in
    let deepened = (.idx_mut(shallow).#a40) in
    ...
    (* from (.a169.#a40) *)
    let shallow : (t169, _) idx_mut = (.a169.#a40) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 575;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a170.#a42) *)
  let idx : (t170, _) idx_mut = (.a170.#a42) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a170) *)
    let shallow : (t170, _) idx_mut = (.a170) in
    let deepened = (.idx_mut(shallow).#a42) in
    ...
    (* from (.a170.#a42) *)
    let shallow : (t170, _) idx_mut = (.a170.#a42) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 579;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 579 failed %d" i;
  );
  (* Deepening to (.a170.#b42) *)
  let idx : (t170, _) idx_mut = (.a170.#b42) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a170) *)
    let shallow : (t170, _) idx_mut = (.a170) in
    let deepened = (.idx_mut(shallow).#b42) in
    ...
    (* from (.a170.#b42) *)
    let shallow : (t170, _) idx_mut = (.a170.#b42) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 581;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a171.#a42) *)
  let idx : (t171, _) idx_mut = (.a171.#a42) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a171) *)
    let shallow : (t171, _) idx_mut = (.a171) in
    let deepened = (.idx_mut(shallow).#a42) in
    ...
    (* from (.a171.#a42) *)
    let shallow : (t171, _) idx_mut = (.a171.#a42) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 584;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 584 failed %d" i;
  );
  (* Deepening to (.a171.#b42) *)
  let idx : (t171, _) idx_mut = (.a171.#b42) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a171) *)
    let shallow : (t171, _) idx_mut = (.a171) in
    let deepened = (.idx_mut(shallow).#b42) in
    ...
    (* from (.a171.#b42) *)
    let shallow : (t171, _) idx_mut = (.a171.#b42) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 586;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a172.#a42) *)
  let idx : (t172, _) idx_mut = (.a172.#a42) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a172) *)
    let shallow : (t172, _) idx_mut = (.a172) in
    let deepened = (.idx_mut(shallow).#a42) in
    ...
    (* from (.a172.#a42) *)
    let shallow : (t172, _) idx_mut = (.a172.#a42) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 590;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 590 failed %d" i;
  );
  (* Deepening to (.a172.#b42) *)
  let idx : (t172, _) idx_mut = (.a172.#b42) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a172) *)
    let shallow : (t172, _) idx_mut = (.a172) in
    let deepened = (.idx_mut(shallow).#b42) in
    ...
    (* from (.a172.#b42) *)
    let shallow : (t172, _) idx_mut = (.a172.#b42) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 592;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a175.#a173) *)
  let idx : (t175, _) idx_mut = (.a175.#a173) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a175) *)
    let shallow : (t175, _) idx_mut = (.a175) in
    ...
    (* from (.a175.#a173) *)
    let shallow : (t175, _) idx_mut = (.a175.#a173) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 596;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 596 failed %d" i;
  );
  (* Deepening to (.a175.#b173) *)
  let idx : (t175, _) idx_mut = (.a175.#b173) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a175) *)
    let shallow : (t175, _) idx_mut = (.a175) in
    ...
    (* from (.a175.#b173) *)
    let shallow : (t175, _) idx_mut = (.a175.#b173) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 598;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b175.#a174) *)
  let idx : (t175, _) idx_mut = (.b175.#a174) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b175) *)
    let shallow : (t175, _) idx_mut = (.b175) in
    ...
    (* from (.b175.#a174) *)
    let shallow : (t175, _) idx_mut = (.b175.#a174) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 601;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 601 failed %d" i;
  );
  (* Deepening to (.b175.#b174) *)
  let idx : (t175, _) idx_mut = (.b175.#b174) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b175) *)
    let shallow : (t175, _) idx_mut = (.b175) in
    ...
    (* from (.b175.#b174) *)
    let shallow : (t175, _) idx_mut = (.b175.#b174) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 603;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a176.#a44) *)
  let idx : (t176, _) idx_mut = (.a176.#a44) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a176) *)
    let shallow : (t176, _) idx_mut = (.a176) in
    let deepened = (.idx_mut(shallow).#a44) in
    ...
    (* from (.a176.#a44) *)
    let shallow : (t176, _) idx_mut = (.a176.#a44) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 606;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 606 failed %d" i;
  );
  (* Deepening to (.a176.#b44) *)
  let idx : (t176, _) idx_mut = (.a176.#b44) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a176) *)
    let shallow : (t176, _) idx_mut = (.a176) in
    let deepened = (.idx_mut(shallow).#b44) in
    ...
    (* from (.a176.#b44) *)
    let shallow : (t176, _) idx_mut = (.a176.#b44) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 608;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a177.#a44) *)
  let idx : (t177, _) idx_mut = (.a177.#a44) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a177) *)
    let shallow : (t177, _) idx_mut = (.a177) in
    let deepened = (.idx_mut(shallow).#a44) in
    ...
    (* from (.a177.#a44) *)
    let shallow : (t177, _) idx_mut = (.a177.#a44) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 611;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 611 failed %d" i;
  );
  (* Deepening to (.a177.#b44) *)
  let idx : (t177, _) idx_mut = (.a177.#b44) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a177) *)
    let shallow : (t177, _) idx_mut = (.a177) in
    let deepened = (.idx_mut(shallow).#b44) in
    ...
    (* from (.a177.#b44) *)
    let shallow : (t177, _) idx_mut = (.a177.#b44) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 613;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a178.#a44) *)
  let idx : (t178, _) idx_mut = (.a178.#a44) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a178) *)
    let shallow : (t178, _) idx_mut = (.a178) in
    let deepened = (.idx_mut(shallow).#a44) in
    ...
    (* from (.a178.#a44) *)
    let shallow : (t178, _) idx_mut = (.a178.#a44) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 617;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 617 failed %d" i;
  );
  (* Deepening to (.a178.#b44) *)
  let idx : (t178, _) idx_mut = (.a178.#b44) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a178) *)
    let shallow : (t178, _) idx_mut = (.a178) in
    let deepened = (.idx_mut(shallow).#b44) in
    ...
    (* from (.a178.#b44) *)
    let shallow : (t178, _) idx_mut = (.a178.#b44) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 619;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
