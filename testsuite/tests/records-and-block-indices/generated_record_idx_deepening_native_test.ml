...
let () =
  ...
  (* Deepening to (.b44.#b43) *)
  let idx : (t44, _) idx_mut = (.b44.#b43) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b44) *)
    let shallow : (t44, _) idx_mut = (.b44) in
    ...
    (* from (.b44.#b43) *)
    let shallow : (t44, _) idx_mut = (.b44.#b43) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 117;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.c65) *)
  let idx : (t65, _) idx_mut = (.c65) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.c65) *)
    let shallow : (t65, _) idx_mut = (.c65) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 159;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.c66) *)
  let idx : (t66, _) idx_mut = (.c66) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.c66) *)
    let shallow : (t66, _) idx_mut = (.c66) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 162;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.c71) *)
  let idx : (t71, _) idx_mut = (.c71) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.c71) *)
    let shallow : (t71, _) idx_mut = (.c71) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 174;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b75.#a19) *)
  let idx : (t75, _) idx_mut = (.b75.#a19) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b75) *)
    let shallow : (t75, _) idx_mut = (.b75) in
    let deepened = (.idx_mut(shallow).#a19) in
    ...
    (* from (.b75.#a19) *)
    let shallow : (t75, _) idx_mut = (.b75.#a19) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 184;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b76.#a21) *)
  let idx : (t76, _) idx_mut = (.b76.#a21) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b76) *)
    let shallow : (t76, _) idx_mut = (.b76) in
    let deepened = (.idx_mut(shallow).#a21) in
    ...
    (* from (.b76.#a21) *)
    let shallow : (t76, _) idx_mut = (.b76.#a21) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 188;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 188 failed %d" i;
  );
  (* Deepening to (.b76.#b21) *)
  let idx : (t76, _) idx_mut = (.b76.#b21) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b76) *)
    let shallow : (t76, _) idx_mut = (.b76) in
    let deepened = (.idx_mut(shallow).#b21) in
    ...
    (* from (.b76.#b21) *)
    let shallow : (t76, _) idx_mut = (.b76.#b21) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 190;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b77.#a23) *)
  let idx : (t77, _) idx_mut = (.b77.#a23) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b77) *)
    let shallow : (t77, _) idx_mut = (.b77) in
    let deepened = (.idx_mut(shallow).#a23) in
    ...
    (* from (.b77.#a23) *)
    let shallow : (t77, _) idx_mut = (.b77.#a23) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 194;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 194 failed %d" i;
  );
  (* Deepening to (.b77.#b23) *)
  let idx : (t77, _) idx_mut = (.b77.#b23) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b77) *)
    let shallow : (t77, _) idx_mut = (.b77) in
    let deepened = (.idx_mut(shallow).#b23) in
    ...
    (* from (.b77.#b23) *)
    let shallow : (t77, _) idx_mut = (.b77.#b23) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 196;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b78.#a29) *)
  let idx : (t78, _) idx_mut = (.b78.#a29) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b78) *)
    let shallow : (t78, _) idx_mut = (.b78) in
    let deepened = (.idx_mut(shallow).#a29) in
    ...
    (* from (.b78.#a29) *)
    let shallow : (t78, _) idx_mut = (.b78.#a29) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 200;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b79.#a31) *)
  let idx : (t79, _) idx_mut = (.b79.#a31) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b79) *)
    let shallow : (t79, _) idx_mut = (.b79) in
    let deepened = (.idx_mut(shallow).#a31) in
    ...
    (* from (.b79.#a31) *)
    let shallow : (t79, _) idx_mut = (.b79.#a31) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 204;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 204 failed %d" i;
  );
  (* Deepening to (.b79.#b31) *)
  let idx : (t79, _) idx_mut = (.b79.#b31) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b79) *)
    let shallow : (t79, _) idx_mut = (.b79) in
    let deepened = (.idx_mut(shallow).#b31) in
    ...
    (* from (.b79.#b31) *)
    let shallow : (t79, _) idx_mut = (.b79.#b31) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 206;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b80.#a33) *)
  let idx : (t80, _) idx_mut = (.b80.#a33) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b80) *)
    let shallow : (t80, _) idx_mut = (.b80) in
    let deepened = (.idx_mut(shallow).#a33) in
    ...
    (* from (.b80.#a33) *)
    let shallow : (t80, _) idx_mut = (.b80.#a33) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 210;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 210 failed %d" i;
  );
  (* Deepening to (.b80.#b33) *)
  let idx : (t80, _) idx_mut = (.b80.#b33) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b80) *)
    let shallow : (t80, _) idx_mut = (.b80) in
    let deepened = (.idx_mut(shallow).#b33) in
    ...
    (* from (.b80.#b33) *)
    let shallow : (t80, _) idx_mut = (.b80.#b33) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 212;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.c109) *)
  let idx : (t109, _) idx_mut = (.c109) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.c109) *)
    let shallow : (t109, _) idx_mut = (.c109) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 274;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b111.#a19) *)
  let idx : (t111, _) idx_mut = (.b111.#a19) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b111) *)
    let shallow : (t111, _) idx_mut = (.b111) in
    let deepened = (.idx_mut(shallow).#a19) in
    ...
    (* from (.b111.#a19) *)
    let shallow : (t111, _) idx_mut = (.b111.#a19) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 281;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b112.#a21) *)
  let idx : (t112, _) idx_mut = (.b112.#a21) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b112) *)
    let shallow : (t112, _) idx_mut = (.b112) in
    let deepened = (.idx_mut(shallow).#a21) in
    ...
    (* from (.b112.#a21) *)
    let shallow : (t112, _) idx_mut = (.b112.#a21) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 285;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 285 failed %d" i;
  );
  (* Deepening to (.b112.#b21) *)
  let idx : (t112, _) idx_mut = (.b112.#b21) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b112) *)
    let shallow : (t112, _) idx_mut = (.b112) in
    let deepened = (.idx_mut(shallow).#b21) in
    ...
    (* from (.b112.#b21) *)
    let shallow : (t112, _) idx_mut = (.b112.#b21) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 287;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b113.#a25) *)
  let idx : (t113, _) idx_mut = (.b113.#a25) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b113) *)
    let shallow : (t113, _) idx_mut = (.b113) in
    let deepened = (.idx_mut(shallow).#a25) in
    ...
    (* from (.b113.#a25) *)
    let shallow : (t113, _) idx_mut = (.b113.#a25) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 291;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 291 failed %d" i;
  );
  (* Deepening to (.b113.#b25) *)
  let idx : (t113, _) idx_mut = (.b113.#b25) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b113) *)
    let shallow : (t113, _) idx_mut = (.b113) in
    let deepened = (.idx_mut(shallow).#b25) in
    ...
    (* from (.b113.#b25) *)
    let shallow : (t113, _) idx_mut = (.b113.#b25) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 293;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b114.#a35) *)
  let idx : (t114, _) idx_mut = (.b114.#a35) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* Note: can't deepen (.b114) because it's a path to a flattened
       float, making its element type [float#] *)
    (* from (.b114.#a35) *)
    let shallow : (t114, _) idx_mut = (.b114.#a35) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 296;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b115.#a37) *)
  let idx : (t115, _) idx_mut = (.b115.#a37) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b115) *)
    let shallow : (t115, _) idx_mut = (.b115) in
    let deepened = (.idx_mut(shallow).#a37) in
    ...
    (* from (.b115.#a37) *)
    let shallow : (t115, _) idx_mut = (.b115.#a37) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 300;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 300 failed %d" i;
  );
  (* Deepening to (.b115.#b37) *)
  let idx : (t115, _) idx_mut = (.b115.#b37) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b115) *)
    let shallow : (t115, _) idx_mut = (.b115) in
    let deepened = (.idx_mut(shallow).#b37) in
    ...
    (* from (.b115.#b37) *)
    let shallow : (t115, _) idx_mut = (.b115.#b37) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 302;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b116.#a39) *)
  let idx : (t116, _) idx_mut = (.b116.#a39) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b116) *)
    let shallow : (t116, _) idx_mut = (.b116) in
    let deepened = (.idx_mut(shallow).#a39) in
    ...
    (* from (.b116.#a39) *)
    let shallow : (t116, _) idx_mut = (.b116.#a39) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 306;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 306 failed %d" i;
  );
  (* Deepening to (.b116.#b39) *)
  let idx : (t116, _) idx_mut = (.b116.#b39) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b116) *)
    let shallow : (t116, _) idx_mut = (.b116) in
    let deepened = (.idx_mut(shallow).#b39) in
    ...
    (* from (.b116.#b39) *)
    let shallow : (t116, _) idx_mut = (.b116.#b39) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 308;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b118.#a117) *)
  let idx : (t118, _) idx_mut = (.b118.#a117) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b118) *)
    let shallow : (t118, _) idx_mut = (.b118) in
    ...
    (* from (.b118.#a117) *)
    let shallow : (t118, _) idx_mut = (.b118.#a117) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 312;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 312 failed %d" i;
  );
  (* Deepening to (.b118.#b117) *)
  let idx : (t118, _) idx_mut = (.b118.#b117) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b118) *)
    let shallow : (t118, _) idx_mut = (.b118) in
    ...
    (* from (.b118.#b117) *)
    let shallow : (t118, _) idx_mut = (.b118.#b117) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 314;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b120.#a119) *)
  let idx : (t120, _) idx_mut = (.b120.#a119) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b120) *)
    let shallow : (t120, _) idx_mut = (.b120) in
    ...
    (* from (.b120.#a119) *)
    let shallow : (t120, _) idx_mut = (.b120.#a119) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 318;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b122.#a121) *)
  let idx : (t122, _) idx_mut = (.b122.#a121) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b122) *)
    let shallow : (t122, _) idx_mut = (.b122) in
    ...
    (* from (.b122.#a121) *)
    let shallow : (t122, _) idx_mut = (.b122.#a121) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 322;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 322 failed %d" i;
  );
  (* Deepening to (.b122.#b121) *)
  let idx : (t122, _) idx_mut = (.b122.#b121) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b122) *)
    let shallow : (t122, _) idx_mut = (.b122) in
    ...
    (* from (.b122.#b121) *)
    let shallow : (t122, _) idx_mut = (.b122.#b121) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 324;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b124.#a123) *)
  let idx : (t124, _) idx_mut = (.b124.#a123) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b124) *)
    let shallow : (t124, _) idx_mut = (.b124) in
    ...
    (* from (.b124.#a123) *)
    let shallow : (t124, _) idx_mut = (.b124.#a123) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 328;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 328 failed %d" i;
  );
  (* Deepening to (.b124.#b123) *)
  let idx : (t124, _) idx_mut = (.b124.#b123) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b124) *)
    let shallow : (t124, _) idx_mut = (.b124) in
    ...
    (* from (.b124.#b123) *)
    let shallow : (t124, _) idx_mut = (.b124.#b123) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 330;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.c127) *)
  let idx : (t127, _) idx_mut = (.c127) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.c127) *)
    let shallow : (t127, _) idx_mut = (.c127) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 336;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.c130) *)
  let idx : (t130, _) idx_mut = (.c130) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.c130) *)
    let shallow : (t130, _) idx_mut = (.c130) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 344;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.c131) *)
  let idx : (t131, _) idx_mut = (.c131) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.c131) *)
    let shallow : (t131, _) idx_mut = (.c131) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 347;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b132.#a35) *)
  let idx : (t132, _) idx_mut = (.b132.#a35) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* Note: can't deepen (.b132) because it's a path to a flattened
       float, making its element type [float#] *)
    (* from (.b132.#a35) *)
    let shallow : (t132, _) idx_mut = (.b132.#a35) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 350;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b133.#a39) *)
  let idx : (t133, _) idx_mut = (.b133.#a39) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b133) *)
    let shallow : (t133, _) idx_mut = (.b133) in
    let deepened = (.idx_mut(shallow).#a39) in
    ...
    (* from (.b133.#a39) *)
    let shallow : (t133, _) idx_mut = (.b133.#a39) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 354;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 354 failed %d" i;
  );
  (* Deepening to (.b133.#b39) *)
  let idx : (t133, _) idx_mut = (.b133.#b39) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b133) *)
    let shallow : (t133, _) idx_mut = (.b133) in
    let deepened = (.idx_mut(shallow).#b39) in
    ...
    (* from (.b133.#b39) *)
    let shallow : (t133, _) idx_mut = (.b133.#b39) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 356;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b134.#a117) *)
  let idx : (t134, _) idx_mut = (.b134.#a117) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b134) *)
    let shallow : (t134, _) idx_mut = (.b134) in
    let deepened = (.idx_mut(shallow).#a117) in
    ...
    (* from (.b134.#a117) *)
    let shallow : (t134, _) idx_mut = (.b134.#a117) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 360;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 360 failed %d" i;
  );
  (* Deepening to (.b134.#b117) *)
  let idx : (t134, _) idx_mut = (.b134.#b117) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b134) *)
    let shallow : (t134, _) idx_mut = (.b134) in
    let deepened = (.idx_mut(shallow).#b117) in
    ...
    (* from (.b134.#b117) *)
    let shallow : (t134, _) idx_mut = (.b134.#b117) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 362;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b135.#a119) *)
  let idx : (t135, _) idx_mut = (.b135.#a119) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b135) *)
    let shallow : (t135, _) idx_mut = (.b135) in
    let deepened = (.idx_mut(shallow).#a119) in
    ...
    (* from (.b135.#a119) *)
    let shallow : (t135, _) idx_mut = (.b135.#a119) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 366;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b136.#a121) *)
  let idx : (t136, _) idx_mut = (.b136.#a121) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b136) *)
    let shallow : (t136, _) idx_mut = (.b136) in
    let deepened = (.idx_mut(shallow).#a121) in
    ...
    (* from (.b136.#a121) *)
    let shallow : (t136, _) idx_mut = (.b136.#a121) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 370;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 370 failed %d" i;
  );
  (* Deepening to (.b136.#b121) *)
  let idx : (t136, _) idx_mut = (.b136.#b121) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b136) *)
    let shallow : (t136, _) idx_mut = (.b136) in
    let deepened = (.idx_mut(shallow).#b121) in
    ...
    (* from (.b136.#b121) *)
    let shallow : (t136, _) idx_mut = (.b136.#b121) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 372;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b137.#a123) *)
  let idx : (t137, _) idx_mut = (.b137.#a123) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b137) *)
    let shallow : (t137, _) idx_mut = (.b137) in
    let deepened = (.idx_mut(shallow).#a123) in
    ...
    (* from (.b137.#a123) *)
    let shallow : (t137, _) idx_mut = (.b137.#a123) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 376;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 376 failed %d" i;
  );
  (* Deepening to (.b137.#b123) *)
  let idx : (t137, _) idx_mut = (.b137.#b123) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b137) *)
    let shallow : (t137, _) idx_mut = (.b137) in
    let deepened = (.idx_mut(shallow).#b123) in
    ...
    (* from (.b137.#b123) *)
    let shallow : (t137, _) idx_mut = (.b137.#b123) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 378;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.c140) *)
  let idx : (t140, _) idx_mut = (.c140) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.c140) *)
    let shallow : (t140, _) idx_mut = (.c140) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 384;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.c144) *)
  let idx : (t144, _) idx_mut = (.c144) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.c144) *)
    let shallow : (t144, _) idx_mut = (.c144) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 395;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b145.#a19) *)
  let idx : (t145, _) idx_mut = (.b145.#a19) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b145) *)
    let shallow : (t145, _) idx_mut = (.b145) in
    let deepened = (.idx_mut(shallow).#a19) in
    ...
    (* from (.b145.#a19) *)
    let shallow : (t145, _) idx_mut = (.b145.#a19) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 399;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b146.#a21) *)
  let idx : (t146, _) idx_mut = (.b146.#a21) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b146) *)
    let shallow : (t146, _) idx_mut = (.b146) in
    let deepened = (.idx_mut(shallow).#a21) in
    ...
    (* from (.b146.#a21) *)
    let shallow : (t146, _) idx_mut = (.b146.#a21) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 403;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 403 failed %d" i;
  );
  (* Deepening to (.b146.#b21) *)
  let idx : (t146, _) idx_mut = (.b146.#b21) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b146) *)
    let shallow : (t146, _) idx_mut = (.b146) in
    let deepened = (.idx_mut(shallow).#b21) in
    ...
    (* from (.b146.#b21) *)
    let shallow : (t146, _) idx_mut = (.b146.#b21) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 405;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b147.#a27) *)
  let idx : (t147, _) idx_mut = (.b147.#a27) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b147) *)
    let shallow : (t147, _) idx_mut = (.b147) in
    let deepened = (.idx_mut(shallow).#a27) in
    ...
    (* from (.b147.#a27) *)
    let shallow : (t147, _) idx_mut = (.b147.#a27) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 409;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 409 failed %d" i;
  );
  (* Deepening to (.b147.#b27) *)
  let idx : (t147, _) idx_mut = (.b147.#b27) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b147) *)
    let shallow : (t147, _) idx_mut = (.b147) in
    let deepened = (.idx_mut(shallow).#b27) in
    ...
    (* from (.b147.#b27) *)
    let shallow : (t147, _) idx_mut = (.b147.#b27) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 411;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b149.#a148) *)
  let idx : (t149, _) idx_mut = (.b149.#a148) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b149) *)
    let shallow : (t149, _) idx_mut = (.b149) in
    ...
    (* from (.b149.#a148) *)
    let shallow : (t149, _) idx_mut = (.b149.#a148) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 415;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 415 failed %d" i;
  );
  (* Deepening to (.b149.#b148) *)
  let idx : (t149, _) idx_mut = (.b149.#b148) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b149) *)
    let shallow : (t149, _) idx_mut = (.b149) in
    let deepened = (.idx_mut(shallow).#b148) in
    mark_test_run 416;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 416 failed %d" i;
    (* from (.b149.#b148) *)
    let shallow : (t149, _) idx_mut = (.b149.#b148) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 417;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b150.#a41) *)
  let idx : (t150, _) idx_mut = (.b150.#a41) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b150) *)
    let shallow : (t150, _) idx_mut = (.b150) in
    let deepened = (.idx_mut(shallow).#a41) in
    ...
    (* from (.b150.#a41) *)
    let shallow : (t150, _) idx_mut = (.b150.#a41) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 421;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b151.#a43) *)
  let idx : (t151, _) idx_mut = (.b151.#a43) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b151) *)
    let shallow : (t151, _) idx_mut = (.b151) in
    let deepened = (.idx_mut(shallow).#a43) in
    ...
    (* from (.b151.#a43) *)
    let shallow : (t151, _) idx_mut = (.b151.#a43) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 425;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 425 failed %d" i;
  );
  (* Deepening to (.b151.#b43) *)
  let idx : (t151, _) idx_mut = (.b151.#b43) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b151) *)
    let shallow : (t151, _) idx_mut = (.b151) in
    let deepened = (.idx_mut(shallow).#b43) in
    ...
    (* from (.b151.#b43) *)
    let shallow : (t151, _) idx_mut = (.b151.#b43) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 427;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b152.#a45) *)
  let idx : (t152, _) idx_mut = (.b152.#a45) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b152) *)
    let shallow : (t152, _) idx_mut = (.b152) in
    let deepened = (.idx_mut(shallow).#a45) in
    ...
    (* from (.b152.#a45) *)
    let shallow : (t152, _) idx_mut = (.b152.#a45) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 431;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 431 failed %d" i;
  );
  (* Deepening to (.b152.#b45) *)
  let idx : (t152, _) idx_mut = (.b152.#b45) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b152) *)
    let shallow : (t152, _) idx_mut = (.b152) in
    let deepened = (.idx_mut(shallow).#b45) in
    ...
    (* from (.b152.#b45) *)
    let shallow : (t152, _) idx_mut = (.b152.#b45) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 433;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a153.#a19) *)
  let idx : (t153, _) idx_mut = (.a153.#a19) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a153) *)
    let shallow : (t153, _) idx_mut = (.a153) in
    let deepened = (.idx_mut(shallow).#a19) in
    ...
    (* from (.a153.#a19) *)
    let shallow : (t153, _) idx_mut = (.a153.#a19) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 436;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a154.#a19) *)
  let idx : (t154, _) idx_mut = (.a154.#a19) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a154) *)
    let shallow : (t154, _) idx_mut = (.a154) in
    let deepened = (.idx_mut(shallow).#a19) in
    ...
    (* from (.a154.#a19) *)
    let shallow : (t154, _) idx_mut = (.a154.#a19) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 439;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a155.#a19) *)
  let idx : (t155, _) idx_mut = (.a155.#a19) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a155) *)
    let shallow : (t155, _) idx_mut = (.a155) in
    let deepened = (.idx_mut(shallow).#a19) in
    ...
    (* from (.a155.#a19) *)
    let shallow : (t155, _) idx_mut = (.a155.#a19) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 443;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a156.#a19) *)
  let idx : (t156, _) idx_mut = (.a156.#a19) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a156) *)
    let shallow : (t156, _) idx_mut = (.a156) in
    let deepened = (.idx_mut(shallow).#a19) in
    ...
    (* from (.a156.#a19) *)
    let shallow : (t156, _) idx_mut = (.a156.#a19) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 447;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a157.#a19) *)
  let idx : (t157, _) idx_mut = (.a157.#a19) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a157) *)
    let shallow : (t157, _) idx_mut = (.a157) in
    let deepened = (.idx_mut(shallow).#a19) in
    ...
    (* from (.a157.#a19) *)
    let shallow : (t157, _) idx_mut = (.a157.#a19) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 451;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a158.#a21) *)
  let idx : (t158, _) idx_mut = (.a158.#a21) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a158) *)
    let shallow : (t158, _) idx_mut = (.a158) in
    let deepened = (.idx_mut(shallow).#a21) in
    ...
    (* from (.a158.#a21) *)
    let shallow : (t158, _) idx_mut = (.a158.#a21) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 455;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 455 failed %d" i;
  );
  (* Deepening to (.a158.#b21) *)
  let idx : (t158, _) idx_mut = (.a158.#b21) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a158) *)
    let shallow : (t158, _) idx_mut = (.a158) in
    let deepened = (.idx_mut(shallow).#b21) in
    ...
    (* from (.a158.#b21) *)
    let shallow : (t158, _) idx_mut = (.a158.#b21) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 457;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a159.#a21) *)
  let idx : (t159, _) idx_mut = (.a159.#a21) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a159) *)
    let shallow : (t159, _) idx_mut = (.a159) in
    let deepened = (.idx_mut(shallow).#a21) in
    ...
    (* from (.a159.#a21) *)
    let shallow : (t159, _) idx_mut = (.a159.#a21) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 460;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 460 failed %d" i;
  );
  (* Deepening to (.a159.#b21) *)
  let idx : (t159, _) idx_mut = (.a159.#b21) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a159) *)
    let shallow : (t159, _) idx_mut = (.a159) in
    let deepened = (.idx_mut(shallow).#b21) in
    ...
    (* from (.a159.#b21) *)
    let shallow : (t159, _) idx_mut = (.a159.#b21) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 462;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a160.#a21) *)
  let idx : (t160, _) idx_mut = (.a160.#a21) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a160) *)
    let shallow : (t160, _) idx_mut = (.a160) in
    let deepened = (.idx_mut(shallow).#a21) in
    ...
    (* from (.a160.#a21) *)
    let shallow : (t160, _) idx_mut = (.a160.#a21) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 466;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 466 failed %d" i;
  );
  (* Deepening to (.a160.#b21) *)
  let idx : (t160, _) idx_mut = (.a160.#b21) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a160) *)
    let shallow : (t160, _) idx_mut = (.a160) in
    let deepened = (.idx_mut(shallow).#b21) in
    ...
    (* from (.a160.#b21) *)
    let shallow : (t160, _) idx_mut = (.a160.#b21) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 468;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a161.#a21) *)
  let idx : (t161, _) idx_mut = (.a161.#a21) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a161) *)
    let shallow : (t161, _) idx_mut = (.a161) in
    let deepened = (.idx_mut(shallow).#a21) in
    ...
    (* from (.a161.#a21) *)
    let shallow : (t161, _) idx_mut = (.a161.#a21) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 472;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 472 failed %d" i;
  );
  (* Deepening to (.a161.#b21) *)
  let idx : (t161, _) idx_mut = (.a161.#b21) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a161) *)
    let shallow : (t161, _) idx_mut = (.a161) in
    let deepened = (.idx_mut(shallow).#b21) in
    ...
    (* from (.a161.#b21) *)
    let shallow : (t161, _) idx_mut = (.a161.#b21) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 474;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a162.#a21) *)
  let idx : (t162, _) idx_mut = (.a162.#a21) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a162) *)
    let shallow : (t162, _) idx_mut = (.a162) in
    let deepened = (.idx_mut(shallow).#a21) in
    ...
    (* from (.a162.#a21) *)
    let shallow : (t162, _) idx_mut = (.a162.#a21) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 478;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 478 failed %d" i;
  );
  (* Deepening to (.a162.#b21) *)
  let idx : (t162, _) idx_mut = (.a162.#b21) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a162) *)
    let shallow : (t162, _) idx_mut = (.a162) in
    let deepened = (.idx_mut(shallow).#b21) in
    ...
    (* from (.a162.#b21) *)
    let shallow : (t162, _) idx_mut = (.a162.#b21) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 480;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a163.#a23) *)
  let idx : (t163, _) idx_mut = (.a163.#a23) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a163) *)
    let shallow : (t163, _) idx_mut = (.a163) in
    let deepened = (.idx_mut(shallow).#a23) in
    ...
    (* from (.a163.#a23) *)
    let shallow : (t163, _) idx_mut = (.a163.#a23) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 484;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 484 failed %d" i;
  );
  (* Deepening to (.a163.#b23) *)
  let idx : (t163, _) idx_mut = (.a163.#b23) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a163) *)
    let shallow : (t163, _) idx_mut = (.a163) in
    let deepened = (.idx_mut(shallow).#b23) in
    ...
    (* from (.a163.#b23) *)
    let shallow : (t163, _) idx_mut = (.a163.#b23) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 486;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a164.#a23) *)
  let idx : (t164, _) idx_mut = (.a164.#a23) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a164) *)
    let shallow : (t164, _) idx_mut = (.a164) in
    let deepened = (.idx_mut(shallow).#a23) in
    ...
    (* from (.a164.#a23) *)
    let shallow : (t164, _) idx_mut = (.a164.#a23) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 489;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 489 failed %d" i;
  );
  (* Deepening to (.a164.#b23) *)
  let idx : (t164, _) idx_mut = (.a164.#b23) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a164) *)
    let shallow : (t164, _) idx_mut = (.a164) in
    let deepened = (.idx_mut(shallow).#b23) in
    ...
    (* from (.a164.#b23) *)
    let shallow : (t164, _) idx_mut = (.a164.#b23) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 491;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a165.#a23) *)
  let idx : (t165, _) idx_mut = (.a165.#a23) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a165) *)
    let shallow : (t165, _) idx_mut = (.a165) in
    let deepened = (.idx_mut(shallow).#a23) in
    ...
    (* from (.a165.#a23) *)
    let shallow : (t165, _) idx_mut = (.a165.#a23) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 495;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 495 failed %d" i;
  );
  (* Deepening to (.a165.#b23) *)
  let idx : (t165, _) idx_mut = (.a165.#b23) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a165) *)
    let shallow : (t165, _) idx_mut = (.a165) in
    let deepened = (.idx_mut(shallow).#b23) in
    ...
    (* from (.a165.#b23) *)
    let shallow : (t165, _) idx_mut = (.a165.#b23) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 497;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a166.#a25) *)
  let idx : (t166, _) idx_mut = (.a166.#a25) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a166) *)
    let shallow : (t166, _) idx_mut = (.a166) in
    let deepened = (.idx_mut(shallow).#a25) in
    ...
    (* from (.a166.#a25) *)
    let shallow : (t166, _) idx_mut = (.a166.#a25) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 501;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 501 failed %d" i;
  );
  (* Deepening to (.a166.#b25) *)
  let idx : (t166, _) idx_mut = (.a166.#b25) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a166) *)
    let shallow : (t166, _) idx_mut = (.a166) in
    let deepened = (.idx_mut(shallow).#b25) in
    ...
    (* from (.a166.#b25) *)
    let shallow : (t166, _) idx_mut = (.a166.#b25) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 503;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a167.#a25) *)
  let idx : (t167, _) idx_mut = (.a167.#a25) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a167) *)
    let shallow : (t167, _) idx_mut = (.a167) in
    let deepened = (.idx_mut(shallow).#a25) in
    ...
    (* from (.a167.#a25) *)
    let shallow : (t167, _) idx_mut = (.a167.#a25) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 506;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 506 failed %d" i;
  );
  (* Deepening to (.a167.#b25) *)
  let idx : (t167, _) idx_mut = (.a167.#b25) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a167) *)
    let shallow : (t167, _) idx_mut = (.a167) in
    let deepened = (.idx_mut(shallow).#b25) in
    ...
    (* from (.a167.#b25) *)
    let shallow : (t167, _) idx_mut = (.a167.#b25) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 508;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a168.#a25) *)
  let idx : (t168, _) idx_mut = (.a168.#a25) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a168) *)
    let shallow : (t168, _) idx_mut = (.a168) in
    let deepened = (.idx_mut(shallow).#a25) in
    ...
    (* from (.a168.#a25) *)
    let shallow : (t168, _) idx_mut = (.a168.#a25) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 512;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 512 failed %d" i;
  );
  (* Deepening to (.a168.#b25) *)
  let idx : (t168, _) idx_mut = (.a168.#b25) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a168) *)
    let shallow : (t168, _) idx_mut = (.a168) in
    let deepened = (.idx_mut(shallow).#b25) in
    ...
    (* from (.a168.#b25) *)
    let shallow : (t168, _) idx_mut = (.a168.#b25) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 514;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a169.#a27) *)
  let idx : (t169, _) idx_mut = (.a169.#a27) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a169) *)
    let shallow : (t169, _) idx_mut = (.a169) in
    let deepened = (.idx_mut(shallow).#a27) in
    ...
    (* from (.a169.#a27) *)
    let shallow : (t169, _) idx_mut = (.a169.#a27) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 518;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 518 failed %d" i;
  );
  (* Deepening to (.a169.#b27) *)
  let idx : (t169, _) idx_mut = (.a169.#b27) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a169) *)
    let shallow : (t169, _) idx_mut = (.a169) in
    let deepened = (.idx_mut(shallow).#b27) in
    ...
    (* from (.a169.#b27) *)
    let shallow : (t169, _) idx_mut = (.a169.#b27) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 520;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a170.#a27) *)
  let idx : (t170, _) idx_mut = (.a170.#a27) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a170) *)
    let shallow : (t170, _) idx_mut = (.a170) in
    let deepened = (.idx_mut(shallow).#a27) in
    ...
    (* from (.a170.#a27) *)
    let shallow : (t170, _) idx_mut = (.a170.#a27) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 523;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 523 failed %d" i;
  );
  (* Deepening to (.a170.#b27) *)
  let idx : (t170, _) idx_mut = (.a170.#b27) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a170) *)
    let shallow : (t170, _) idx_mut = (.a170) in
    let deepened = (.idx_mut(shallow).#b27) in
    ...
    (* from (.a170.#b27) *)
    let shallow : (t170, _) idx_mut = (.a170.#b27) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 525;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a171.#a27) *)
  let idx : (t171, _) idx_mut = (.a171.#a27) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a171) *)
    let shallow : (t171, _) idx_mut = (.a171) in
    let deepened = (.idx_mut(shallow).#a27) in
    ...
    (* from (.a171.#a27) *)
    let shallow : (t171, _) idx_mut = (.a171.#a27) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 529;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 529 failed %d" i;
  );
  (* Deepening to (.a171.#b27) *)
  let idx : (t171, _) idx_mut = (.a171.#b27) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a171) *)
    let shallow : (t171, _) idx_mut = (.a171) in
    let deepened = (.idx_mut(shallow).#b27) in
    ...
    (* from (.a171.#b27) *)
    let shallow : (t171, _) idx_mut = (.a171.#b27) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 531;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a174.#a172) *)
  let idx : (t174, _) idx_mut = (.a174.#a172) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a174) *)
    let shallow : (t174, _) idx_mut = (.a174) in
    ...
    (* from (.a174.#a172) *)
    let shallow : (t174, _) idx_mut = (.a174.#a172) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 535;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 535 failed %d" i;
  );
  (* Deepening to (.a174.#b172) *)
  let idx : (t174, _) idx_mut = (.a174.#b172) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a174) *)
    let shallow : (t174, _) idx_mut = (.a174) in
    ...
    (* from (.a174.#b172) *)
    let shallow : (t174, _) idx_mut = (.a174.#b172) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 537;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b174.#a173) *)
  let idx : (t174, _) idx_mut = (.b174.#a173) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b174) *)
    let shallow : (t174, _) idx_mut = (.b174) in
    let deepened = (.idx_mut(shallow).#a173) in
    mark_test_run 539;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 539 failed %d" i;
    (* from (.b174.#a173) *)
    let shallow : (t174, _) idx_mut = (.b174.#a173) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 540;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 540 failed %d" i;
  );
  (* Deepening to (.b174.#b173) *)
  let idx : (t174, _) idx_mut = (.b174.#b173) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b174) *)
    let shallow : (t174, _) idx_mut = (.b174) in
    let deepened = (.idx_mut(shallow).#b173) in
    mark_test_run 541;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 541 failed %d" i;
    (* from (.b174.#b173) *)
    let shallow : (t174, _) idx_mut = (.b174.#b173) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 542;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a177.#a175) *)
  let idx : (t177, _) idx_mut = (.a177.#a175) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a177) *)
    let shallow : (t177, _) idx_mut = (.a177) in
    ...
    (* from (.a177.#a175) *)
    let shallow : (t177, _) idx_mut = (.a177.#a175) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 545;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 545 failed %d" i;
  );
  (* Deepening to (.a177.#b175) *)
  let idx : (t177, _) idx_mut = (.a177.#b175) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a177) *)
    let shallow : (t177, _) idx_mut = (.a177) in
    let deepened = (.idx_mut(shallow).#b175) in
    mark_test_run 546;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 546 failed %d" i;
    (* from (.a177.#b175) *)
    let shallow : (t177, _) idx_mut = (.a177.#b175) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 547;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b177.#a176) *)
  let idx : (t177, _) idx_mut = (.b177.#a176) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b177) *)
    let shallow : (t177, _) idx_mut = (.b177) in
    let deepened = (.idx_mut(shallow).#a176) in
    mark_test_run 549;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 549 failed %d" i;
    (* from (.b177.#a176) *)
    let shallow : (t177, _) idx_mut = (.b177.#a176) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 550;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 550 failed %d" i;
  );
  (* Deepening to (.b177.#b176) *)
  let idx : (t177, _) idx_mut = (.b177.#b176) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b177) *)
    let shallow : (t177, _) idx_mut = (.b177) in
    let deepened = (.idx_mut(shallow).#b176) in
    mark_test_run 551;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 551 failed %d" i;
    (* from (.b177.#b176) *)
    let shallow : (t177, _) idx_mut = (.b177.#b176) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 552;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a180.#a178) *)
  let idx : (t180, _) idx_mut = (.a180.#a178) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a180) *)
    let shallow : (t180, _) idx_mut = (.a180) in
    let deepened = (.idx_mut(shallow).#a178) in
    mark_test_run 554;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 554 failed %d" i;
    (* from (.a180.#a178) *)
    let shallow : (t180, _) idx_mut = (.a180.#a178) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 555;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 555 failed %d" i;
  );
  (* Deepening to (.a180.#b178) *)
  let idx : (t180, _) idx_mut = (.a180.#b178) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a180) *)
    let shallow : (t180, _) idx_mut = (.a180) in
    let deepened = (.idx_mut(shallow).#b178) in
    mark_test_run 556;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 556 failed %d" i;
    (* from (.a180.#b178) *)
    let shallow : (t180, _) idx_mut = (.a180.#b178) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 557;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b180.#b179) *)
  let idx : (t180, _) idx_mut = (.b180.#b179) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b180) *)
    let shallow : (t180, _) idx_mut = (.b180) in
    let deepened = (.idx_mut(shallow).#b179) in
    mark_test_run 561;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 561 failed %d" i;
    (* from (.b180.#b179) *)
    let shallow : (t180, _) idx_mut = (.b180.#b179) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 562;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a182.#a178) *)
  let idx : (t182, _) idx_mut = (.a182.#a178) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a182) *)
    let shallow : (t182, _) idx_mut = (.a182) in
    let deepened = (.idx_mut(shallow).#a178) in
    ...
    (* from (.a182.#a178) *)
    let shallow : (t182, _) idx_mut = (.a182.#a178) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 565;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 565 failed %d" i;
  );
  (* Deepening to (.a182.#b178) *)
  let idx : (t182, _) idx_mut = (.a182.#b178) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a182) *)
    let shallow : (t182, _) idx_mut = (.a182) in
    let deepened = (.idx_mut(shallow).#b178) in
    ...
    (* from (.a182.#b178) *)
    let shallow : (t182, _) idx_mut = (.a182.#b178) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 567;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b182.#a181) *)
  let idx : (t182, _) idx_mut = (.b182.#a181) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b182) *)
    let shallow : (t182, _) idx_mut = (.b182) in
    let deepened = (.idx_mut(shallow).#a181) in
    mark_test_run 569;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 569 failed %d" i;
    (* from (.b182.#a181) *)
    let shallow : (t182, _) idx_mut = (.b182.#a181) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 570;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 570 failed %d" i;
  );
  (* Deepening to (.b182.#b181) *)
  let idx : (t182, _) idx_mut = (.b182.#b181) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b182) *)
    let shallow : (t182, _) idx_mut = (.b182) in
    let deepened = (.idx_mut(shallow).#b181) in
    mark_test_run 571;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 571 failed %d" i;
    (* from (.b182.#b181) *)
    let shallow : (t182, _) idx_mut = (.b182.#b181) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 572;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a183.#a29) *)
  let idx : (t183, _) idx_mut = (.a183.#a29) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a183) *)
    let shallow : (t183, _) idx_mut = (.a183) in
    let deepened = (.idx_mut(shallow).#a29) in
    ...
    (* from (.a183.#a29) *)
    let shallow : (t183, _) idx_mut = (.a183.#a29) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 575;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a184.#a29) *)
  let idx : (t184, _) idx_mut = (.a184.#a29) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a184) *)
    let shallow : (t184, _) idx_mut = (.a184) in
    let deepened = (.idx_mut(shallow).#a29) in
    ...
    (* from (.a184.#a29) *)
    let shallow : (t184, _) idx_mut = (.a184.#a29) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 578;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a185.#a29) *)
  let idx : (t185, _) idx_mut = (.a185.#a29) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a185) *)
    let shallow : (t185, _) idx_mut = (.a185) in
    let deepened = (.idx_mut(shallow).#a29) in
    ...
    (* from (.a185.#a29) *)
    let shallow : (t185, _) idx_mut = (.a185.#a29) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 582;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a186.#a31) *)
  let idx : (t186, _) idx_mut = (.a186.#a31) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a186) *)
    let shallow : (t186, _) idx_mut = (.a186) in
    let deepened = (.idx_mut(shallow).#a31) in
    ...
    (* from (.a186.#a31) *)
    let shallow : (t186, _) idx_mut = (.a186.#a31) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 586;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 586 failed %d" i;
  );
  (* Deepening to (.a186.#b31) *)
  let idx : (t186, _) idx_mut = (.a186.#b31) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a186) *)
    let shallow : (t186, _) idx_mut = (.a186) in
    let deepened = (.idx_mut(shallow).#b31) in
    ...
    (* from (.a186.#b31) *)
    let shallow : (t186, _) idx_mut = (.a186.#b31) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 588;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a187.#a31) *)
  let idx : (t187, _) idx_mut = (.a187.#a31) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a187) *)
    let shallow : (t187, _) idx_mut = (.a187) in
    let deepened = (.idx_mut(shallow).#a31) in
    ...
    (* from (.a187.#a31) *)
    let shallow : (t187, _) idx_mut = (.a187.#a31) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 591;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 591 failed %d" i;
  );
  (* Deepening to (.a187.#b31) *)
  let idx : (t187, _) idx_mut = (.a187.#b31) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a187) *)
    let shallow : (t187, _) idx_mut = (.a187) in
    let deepened = (.idx_mut(shallow).#b31) in
    ...
    (* from (.a187.#b31) *)
    let shallow : (t187, _) idx_mut = (.a187.#b31) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 593;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a188.#a31) *)
  let idx : (t188, _) idx_mut = (.a188.#a31) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a188) *)
    let shallow : (t188, _) idx_mut = (.a188) in
    let deepened = (.idx_mut(shallow).#a31) in
    ...
    (* from (.a188.#a31) *)
    let shallow : (t188, _) idx_mut = (.a188.#a31) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 597;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 597 failed %d" i;
  );
  (* Deepening to (.a188.#b31) *)
  let idx : (t188, _) idx_mut = (.a188.#b31) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a188) *)
    let shallow : (t188, _) idx_mut = (.a188) in
    let deepened = (.idx_mut(shallow).#b31) in
    ...
    (* from (.a188.#b31) *)
    let shallow : (t188, _) idx_mut = (.a188.#b31) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 599;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a189.#a33) *)
  let idx : (t189, _) idx_mut = (.a189.#a33) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a189) *)
    let shallow : (t189, _) idx_mut = (.a189) in
    let deepened = (.idx_mut(shallow).#a33) in
    ...
    (* from (.a189.#a33) *)
    let shallow : (t189, _) idx_mut = (.a189.#a33) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 603;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 603 failed %d" i;
  );
  (* Deepening to (.a189.#b33) *)
  let idx : (t189, _) idx_mut = (.a189.#b33) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a189) *)
    let shallow : (t189, _) idx_mut = (.a189) in
    let deepened = (.idx_mut(shallow).#b33) in
    ...
    (* from (.a189.#b33) *)
    let shallow : (t189, _) idx_mut = (.a189.#b33) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 605;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a190.#a33) *)
  let idx : (t190, _) idx_mut = (.a190.#a33) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a190) *)
    let shallow : (t190, _) idx_mut = (.a190) in
    let deepened = (.idx_mut(shallow).#a33) in
    ...
    (* from (.a190.#a33) *)
    let shallow : (t190, _) idx_mut = (.a190.#a33) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 608;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 608 failed %d" i;
  );
  (* Deepening to (.a190.#b33) *)
  let idx : (t190, _) idx_mut = (.a190.#b33) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a190) *)
    let shallow : (t190, _) idx_mut = (.a190) in
    let deepened = (.idx_mut(shallow).#b33) in
    ...
    (* from (.a190.#b33) *)
    let shallow : (t190, _) idx_mut = (.a190.#b33) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 610;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a191.#a33) *)
  let idx : (t191, _) idx_mut = (.a191.#a33) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a191) *)
    let shallow : (t191, _) idx_mut = (.a191) in
    let deepened = (.idx_mut(shallow).#a33) in
    ...
    (* from (.a191.#a33) *)
    let shallow : (t191, _) idx_mut = (.a191.#a33) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 614;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 614 failed %d" i;
  );
  (* Deepening to (.a191.#b33) *)
  let idx : (t191, _) idx_mut = (.a191.#b33) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a191) *)
    let shallow : (t191, _) idx_mut = (.a191) in
    let deepened = (.idx_mut(shallow).#b33) in
    ...
    (* from (.a191.#b33) *)
    let shallow : (t191, _) idx_mut = (.a191.#b33) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 616;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a192.#a35) *)
  let idx : (t192, _) idx_mut = (.a192.#a35) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* Note: can't deepen (.a192) because it's a path to a flattened
       float, making its element type [float#] *)
    (* from (.a192.#a35) *)
    let shallow : (t192, _) idx_mut = (.a192.#a35) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 619;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a193.#a35) *)
  let idx : (t193, _) idx_mut = (.a193.#a35) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a193) *)
    let shallow : (t193, _) idx_mut = (.a193) in
    let deepened = (.idx_mut(shallow).#a35) in
    ...
    (* from (.a193.#a35) *)
    let shallow : (t193, _) idx_mut = (.a193.#a35) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 622;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a194.#a35) *)
  let idx : (t194, _) idx_mut = (.a194.#a35) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* Note: can't deepen (.a194) because it's a path to a flattened
       float, making its element type [float#] *)
    (* from (.a194.#a35) *)
    let shallow : (t194, _) idx_mut = (.a194.#a35) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 625;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a195.#a35) *)
  let idx : (t195, _) idx_mut = (.a195.#a35) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* Note: can't deepen (.a195) because it's a path to a flattened
       float, making its element type [float#] *)
    (* from (.a195.#a35) *)
    let shallow : (t195, _) idx_mut = (.a195.#a35) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 628;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a196.#a37) *)
  let idx : (t196, _) idx_mut = (.a196.#a37) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a196) *)
    let shallow : (t196, _) idx_mut = (.a196) in
    let deepened = (.idx_mut(shallow).#a37) in
    ...
    (* from (.a196.#a37) *)
    let shallow : (t196, _) idx_mut = (.a196.#a37) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 632;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 632 failed %d" i;
  );
  (* Deepening to (.a196.#b37) *)
  let idx : (t196, _) idx_mut = (.a196.#b37) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a196) *)
    let shallow : (t196, _) idx_mut = (.a196) in
    let deepened = (.idx_mut(shallow).#b37) in
    ...
    (* from (.a196.#b37) *)
    let shallow : (t196, _) idx_mut = (.a196.#b37) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 634;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a197.#a37) *)
  let idx : (t197, _) idx_mut = (.a197.#a37) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a197) *)
    let shallow : (t197, _) idx_mut = (.a197) in
    let deepened = (.idx_mut(shallow).#a37) in
    ...
    (* from (.a197.#a37) *)
    let shallow : (t197, _) idx_mut = (.a197.#a37) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 637;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 637 failed %d" i;
  );
  (* Deepening to (.a197.#b37) *)
  let idx : (t197, _) idx_mut = (.a197.#b37) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a197) *)
    let shallow : (t197, _) idx_mut = (.a197) in
    let deepened = (.idx_mut(shallow).#b37) in
    ...
    (* from (.a197.#b37) *)
    let shallow : (t197, _) idx_mut = (.a197.#b37) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 639;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a198.#a37) *)
  let idx : (t198, _) idx_mut = (.a198.#a37) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a198) *)
    let shallow : (t198, _) idx_mut = (.a198) in
    let deepened = (.idx_mut(shallow).#a37) in
    ...
    (* from (.a198.#a37) *)
    let shallow : (t198, _) idx_mut = (.a198.#a37) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 643;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 643 failed %d" i;
  );
  (* Deepening to (.a198.#b37) *)
  let idx : (t198, _) idx_mut = (.a198.#b37) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a198) *)
    let shallow : (t198, _) idx_mut = (.a198) in
    let deepened = (.idx_mut(shallow).#b37) in
    ...
    (* from (.a198.#b37) *)
    let shallow : (t198, _) idx_mut = (.a198.#b37) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 645;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a199.#a39) *)
  let idx : (t199, _) idx_mut = (.a199.#a39) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a199) *)
    let shallow : (t199, _) idx_mut = (.a199) in
    let deepened = (.idx_mut(shallow).#a39) in
    ...
    (* from (.a199.#a39) *)
    let shallow : (t199, _) idx_mut = (.a199.#a39) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 649;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 649 failed %d" i;
  );
  (* Deepening to (.a199.#b39) *)
  let idx : (t199, _) idx_mut = (.a199.#b39) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a199) *)
    let shallow : (t199, _) idx_mut = (.a199) in
    let deepened = (.idx_mut(shallow).#b39) in
    ...
    (* from (.a199.#b39) *)
    let shallow : (t199, _) idx_mut = (.a199.#b39) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 651;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a200.#a39) *)
  let idx : (t200, _) idx_mut = (.a200.#a39) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a200) *)
    let shallow : (t200, _) idx_mut = (.a200) in
    let deepened = (.idx_mut(shallow).#a39) in
    ...
    (* from (.a200.#a39) *)
    let shallow : (t200, _) idx_mut = (.a200.#a39) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 654;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 654 failed %d" i;
  );
  (* Deepening to (.a200.#b39) *)
  let idx : (t200, _) idx_mut = (.a200.#b39) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a200) *)
    let shallow : (t200, _) idx_mut = (.a200) in
    let deepened = (.idx_mut(shallow).#b39) in
    ...
    (* from (.a200.#b39) *)
    let shallow : (t200, _) idx_mut = (.a200.#b39) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 656;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a201.#a39) *)
  let idx : (t201, _) idx_mut = (.a201.#a39) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a201) *)
    let shallow : (t201, _) idx_mut = (.a201) in
    let deepened = (.idx_mut(shallow).#a39) in
    ...
    (* from (.a201.#a39) *)
    let shallow : (t201, _) idx_mut = (.a201.#a39) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 660;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 660 failed %d" i;
  );
  (* Deepening to (.a201.#b39) *)
  let idx : (t201, _) idx_mut = (.a201.#b39) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a201) *)
    let shallow : (t201, _) idx_mut = (.a201) in
    let deepened = (.idx_mut(shallow).#b39) in
    ...
    (* from (.a201.#b39) *)
    let shallow : (t201, _) idx_mut = (.a201.#b39) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 662;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a202.#a39) *)
  let idx : (t202, _) idx_mut = (.a202.#a39) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a202) *)
    let shallow : (t202, _) idx_mut = (.a202) in
    let deepened = (.idx_mut(shallow).#a39) in
    ...
    (* from (.a202.#a39) *)
    let shallow : (t202, _) idx_mut = (.a202.#a39) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 666;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 666 failed %d" i;
  );
  (* Deepening to (.a202.#b39) *)
  let idx : (t202, _) idx_mut = (.a202.#b39) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a202) *)
    let shallow : (t202, _) idx_mut = (.a202) in
    let deepened = (.idx_mut(shallow).#b39) in
    ...
    (* from (.a202.#b39) *)
    let shallow : (t202, _) idx_mut = (.a202.#b39) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 668;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a203.#a117) *)
  let idx : (t203, _) idx_mut = (.a203.#a117) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a203) *)
    let shallow : (t203, _) idx_mut = (.a203) in
    let deepened = (.idx_mut(shallow).#a117) in
    ...
    (* from (.a203.#a117) *)
    let shallow : (t203, _) idx_mut = (.a203.#a117) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 672;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 672 failed %d" i;
  );
  (* Deepening to (.a203.#b117) *)
  let idx : (t203, _) idx_mut = (.a203.#b117) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a203) *)
    let shallow : (t203, _) idx_mut = (.a203) in
    let deepened = (.idx_mut(shallow).#b117) in
    ...
    (* from (.a203.#b117) *)
    let shallow : (t203, _) idx_mut = (.a203.#b117) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 674;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a204.#a117) *)
  let idx : (t204, _) idx_mut = (.a204.#a117) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a204) *)
    let shallow : (t204, _) idx_mut = (.a204) in
    let deepened = (.idx_mut(shallow).#a117) in
    ...
    (* from (.a204.#a117) *)
    let shallow : (t204, _) idx_mut = (.a204.#a117) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 677;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 677 failed %d" i;
  );
  (* Deepening to (.a204.#b117) *)
  let idx : (t204, _) idx_mut = (.a204.#b117) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a204) *)
    let shallow : (t204, _) idx_mut = (.a204) in
    let deepened = (.idx_mut(shallow).#b117) in
    ...
    (* from (.a204.#b117) *)
    let shallow : (t204, _) idx_mut = (.a204.#b117) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 679;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a205.#a117) *)
  let idx : (t205, _) idx_mut = (.a205.#a117) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a205) *)
    let shallow : (t205, _) idx_mut = (.a205) in
    let deepened = (.idx_mut(shallow).#a117) in
    ...
    (* from (.a205.#a117) *)
    let shallow : (t205, _) idx_mut = (.a205.#a117) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 683;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 683 failed %d" i;
  );
  (* Deepening to (.a205.#b117) *)
  let idx : (t205, _) idx_mut = (.a205.#b117) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a205) *)
    let shallow : (t205, _) idx_mut = (.a205) in
    let deepened = (.idx_mut(shallow).#b117) in
    ...
    (* from (.a205.#b117) *)
    let shallow : (t205, _) idx_mut = (.a205.#b117) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 685;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a206.#a119) *)
  let idx : (t206, _) idx_mut = (.a206.#a119) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a206) *)
    let shallow : (t206, _) idx_mut = (.a206) in
    let deepened = (.idx_mut(shallow).#a119) in
    ...
    (* from (.a206.#a119) *)
    let shallow : (t206, _) idx_mut = (.a206.#a119) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 689;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a207.#a119) *)
  let idx : (t207, _) idx_mut = (.a207.#a119) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a207) *)
    let shallow : (t207, _) idx_mut = (.a207) in
    let deepened = (.idx_mut(shallow).#a119) in
    ...
    (* from (.a207.#a119) *)
    let shallow : (t207, _) idx_mut = (.a207.#a119) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 692;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a208.#a119) *)
  let idx : (t208, _) idx_mut = (.a208.#a119) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a208) *)
    let shallow : (t208, _) idx_mut = (.a208) in
    let deepened = (.idx_mut(shallow).#a119) in
    ...
    (* from (.a208.#a119) *)
    let shallow : (t208, _) idx_mut = (.a208.#a119) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 696;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a209.#a121) *)
  let idx : (t209, _) idx_mut = (.a209.#a121) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a209) *)
    let shallow : (t209, _) idx_mut = (.a209) in
    let deepened = (.idx_mut(shallow).#a121) in
    ...
    (* from (.a209.#a121) *)
    let shallow : (t209, _) idx_mut = (.a209.#a121) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 700;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 700 failed %d" i;
  );
  (* Deepening to (.a209.#b121) *)
  let idx : (t209, _) idx_mut = (.a209.#b121) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a209) *)
    let shallow : (t209, _) idx_mut = (.a209) in
    let deepened = (.idx_mut(shallow).#b121) in
    ...
    (* from (.a209.#b121) *)
    let shallow : (t209, _) idx_mut = (.a209.#b121) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 702;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a210.#a121) *)
  let idx : (t210, _) idx_mut = (.a210.#a121) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a210) *)
    let shallow : (t210, _) idx_mut = (.a210) in
    let deepened = (.idx_mut(shallow).#a121) in
    ...
    (* from (.a210.#a121) *)
    let shallow : (t210, _) idx_mut = (.a210.#a121) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 705;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 705 failed %d" i;
  );
  (* Deepening to (.a210.#b121) *)
  let idx : (t210, _) idx_mut = (.a210.#b121) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a210) *)
    let shallow : (t210, _) idx_mut = (.a210) in
    let deepened = (.idx_mut(shallow).#b121) in
    ...
    (* from (.a210.#b121) *)
    let shallow : (t210, _) idx_mut = (.a210.#b121) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 707;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a211.#a121) *)
  let idx : (t211, _) idx_mut = (.a211.#a121) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a211) *)
    let shallow : (t211, _) idx_mut = (.a211) in
    let deepened = (.idx_mut(shallow).#a121) in
    ...
    (* from (.a211.#a121) *)
    let shallow : (t211, _) idx_mut = (.a211.#a121) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 711;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 711 failed %d" i;
  );
  (* Deepening to (.a211.#b121) *)
  let idx : (t211, _) idx_mut = (.a211.#b121) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a211) *)
    let shallow : (t211, _) idx_mut = (.a211) in
    let deepened = (.idx_mut(shallow).#b121) in
    ...
    (* from (.a211.#b121) *)
    let shallow : (t211, _) idx_mut = (.a211.#b121) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 713;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a212.#a123) *)
  let idx : (t212, _) idx_mut = (.a212.#a123) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a212) *)
    let shallow : (t212, _) idx_mut = (.a212) in
    let deepened = (.idx_mut(shallow).#a123) in
    ...
    (* from (.a212.#a123) *)
    let shallow : (t212, _) idx_mut = (.a212.#a123) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 717;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 717 failed %d" i;
  );
  (* Deepening to (.a212.#b123) *)
  let idx : (t212, _) idx_mut = (.a212.#b123) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a212) *)
    let shallow : (t212, _) idx_mut = (.a212) in
    let deepened = (.idx_mut(shallow).#b123) in
    ...
    (* from (.a212.#b123) *)
    let shallow : (t212, _) idx_mut = (.a212.#b123) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 719;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a213.#a123) *)
  let idx : (t213, _) idx_mut = (.a213.#a123) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a213) *)
    let shallow : (t213, _) idx_mut = (.a213) in
    let deepened = (.idx_mut(shallow).#a123) in
    ...
    (* from (.a213.#a123) *)
    let shallow : (t213, _) idx_mut = (.a213.#a123) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 722;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 722 failed %d" i;
  );
  (* Deepening to (.a213.#b123) *)
  let idx : (t213, _) idx_mut = (.a213.#b123) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a213) *)
    let shallow : (t213, _) idx_mut = (.a213) in
    let deepened = (.idx_mut(shallow).#b123) in
    ...
    (* from (.a213.#b123) *)
    let shallow : (t213, _) idx_mut = (.a213.#b123) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 724;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a214.#a123) *)
  let idx : (t214, _) idx_mut = (.a214.#a123) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a214) *)
    let shallow : (t214, _) idx_mut = (.a214) in
    let deepened = (.idx_mut(shallow).#a123) in
    ...
    (* from (.a214.#a123) *)
    let shallow : (t214, _) idx_mut = (.a214.#a123) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 728;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 728 failed %d" i;
  );
  (* Deepening to (.a214.#b123) *)
  let idx : (t214, _) idx_mut = (.a214.#b123) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a214) *)
    let shallow : (t214, _) idx_mut = (.a214) in
    let deepened = (.idx_mut(shallow).#b123) in
    ...
    (* from (.a214.#b123) *)
    let shallow : (t214, _) idx_mut = (.a214.#b123) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 730;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a217.#a215) *)
  let idx : (t217, _) idx_mut = (.a217.#a215) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a217) *)
    let shallow : (t217, _) idx_mut = (.a217) in
    ...
    (* from (.a217.#a215) *)
    let shallow : (t217, _) idx_mut = (.a217.#a215) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 734;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 734 failed %d" i;
  );
  (* Deepening to (.a217.#b215) *)
  let idx : (t217, _) idx_mut = (.a217.#b215) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a217) *)
    let shallow : (t217, _) idx_mut = (.a217) in
    ...
    (* from (.a217.#b215) *)
    let shallow : (t217, _) idx_mut = (.a217.#b215) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 736;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.b217.#a216) *)
  let idx : (t217, _) idx_mut = (.b217.#a216) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b217) *)
    let shallow : (t217, _) idx_mut = (.b217) in
    ...
    (* from (.b217.#a216) *)
    let shallow : (t217, _) idx_mut = (.b217.#a216) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 739;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 739 failed %d" i;
  );
  (* Deepening to (.b217.#b216) *)
  let idx : (t217, _) idx_mut = (.b217.#b216) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.b217) *)
    let shallow : (t217, _) idx_mut = (.b217) in
    ...
    (* from (.b217.#b216) *)
    let shallow : (t217, _) idx_mut = (.b217.#b216) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 741;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a218.#a41) *)
  let idx : (t218, _) idx_mut = (.a218.#a41) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a218) *)
    let shallow : (t218, _) idx_mut = (.a218) in
    let deepened = (.idx_mut(shallow).#a41) in
    ...
    (* from (.a218.#a41) *)
    let shallow : (t218, _) idx_mut = (.a218.#a41) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 744;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a219.#a41) *)
  let idx : (t219, _) idx_mut = (.a219.#a41) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a219) *)
    let shallow : (t219, _) idx_mut = (.a219) in
    let deepened = (.idx_mut(shallow).#a41) in
    ...
    (* from (.a219.#a41) *)
    let shallow : (t219, _) idx_mut = (.a219.#a41) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 747;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a220.#a41) *)
  let idx : (t220, _) idx_mut = (.a220.#a41) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a220) *)
    let shallow : (t220, _) idx_mut = (.a220) in
    let deepened = (.idx_mut(shallow).#a41) in
    ...
    (* from (.a220.#a41) *)
    let shallow : (t220, _) idx_mut = (.a220.#a41) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 751;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a221.#a43) *)
  let idx : (t221, _) idx_mut = (.a221.#a43) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a221) *)
    let shallow : (t221, _) idx_mut = (.a221) in
    let deepened = (.idx_mut(shallow).#a43) in
    ...
    (* from (.a221.#a43) *)
    let shallow : (t221, _) idx_mut = (.a221.#a43) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 755;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 755 failed %d" i;
  );
  (* Deepening to (.a221.#b43) *)
  let idx : (t221, _) idx_mut = (.a221.#b43) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a221) *)
    let shallow : (t221, _) idx_mut = (.a221) in
    let deepened = (.idx_mut(shallow).#b43) in
    ...
    (* from (.a221.#b43) *)
    let shallow : (t221, _) idx_mut = (.a221.#b43) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 757;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a222.#a43) *)
  let idx : (t222, _) idx_mut = (.a222.#a43) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a222) *)
    let shallow : (t222, _) idx_mut = (.a222) in
    let deepened = (.idx_mut(shallow).#a43) in
    ...
    (* from (.a222.#a43) *)
    let shallow : (t222, _) idx_mut = (.a222.#a43) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 760;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 760 failed %d" i;
  );
  (* Deepening to (.a222.#b43) *)
  let idx : (t222, _) idx_mut = (.a222.#b43) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a222) *)
    let shallow : (t222, _) idx_mut = (.a222) in
    let deepened = (.idx_mut(shallow).#b43) in
    ...
    (* from (.a222.#b43) *)
    let shallow : (t222, _) idx_mut = (.a222.#b43) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 762;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a223.#a43) *)
  let idx : (t223, _) idx_mut = (.a223.#a43) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a223) *)
    let shallow : (t223, _) idx_mut = (.a223) in
    let deepened = (.idx_mut(shallow).#a43) in
    ...
    (* from (.a223.#a43) *)
    let shallow : (t223, _) idx_mut = (.a223.#a43) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 766;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 766 failed %d" i;
  );
  (* Deepening to (.a223.#b43) *)
  let idx : (t223, _) idx_mut = (.a223.#b43) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a223) *)
    let shallow : (t223, _) idx_mut = (.a223) in
    let deepened = (.idx_mut(shallow).#b43) in
    ...
    (* from (.a223.#b43) *)
    let shallow : (t223, _) idx_mut = (.a223.#b43) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 768;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a226.#a224) *)
  let idx : (t226, _) idx_mut = (.a226.#a224) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a226) *)
    let shallow : (t226, _) idx_mut = (.a226) in
    let deepened = (.idx_mut(shallow).#a224) in
    mark_test_run 771;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 771 failed %d" i;
    (* from (.a226.#a224) *)
    let shallow : (t226, _) idx_mut = (.a226.#a224) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 772;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 772 failed %d" i;
  );
  (* Deepening to (.a226.#b224) *)
  let idx : (t226, _) idx_mut = (.a226.#b224) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a226) *)
    let shallow : (t226, _) idx_mut = (.a226) in
    let deepened = (.idx_mut(shallow).#b224) in
    mark_test_run 773;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 773 failed %d" i;
    (* from (.a226.#b224) *)
    let shallow : (t226, _) idx_mut = (.a226.#b224) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 774;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a227.#a45) *)
  let idx : (t227, _) idx_mut = (.a227.#a45) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a227) *)
    let shallow : (t227, _) idx_mut = (.a227) in
    let deepened = (.idx_mut(shallow).#a45) in
    ...
    (* from (.a227.#a45) *)
    let shallow : (t227, _) idx_mut = (.a227.#a45) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 782;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 782 failed %d" i;
  );
  (* Deepening to (.a227.#b45) *)
  let idx : (t227, _) idx_mut = (.a227.#b45) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a227) *)
    let shallow : (t227, _) idx_mut = (.a227) in
    let deepened = (.idx_mut(shallow).#b45) in
    ...
    (* from (.a227.#b45) *)
    let shallow : (t227, _) idx_mut = (.a227.#b45) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 784;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a228.#a45) *)
  let idx : (t228, _) idx_mut = (.a228.#a45) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a228) *)
    let shallow : (t228, _) idx_mut = (.a228) in
    let deepened = (.idx_mut(shallow).#a45) in
    ...
    (* from (.a228.#a45) *)
    let shallow : (t228, _) idx_mut = (.a228.#a45) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 787;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 787 failed %d" i;
  );
  (* Deepening to (.a228.#b45) *)
  let idx : (t228, _) idx_mut = (.a228.#b45) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a228) *)
    let shallow : (t228, _) idx_mut = (.a228) in
    let deepened = (.idx_mut(shallow).#b45) in
    ...
    (* from (.a228.#b45) *)
    let shallow : (t228, _) idx_mut = (.a228.#b45) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 789;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
  ...
  (* Deepening to (.a229.#a45) *)
  let idx : (t229, _) idx_mut = (.a229.#a45) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a229) *)
    let shallow : (t229, _) idx_mut = (.a229) in
    let deepened = (.idx_mut(shallow).#a45) in
    ...
    (* from (.a229.#a45) *)
    let shallow : (t229, _) idx_mut = (.a229.#a45) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 793;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
    if not test then failwithf "test 793 failed %d" i;
  );
  (* Deepening to (.a229.#b45) *)
  let idx : (t229, _) idx_mut = (.a229.#b45) in
  iter indices_in_deepening_tests ~f:(fun i ->
    (* from (.a229) *)
    let shallow : (t229, _) idx_mut = (.a229) in
    let deepened = (.idx_mut(shallow).#b45) in
    ...
    (* from (.a229.#b45) *)
    let shallow : (t229, _) idx_mut = (.a229.#b45) in
    let deepened = (.idx_mut(shallow)) in
    mark_test_run 795;
    let test = Idx_repr.equal (Idx_repr.of_idx_mut idx) (Idx_repr.of_idx_mut deepened) in
