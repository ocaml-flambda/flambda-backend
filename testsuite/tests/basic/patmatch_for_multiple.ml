(* TEST
 flags = "-drawlambda -dlambda";
 expect;
*)

(* Note: the tests below contain *both* the -drawlambda and
   the -dlambda intermediate representations:
   -drawlambda is the Lambda code generated directly by the
     pattern-matching compiler; it contain "alias" bindings or static
     exits that are unused, and will be removed by simplification, or
     that are used only once, and will be inlined by simplification.
   -dlambda is the Lambda code resulting from simplification.

  The -drawlambda output more closely matches what the
  pattern-compiler produces, and the -dlambda output more closely
  matches the final generated code.

  In this test we decided to show both to notice that some allocations
  are "optimized away" during simplification (see "here flattening is
  an optimization" below).
*)

match (3, 2, 1) with
| (_, 3, _)
| (1, _, _) -> true
| _ -> false
;;
[%%expect{|
(let (*match*/281 =[int] 3 *match*/282 =[int] 2 *match*/283 =[int] 1)
  (catch
    (catch
      (catch (if (!= *match*/282 3) (exit 3) (exit 1)) with (3)
        (if (!= *match*/281 1) (exit 2) (exit 1)))
     with (2) 0)
   with (1) 1))
(let (*match*/281 =[int] 3 *match*/282 =[int] 2 *match*/283 =[int] 1)
  (catch (if (!= *match*/282 3) (if (!= *match*/281 1) 0 (exit 1)) (exit 1))
   with (1) 1))
- : bool = false
|}];;

(* This tests needs to allocate the tuple to bind 'x',
   but this is only done in the branches that use it. *)
match (3, 2, 1) with
| ((_, 3, _) as x)
| ((1, _, _) as x) -> ignore x; true
| _ -> false
;;
[%%expect{|
(let (*match*/286 =[int] 3 *match*/287 =[int] 2 *match*/288 =[int] 1)
  (catch
    (catch
      (catch
        (if (!= *match*/287 3) (exit 6)
          (let
            (x/290 =a[(consts ()) (non_consts ([0: [int], [int], [int]]))]
               (makeblock 0 *match*/286 *match*/287 *match*/288))
            (exit 4 x/290)))
       with (6)
        (if (!= *match*/286 1) (exit 5)
          (let
            (x/289 =a[(consts ()) (non_consts ([0: [int], [int], [int]]))]
               (makeblock 0 *match*/286 *match*/287 *match*/288))
            (exit 4 x/289))))
     with (5) 0)
   with (4 x/284[(consts ()) (non_consts ([0: [int], [int], [int]]))])
    (seq (ignore x/284) 1)))
(let (*match*/286 =[int] 3 *match*/287 =[int] 2 *match*/288 =[int] 1)
  (catch
    (if (!= *match*/287 3)
      (if (!= *match*/286 1) 0
        (exit 4 (makeblock 0 *match*/286 *match*/287 *match*/288)))
      (exit 4 (makeblock 0 *match*/286 *match*/287 *match*/288)))
   with (4 x/284[(consts ()) (non_consts ([0: [int], [int], [int]]))])
    (seq (ignore x/284) 1)))
- : bool = false
|}];;

(* Regression test for #3780 *)
let _ = fun a b ->
  match a, b with
  | ((true, _) as _g)
  | ((false, _) as _g) -> ()
[%%expect{|
(function {nlocal = 0} a/291[int] b/292 : int 0)
(function {nlocal = 0} a/291[int] b/292 : int 0)
- : bool -> 'a -> unit = <fun>
|}];;

(* More complete tests.

   The test cases below compare the compiler output on alias patterns
   that are outside an or-pattern (handled during half-simplification,
   then flattened) or inside an or-pattern (handled during simplification).

   We used to have a Cannot_flatten exception that would result in fairly
   different code generated in both cases, but now the compilation strategy
   is fairly similar.
*)
let _ = fun a b -> match a, b with
| (true, _) as p -> p
| (false, _) as p -> p
(* outside, trivial *)
[%%expect {|
(function {nlocal = 0} a/295[int] b/296
  [(consts ()) (non_consts ([0: [int], *]))](let
                                              (p/297 =a[(consts ())
                                                        (non_consts (
                                                        [0: [int], *]))]
                                                 (makeblock 0 a/295 b/296))
                                              p/297))
(function {nlocal = 0} a/295[int] b/296
  [(consts ()) (non_consts ([0: [int], *]))](makeblock 0 a/295 b/296))
- : bool -> 'a -> bool * 'a = <fun>
|}]

let _ = fun a b -> match a, b with
| ((true, _) as p)
| ((false, _) as p) -> p
(* inside, trivial *)
[%%expect{|
(function {nlocal = 0} a/299[int] b/300
  [(consts ()) (non_consts ([0: [int], *]))](let
                                              (p/301 =a[(consts ())
                                                        (non_consts (
                                                        [0: [int], *]))]
                                                 (makeblock 0 a/299 b/300))
                                              p/301))
(function {nlocal = 0} a/299[int] b/300
  [(consts ()) (non_consts ([0: [int], *]))](makeblock 0 a/299 b/300))
- : bool -> 'a -> bool * 'a = <fun>
|}];;

let _ = fun a b -> match a, b with
| (true as x, _) as p -> x, p
| (false as x, _) as p -> x, p
(* outside, simple *)
[%%expect {|
(function {nlocal = 0} a/305[int] b/306
  [(consts ())
   (non_consts ([0: [int], [(consts ()) (non_consts ([0: [int], *]))]]))]
  (let
    (x/307 =a[int] a/305
     p/308 =a[(consts ()) (non_consts ([0: [int], *]))]
       (makeblock 0 a/305 b/306))
    (makeblock 0 (int,[(consts ()) (non_consts ([0: [int], *]))]) x/307
      p/308)))
(function {nlocal = 0} a/305[int] b/306
  [(consts ())
   (non_consts ([0: [int], [(consts ()) (non_consts ([0: [int], *]))]]))]
  (makeblock 0 (int,[(consts ()) (non_consts ([0: [int], *]))]) a/305
    (makeblock 0 a/305 b/306)))
- : bool -> 'a -> bool * (bool * 'a) = <fun>
|}]

let _ = fun a b -> match a, b with
| ((true as x, _) as p)
| ((false as x, _) as p) -> x, p
(* inside, simple *)
[%%expect {|
(function {nlocal = 0} a/311[int] b/312
  [(consts ())
   (non_consts ([0: [int], [(consts ()) (non_consts ([0: [int], *]))]]))]
  (let
    (x/313 =a[int] a/311
     p/314 =a[(consts ()) (non_consts ([0: [int], *]))]
       (makeblock 0 a/311 b/312))
    (makeblock 0 (int,[(consts ()) (non_consts ([0: [int], *]))]) x/313
      p/314)))
(function {nlocal = 0} a/311[int] b/312
  [(consts ())
   (non_consts ([0: [int], [(consts ()) (non_consts ([0: [int], *]))]]))]
  (makeblock 0 (int,[(consts ()) (non_consts ([0: [int], *]))]) a/311
    (makeblock 0 a/311 b/312)))
- : bool -> 'a -> bool * (bool * 'a) = <fun>
|}]

let _ = fun a b -> match a, b with
| (true as x, _) as p -> x, p
| (false, x) as p -> x, p
(* outside, complex *)
[%%expect{|
(function {nlocal = 0} a/321[int] b/322[int]
  [(consts ())
   (non_consts ([0: [int], [(consts ()) (non_consts ([0: [int], [int]]))]]))]
  (if a/321
    (let
      (x/323 =a[int] a/321
       p/324 =a[(consts ()) (non_consts ([0: [int], [int]]))]
         (makeblock 0 a/321 b/322))
      (makeblock 0 (int,[(consts ()) (non_consts ([0: [int], [int]]))]) x/323
        p/324))
    (let
      (x/325 =a[(consts ()) (non_consts ([0: ]))] b/322
       p/326 =a[(consts ()) (non_consts ([0: [int], [int]]))]
         (makeblock 0 a/321 b/322))
      (makeblock 0 (int,[(consts ()) (non_consts ([0: [int], [int]]))]) x/325
        p/326))))
(function {nlocal = 0} a/321[int] b/322[int]
  [(consts ())
   (non_consts ([0: [int], [(consts ()) (non_consts ([0: [int], [int]]))]]))]
  (if a/321
    (makeblock 0 (int,[(consts ()) (non_consts ([0: [int], [int]]))]) a/321
      (makeblock 0 a/321 b/322))
    (makeblock 0 (int,[(consts ()) (non_consts ([0: [int], [int]]))]) b/322
      (makeblock 0 a/321 b/322))))
- : bool -> bool -> bool * (bool * bool) = <fun>
|}]

let _ = fun a b -> match a, b with
| ((true as x, _) as p)
| ((false, x) as p)
  -> x, p
(* inside, complex *)
[%%expect{|
(function {nlocal = 0} a/327[int] b/328[int]
  [(consts ())
   (non_consts ([0: [int], [(consts ()) (non_consts ([0: [int], [int]]))]]))]
  (catch
    (if a/327
      (let
        (x/335 =a[int] a/327
         p/336 =a[(consts ()) (non_consts ([0: [int], [int]]))]
           (makeblock 0 a/327 b/328))
        (exit 10 x/335 p/336))
      (let
        (x/333 =a[(consts ()) (non_consts ([0: ]))] b/328
         p/334 =a[(consts ()) (non_consts ([0: [int], [int]]))]
           (makeblock 0 a/327 b/328))
        (exit 10 x/333 p/334)))
   with (10 x/329[int] p/330[(consts ()) (non_consts ([0: [int], [int]]))])
    (makeblock 0 (int,[(consts ()) (non_consts ([0: [int], [int]]))]) x/329
      p/330)))
(function {nlocal = 0} a/327[int] b/328[int]
  [(consts ())
   (non_consts ([0: [int], [(consts ()) (non_consts ([0: [int], [int]]))]]))]
  (catch
    (if a/327 (exit 10 a/327 (makeblock 0 a/327 b/328))
      (exit 10 b/328 (makeblock 0 a/327 b/328)))
   with (10 x/329[int] p/330[(consts ()) (non_consts ([0: [int], [int]]))])
    (makeblock 0 (int,[(consts ()) (non_consts ([0: [int], [int]]))]) x/329
      p/330)))
- : bool -> bool -> bool * (bool * bool) = <fun>
|}]

(* here flattening is an optimisation: the allocation is moved as an
   alias within each branch, and in the first branch it is unused and
   will be removed by simplification, so the final code
   (see the -dlambda output) will not allocate in the first branch. *)
let _ = fun a b -> match a, b with
| (true as x, _) as _p -> x, (true, true)
| (false as x, _) as p -> x, p
(* outside, onecase *)
[%%expect {|
(function {nlocal = 0} a/337[int] b/338[int]
  [(consts ())
   (non_consts ([0: [int], [(consts ()) (non_consts ([0: [int], [int]]))]]))]
  (if a/337
    (let
      (x/339 =a[int] a/337
       _p/340 =a[(consts ()) (non_consts ([0: [int], [int]]))]
         (makeblock 0 a/337 b/338))
      (makeblock 0 (int,[(consts ()) (non_consts ([0: [int], [int]]))]) x/339
        [0: 1 1]))
    (let
      (x/341 =a[int] a/337
       p/342 =a[(consts ()) (non_consts ([0: [int], [int]]))]
         (makeblock 0 a/337 b/338))
      (makeblock 0 (int,[(consts ()) (non_consts ([0: [int], [int]]))]) x/341
        p/342))))
(function {nlocal = 0} a/337[int] b/338[int]
  [(consts ())
   (non_consts ([0: [int], [(consts ()) (non_consts ([0: [int], [int]]))]]))]
  (if a/337
    (makeblock 0 (int,[(consts ()) (non_consts ([0: [int], [int]]))]) a/337
      [0: 1 1])
    (makeblock 0 (int,[(consts ()) (non_consts ([0: [int], [int]]))]) a/337
      (makeblock 0 a/337 b/338))))
- : bool -> bool -> bool * (bool * bool) = <fun>
|}]

let _ = fun a b -> match a, b with
| ((true as x, _) as p)
| ((false as x, _) as p) -> x, p
(* inside, onecase *)
[%%expect{|
(function {nlocal = 0} a/343[int] b/344
  [(consts ())
   (non_consts ([0: [int], [(consts ()) (non_consts ([0: [int], *]))]]))]
  (let
    (x/345 =a[int] a/343
     p/346 =a[(consts ()) (non_consts ([0: [int], *]))]
       (makeblock 0 a/343 b/344))
    (makeblock 0 (int,[(consts ()) (non_consts ([0: [int], *]))]) x/345
      p/346)))
(function {nlocal = 0} a/343[int] b/344
  [(consts ())
   (non_consts ([0: [int], [(consts ()) (non_consts ([0: [int], *]))]]))]
  (makeblock 0 (int,[(consts ()) (non_consts ([0: [int], *]))]) a/343
    (makeblock 0 a/343 b/344)))
- : bool -> 'a -> bool * (bool * 'a) = <fun>
|}]

type 'a tuplist = Nil | Cons of ('a * 'a tuplist)
[%%expect{|
0
0
type 'a tuplist = Nil | Cons of ('a * 'a tuplist)
|}]

(* another example where we avoid an allocation in the first case *)
let _ =fun a b -> match a, b with
| (true, Cons p) -> p
| (_, _) as p -> p
(* outside, tuplist *)
[%%expect {|
(function {nlocal = 0} a/356[int]
  b/357[(consts (0))
        (non_consts ([0: [(consts ()) (non_consts ([0: *, *]))]]))]
  [(consts ())
   (non_consts ([0: [int], [(consts (0)) (non_consts ([0: *]))]]))](catch
                                                                    (if a/356
                                                                    (if b/357
                                                                    (let
                                                                    (p/358 =a
                                                                    (field_imm 0
                                                                    b/357))
                                                                    p/358)
                                                                    (exit 12))
                                                                    (exit 12))
                                                                    with (12)
                                                                    (let
                                                                    (p/359 =a
                                                                    [(consts ())
                                                                    (non_consts (
                                                                    [0:
                                                                    [int],
                                                                    [(consts (0))
                                                                    (non_consts (
                                                                    [0: *]))]]))]
                                                                    (makeblock 0
                                                                    a/356
                                                                    b/357))
                                                                    p/359)))
(function {nlocal = 0} a/356[int]
  b/357[(consts (0))
        (non_consts ([0: [(consts ()) (non_consts ([0: *, *]))]]))]
  [(consts ())
   (non_consts ([0: [int], [(consts (0)) (non_consts ([0: *]))]]))](catch
                                                                    (if a/356
                                                                    (if b/357
                                                                    (field_imm 0
                                                                    b/357)
                                                                    (exit 12))
                                                                    (exit 12))
                                                                    with (12)
                                                                    (makeblock 0
                                                                    a/356
                                                                    b/357)))
- : bool -> bool tuplist -> bool * bool tuplist = <fun>
|}]

let _ = fun a b -> match a, b with
| (true, Cons p)
| ((_, _) as p) -> p
(* inside, tuplist *)
[%%expect{|
(function {nlocal = 0} a/360[int]
  b/361[(consts (0))
        (non_consts ([0: [(consts ()) (non_consts ([0: *, *]))]]))]
  [(consts ())
   (non_consts ([0: [int], [(consts (0)) (non_consts ([0: *]))]]))](catch
                                                                    (catch
                                                                    (if a/360
                                                                    (if b/361
                                                                    (let
                                                                    (p/365 =a
                                                                    (field_imm 0
                                                                    b/361))
                                                                    (exit 13
                                                                    p/365))
                                                                    (exit 14))
                                                                    (exit 14))
                                                                    with (14)
                                                                    (let
                                                                    (p/364 =a
                                                                    [(consts ())
                                                                    (non_consts (
                                                                    [0:
                                                                    [int],
                                                                    [(consts (0))
                                                                    (non_consts (
                                                                    [0: *]))]]))]
                                                                    (makeblock 0
                                                                    a/360
                                                                    b/361))
                                                                    (exit 13
                                                                    p/364)))
                                                                    with (13 p/362
                                                                    [(consts ())
                                                                    (non_consts (
                                                                    [0:
                                                                    [int],
                                                                    [(consts (0))
                                                                    (non_consts (
                                                                    [0: *]))]]))])
                                                                    p/362))
(function {nlocal = 0} a/360[int]
  b/361[(consts (0))
        (non_consts ([0: [(consts ()) (non_consts ([0: *, *]))]]))]
  [(consts ())
   (non_consts ([0: [int], [(consts (0)) (non_consts ([0: *]))]]))](catch
                                                                    (catch
                                                                    (if a/360
                                                                    (if b/361
                                                                    (exit 13
                                                                    (field_imm 0
                                                                    b/361))
                                                                    (exit 14))
                                                                    (exit 14))
                                                                    with (14)
                                                                    (exit 13
                                                                    (makeblock 0
                                                                    a/360
                                                                    b/361)))
                                                                    with (13 p/362
                                                                    [(consts ())
                                                                    (non_consts (
                                                                    [0:
                                                                    [int],
                                                                    [(consts (0))
                                                                    (non_consts (
                                                                    [0: *]))]]))])
                                                                    p/362))
- : bool -> bool tuplist -> bool * bool tuplist = <fun>
|}]
