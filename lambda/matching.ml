...
(* CR rtjoa: This, along with [divide_record_unboxed_product], is one of the sadder parts
   to duplicate between legacy and unboxed records. We *could* de-duplicate them using
   [Types.record_form], but that would hurt readability, so I think this duplication is
   ultimately more maintainable. It's a close call, to me - I don't feel very strongly
   either way.
*)
let get_expr_args_record_unboxed_product ~scopes head (arg, _mut, _sort, _layout) rem =
  ...
  let all_labels =
    let open Patterns.Head in
    match head.pat_desc with
    | Record_unboxed_product (lbl :: _) -> lbl.lbl_all
...
let is_record_with_mutable_field p =
  let fields_has_mutable lps =
    List.exists (fun (_, lbl, _) -> Types.is_mutable lbl.lbl_mut) lps
  in
  match p.pat_desc with
  | Tpat_record (lps, _) -> fields_has_mutable lps
  | Tpat_record_unboxed_product (lps, _) -> fields_has_mutable lps
  | Tpat_alias _
  | Tpat_variant _
  | Tpat_lazy _
  | Tpat_tuple _
