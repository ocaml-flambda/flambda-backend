           ...
           | Some(e1) -> begin 
            ...
            | Texp_apply(f2, ae_l2) -> 
              if is_ignore f2 then
                mapper.expr mapper {e with exp_desc = mkTexp_apply(f, ae_l2)}
        ...
        Tast_mapper.default.expr mapper
          ...
          | Texp_apply(f, ae_l) -> 
            if is_dummy f && should_remove () then
              (
            { e with 
              exp_desc = make_sequence
                (List.fold_left
                  (fun l (_,eo) ->
                    fold_arg_or_omitted
                      (fun l e ->
                         mkTexp_apply (Dummy.ignore, [(Nolabel, mkArg e)]) :: l
                      ) l eo)
                  []
                  ae_l)
                })
...
let minimizer = {
  minimizer_name = "sequentialize-functions" ;
