...
let a_to_unit = 
  {
    ...
    ctyp_type = newty2 ~level:0 (mkTarrow(Nolabel, a_typ.ctyp_type, unit_typ.ctyp_type, commu_ok));
    ctyp_env = Env.empty ;
    ctyp_loc = Location.none;
    ctyp_attributes = [];
  }
...
let unit_to_a = 
  {
    ...
    ctyp_type = newty2 ~level:0 (mkTarrow(Nolabel,  unit_typ.ctyp_type, a_typ.ctyp_type, commu_ok));
    ctyp_env = Env.empty ;
    ctyp_loc = Location.none;
    ctyp_attributes = [];
  }
...
let apply_dummy1 = exp_desc_to_exp (mkTexp_apply( (dummy1), [(Nolabel, mkArg (exp_desc_to_exp (mkTexp_tuple [])))]))

let apply_dummy2 = exp_desc_to_exp (mkTexp_apply( (dummy2), [(Nolabel, mkArg (exp_desc_to_exp (mkTexp_tuple [])))]))
