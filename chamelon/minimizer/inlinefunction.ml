    ...
    structure_item = (fun _ str_it -> 
      ...
      | Tstr_value(Nonrecursive, vb_l) -> 
        let vb1 = List.filter
          ...
          vb_l
        in 
        if vb1 <> [] then (fun_body := (List.hd vb1).vb_expr ; non_rec := true) ;
        str_it
      | _ -> str_it);
    expr = fun mapper e -> 
      ...
      | _ -> Tast_mapper.default.expr mapper e ; } in 
        ...
        Tast_mapper.default.expr mapper
          ...
          | Texp_ident(Pident id, _, vd) ->
            if is_function vd.val_type then
              ...
              if non_rec && should_remove () then
                { e with exp_desc = body }
              else e
            else e
          | _ -> e) }
  in
...
let minimizer = {
  minimizer_name = "inline-function" ;
