...
let rec arity_min l min = 
  match l with
  | cd :: q -> 
    if arity cd < arity min then arity_min q cd 
    else arity_min q min 
  | [] -> min 

let is_recursive p cons_decl = 
  match cons_decl.cd_args with 
  | Cstr_tuple(etyp_list) -> 
    List.exists (contain_type p) etyp_list
...
let rec find_value_of_typ t s = 
  ...
  | Env_value(s,id,vd) -> begin 
    match get_desc vd.val_type with 
    | Tvar(Some(s2)) when t = s2 -> id, vd
    | _ ->  find_value_of_typ t s 
  end 
  | Env_type(s,_,_) | Env_extension(s, _, _) | Env_module(s,_,_,_)  
  | Env_modtype(s,_,_) | Env_class (s,_,_)  | Env_cltype (s,_,_)  
  | Env_open (s,_)  | Env_functor_arg(s,_)   | Env_constraints(s,_)
  | Env_copy_types s  | Env_persistent (s,_) | Env_value_unbound (s,_,_)  
  | Env_module_unbound (s,_,_) -> find_value_of_typ t s 
  ...
  let rec find_value_of_typ_none s = 
    ...
    | Env_open (s,_)  | Env_functor_arg(s,_)   | Env_constraints(s,_)
    | Env_copy_types s  | Env_persistent (s,_) | Env_value_unbound (s,_,_)  
    | Env_module_unbound (s,_,_) -> find_value_of_typ_none s 

let is_known id = 
  match name id with 
    | "int" | "char" | "string" | "unit" -> true
    | _ -> false 
...
let rec generate_dummy_expr env etyp = 
  ...
  | Tvar(Some(vtyp)) -> 
    ...
    Texp_ident( Pident id , {txt = Lident(Ident.name id); loc = Location.none} , vd)
  | Tvar(None) ->
    let id,vd = find_value_of_typ_none (summary env) in 
    Texp_ident( Pident id , {txt = Lident(Ident.name id); loc = Location.none} , vd)
  | Ttuple(etyp_list) -> Texp_tuple( List.map (generate_dummy_expr env) etyp_list)
  | Tarrow(arg_label, etyp1, etyp2, _) -> 
    ...
    Texp_function(
      { arg_label ; param ; partial = Partial ;
        cases = 
        [{
            c_lhs = { any_pat with pat_desc = Tpat_var(param,{ loc = Location.none ; txt = "x"})} ;
            c_guard = None ; 
            c_rhs = generate_dummy_expr nenv etyp2 
          }
        ];}
    )
  | Tconstr(Pident id, [], _) when is_known id -> begin
      match name id with 
