...
let remove_module id = 
  { Tast_mapper.default with 
    expr = (fun mapper e -> 
      ...
      | Texp_ident(path, t_loc,vd) -> begin 
        match path, t_loc.txt with  
        | Pdot(Pident(id1),id2), Ldot(_,lid2) -> 
          if Ident.same id id1 then (
            { e with exp_desc = 
              mkTexp_ident(
                Pident(Ident.create_local ((String.lowercase_ascii (Ident.name id1)) ^ "_" ^ id2)),
                {t_loc with txt = Lident((String.lowercase_ascii (Ident.name id1)) ^ "_" ^lid2)},
                vd)
            })
...
let rec replace_in_pat : type k. _ -> k general_pattern ->  k general_pattern = 
  fun mod_name pat ->
   { pat with pat_desc = 
    ...
    | Tpat_alias(p,id,str) -> 
       mkTpat_alias(
        replace_in_pat mod_name p,
          ...
          {str with txt = mod_name ^"_" ^ str.txt})
    | O (Tpat_tuple(vl)) -> 
       Tpat_tuple(List.map  (replace_in_pat mod_name) vl) 
    | Tpat_array(vl) -> 
      mkTpat_array(List.map  (replace_in_pat mod_name) vl) 
    | O (Tpat_construct(a1,a2,vl,a3)) -> 
      Tpat_construct(a1,a2,List.map  (replace_in_pat mod_name) vl, a3)
    | O (Tpat_record(r,a1)) -> Tpat_record(
      List.map (fun (e1,e2,pat) -> (e1, e2, replace_in_pat mod_name pat)) r, a1) 
    | O (Tpat_or(p1, p2, a1)) -> Tpat_or(replace_in_pat mod_name p1,replace_in_pat mod_name p2,a1)
    ...
    | O (Tpat_value _) (* p) -> as_computation_pattern (replace_in_pat mod_name p) *)
    | O (Tpat_any | Tpat_constant _ | Tpat_variant _  | Tpat_exception _) ->  pat.pat_desc
...
let add_module_name_mapper mod_name l = 
  { Tast_mapper.default with 
      structure_item = fun _ str_it -> 
        ...
        | Tstr_value(rf, vb_l) -> 
           l := List.fold_left (fun l vb -> Removedeadcode.var_from_pat vb.vb_pat.pat_desc l) !l vb_l ;
          { str_it with str_desc = 
            Tstr_value(rf,
                List.map (fun vb -> { vb with vb_pat = replace_in_pat mod_name vb.vb_pat }) vb_l)}
        | Tstr_type(rf, td_l) ->  
          ...
          Tstr_type(
            ...
            (fun td -> 
              { td with 
                typ_id = create_local (mod_name ^"_" ^ Ident.name td.typ_id) ;
                typ_name = { td.typ_name with txt = mod_name ^"_"^td.typ_name.txt}}
            ) td_l
          ...
          | Tstr_module(mb) -> begin
            ...
            | Some(str), Some(mod_name) ->
              if should_remove () then (
                id := mod_name;
                let rep_mapper = add_module_name_mapper (String.lowercase_ascii (Ident.name !id)) vars in 
               (List.rev ( List.map (rep_mapper.structure_item rep_mapper) str.str_items ))@ l)
              else str_it :: l  
            | _ -> str_it :: l end
          | _ -> str_it :: l)
          []
  ...
  let minimize should_remove map cur_name = 
    ...
    if !to_change <> [] then 
      ...
      let corrected_str = 
        List.fold_left 
          (fun nstr id ->
            let rep_mapper = replace_id id (Ident.create_local (mod_name^"_"^(Ident.name id)))in 
            let nstr = rep_mapper.structure rep_mapper nstr in            
            let to_find = Pdot(Pident (create_local (String.capitalize_ascii mod_name)), Ident.name id) in 
            let replacement = Pident(create_local (mod_name^"_"^(Ident.name id))) in 
        ...
        let correcter nstr = 
          List.fold_left (fun nstr id -> 
            let origin_file = Pident (create_local (String.capitalize_ascii (String.sub cur_name 0 (String.length cur_name -3)))) in
            let to_find = Pdot(Pdot(origin_file, String.capitalize_ascii mod_name), Ident.name id) in 
            let replacement = Pdot(origin_file, mod_name^"_"^(Ident.name id)) in
            let rep_mapper = replace_path to_find replacement in 
            rep_mapper.structure rep_mapper nstr)
            nstr !to_change
          in 
...
let minimizer = {
  minimizer_name = "flatten-modules" ;
