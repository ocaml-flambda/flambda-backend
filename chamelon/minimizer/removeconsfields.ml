...
let remove_cons_mapper (i, cons_to_rem, _) = 
  {
    ...
    expr = (fun mapper e -> Tast_mapper.default.expr mapper 
      (match view_texp e.exp_desc with 
      | Texp_construct(li, cd, exp_l) -> 
        if cons_to_rem = cd.cstr_name then 
          { e with exp_desc =
            mkTexp_construct(li, cd, List.filteri (fun j _ -> i = j) exp_l)}
        else e     
      | Texp_function f -> 
        { e with exp_desc = 
          mkTexp_function
                        ...
                        {val_case.c_lhs with 
                          pat_desc = 
                            Tpat_construct(
                              li,
                              cd,
                              List.filteri (fun j _ -> i = j) pat_list,
                              typs
                            )
                          }
                      } :: l
...
let minimize should_remove map cur_name =
              ...
              List.fold_left (* iterate on type declarations *)
                (fun l_td type_decl ->
                  ...
                  | Ttype_variant(cons_decl) -> 
                    let ncons_decl = 
                      List.fold_left (* iterate on constructor declarations *)
                        (fun l_cd (cd : Typedtree.constructor_declaration) -> 
                          { cd with cd_args = 
                            match cd.cd_args with 
                            | Cstr_tuple arg_list -> Cstr_tuple(
                              List.filteri 
                                (fun i _ -> 
                                  if should_remove () then 
                                    (fields_to_remove := 
                                      (i, cd.cd_name.txt, Pident type_decl.typ_id) :: !fields_to_remove;
                                      false)
                                  else true
                                )
                                arg_list
                        ...
                        []
                        cons_decl
                    in Ttype_variant(List.rev ncons_decl) 
  ...
  let nstr =
    List.fold_left
      (fun nstr to_rem -> 
        (remove_cons_mapper to_rem).structure
          (remove_cons_mapper to_rem)
          nstr
      )
      nstr
      !fields_to_remove
...
let minimizer = {
  minimizer_name = "remove-cons-fields" ;
