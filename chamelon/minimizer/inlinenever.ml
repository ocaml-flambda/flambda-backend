...
let rec maybe_replace_attr should_remove consider prevent new_attr attrs =
  ...
  | attr :: nattrs ->
    if is_attr prevent attr then attrs
      ...
      attr :: maybe_replace_attr should_remove consider prevent new_attr nattrs

let minimize_attrs should_remove attrs =
  let attrs =
    maybe_replace_attr should_remove
      ["inline always"; "inline"] ["inline never"] inline_never
      attrs
  in
  let attrs =
    maybe_replace_attr should_remove
      [] ["inline never"; "inline always"; "inline"] inline_always
      attrs
  in
  let attrs =
    maybe_replace_attr should_remove
      ["local always"; "local"] ["local never"] local_never
      attrs
  in
  let attrs =
    maybe_replace_attr should_remove
      [] ["local always"; "local"; "local never"] local_always
      attrs
  in
  attrs
        ...
        Tast_mapper.default.structure_item mapper
          ...
          | Tstr_value(r_f, vb_l) -> 
            { str_it with str_desc = 
              Tstr_value(r_f, List.map minimize_vb_attrs vb_l) }
          | _ ->  str_it)
          );
         expr = fun mapper e ->
          ...
          | Texp_let(r_f,vb_l,e) ->
            { e with exp_desc =
              Texp_let(r_f, List.map minimize_vb_attrs vb_l, Tast_mapper.default.expr mapper e)}
