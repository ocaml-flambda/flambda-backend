...
let minimize should_remove map cur_name =
        ...
        Tast_mapper.default.expr mapper
          ...
          | Texp_match(e_match, cc_l, _) -> 
            ...
            if List.length cc_l = 1 then
              ...
              |  Tpat_value(tva), None -> begin 
                match view_tpat (tva :> value general_pattern).pat_desc with 
                | Tpat_var(id, _) -> 
  ...
  in 
  let nstr = simplify_match_mapper.structure simplify_match_mapper (Smap.find cur_name map) in 
  Smap.add cur_name nstr map

let minimizer = {
  minimizer_name = "simplify-match" ;
