...
let rec is_simplified env e = 
  ...
  | Texp_construct(_,cons,el) -> begin 
    ...
    | Tconstr(path, _, _) ->  
      ...
      | Type_variant(cstr_list, _)  -> 
        ...
        && cons.cstr_name = Ident.name (find_simpler_cons path cstr_list).cd_id 
      | _ -> false
      end
  | _ -> false 
  end
  | Texp_tuple(exp_list) | Texp_array(exp_list) -> List.for_all (is_simplified env) exp_list
  ...
  | Texp_constant(Const_string("", _, None)) -> true 
  | _ -> false
  ...
  let rec reduce_def_mapper = 
    {
      ...
      expr = fun mapper e ->
        ...
        if to_remove = !nth_expr && not (is_simplified e.exp_env e) then
          (is_removed := true ; generate_dummy_expr e.exp_env e.exp_type)
        else Tast_mapper.default.expr mapper e 
        ;
    }
  in 
  let nstr = reduce_def_mapper.structure reduce_def_mapper (Smap.find cur_name map) in 
