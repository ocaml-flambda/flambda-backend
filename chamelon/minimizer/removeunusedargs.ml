...
let rec fun_wrapper arg_list acc_id depth path_fun n = 
  if depth = 0 then 
    let n = ref 0 in 
    mkTexp_apply( exp_desc_to_exp
      (mkTexp_ident(
        path_fun,
        { txt = Lident (Path.name path_fun);
          loc = Location.none},
          dummy_value_description
      )),
      List.map2
        ( fun arg_lab id -> incr n ; 
          (arg_lab, 
          mkArg(exp_desc_to_exp (
            mkTexp_ident(
              Pident id,
    ...
    mkTexp_function{ 
      ...
      cases = [ {
        ...
        c_guard = None;
        c_rhs = exp_desc_to_exp
          (fun_wrapper arg_list (if n = 1 then acc_id else id_arg :: acc_id) (depth - 1) path_fun (n+1) );
      }] ;
    }
...
let replace_mapper id to_replace label = 
  { Tast_mapper.default with 
      expr = fun mapper e -> 
        ...
        | Texp_apply(e, ae_l) -> 
          { e with exp_desc = mkTexp_apply(
            mapper.expr mapper e, List.rev
            (List.fold_left(
              fun l (a,e) -> 
                match a, label with 
                | Optional s1, Optional s2 | Labelled s1,  Labelled s2-> 
                  if (s1 = s2 ) then (Nolabel,map_arg_or_omitted (mapper.expr mapper) e)::l 
                  else (a,map_arg_or_omitted (mapper.expr mapper) e) :: l 
                | _ ->  (a,map_arg_or_omitted (mapper.expr mapper) e) ::l
            )
            []
            ae_l))}
  ...
  let mapper = {
    Tast_mapper.default with 
      expr = fun mapper e -> 
        ...
        | Texp_function({cases = vc_l ; _} as f) -> incr depth;
          if List.length vc_l = 1 && not !is_modified then (
            ...
            | Tpat_var(id,_) -> 
              ...
              if not !is_used then 
                ...
                arg := f.arg_label :: !arg ; 
                {e with exp_desc = Texp_function(
                  {f with 
                    cases = [{vc with c_rhs = mapper.expr mapper vc.c_rhs}]
                  }
              )})
            | _ -> e)
          else e
          ...
          (List.fold_left
            (fun l vb -> 
              if Inlinenever.is_function vb.vb_expr.exp_type then
                let is_rep, nfun, depth, arg_list, arg_label  = find_unused_arg_mapper vb.vb_expr in  
                if is_rep then
                  match view_tpat vb.vb_pat.pat_desc with 
                  | Tpat_var(id, _) when should_remove () ->
                    to_suppress := (id, depth, arg_list, arg_label):: !to_suppress;
...
let minimize should_remove map cur_name =
  ...
  let global_replace = fun str -> 
      List.fold_left
        (fun nstr (id_fun, depth, arg_list, arg_label) -> 
          let id_fun_ext = Path.Pdot(Pident (Ident.create_local name_clr), Ident.name id_fun) in
...
let minimizer = {
  minimizer_name = "remove-unused-args" ;
