   ...
   expr = fun mapper e -> 
     ...
     | Texp_ident(path, _ , _) -> 
       if Ident.same (Path.head path) id then
          is_used := true
     | _ -> ());
     if not !is_used then Tast_mapper.default.expr mapper e 
     else e 
   }
    ...
    {Tast_mapper.default with
      typ = (fun mapper ct -> 
        (match ct.ctyp_desc with 
          ...
          | Ttyp_constr(_,li,_) -> if Longident.last li.txt = typ_name then is_used := true
          | _ -> ());
        if not !is_used then Tast_mapper.default.typ mapper ct
        else ct) ;
      expr = (fun mapper e -> 
...
let rec rem_in_pat str pat should_remove = 
    ...
    | Tpat_alias(pat1, id, a) ->
      ...
      if not is_used && should_remove () then
        ...
        pat with pat_desc =
                   mkTpat_alias (rem_in_pat str pat1 should_remove, id, a)
      }
    | O (Tpat_tuple(vl)) ->
      {pat with pat_desc = Tpat_tuple(
           List.map (fun pat -> rem_in_pat str pat should_remove) vl) }
    | Tpat_array(vl) ->
      {pat with pat_desc = mkTpat_array(
           List.map (fun pat -> rem_in_pat str pat should_remove) vl) }
    | O (Tpat_construct(a1,a2,vl,a3)) -> {pat with pat_desc = Tpat_construct(a1,a2,
      List.map (fun pat -> rem_in_pat str pat should_remove) vl, a3) }
    | O (Tpat_record(r,a1)) -> {pat with pat_desc = Tpat_record(
      List.map (fun (e1,e2,pat) -> (e1, e2, rem_in_pat str pat should_remove)) r, a1) }
    | O (Tpat_or(p1, p2, a1)) ->
                ...
                (List.fold_left 
                  ( fun l str_it -> 
                    ...
                    | Tstr_value(r_f, vb_l) ->
                      let vb_l = List.rev @@
                        List.fold_left
                          (fun (l : value_binding list) vb -> 
                            let npat = rem_in_pat str vb.vb_pat should_remove in 
                      ...
                      in
                      if vb_l = [] then l 
                      else { str_it with str_desc = Tstr_value(r_f, vb_l)} :: l
                    | Tstr_primitive(vd) -> 
                    ...
                    | Tstr_exception(te) -> 
                      let is_used = is_used_exc str te.tyexn_constructor.ext_name.txt in 
                      if not is_used && should_remove () then
                        l
                      else str_it :: l
    
                    | Tstr_type(rf, tdl) -> 
                      let nl = List.rev (List.fold_left
                        (fun l td ->
...
let minimizer = {
  minimizer_name = "remove-dead-code" ;
