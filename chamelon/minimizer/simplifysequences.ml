...
let break_sequence e = 
  ...
  let remove_dummy_mapper should_remove = 
    {
      Tast_mapper.default with 
        expr = fun mapper e -> Tast_mapper.default.expr mapper 
          (match view_texp e.exp_desc with 
          | Texp_sequence(e1, e2) -> 
            if is_dummy e2 && should_remove ()  then 
              e1
            else e 
          | _ -> e )
    }
  ...
  let minimize should_remove map cur_name =
      let simp_seq_mapper = 
        {
          ...
          expr = fun mapper e ->
            ...
            (match view_texp e.exp_desc with 
            | Texp_sequence(e1,e2) -> 
              if (Removeunit.is_unit e1 || is_dummy e1) && should_remove () then
                e2
            ...
            | O (Texp_ifthenelse(e1,e2,Some(e3))) -> 
              if Reduceexpr.is_simplified e1 && (Removeunit.is_unit e2 || Removeunit.is_unit e3) && should_remove () then
                if Removeunit.is_unit e2 then e3 else
                  { e with exp_desc = Texp_ifthenelse(e1,e2,None) }
              else e
            | Texp_apply(f, ael) -> 
              if is_dummy f  then
                begin 
                  { e with exp_desc  =  mkTexp_apply(f, List.rev 
                    (List.fold_left 
                      (fun l (a,e) ->
                        fold_arg_or_omitted
                          (fun l e1 ->
                          if Reduceexpr.is_simplified e1 && should_remove () then
                          l else (a,mkArg(e1)) :: l
                          ) l e)
                      []
                      ael)) }
                    ...
                    (fun l (a,e) ->
                      fold_arg_or_omitted
                        ...
                        if is_sequence e1 then
                          let rdm = remove_dummy_mapper should_remove in 
      ...
      in 
      let nstr = simp_seq_mapper.structure simp_seq_mapper (Smap.find cur_name map) in  
      Smap.add cur_name nstr map

let minimizer = {
  minimizer_name = "simplify-sequences" ;
