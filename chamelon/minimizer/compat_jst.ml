...
let mkTexp_apply (exp, args) = Texp_apply (exp, args, Default, Amode Global)
let mkTexp_tuple exps = Texp_tuple (exps, Amode Global)
...
let mkTpat_var (ident, name) = Tpat_var (ident, name, dummy_value_mode)
let mkTpat_alias (p, ident, name) = Tpat_alias (p, ident, name, dummy_value_mode)
...
let view_tstr (si : structure_item_desc) =
  match si with Tstr_eval (e, _, attrs) -> Tstr_eval (e, attrs) | _ -> O si
...
let fold_arg_or_omitted f init arg =
  match arg with Arg (e, _) -> f init e | Omitted _ -> init

let option_of_arg_or_omitted arg =
  match arg with Arg (e, _) -> Some e | Omitted _ -> None
