              ...
              str_desc = 
                Tstr_open(
                  {decl with 
                    open_expr =
                      {decl.open_expr with 
                        mod_desc = Tmod_ident(Pident(create_local n2), { li with txt = Lident(n2)})
                        }
                  }
                )
            }
...
let remove_file_name_mapper n1 = 
  ...
  {Tast_mapper.default with 
    expr = (fun mapper e -> 
      ...
      | Texp_ident(p, li, vd) -> begin 
        match p with 
        | Pdot(p1, _) -> 
          if Ident.name (Path.head p1) = n1 then 
            ({ e with exp_desc = 
              mkTexp_ident( remove_head_path p, {li with txt =  li.txt}, vd)})
          else e
        | _ -> e 
        end 
    ...
    typ = fun mapper ct -> 
      ...
      | Ttyp_constr(p, li, c) -> begin 
        ...
        | Pdot(p1, _) -> 
          ...
          if Ident.name (Path.head p1) = n1 then 
          { ct with ctyp_desc =
          Ttyp_constr(remove_head_path p, {li with txt =   li.txt}, c) }
          else ct )
        | _ -> ct 
        end 
...
let merge str1 str2 n1 n2 = 
  ...
  let rem_open_mapper = remove_open (String.capitalize_ascii (String.sub n1 0 (String.length n1 -3))) in 
...
let merge_strategy c (names, strs) =
  ...
  while List.length !to_merge > 1 do 
    ...
    let correcter nstr = 
      List.fold_left (fun nstr id -> 
        let to_find = Pdot( Pident (create_local merged_file ) , Ident.name id) in 
        let replacement = Pident(create_local (merged_file^"_"^(Ident.name id))) in
        ...
        nstr to_rename
      in 
    let new_str = List.map (fun (name,str) -> (name, correcter str) ) new_merged in 
  ...
  in 
  List.fold_left ( fun (l1,l2) (a,b) -> (a :: l1, b :: l2) ) ([],[]) final_merge
