...
let is_simplified e =
  ...
  | Texp_tuple [] | O (Texp_constant(Const_int 0))
  | O (Texp_constant(Const_char '0'))
  | O (Texp_constant(Const_string("", _, None))) -> true
  | Texp_ident(_, name,_) -> (Longident.last name.txt = "__dummy2__")
  | Texp_construct(name,_,_) -> (Longident.last name.txt = "" )
  ...
  | Tconstr(path, [], _) -> begin 
    ...
    | Path.Pident id -> begin 
      match name id with 
        ...
        | "string" -> { e with exp_desc = Texp_constant(Const_string("", Location.none, None)) }
