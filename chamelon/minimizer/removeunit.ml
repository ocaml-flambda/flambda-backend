...
let is_unit e =
  ...
  | Texp_construct( {txt = Lident "()" ; _},_,_)
  | Texp_tuple [] -> true
  | _ -> false
  ...
  let remove_unit_mapper = 
    {
      Tast_mapper.default with
        expr = fun mapper e ->
          if is_unit_typ e.exp_type && not (is_unit e) && should_remove () then
            {e with exp_desc = eunit}
          else
            Tast_mapper.default.expr mapper e
    }
  in 
  let nstr = remove_unit_mapper.structure remove_unit_mapper (Smap.find cur_name map) in 
  Smap.add cur_name nstr map

let minimizer = {
  minimizer_name = "remove-unit" ;
