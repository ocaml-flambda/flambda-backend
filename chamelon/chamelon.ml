...
let usage_msg = "minimize <file1> [<file2>] ... -c \"<command>\" [-m <minimizers>] [-e <error>] [-t <typing command>] [-o <output>]"
let input_files = ref []
...
let all_minimizers =
  List.fold_left (fun minimizers m -> Smap.add m.minimizer_name m minimizers)
    Smap.empty
...
let minimizers_to_run =
  List.map (fun name ->
      ...
      with Not_found ->
        (Format.eprintf "Minimizer %S not found@." name; exit 1)
    )
    (if !arg_minimizers = "" then
       default_iteration
     else
       String.split_on_char ',' !arg_minimizers)
...
let one_file_minimize c (map :structure Smap.t) file : structure Smap.t * bool =
  if !test then begin
    if List.compare_length_with minimizers_to_run 1 <> 0 then
      (Format.eprintf "Please provide exactly one minimizer in test mode@."; exit 1);
...
let main () =
  ...
  let file_strs = List.map (fun cmt_info -> (extract_cmt cmt_info.cmt_annots)) cmt_infos in

  (* CHECKING ERROR PRESENCE *)
  let c = List.fold_left (fun c output -> c^" "^output) !command file_names in 
  if not (raise_error c) then
    (Format.eprintf "This command does not raise the error %S. @." !Utils.error_str; exit 1);
  ...
  if List.length file_names = 1 then begin
    ...
    while !has_changed do 
      let a, b = one_file_minimize c (Smap.singleton output_file !input_str) output_file in
      input_str := Smap.find output_file a ; 
      has_changed := b ; 
    done ;
      let a, _  = apply_minimizer false (Smap.singleton output_file !input_str) output_file Remdef.minimizer c in 
      input_str := Smap.find output_file a ; 
    end
  else
    ...
    let str_map = List.fold_left2 (fun map key str -> Smap.add key str map) Smap.empty file_names file_strs in 
    let c = ref (List.fold_left (fun c output -> c^" "^output) !command file_names) in 
    let has_changed = ref true in 
    ...
    while !has_changed do 
      ...
      in 
      rfile_names := fn ; rfile_strs := fs ; 
      nmap := List.fold_left2 (fun map key str -> Smap.add key str map) Smap.empty file_names file_strs ;
      c := make_command !command fn ; 
      let a,b = List.fold_left 
        (fun (map,b) name -> 
          let nmap,ch = one_file_minimize !c map name in nmap, b || ch)
        (!nmap, false)
        file_names
      in
      nmap := a ;
