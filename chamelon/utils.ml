...
let rec replace_id_in_path path to_rep =
  ...
  | Papply(p1, p2) ->
    Papply(
      replace_id_in_path p1 to_rep,
      replace_id_in_path p2 to_rep )
...
let generate_cmt typing_command (filenames : string list) =
  ...
  if (Sys.command (typing_command ^ " -bin-annot -stop-after typing " ^ params)) = 0 then
    let l = List.map (fun s -> read_cmt ((String.sub s 0 (String.length s-3)) ^ ".cmt")) filenames in 
    List.iter (fun s -> Stdlib.ignore (Sys.command ("rm "^(String.sub s 0 (String.length s-3)) ^ ".cm*"))) filenames;
    l
  else
    failwith "Fail generating.cmt"
...
let add_def str =
  { str with str_items = dummy1_def :: dummy2_def :: ignore_def :: str.str_items}

(** [update_output map] replaces the content of each file
    by its associated structure in [map] *)
let update_output map =
  Smap.iter update_single (Smap.map (add_def) map)
