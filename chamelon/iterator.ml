...
type 'a minimized_step_result =
  ...
  | Change_removes_error (* This change removes the error, but other changes might be possible *)
  | No_more_changes (* The last possible position for changes has been reached *)

let minimize_basic (state : 'a) (f : 'a -> pos:int -> 'a minimized_step_result) : 'a * bool =
...
let minimize_ranged (state : 'a) (f : 'a -> pos:int -> len:int -> 'a minimized_step_result) : 'a * bool =
  let rec loop_increasing (state : 'a) (pos : int) (len : int) (ever_changed : bool) =
  ...
  and loop_decreasing (state : 'a) (pos : int) (len : int) (ever_changed : bool) =
    if len = 0 then
      loop_increasing state (pos + 1) 1 ever_changed
    else
      match f state ~pos ~len with
...
let apply_minimizer test map cur_file minimize (c : string) =
  let result, has_changed =
    if test then
      minimize_at minimize cur_file map ~pos:0 ~len:1
    else if dicho then
      minimize_ranged map (step_minimizer c minimize cur_file)
    else
      minimize_basic map (step_minimizer c minimize cur_file ~len:1)
  in
