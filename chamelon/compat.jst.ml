...
let mkTexp_ident ?id:(ident_kind, uu = Id_value, shared_many_use) (path, longident, vd) =
...
type texp_function_param_identifier =
  ...
  ; param_newtypes : (string Location.loc * Jane_asttypes.jkind_annotation option) list
  }
...
let mkTexp_function ?(id = texp_function_defaults) ({ params; body } : texp_function) =
  Texp_function
    { params =
        List.map
          (fun { arg_label
    ...
    ; body =
        (match body with
         ...
         | Function_cases { cases; param; partial; function_cases_identifier = id } ->
           Tfunction_cases
             { fc_cases = cases
             ; fc_param = param
             ; fc_partial = partial
...
let mkTexp_sequence ?id:(sort = Jkind.Sort.value) (e1, e2) = Texp_sequence (e1, sort, e2)
...
type matched_expression_desc =
  | Texp_ident of
      Path.t * Longident.t Location.loc * value_description * texp_ident_identifier
  | Texp_apply of
  ...
  | Texp_match of expression * computation case list * partial * texp_match_identifier
...
let view_texp (e : expression_desc) =
  ...
  | Texp_construct (name, desc, args, mode) -> Texp_construct (name, desc, args, mode)
...
type 'a matched_pattern_desc =
  ...
  | Tpat_alias :
      value general_pattern * Ident.t * string Location.loc * tpat_alias_identifier
      -> value matched_pattern_desc
  | Tpat_array :
...
let mkTstr_eval ?id:(sort = Jkind.Sort.value) (e, attrs) = Tstr_eval (e, sort, attrs)
...
let fold_arg_or_omitted f init arg =
  match arg with
  | Arg (e, sort) -> f init (e, sort)
  | Omitted _ -> init
;;

let option_of_arg_or_omitted arg =
  match arg with
  | Arg (e, sort) -> Some (e, sort)
  | Omitted _ -> None
;;
...
let mk_value_binding ~vb_pat ~vb_expr ~vb_attributes =
  { vb_pat; vb_expr; vb_attributes; vb_loc = Location.none; vb_sort = Jkind.Sort.value }
;;
...
let rec replace_id_in_path path to_rep : Path.t =
  ...
  | Papply (p1, p2) -> Papply (replace_id_in_path p1 to_rep, replace_id_in_path p2 to_rep)
