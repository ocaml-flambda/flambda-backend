...
module Layout_and_axes = struct
  ...
  (* Normalize the jkind. If mode is [Require_best], only jkinds that have quality [Best]
     ...
     axes.
  *)
  let normalize (type layout l r1 r2) ?(existentials = Btype.TypeSet.empty)
        ...
        | (ty, ti) :: bs -> (
          ...
          | false -> (
            ...
            let found_jkind_for_ty new_ctl b_upper_bounds b_with_bounds quality
              ...
              match quality, mode, Btype.TypeSet.mem ty existentials with
              | Best, _, _
...
let[@inline] normalize ?(existentials = Btype.TypeSet.empty) ~mode
    ~jkind_of_type t =
  ...
  let jkind, fuel_result =
    Layout_and_axes.normalize ~existentials ~jkind_of_type
