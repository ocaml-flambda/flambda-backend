...
let all_void_labels lbls =
  List.for_all
    (fun (_lbl : Types.label_declaration) -> (* CR: fix this *) false)
    lbls
...
let for_boxed_variant cstrs =
  ...
  if List.for_all
       (fun cstr ->
         ...
         | Cstr_tuple args ->
           List.for_all (fun _arg -> (* CR: fix this *) false) args
...
let () =
  Types.set_sort_equal (fun left right ->
      let left = Option.bind (get_layout left) Layout.Const.get_sort in
      let right = Option.bind (get_layout right) Layout.Const.get_sort in
      match left, right with
      | Some left, Some right -> Sort.Const.equal left right
      | None, _ | _, None -> false)
