...
module With_bounds = struct
  ...
  module Type_info = struct
    ...
    let print ~print_type_expr ppf Types.{ type_expr; modality; nullability } =
      let open Format in
      ...
      then print_type_expr ppf type_expr
        ...
        fprintf ppf "(@[%a@ @@@@ %a%a])" print_type_expr type_expr
          ...
          (fun ppf nullable -> if nullable then fprintf ppf "nullable")
          nullability
      end
    ...
    let is_on_axis (type a) ~(axis : a Jkind_axis.Axis.t) (t : t) =
      match axis with
    ...
    let create ~type_expr ~modality ~deep_only : t =
