...
let print_jkind = ref (fun ~print_type_expr:_ _ _ -> ())
    ...
    let rec loop ctl bounds_so_far :
      ...
      | (ty, ti) :: bs -> (
        ...
        | Continue ctl_after_unpacking_b -> (
          ...
          | Some b_jkind ->
            found_jkind_for_ty ctl_after_unpacking_b b_jkind.jkind.upper_bounds
          ...
          | None ->
            ...
            found_jkind_for_ty ctl_after_unpacking_b Bounds.max No_with_bounds
              Not_best))
    in
    let upper_bounds, with_bounds =
      loop Loop_control.starting t.upper_bounds
        (With_bounds.to_list t.with_bounds)
    in
    { t with upper_bounds; with_bounds }

  let sub (type l r) ~type_equal:_ ~jkind_of_type
      (sub : (allowed * r) jkind_desc)
      ({ layout = lay2; upper_bounds = bounds2; with_bounds = No_with_bounds } :
        (l * allowed) jkind_desc) =
    let { layout = lay1; upper_bounds = bounds1; with_bounds = _ } =
...
module Builtin = struct
  ...
  let value_v1_safety_check =
    { jkind = Jkind_desc.Builtin.value_or_null;
      ...
      has_warned = false;
      quality = Not_best (* CR aspsmith: FIXME *)
    }
...
let get_modal_upper_bounds (type l r) ~jkind_of_type (jk : (l * r) jkind) :
  ...
  let get axis = Bounds.get jk.jkind.upper_bounds ~axis in
...
let get_externality_upper_bound ~jkind_of_type jk =
  let jk = normalize ~mode:Ignore_best ~jkind_of_type jk in
  Bounds.get jk.jkind.upper_bounds ~axis:(Nonmodal Externality)
...
module Debug_printers = struct
  ...
  let t (type l r) ~print_type_expr ppf
      ({ jkind; annotation = a; history = h; has_warned = _; quality } :
        (l * r) jkind) : unit =
    fprintf ppf
      "@[<v 2>{ jkind = %a@,\
       ; annotation = %a@,\
      ...
      (match quality with Best -> "Best" | Not_best -> "Not_best")
