...
let report_privacy_mismatch ppf err =
  let singular, item =
    ...
    | Private_record_unboxed_product_type        -> true,  "record_unboxed_product constructor"
...
module Record_diffing = struct
  ...
  let compare_with_representation_flat ~loc env params1 params2 l r rep1 rep2 =
    if not (equal ~loc env params1 params2 l r) then
     ...
     | Record_unboxed_product _jkinds1, Record_unboxed_product _jkinds2 -> None
...
let type_declarations ?(equality = false) ~loc env ~mark name
  ...
  let err = match (decl1.type_kind, decl2.type_kind) with
    ...
    | (Type_record(labels1,rep1), Type_record(labels2,rep2)) ->
        if mark then begin
          let mark usage lbls =
            List.iter (Env.mark_label_used Legacy usage) lbls
          in
        ...
        Record_diffing.compare_with_representation ~loc env
          ...
          labels1 labels2
          rep1 rep2
    (* CR rtjoa: maybe deduplicate *)
    | (Type_record_unboxed_product(labels1,rep1), Type_record_unboxed_product(labels2,rep2)) ->
        if mark then begin
          let mark usage lbls =
            List.iter (Env.mark_label_used Unboxed_product usage) lbls
          in
          let usage : Env.label_usage =
            if decl2.type_private = Public then Env.Exported
            else Env.Exported_private
