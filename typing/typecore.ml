...
module Datatype_kind = struct
  ...
  let type_name = function
    | Record -> "record"
    | Record_flat_dk -> "record_flat"
    | Variant -> "variant"

  let label_name = function
    | Record -> "field"
    | Record_flat_dk -> "field_flat"
    | Variant -> "constructor"
end
...
type error =
  ...
  *)
  ...
  | Expr_not_a_record_flat_type of type_expr
  | Submode_failed of
...
and build_as_type_aux (env : Env.t) p ~mode =
  ...
  | Tpat_record (lpl,_) ->
      ...
      ty, mode
  (* CR rtjoa: rtjoa: fix soon *)
  | Tpat_record_flat (lpl,_) ->
      let lbl = snd3 (List.hd lpl) in
      if lbl.lbl_private = Private then p.pat_type, mode else
      (* The jkind here is filled in via unification with [ty_res] in
         [unify_pat]. *)
      ...
      let ppl = List.map (fun (_, l, p) -> l.lbl_num, p) lpl in
      let do_label lbl =
        let _, ty_arg, ty_res = instance_label_flat ~fixed:false lbl in
        unify_pat env {p with pat_type = ty} ty_res;
        let refinable =
          lbl.lbl_mut = Immutable && List.mem_assoc lbl.lbl_num ppl &&
          match get_desc lbl.lbl_arg with Tpoly _ -> false | _ -> true in
...
let solve_Ppat_record_flat_field ~refine loc penv label label_lid record_ty =
  with_local_level_iter ~post:generalize_structure begin fun () ->
    let (_, ty_arg, ty_res) = instance_label_flat ~fixed:false label in
    begin try
      unify_pat_types_refine ~refine loc penv ty_res (instance record_ty)
    with Error(_loc, _env, Pattern_type_clash(err, _)) ->
      raise(Error(label_lid.loc, !!penv,
...
module Label_flat = NameChoice (struct
  type t = label_flat_description
  type usage = Env.label_usage
  let kind = Datatype_kind.Record_flat_dk
  let get_name lbl = lbl.lbl_name
  let get_type lbl = lbl.lbl_res
  let lookup_all_from_type loc usage path env =
    Env.lookup_all_label_flats_from_type ~loc usage path env
  let in_env (lbl : t) =
    match lbl.lbl_repres with
    | Record_flat _ -> true
end)
...
(* Only issue warnings once per record constructor/pattern *)
let disambiguate_sort_lid_a_list_flat loc closed env usage expected_type lid_a_list =
  let ids = List.map (fun (lid, _) -> Longident.last lid.txt) lid_a_list in
  let w_pr = ref false and w_amb = ref []
  and w_scope = ref [] and w_scope_ty = ref "" in
  let warn loc msg =
  ...
  in
  let process_label lid =
    let scope = Env.lookup_all_label_flats ~loc:lid.loc usage lid.txt env in
    let filter : Label_flat.nonempty_candidate_filter =
      disambiguate_label_by_ids closed ids in
    Label_flat.disambiguate ~warn ~filter usage lid env expected_type scope in
  let lbl_a_list =
    (* If one label is qualified [{ foo = ...; M.bar = ... }],
       we will disambiguate all labels using one of the qualifying modules,
       as if the user had written [{ M.foo = ...; M.bar = ... }].
  ...
  if !w_pr then
    ...
    match List.rev !w_amb with
      (_,types,ex)::_ as amb ->
        let paths =
          List.map (fun (_,lbl,_) -> Label_flat.get_type_path lbl) lbl_a_list in
        let path = List.hd paths in
        let fst3 (x,_,_) = x in
        if List.for_all (compare_type_path env path) (List.tl paths) then
          Location.prerr_warning loc
...
let check_recordpat_flat_labels loc lbl_pat_list closed =
  match lbl_pat_list with
  | [] -> ()                            (* should not happen *)
  | (_, label1, _) :: _ ->
      let all = label1.lbl_all in
...
and type_pat_aux
  ...
  | Ppat_record(lid_sp_list, closed) ->
      ...
      let expected_type, record_ty =
        ...
        (* CR rtjoa:  *)
        | Record_flat_type _ -> assert false
        | Maybe_a_record_type ->
  ...
  (* CR rtjoa:  *)
  | Ppat_record_flat(lid_sp_list, closed) ->
      ...
      let expected_type, record_ty =
        ...
        | Not_a_record_type ->
          ...
          raise (Error (loc, !!penv, error))
      in
      let type_label_flat_pat (label_lid, label, sarg) =
        let ty_arg =
          solve_Ppat_record_flat_field ~refine:false loc penv label label_lid
            record_ty in
        check_project_mutability ~loc ~env:!!penv label.lbl_mut alloc_mode.mode;
        let mode =
          Modality.Value.Const.apply label.lbl_modalities alloc_mode.mode
        ...
        (label_lid, label, type_pat tps Value ~alloc_mode sarg ty_arg)
      in
      let make_record_flat_pat lbl_pat_list =
        check_recordpat_flat_labels loc lbl_pat_list closed;
        {
          pat_desc = Tpat_record_flat (lbl_pat_list, closed);
          pat_loc = loc; pat_extra=[];
          pat_type = instance record_ty;
          pat_attributes = sp.ppat_attributes;
          pat_env = !!penv;
      ...
      let lbl_a_list =
        wrap_disambiguate "This record pattern is expected to have"
          (mk_expected expected_ty)
          (disambiguate_sort_lid_a_list_flat loc false !!penv Env.Projection
             expected_type)
          lid_sp_list
      in
      let lbl_a_list = List.map type_label_flat_pat lbl_a_list in
      rvp @@ solve_expected (make_record_flat_pat lbl_a_list)
  | Ppat_array spl ->
...
let rec pat_tuple_arity spat =
  ...
  | Ppat_record _ | Ppat_record_flat _ | Ppat_array _ | Ppat_type _ | Ppat_lazy _
...
let rec check_counter_example_pat
  ...
  | Tpat_record_flat(fields, closed) ->
      let record_ty = generic_instance expected_ty in
      let type_label_flat_pat (label_lid, label, targ) k =
        let ty_arg =
          solve_Ppat_record_flat_field ~refine loc penv label label_lid record_ty in
        check_rec targ ty_arg (fun arg -> k (label_lid, label, arg))
      in
      map_fold_cont type_label_flat_pat fields
        (fun fields -> mkp k (Tpat_record_flat (fields, closed)))
...
let rec is_nonexpansive exp =
  ...
  | Texp_field_flat(exp, _, _, _) -> is_nonexpansive exp
    ...
    | Tarrow _ ->
        let rec check {exp_desc; exp_loc; exp_extra; _} =
          if List.exists (function
            ...
            | Texp_record_flat _
            | Texp_field _ | Texp_field_flat _ | Texp_setfield _ | Texp_setfield_flat _ | Texp_array _
...
let shallow_iter_ppat f p =
  ...
  | Ppat_record (args, _flag) | Ppat_record_flat (args, _flag) ->
...
and type_expect_
  ...
  let type_expect_record_flat
    ...
    let ty_record, expected_type =
      ...
      let opt_exp_opath =
        ...
        | Some (exp, _) ->
          ...
          | Maybe_a_record_type -> None
          | Not_a_record_type ->
            let error = Expr_not_a_record_flat_type exp.exp_type in
            raise (Error (exp.exp_loc, env, error))
      in
      match expected_opath, opt_exp_opath with
      | None, None ->
    ...
    let lbl_a_list =
      wrap_disambiguate "This flat record expression is expected to have"
        (mk_expected ty_record)
        (disambiguate_sort_lid_a_list_flat loc closed env Env.Construct expected_type)
        lid_sexp_list
    in
    let alloc_mode, argument_mode =
      if List.exists
    ...
    let type_label_exp ((_, label, _) as x) =
      check_construct_mutability ~loc ~env label.lbl_mut argument_mode;
      let argument_mode = mode_modality label.lbl_modalities argument_mode in
      type_label_flat_exp true env argument_mode loc ty_record x
    in
    let lbl_exp_list = List.map type_label_exp lbl_a_list in
    with_explanation (fun () ->
      unify_exp_types loc env (instance ty_record) (instance ty_expected));
    ...
    let opt_exp, label_definitions =
      ...
      | Some (exp, mode) ->
          let ty_exp = instance exp.exp_type in
          let unify_kept lbl =
            let _, ty_arg1, ty_res1 = instance_label_flat ~fixed:false lbl in
            unify_exp_types exp.exp_loc env ty_exp ty_res1;
            match matching_label lbl with
            | lid, _lbl, lbl_exp ->
                (* do not connect result types for overridden labels *)
                Overridden (lid, lbl_exp)
            | exception Not_found -> begin
                let _, ty_arg2, ty_res2 = instance_label_flat ~fixed:false lbl in
                unify_exp_types loc env ty_arg1 ty_arg2;
                with_explanation (fun () ->
                  unify_exp_types loc env (instance ty_expected) ty_res2);
                check_project_mutability ~loc:exp.exp_loc ~env lbl.lbl_mut mode;
  ...
  | Pexp_field(srecord, lid) ->
      ...
      rue {
        ...
        exp_attributes = sexp.pexp_attributes;
        exp_env = env }
  | Pexp_field_flat(srecord, lid) ->
      Jane_syntax_parsing.assert_extension_enabled ~loc Layouts Language_extension.Beta;
      let (record, rmode, label, _) =
        type_label_flat_access env srecord Env.Projection lid
      in
      let ty_arg =
        with_local_level_if_principal begin fun () ->
          (* [ty_arg] is the type of field, [ty_res] is the type of record, they
      ...
      in
      rue {
        exp_desc = Texp_field_flat(record, lid, label, boxing);
        exp_loc = loc; exp_extra = [];
        exp_type = ty_arg;
        exp_attributes = sexp.pexp_attributes;
        exp_env = env }
  | Pexp_setfield(srecord, lid, snewval) ->
      ...
      rue {
        ...
        exp_attributes = sexp.pexp_attributes;
        exp_env = env }
  | Pexp_setfield_flat(srecord, lid, snewval) ->
      Jane_syntax_parsing.assert_extension_enabled ~loc Layouts Language_extension.Beta;
      (* CR rtjoa: fix soon *)
      let (record, rmode, label, expected_type) =
        type_label_flat_access env srecord Env.Mutation lid in
      let ty_record =
        if expected_type = None
        then newvar (Jkind.of_new_sort ~why:Record_assignment)
        else record.exp_type
      ...
      let (label_loc, label, newval) =
        ...
        | Mutable m0 ->
          ...
          let mode = mutable_mode m0 |> mode_default in
          let mode = mode_modality label.lbl_modalities mode in
          type_label_flat_exp false env mode loc ty_record (lid, label, snewval)
        | Immutable ->
          raise(Error(loc, env, Label_not_mutable lid.txt))
      in
      unify_exp env record ty_record;
      rue {
        exp_desc = Texp_setfield_flat(record,
          Locality.disallow_right (regional_to_local
            (Value.proj (Comonadic Areality) rmode)),
          label_loc, label, newval);
        exp_loc = loc; exp_extra = [];
...
and type_label_flat_access env srecord usage lid =
  let mode = Value.newvar () in
  let record =
    with_local_level_if_principal ~post:generalize_structure_exp
      (fun () -> type_exp ~recarg:Allowed env (mode_default mode) srecord)
  ...
  let expected_type =
    ...
    | Record_type _
    | Not_a_record_type ->
        let error = Expr_not_a_record_flat_type ty_exp in
        raise (Error (record.exp_loc, env, error))
  in
  let labels = Env.lookup_all_label_flats ~loc:lid.loc usage lid.txt env in
  let label =
    wrap_disambiguate "This expression has" (mk_expected ty_exp)
      (Label_flat.disambiguate usage lid env expected_type) labels in
  (record, mode, label, expected_type)
...
and type_label_flat_exp create env (arg_mode : expected_mode) loc ty_expected
          ((lid, label, sarg) :
             Longident.t loc * label_flat_description * Parsetree.expression ) :

(Longident.t loc * label_flat_description * Typedtree.expression ) =
  (* Here also ty_expected may be at generic_level *)
  let separate = !Clflags.principal || Env.has_local_constraints env in
  (* #4682: we try two type-checking approaches for [arg] using backtracking:
     - first try: we try with [ty_arg] as expected type;
        ...
        with_local_level_iter_if separate begin fun () ->
          let (vars, ty_arg, ty_res) =
            with_local_level_iter_if separate ~post:generalize_structure
              begin fun () ->
                let ((_, ty_arg, ty_res) as r) =
                  instance_label_flat ~fixed:true label in
                (r, [ty_arg; ty_res])
              end
          in
          begin try
...
let report_error ~loc env = function
  ...
  | Expr_not_a_record_type ty ->
      Location.errorf ~loc
        ...
        (Style.as_inline_code Printtyp.type_expr) ty
  | Expr_not_a_record_flat_type ty ->
      Location.errorf ~loc
        "This expression has type %a@ \
