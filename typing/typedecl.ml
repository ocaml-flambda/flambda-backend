    ...
    if normalized_jkind != original_decl.type_jkind then begin
      ...
      | Ok _ ->
        if allow_any_crossing then
          ...
          in
          let umc = Some {
            (* CR aspsmith: make getters *)
            unsafe_mod_bounds = Mode.Crossing.of_bounds (Types.Jkind_mod_bounds.
                                                           { comonadic =
                                                               { areality = locality type_jkind.jkind.mod_bounds;
                                                                 linearity = linearity type_jkind.jkind.mod_bounds;
                                                                 portability = portability type_jkind.jkind.mod_bounds;
                                                                 yielding = yielding type_jkind.jkind.mod_bounds
                                                               };
                                                             monadic =
                                                               { uniqueness = uniqueness type_jkind.jkind.mod_bounds;
                                                                 contention = contention type_jkind.jkind.mod_bounds
                                                               }
                                                           });
