...
let all_coherent column =
  ...
  let coherent_heads hp1 hp2 =
    ...
    | Record_flat (lbl1 :: _), Record_flat (lbl2 :: _) ->
      Array.length lbl1.lbl_all = Array.length lbl2.lbl_all
    | Array (am1, _, _), Array (am2, _, _) -> am1 = am2
    | Any, _
    | _, Any
    | Record [], Record []
    | Record_flat [], Record_flat []
    | Variant _, Variant _
    | Lazy, Lazy -> true
    | ( Construct _ | Constant _ | Tuple _ | Unboxed_tuple _ | Record _ | Record_flat _
...
(* extract record fields as a whole *)
let record_flat_arg ph =
  let open Patterns.Head in
  match ph.pat_desc with
  | Any -> []
  | Record_flat args -> args
  | _ -> fatal_error "Parmatch.as_record_flat"
...
let simple_match_args discr head args =
  ...
  | Record_flat lbls -> extract_fields (record_flat_arg discr) (List.combine lbls args)
  | Any ->
      ...
      | Record_flat lbls ->  omega_list lbls
...
| {pat_desc = Tpat_record (omegas,closed)} ->
    ...
    rest
| {pat_desc = Tpat_record_flat (omegas,closed)} ->
    let args,rest = read_args omegas r in
    make_pat
      (Tpat_record_flat
         (List.map2 (fun (lid, lbl,_) arg ->
...
let rec has_instance p = match p.pat_desc with
  ...
  | Tpat_record_flat (lps,_) -> has_instances (List.map (fun (_,_,x) -> x) lps)
  | Tpat_lazy p
    -> has_instance p
