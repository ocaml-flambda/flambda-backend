...
let all_coherent column =
  ...
  let coherent_heads hp1 hp2 =
    ...
    | Variant _, Variant _
    | Lazy, Lazy -> true
    | ( Construct _ | Constant _ | Tuple _ | Unboxed_tuple _ | Record _ | Record_unboxed_product _
...
let simple_match_args discr head args =
  ...
  | Record_unboxed_product lbls -> extract_fields (record_unboxed_product_arg discr) (List.combine lbls args)
  | Any ->
...
let pats_of_type env ty =
  ...
  | Typedecl (_, path, {type_kind = Type_variant _ | Type_record _ | Type_record_unboxed_product _ }) ->
...
let rec has_instance p = match p.pat_desc with
  ...
  | Tpat_record_unboxed_product (lps,_) -> has_instances (List.map (fun (_,_,x) -> x) lps)
  | Tpat_lazy p
    -> has_instance p
