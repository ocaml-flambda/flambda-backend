...
module TyVarEnv : sig
  ...
  *)
  ...
  let mk_poly_univars_tuple_with_jkind ~context env var jkind_annot =
    let name = var.txt in
    let original_jkind =
      Jkind.of_annotation
        ~type_equal:(Ctype.type_equal env)
        ~context:(context name) jkind_annot
    in
  ...
  let make_poly_univars_jkinds ~context env vars_jkinds =
    let mk_trip = function
        ...
        | (v, Some l) -> mk_poly_univars_tuple_with_jkind ~context env v l
    in
...
let transl_type_param env path jkind_default styp =
  ...
  let transl_jkind_and_annot_opt jkind_annot name =
    ...
    | Some jkind_annot ->
      Jkind.of_annotation
        ~type_equal:(Ctype.type_equal env)
        ~context:(Type_parameter (path, name)) jkind_annot,
      Some jkind_annot
  in
...
let jkind_of_annotation env annotation_context attrs jkind =
  Jkind.of_annotation
    ~type_equal:(Ctype.type_equal env)
    ~context:(enrich_with_attributes attrs annotation_context)
    jkind
  ...
  let cty, jkind_annot = match name_opt with
    | Some { txt = alias; loc = alias_loc } ->
      begin try
        ...
        | Some jkind_annot ->
          let jkind =
            jkind_of_annotation env (Type_variable ("'" ^ alias)) attrs jkind_annot
          in
    ...
    | None ->
      ...
      in
      let jkind =
        jkind_of_annotation env (Type_wildcard jkind_annot.pjkind_loc)
          attrs jkind_annot
      in
