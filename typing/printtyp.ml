...
module Namespace = struct
  ...
  let lookup =
    ...
    | None | Some(Value|Extension_constructor|Constructor|Label|Label_flat) ->
  ...
  let location namespace id =
    ...
    try Some (
        ...
        | Some (Extension_constructor|Value|Constructor|Label|Label_flat) | None ->
            Location.none
      ) with Not_found -> None
...
let tree_of_type_decl id decl =
  ...
  let ty, priv, unboxed =
    ...
    | Type_record_flat(lbls, Record_flat _) ->
        tree_of_manifest (Otyp_record_flat (List.map tree_of_label lbls)),
        decl.type_private,
        false
    | Type_open ->
