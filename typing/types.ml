...
let find_unboxed_type decl =
  ...
  | Type_record_flat
                ([{ld_type = arg; _}], Record_flat _)
  | Type_variant ([{cd_args = Cstr_tuple [{ca_type = arg; _}]; _}], Variant_unboxed)
  | Type_variant ([{cd_args = Cstr_record [{ld_type = arg; _}]; _}],
                  Variant_unboxed) ->
    Some arg
  | Type_record_flat (({ld_type; _}::_ as lbls), Record_flat _) ->
    (* CR rtjoa: this is super hacky. If we keep it, we probably change
       [Tunboxed_tuple labeled_types] to
       [Tunboxed_product (history, labeled_types)]
       and give better error messages. *)
    incr new_id;
    Some {
       desc = Tunboxed_tuple (
          List.map (fun ld -> None, ld.ld_type ) lbls
        );
       level = Ident.highest_scope;
       scope = ld_type.scope;
       id = !new_id;
    }
