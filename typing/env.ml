...
let env_labels (type rep) (record_form : rep record_form) env : rep gen_label_description TycompTbl.t  =
  match record_form with
  | Legacy -> env.labels
...
and store_label
  ...
  then begin
    ...
    if not (Types.Uid.Tbl.mem (used_labels_by_form record_form) lbl.lbl_uid) then
      let used = label_usages () in
      Types.Uid.Tbl.add
        (used_labels_by_form record_form) lbl.lbl_uid (add_label_usage used);
...
let lookup_all_ident_labels (type rep) ~(record_form : rep record_form) ~errors ~use ~loc
      usage s env =
