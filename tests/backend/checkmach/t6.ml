...
(* test for off  *)
let[@zero_alloc off] test1 x = (x, x)

(* assume still works *)
let[@zero_alloc assume][@inline never][@specialise never][@local never] fail_loud x = [ x; x+1 ]

let call_loud x = fail_loud (x*2)

(* assume strict still works *)
let[@inline never][@specialise never][@local never][@zero_alloc strict assume] fail_loud2 x = raise (Exn (x,x))
...
let[@inline never][@specialise never][@zero_alloc strict assume][@zero_alloc off][@local never] fail_loud1 x = raise (Exn (x,x))
