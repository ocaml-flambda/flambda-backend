...
let get_rosetta2_status () =
  ...
  let command =
    Printf.sprintf
      "/usr/sbin/sysctl -n sysctl.proc_translated > %S 2> /dev/null" file_path
  in
  ...
  | 0 -> begin
    try
      ...
      match first_line with "0" -> Off | "1" -> On | _ -> Unknown
    with _ -> Unknown
  end
  | _ -> Unknown

let run_if_not_under_rosetta2 ~f =
  match get_rosetta2_status () with Unknown | On -> () | Off -> f ()
