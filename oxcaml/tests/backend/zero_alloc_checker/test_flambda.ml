(* These checks fail with closure but pass with flambda and flambda2, so we have
   a separate file for them. *)
...
let[@zero_alloc strict] test9 n = try test8 n with _ -> 0

(* This should work with closure too, but there is a problem with stdlib: stdlib
   is not compiled with -zero-alloc-check and with closure "failwith" does not
   get inlined. With flambda, ( ^ ) does not get inlined. *)
...
(* [test37] allocates on exn return in closure but not in flambda or flambda2,
   and conservative handling of callees causes test36 to fail zero_alloc check
   even though it does not allocate on normal return. *)
let[@zero_alloc] rec test36 x =
  if Sys.opaque_identity true
  then try test37 x with _ -> ()
