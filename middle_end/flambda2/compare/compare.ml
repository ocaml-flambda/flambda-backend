...
(*
  let map2 ~f c1 c2 =
    ...
    | different, Equivalent | Equivalent, different -> different
...
module Env = struct
  ...
  let add_symbol t symbol1 symbol2 =
    Symbol.Tbl.add t.symbols symbol1 symbol2
  ;;
  ...
  let find_symbol t sym =
    Symbol.Tbl.find_opt t.symbols sym
  ;;
  let find_code_id t code_id =
    Code_id.Tbl.find_opt t.code_ids code_id
  ;;
...
let subst_closure_id (env : Env.t) closure_id =
  Env.find_closure_id env closure_id
  |> Option.value ~default:closure_id
;;

let subst_code_id (env : Env.t) code_id =
  Env.find_code_id env code_id
  |> Option.value ~default:code_id
;;

let subst_symbol (env : Env.t) symbol =
  Env.find_symbol env symbol
  |> Option.value ~default:symbol
;;

let subst_closure_var (env : Env.t) var =
  Env.find_closure_var env var
...
module Comparator = struct
  ...
  let of_predicate ~(f : 'a -> 'a -> bool) ?(subst : (Env.t -> 'a -> 'a) option)
        : 'a t =
    fun env a1 a2 ->
      if f a1 a2 then Equivalent else
        let approximant = match subst with
          | Some subst -> subst env a1
          | None -> a1
        in Different { approximant }
...
let pairs
      ~(f1 : 'a Comparator.t)
      ~(f2 : 'b Comparator.t)
      ?(subst2 : (Env.t -> 'b -> 'b) option)
      : ('a * 'b) Comparator.t =
  fun env (a1, b1) (a2, b2) ->
    match f1 env a1 a2 with
    | Equivalent ->
      f2 env b1 b2 |> Comparison.map ~f:(fun b1' -> (a2, b1'))
    | Different { approximant = a1' } ->
      begin
        match subst2 with
        | Some subst2 ->
          Different { approximant = (a1', subst2 env b1) }
        | None ->
          begin
            match f2 env b1 b2 with
            | Equivalent ->
              Different { approximant = (a1', b2) }
            | Different { approximant = b1' } ->
              Different { approximant = (a1', b1') }
          end
      end
...
let triples
      ...
      ~(f3 : 'c Comparator.t)
      ?(subst2 : (Env.t -> 'b -> 'b) option)
      ?(subst3 : (Env.t -> 'c -> 'c) option)
      : ('a * 'b * 'c) Comparator.t =
  fun env (a1, b1, c1) (a2, b2, c2) ->
    let subst23 = match subst2, subst3 with
    ...
    in
    let f2 env bc1 bc2 =
      pairs ~f1:f2 ~f2:f3 ?subst2:subst3 env bc1 bc2
    in
...
let rec lists
  ...
  fun env list1 list2 ->
    ...
    | a1 :: list1, a2 :: list2 ->
      ...
      in
      pairs ~f1:f ~f2:(lists ~f ~subst ~subst_snd) ?subst2
        env (a1, list1) (a2, list2)
...
let options ~f ~subst env o1 o2 =
  ...
  | Some a1, Some a2 ->
    f env a1 a2
    |> Comparison.map ~f:(fun a1' -> Some a1')
  | None, None ->
    Comparison.Equivalent
  | Some a1, None ->
    Comparison.Different { approximant = Some (subst env a1) }
  | None, Some _ ->
    Comparison.Different { approximant = None }
;;
...
let closure_ids env closure_id1 closure_id2 : Closure_id.t Comparison.t =
  ...
  | None ->
    ...
    | None -> Env.add_closure_id env closure_id1 closure_id2; Equivalent
;;
...
let closure_vars env closure_var1 closure_var2
  ...
  | None ->
    ...
    | None -> Env.add_closure_var env closure_var1 closure_var2; Equivalent
;;
...
let names env name1 name2 : Name.t Comparison.t =
  ...
  Name.pattern_match name1
      ...
      Name.pattern_match name2
        ~var:(fun var2 : Name.t Comparison.t ->
          if Variable.equal var1 var2
            then Equivalent
            else Different { approximant = name1 } (* don't bother with
                                                       subst when name1 is
                                                       var *)
        )
    ...
    ~symbol:(fun symbol1 ->
      Name.pattern_match name2
        ~var:(fun _ -> Comparison.Different {
          approximant = subst_name env name1
        })
        ~symbol:(fun symbol2 ->
          symbols env symbol1 symbol2
          |> Comparison.map ~f:Name.symbol
        )
    ))
;;
  ...
  Simple.pattern_match simple1
    ~name:(fun name1 ~coercion:_ ->
      ...
      Simple.pattern_match simple2
        ...
        ~const:(fun _ -> Comparison.Different {
          approximant = subst_simple env simple1
        })
    )
    ~const:(fun const1 ->
      Simple.pattern_match simple2
...
let unary_prim_ops env (prim_op1 : Flambda_primitive.unary_primitive)
  ...
  | Select_closure { move_from = move_from1; move_to = move_to1 },
    ...
    pairs ~f1:closure_ids ~f2:closure_ids
      env (move_from1, move_to1) (move_from2, move_to2)
  ...
  | Project_var { project_from = closure_id1; var = var1 },
    ...
    pairs ~f1:closure_ids ~f2:closure_vars
      env (closure_id1, var1) (closure_id2, var2)
...
let primitives env prim1 prim2 : Flambda_primitive.t Comparison.t =
  ...
  | Unary (prim_op1, arg1), Unary (prim_op2, arg2) ->
    pairs ~f1:unary_prim_ops ~f2:simple_exprs ~subst2:subst_simple
      env (prim_op1, arg1) (prim_op2, arg2)
  ...
  | Binary (prim_op1, arg1_1, arg2_1), Binary (prim_op2, arg1_2, arg2_2) ->
    if Flambda_primitive.equal_binary_primitive prim_op1 prim_op2
      then
        ...
        let approximant =
          Flambda_primitive.Binary (prim_op1, subst_simple env arg1_1,
  ...
  | Ternary (prim_op1, arg1_1, arg2_1, arg3_1),
    ...
    if Flambda_primitive.equal_ternary_primitive prim_op1 prim_op2
      then
        ...
        let approximant =
          Flambda_primitive.Ternary (prim_op1, subst_simple env arg1_1,
            subst_simple env arg2_1, subst_simple env arg3_1)
        in
        Different { approximant }
...
let named_exprs env named1 named2 : Named.t Comparison.t =
  ...
  | Simple simple1, Simple simple2 ->
    simple_exprs env simple1 simple2
    |> Comparison.map ~f:Named.create_simple
...
let fields env (field1 : Static_const.Field_of_block.t)
      ...
      : Static_const.Field_of_block.t Comparison.t =
  match field1, field2 with
  | Symbol symbol1, Symbol symbol2 ->
    symbols env symbol1 symbol2
    |> Comparison.map ~f:(fun symbol1' ->
        Static_const.Field_of_block.Symbol symbol1')
  | _, _ ->
    Comparator.of_predicate ~f:Static_const.Field_of_block.equal
      env field1 field2
;;
...
let method_kinds _env
  (method_kind1 : Call_kind.method_kind)
...
let call_kinds env (call_kind1 : Call_kind.t) (call_kind2 : Call_kind.t)
  ...
  | Function (Direct {
    ...
    |> Comparison.map ~f:(fun (code_id, closure_id, return_arity) ->
         Call_kind.direct_function_call code_id closure_id ~return_arity)
  | Function (Indirect_known_arity { param_arity = param_arity1;
                                     return_arity = return_arity1 }),
  ...
  | Method { kind = kind1; obj = obj1 }, Method { kind = kind2; obj = obj2 } ->
    pairs ~f1:method_kinds ~f2:simple_exprs ~subst2:subst_simple
      env (kind1, obj1) (kind2, obj2)
...
let apply_cont_exprs env apply_cont1 apply_cont2 : Apply_cont.t Comparison.t =
  ...
  if Option.compare Trap_action.compare
       (Apply_cont.trap_action apply_cont1)
       (Apply_cont.trap_action apply_cont2) = 0
     && Continuation.equal
  ...
  then
    ...
    |> Comparison.map ~f:(fun args1' ->
         Apply_cont.create ?trap_action:(Apply_cont.trap_action apply_cont1)
           (Apply_cont.continuation apply_cont1)
           ~args:args1'
           ~dbg:(Apply_cont.debuginfo apply_cont1))
  else
    Different { approximant = subst_apply_cont env apply_cont1 }
;;
...
let switch_exprs env switch1 switch2 : Expr.t Comparison.t =
  let compare_arms env arms1 arms2 =
    lists
      ...
      ~subst_snd:true
      env (Targetint_31_63.Map.bindings arms1) (Targetint_31_63.Map.bindings arms2)
...
and let_cont_exprs env (let_cont1 : Let_cont.t) (let_cont2 : Let_cont.t)
  ...
  | Recursive handlers1, Recursive handlers2 ->
    let compare_handler_maps env map1 map2
      ...
      lists ~f:(fun env (cont, handler1) (_cont, handler2) ->
        ...
        ~subst:(fun env (cont, handler) ->
          (cont, subst_cont_handler env handler)
        )
        ~subst_snd:false
        env
    ...
    in
    Recursive_let_cont_handlers.pattern_match_pair handlers1 handlers2 ~f:(
      fun ~body1 ~body2 cont_handlers1 cont_handlers2 ->
