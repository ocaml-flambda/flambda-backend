...
let subst_func_decl env (code_id : Function_declarations.code_id_in_function_declaration) : Function_declarations.code_id_in_function_declaration =
  match code_id with
  | Deleted -> Deleted
...
let sets_of_closures env set1 set2 : Set_of_closures.t Comparison.t =
  ...
  let function_slots_and_fun_decls_by_code_id set =
    ...
    |> List.partition_map (fun (function_slot, (code_id : Function_declarations.code_id_in_function_declaration)) ->
           match code_id with Deleted -> Right function_slot | Code_id code_id0 -> Left (subst_code_id env code_id0, (function_slot, code_id)))
    in
    Code_id.Map.of_list function_slot_map, Function_slot.Set.of_list deleted_function_slot_set
  in
  let function_slot_map1, deleted_function_slot_set1 = function_slots_and_fun_decls_by_code_id set1 in
  let function_slot_map2, deleted_function_slot_set2 = function_slots_and_fun_decls_by_code_id set2 in
  ...
  let (_ : unit Code_id.Map.t) =
    Code_id.Map.merge
      ...
      function_slot_map2
  in
  (* Trying to find a mapping between the deleted function_slots is not easy,
   so we simply say the sets are equal if they have the same cardinality *)
  if Function_slot.Set.cardinal deleted_function_slot_set1 <> Function_slot.Set.cardinal deleted_function_slot_set2 then ok := false;
  if !ok
  then Equivalent
