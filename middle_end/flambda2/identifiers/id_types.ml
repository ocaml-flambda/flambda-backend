...
module Id (_ : sig end) : Id = struct
  ...
  let name (_, name) = if name == empty_string then None else Some name
...
module UnitId (Innerid : Id) (Compilation_unit : Container_types.Thing) :
  ...
  let rename t = { id = Innerid.rename t.id; unit = t.unit }

  let to_string x =
    Format.asprintf "%a.%a" Compilation_unit.print x.unit Innerid.print x.id
