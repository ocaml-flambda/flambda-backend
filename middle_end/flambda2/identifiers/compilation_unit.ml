...
let compare0 v1 v2 =
  ...
  then 0
    ...
    (* CR mshinwell: is [hash] definitely >=0 on a 32-bit platform? *)
    ...
    then
      ...
      if c = 0 then Linkage_name.compare v1.linkage_name v2.linkage_name else c
    else c
...
(* CR mshinwell: This shouldn't be necessary now we force inlining of the
   functor *)
(* Exposing [compare] and [equal] not via the functor application ensures that
   [Closure] will give an approximation for them. Otherwise they won't be called
   directly. *)
...
let create (id : Ident.t) linkage_name =
  ...
  then Misc.fatal_error "Compilation_unit.create with non-persistent Ident.t";
...
let external_symbols_t =
  create (Ident.create_persistent ".extern") (Linkage_name.create ".extern")
