...
module Effect = struct
  ...
  let print ppf t =
    ...
    | Resume { stack; f; arg } ->
      fprintf ppf "@[<hov 1>(%tResume%t (stack@ %a)@ (f@ %a)@ (arg@ %a))@]"
        Flambda_colours.effect Flambda_colours.pop Simple.print stack
        Simple.print f Simple.print arg
  ...
  let apply_renaming t renaming =
    ...
    | Resume { stack; f; arg } ->
      ...
      if stack == stack' && f == f' && arg == arg'
      then t
      else Resume { stack = stack'; f = f'; arg = arg' }
...
let perform ~eff = Effect (Perform { eff })

let reperform ~eff ~cont ~last_fiber =
  Effect (Reperform { eff; cont; last_fiber })

let run_stack ~stack ~f ~arg = Effect (Run_stack { stack; f; arg })

let resume ~stack ~f ~arg = Effect (Resume { stack; f; arg })
