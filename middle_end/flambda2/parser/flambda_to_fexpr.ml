...
let mixed_field_of_block env (field : Field_of_static_block.Mixed_field.t) :
    Fexpr.mixed_field_of_block =
  match field with
...
let static_const env (sc : Static_const.t) : Fexpr.static_data =
  ...
  | Mixed_block (tag, mutability, shape, fields) ->
    ...
    let shape = Flambda_kind.Mixed_block_shape.to_lambda shape in
    Mixed_block { tag; mutability; shape; elements }
