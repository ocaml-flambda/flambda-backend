  ...
  if Option.is_some cu then
    ...
    let cunit =
      ...
      | Some { Fexpr.ident; linkage_name } ->
        let linkage_name =
          linkage_name |> Option.value ~default:ident
        in
    ...
    in
    let symbol =
      Symbol.unsafe_create cunit (Linkage_name.create name)
    in
    symbol,
    { env with
...
let const (c:Fexpr.const) : Reg_width_const.t =
  match c with
  | Tagged_immediate i ->
    Reg_width_const.tagged_immediate (i |> immediate)
  | Naked_immediate i ->
    Reg_width_const.naked_immediate (i |> immediate)
  | Naked_float f ->
    Reg_width_const.naked_float (f |> float)
  | Naked_int32 i ->
...
let rec rec_info env (ri:Fexpr.rec_info) : Rec_info_expr.t =
  ...
  match ri with
  | Depth d ->
    Rec_info_expr.const ~depth:(Finite d) ~unrolling:US.not_unrolling
  | Infinity ->
    Rec_info_expr.const ~depth:Infinity ~unrolling:US.not_unrolling
  | Do_not_inline ->
    Rec_info_expr.do_not_inline
  | Var dv ->
    Rec_info_expr.var (find_var env dv)
  | Succ ri ->
    Rec_info_expr.succ (rec_info env ri)
  | Unroll (u, Depth d) ->
    Rec_info_expr.const
      ~depth:(Finite d)
  ...
  | Unroll (u, Infinity) ->
    Rec_info_expr.const
      ...
      ~unrolling:(US.unrolling ~remaining_depth:u)
  | Unroll (d, ri) ->
    Rec_info_expr.unroll_to d (rec_info env ri)
...
let rec simple env (s:Fexpr.simple) : Simple.t =
  ...
  | Coerce (s, co) ->
    Simple.apply_coercion_exn (simple env s) (coercion env co)
    ...
    let static_const env (b : Fexpr.symbol_binding) : Flambda.Static_const.t =
      ...
      | Code { id; newer_version_of; param_arity; ret_arity; recursive; inline;
        ...
        let params_and_body : _ Or_deleted.t =
          match params_and_body with
          | Deleted -> Deleted
          | Present { params; closure_var; depth_var; ret_cont;
                      exn_cont; body } ->
            let params, env =
              map_accum_left
