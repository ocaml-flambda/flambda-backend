...
let meet_variants_don't_lose_aliases () =
  ...
  let define env v =
    let v' =
      Bound_var.create v Flambda_debug_uid.none
        (* CR sspies: fix *)
        Name_mode.normal
    in
  ...
  | Ok (meet_ty, env) ->
    ...
    let bv_naked =
      Bound_var.create v_naked Flambda_debug_uid.none (* CR sspies: fix *)
        Name_mode.normal
    in
    let env =
...
let test_join_with_extensions () =
  let define ?(kind = K.value) env v =
    let v' =
      Bound_var.create v Flambda_debug_uid.none (* CR sspies: fix *)
        Name_mode.normal
    in
