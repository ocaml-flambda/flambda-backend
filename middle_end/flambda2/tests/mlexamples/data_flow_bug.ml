...
let[@inline never] fixity_of_string  = function
  (* | "" -> `Normal *)
  (*
  | s when id false (* List.mem s special_infix_strings *) -> `Infix s
  | s when id false (* List.mem s.[0] infix_symbols *) -> `Infix s
  | s when id false (* List.mem s.[0] prefix_symbols *) -> `Prefix s
  | s when s.[0] = '.' -> `Mixfix s
  | s when id false (* letop s *) -> `Letop s
  *)
