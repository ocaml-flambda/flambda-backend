...
module LargeFile = struct
  ...
  external out_channel_length : out_channel -> int64 = "caml_ml_channel_size_64"
 ...
 fun k acc fmt ->
  ...
  | Caml_char rest ->
    fun c ->
      ...
      make_printf k new_acc rest
  | String (pad, rest) -> make_padding k acc rest pad (fun str -> str)
  | Caml_string (pad, rest) -> make_padding k acc rest pad string_to_caml_string
  | Int (iconv, pad, prec, rest) ->
  ...
  | Theta rest -> fun f -> make_printf k (Acc_delay (acc, f)) rest
  | Custom (arity, f, rest) -> make_custom k acc rest arity (f ())
  | Reader _ ->
    (* This case is impossible, by typing of formats. *)
    (* Indeed, since printf and co. take a format4 as argument, the 'd and 'e
       type parameters of fmt are obviously equals. The Reader is the only
       constructor which touch 'd and 'e type parameters of the format type, it
       adds an (->) to the 'd parameters. Consequently, a format4 cannot contain
       a Reader node, except in the sub-format associated to an %{...%}. It's
       not a problem because make_printf do not call itself recursively on the
       sub-format associated to %{...%}. *)
    assert false
  | Flush rest -> make_printf k (Acc_flush acc) rest
  | String_literal (str, rest) ->
    make_printf k (Acc_string_literal (acc, str)) rest
  | Char_literal (chr, rest) -> make_printf k (Acc_char_literal (acc, chr)) rest
  ...
  | Format_subst (_, fmtty, rest) ->
    fun (Format (fmt, _)) ->
      make_printf k acc (concat_fmt (recast fmt fmtty) rest)
  ...
  | Scan_next_char rest ->
    fun c ->
      let new_acc = Acc_data_char (acc, c) in
      make_printf k new_acc rest
  | Ignored_param (ign, rest) -> make_ignored_param k acc ign rest
  | Formatting_lit (fmting_lit, rest) ->
    make_printf k (Acc_formatting_lit (acc, fmting_lit)) rest
  | Formatting_gen (Open_tag (Format (fmt', _)), rest) ->
    let k' kacc =
      make_printf k (Acc_formatting_gen (acc, Acc_open_tag kacc)) rest
    in
    make_printf k' End_of_acc fmt'
  | Formatting_gen (Open_box (Format (fmt', _)), rest) ->
    let k' kacc =
      make_printf k (Acc_formatting_gen (acc, Acc_open_box kacc)) rest
    in
    make_printf k' End_of_acc fmt'
  | End_of_format -> k acc
...
and make_int_padding_precision :
    ...
    int_conv ->
    x =
 fun k acc fmt pad prec trans iconv ->
  match pad, prec with
...
and make_custom :
    ...
    x ->
    y =
 fun k acc rest arity f ->
  match arity with
  | Custom_zero -> make_printf k (Acc_data_string (acc, f)) rest
  | Custom_succ arity -> fun x -> make_custom k acc rest arity (f x)

let const x _ = x

let rec make_iprintf :
    type a b c d e f state. (state -> f) -> state -> (a, b, c, d, e, f) fmt -> a
    =
 fun k o fmt ->
  match fmt with
  | Char rest -> const (make_iprintf k o rest)
  | Caml_char rest -> const (make_iprintf k o rest)
  | String (No_padding, rest) -> const (make_iprintf k o rest)
  | String (Lit_padding _, rest) -> const (make_iprintf k o rest)
  | String (Arg_padding _, rest) -> const (const (make_iprintf k o rest))
  | Caml_string (No_padding, rest) -> const (make_iprintf k o rest)
  | Caml_string (Lit_padding _, rest) -> const (make_iprintf k o rest)
  | Caml_string (Arg_padding _, rest) -> const (const (make_iprintf k o rest))
  | Int (_, pad, prec, rest) -> fn_of_padding_precision k o rest pad prec
  | Int32 (_, pad, prec, rest) -> fn_of_padding_precision k o rest pad prec
  | Nativeint (_, pad, prec, rest) -> fn_of_padding_precision k o rest pad prec
  | Int64 (_, pad, prec, rest) -> fn_of_padding_precision k o rest pad prec
  | Float (_, pad, prec, rest) -> fn_of_padding_precision k o rest pad prec
  | Bool (No_padding, rest) -> const (make_iprintf k o rest)
  | Bool (Lit_padding _, rest) -> const (make_iprintf k o rest)
  | Bool (Arg_padding _, rest) -> const (const (make_iprintf k o rest))
  | Alpha rest -> const (const (make_iprintf k o rest))
  | Theta rest -> const (make_iprintf k o rest)
  | Custom (arity, _, rest) -> fn_of_custom_arity k o rest arity
  | Reader _ ->
    (* This case is impossible, by typing of formats. See the note in the
       corresponding case for make_printf. *)
    assert false
  | Flush rest -> make_iprintf k o rest
  | String_literal (_, rest) -> make_iprintf k o rest
  | Char_literal (_, rest) -> make_iprintf k o rest
  | Format_arg (_, _, rest) -> const (make_iprintf k o rest)
  | Format_subst (_, fmtty, rest) ->
    fun (Format (fmt, _)) ->
      make_iprintf k o (concat_fmt (recast fmt fmtty) rest)
  | Scan_char_set (_, _, rest) -> const (make_iprintf k o rest)
  | Scan_get_counter (_, rest) -> const (make_iprintf k o rest)
  | Scan_next_char rest -> const (make_iprintf k o rest)
  | Ignored_param (ign, rest) ->
    make_ignored_param (fun _ -> k o) End_of_acc ign rest
  | Formatting_lit (_, rest) -> make_iprintf k o rest
  | Formatting_gen (Open_tag (Format (fmt', _)), rest) ->
...
and fn_of_padding_precision :
    ...
    (y, z -> a) precision ->
    x =
 fun k o fmt pad prec ->
  match pad, prec with
  | No_padding, No_precision -> const (make_iprintf k o fmt)
  | No_padding, Lit_precision _ -> const (make_iprintf k o fmt)
  | No_padding, Arg_precision -> const (const (make_iprintf k o fmt))
  | Lit_padding _, No_precision -> const (make_iprintf k o fmt)
  | Lit_padding _, Lit_precision _ -> const (make_iprintf k o fmt)
...
and fn_of_custom_arity :
    type x y a b c d e f state.
    (state -> f) ->
    state ->
    (a, b, c, d, e, f) fmt ->
    (a, x, y) custom_arity ->
 ...
 fun k o fmt -> function
  | Custom_zero -> make_iprintf k o fmt
  | Custom_succ arity -> const (fn_of_custom_arity k o fmt arity)

(******************************************************************************)
(* Continuations for make_printf *)

(* Recursively output an "accumulator" containing a reversed list of printing
   entities (string, char, flus, ...) in an output_stream. *)
(* Used as a continuation of make_printf. *)
let rec output_acc o acc =
  match acc with
  ...
  | Acc_formatting_gen (p, Acc_open_box acc') ->
    ...
    output_string o "@[";
    output_acc o acc'
  | Acc_string_literal (p, s) | Acc_data_string (p, s) ->
    output_acc o p;
    output_string o s
  | Acc_char_literal (p, c) | Acc_data_char (p, c) ->
    output_acc o p;
    output_char o c
  | Acc_delay (p, f) ->
    output_acc o p;
...
(* Recursively output an "accumulator" containing a reversed list of printing
   entities (string, char, flus, ...) in a buffer. *)
(* Used as a continuation of make_printf. *)
let rec bufput_acc b acc =
  match acc with
  | Acc_formatting_lit (p, fmting_lit) ->
    let s = string_of_formatting_lit fmting_lit in
    bufput_acc b p;
...
let open_box_of_string str =
  ...
  then 0, Pp_box
    ...
    let rec parse_spaces i =
      if i = len
      then i
      else match str.[i] with ' ' | '\t' -> parse_spaces (i + 1) | _ -> i
    and parse_lword i j =
      if j = len
      then j
      else match str.[j] with 'a' .. 'z' -> parse_lword i (j + 1) | _ -> j
    and parse_int i j =
      if j = len
      then j
      else match str.[j] with '0' .. '9' | '-' -> parse_int i (j + 1) | _ -> j
    in
...
let make_padding_fmt_ebb :
    type x y.
    (x, y) padding -> (_, _, _, _, _, _) fmt -> (_, _, _, _, _) padding_fmt_ebb
    =
 fun pad fmt ->
  match pad with
...
let fmt_ebb_of_string ?legacy_behavior str =
  ...
  let legacy_behavior =
    match legacy_behavior with Some flag -> flag | None -> true
    (* When this flag is enabled, the format parser tries to behave as the <4.02
       implementations, in particular it ignores most benine nonsensical format.
       When the flag is disabled, it will reject any format that is not accepted
  ...
  and parse_literal : type e f. int -> int -> int -> (_, _, e, f) fmt_ebb =
   fun lit_start str_ind end_ind ->
    if str_ind = end_ind
    then add_literal lit_start str_ind End_of_format
    else
      ...
      | '@' ->
        ...
        add_literal lit_start str_ind fmt_rest
      | _ -> parse_literal lit_start (str_ind + 1) end_ind
  ...
  and parse_flags : type e f. int -> int -> int -> bool -> (_, _, e, f) fmt_ebb
   ...
   fun pct_ind str_ind end_ind ign ->
    ...
    let set_flag str_ind flag =
      ...
      if !flag && not legacy_behavior
      then
        failwith_message
          "invalid format %S: at character number %d, duplicate flag %C" str
          str_ind str.[str_ind];
      flag := true
    in
    let rec read_flags str_ind =
      ...
      | '+' ->
        ...
        read_flags (str_ind + 1)
      | '#' ->
        set_flag str_ind hash;
        read_flags (str_ind + 1)
      | ' ' ->
        set_flag str_ind space;
        read_flags (str_ind + 1)
      | _ ->
        parse_padding pct_ind str_ind end_ind !zero !minus !plus !hash !space
          ign
    in
    read_flags str_ind
   ...
   fun pct_ind str_ind end_ind zero minus plus hash space ign ->
    ...
    | _ -> begin
      ...
      | Left ->
        ...
        then invalid_format_without (str_ind - 1) '-' "padding";
      ...
      | Zeros ->
        (* a '0' padding indication not followed by anything should be
           interpreted as a Right padding of width 0. This is used by scanning
           conversions %0s and %0c *)
        parse_after_padding pct_ind str_ind end_ind minus plus hash space ign
          (Lit_padding (Right, 0))
      | Right ->
        parse_after_padding pct_ind str_ind end_ind minus plus hash space ign
   ...
   fun pct_ind str_ind end_ind minus plus hash space ign pad ->
    ...
    let parse_literal minus str_ind =
      ...
      parse_after_precision pct_ind new_ind end_ind minus plus hash space ign
        pad (Lit_precision prec)
    in
    match str.[str_ind] with
    | '0' .. '9' -> parse_literal minus str_ind
    | ('+' | '-') as symb when legacy_behavior ->
      (* Legacy mode would accept and ignore '+' or '-' before the integer
         describing the desired precision; note that this cannot happen for
         padding width, as '+' and '-' already have a semantics there.

         That said, the idea (supported by this tweak) that width and precision
         literals are "integer literals" in the OCaml sense is still blatantly
         wrong, as 123_456 or 0xFF are rejected. *)
    ...
    | _ ->
      if legacy_behavior
      then
        (* note that legacy implementation did not ignore '.' without a number
           (as it does for padding indications), but interprets it as '.0' *)
        parse_after_precision pct_ind str_ind end_ind minus plus hash space ign
          pad (Lit_precision 0)
      else invalid_format_without (str_ind - 1) '.' "precision"
  (* Try to read the conversion. *)
  and parse_after_precision :
      type x y z t e f.
      int ->
      int ->
      int ->
   ...
   fun pct_ind str_ind end_ind minus plus hash space ign pad prec ->
    if str_ind = end_ind then unexpected_end_of_format end_ind;
    let parse_conv (type u v) (padprec : (u, v) padding) =
      parse_conversion pct_ind (str_ind + 1) end_ind plus hash space ign pad
        prec padprec str.[str_ind]
    in
    (* in legacy mode, some formats (%s and %S) accept a weird mix of padding
       and precision, which is merged as a single padding information. For
       example, in %.10s the precision is implicitly understood as padding %10s,
       but the left-padding component may be specified either as a left padding
       or a negative precision: %-.3s and %.-3s are equivalent to %-3s *)
    match pad with
    | No_padding -> (
  ...
  and parse_conversion :
   ...
   fun pct_ind str_ind end_ind plus hash space ign pad prec padprec symb ->
    ...
    let plus_used = ref false
    and hash_used = ref false
    and space_used = ref false
    ...
    let get_int_pad () =
      (* %5.3d is accepted and meaningful: pad to length 5 with spaces, but
         ...
         "precision" for integer formats).

         %05.3d is redundant: pad to length 5 *with zeros*, but first pad with
         zeros... To add insult to the injury, the legacy implementation ignores
         the 0-padding indication and does the 5 padding with spaces instead. We
         reuse this interpretation for compatibility, but statically reject this
         format when the legacy mode is disabled, to protect strict users from
         this corner case. *)
    ...
    (* Get padding as a pad_option (see "%_", "%{", "%(" and "%["). (no need for
       ...
       well) *)
    let opt_of_pad c (type a b) (pad : (a, b) padding) =
      match pad with
      | No_padding -> None
    ...
    let fmt_result =
      ...
      | 'C' ->
        let (Fmt_EBB fmt_rest) = parse str_ind end_ind in
        if get_ign ()
        then Fmt_EBB (Ignored_param (Ignored_caml_char, fmt_rest))
      ...
      | 's' ->
        ...
        then
          ...
          let (Padding_fmt_EBB (pad', fmt_rest')) =
            make_padding_fmt_ebb pad fmt_rest
          in
      ...
      | 'd' | 'i' | 'x' | 'X' | 'o' | 'u' ->
        let iconv =
          compute_int_conv pct_ind str_ind (get_plus ()) (get_hash ())
            (get_space ()) symb
        in
        let (Fmt_EBB fmt_rest) = parse str_ind end_ind in
        if get_ign ()
        then
          ...
          let (Padprec_fmt_EBB (pad', prec', fmt_rest')) =
            make_padprec_fmt_ebb (get_int_pad ()) (get_prec ()) fmt_rest
          in
      ...
      | 'N' ->
        ...
        then
          ...
          Fmt_EBB (Ignored_param (ignored, fmt_rest))
        else Fmt_EBB (Scan_get_counter (counter, fmt_rest))
      | ('l' | 'n' | 'L')
        when str_ind = end_ind || not (is_int_base str.[str_ind]) ->
        let (Fmt_EBB fmt_rest) = parse str_ind end_ind in
      ...
      | 'f' | 'e' | 'E' | 'g' | 'G' | 'F' | 'h' | 'H' ->
        let fconv =
          compute_float_conv pct_ind str_ind (get_plus ()) (get_space ()) symb
        in
        let (Fmt_EBB fmt_rest) = parse str_ind end_ind in
        if get_ign ()
        then
          ...
          let (Padprec_fmt_EBB (pad', prec', fmt_rest')) =
            make_padprec_fmt_ebb (get_pad ()) (get_prec ()) fmt_rest
          in
      ...
      | 'r' ->
        let (Fmt_EBB fmt_rest) = parse str_ind end_ind in
        if get_ign ()
        then Fmt_EBB (Ignored_param (Ignored_reader, fmt_rest))
      ...
      | '{' ->
        ...
        then
          ...
          Fmt_EBB (Ignored_param (ignored, fmt_rest))
        else Fmt_EBB (Format_arg (get_pad_opt '{', sub_fmtty, fmt_rest))
      | '(' ->
        let sub_end = search_subformat_end str_ind end_ind ')' in
        let (Fmt_EBB fmt_rest) = parse (sub_end + 2) end_ind in
        let (Fmt_EBB sub_fmt) = parse str_ind sub_end in
        ...
        then
          ...
          Fmt_EBB (Ignored_param (ignored, fmt_rest))
        else Fmt_EBB (Format_subst (get_pad_opt '(', sub_fmtty, fmt_rest))
      | '[' ->
        ...
        then
          ...
          Fmt_EBB (Ignored_param (ignored, fmt_rest))
        else Fmt_EBB (Scan_char_set (get_pad_opt '[', char_set, fmt_rest))
      | '-' | '+' | '#' | ' ' | '_' ->
        failwith_message
          "invalid format %S: at character number %d, flag %C is only allowed \
           after the '%%', before padding and precision"
          str pct_ind symb
      | _ ->
        failwith_message
          "invalid format %S: at character number %d, invalid conversion \
           \"%%%c\""
          str (str_ind - 1) symb
    in
    ...
    then begin
      ...
      then incompatible_flag pct_ind str_ind symb "'#'";
      if (not !space_used) && space
      then incompatible_flag pct_ind str_ind symb "' '";
      if (not !pad_used) && Padding_EBB pad <> Padding_EBB No_padding
      then incompatible_flag pct_ind str_ind symb "`padding'";
      if (not !prec_used) && Precision_EBB prec <> Precision_EBB No_precision
      then
  ...
  and check_open_box : type a b c d e f. (a, b, c, d, e, f) fmt -> unit =
   fun fmt ->
    ...
    | String_literal (str, End_of_format) -> (
      try ignore (open_box_of_string str)
      with Failure _ -> ( (* Emit warning: invalid open box *) ))
    | _ -> ()
  ...
  and parse_tag : type e f. bool -> int -> int -> (_, _, e, f) fmt_ebb =
   fun is_open_tag str_ind end_ind ->
    ...
    with Not_found ->
      ...
      let formatting =
        if is_open_tag then Open_tag sub_format else Open_box sub_format
      in
      Fmt_EBB (Formatting_gen (formatting, fmt_rest))
    ...
    let next_ind, formatting_lit =
      try
        ...
        | '0' .. '9' | '-' -> (
          ...
          | _ -> raise Not_found)
        | _ -> raise Not_found
      with Not_found | Failure _ -> str_ind, Break ("@;", 1, 0)
    in
    let (Fmt_EBB fmt_rest) = parse next_ind end_ind in
  ...
  and parse_char_set str_ind end_ind =
    ...
    and parse_char_set_content str_ind end_ind =
      ...
      | c -> parse_char_set_after_char (str_ind + 1) end_ind c
    ...
    and parse_char_set_after_char str_ind end_ind c =
      ...
      | c' ->
        ...
        (* note that '@' alone is accepted, as done by the legacy
           implementation; the documentation specifically requires %@ so we
           could warn on that *)
        add_char c;
        parse_char_set_after_char (str_ind + 1) end_ind c'
    ...
    let str_ind, reverse =
      if str_ind = end_ind then unexpected_end_of_format end_ind;
      match str.[str_ind] with '^' -> str_ind + 1, true | _ -> str_ind, false
    in
  ...
  and parse_positive str_ind end_ind acc =
    ...
    | '0' .. '9' as c ->
      ...
      then
        failwith_message
          "invalid format %S: integer %d is greater than the limit %d" str
          new_acc Sys.max_string_length
      else parse_positive (str_ind + 1) end_ind new_acc
    | _ -> str_ind, acc
  ...
  and parse_integer str_ind end_ind =
    ...
    | '-' -> (
      ...
      | '0' .. '9' ->
        ...
        next_ind, -n
      | c -> expected_character (str_ind + 1) "digit" c)
    | _ -> assert false
  (* Add a literal to a format from a literal character sub-sequence. *)
  and add_literal :
      type a d e f. int -> int -> (a, _, _, d, e, f) fmt -> (_, _, e, f) fmt_ebb
      =
   fun lit_start str_ind fmt ->
    match str_ind - lit_start with
    | 0 -> Fmt_EBB fmt
  ...
  and search_subformat_end str_ind end_ind c =
    ...
    then
      failwith_message
        "invalid format %S: unclosed sub-format, expected \"%%%c\" at \
         character number %d"
        str c end_ind;
    match str.[str_ind] with
    | '%' -> (
  ...
  and compute_int_conv pct_ind str_ind plus hash space symb =
    ...
    | false, false, false, 'd' -> Int_d
    | false, false, false, 'i' -> Int_i
    | false, false, true, 'd' -> Int_sd
    | false, false, true, 'i' -> Int_si
    | true, false, false, 'd' -> Int_pd
    | true, false, false, 'i' -> Int_pi
    | false, false, false, 'x' -> Int_x
    | false, false, false, 'X' -> Int_X
    | false, true, false, 'x' -> Int_Cx
    | false, true, false, 'X' -> Int_CX
  ...
  and incompatible_flag : type a. int -> int -> char -> string -> a =
   fun pct_ind str_ind symb option ->
    ...
    failwith_message
      "invalid format %S: at character number %d, %s is incompatible with '%c' \
       in sub-format %S"
...
let format_of_string_fmtty str fmtty =
  ...
  with Type_mismatch ->
    failwith_message "bad input: format type mismatch between %S and %S" str
