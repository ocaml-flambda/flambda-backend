...
let stats h =
  let mbl = Array.fold_left (fun m b -> max m (bucket_length 0 b)) 0 h.data in
...
let to_seq tbl =
  ...
  let rec aux i buck () =
    match buck with
    | Empty ->
      if i = Array.length tbl_data then Seq.Nil else aux (i + 1) tbl_data.(i) ()
...
module MakeSeeded (H : SeededHashedType) : SeededS with type key = H.t = struct
  ...
  let key_index h key = H.hash h.seed key land (Array.length h.data - 1)
  ...
  let find_all h key =
    let rec find_in_bucket = function
      ...
      | Cons { key = k; data = d; next } ->
        if H.equal k key then d :: find_in_bucket next else find_in_bucket next
    in
    find_in_bucket h.data.(key_index h key)
