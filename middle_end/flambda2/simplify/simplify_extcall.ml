...
let simplify_caml_make_vect dacc ~len_ty ~init_value_ty : t =
  ...
  let array_kind : Array_kind.t Or_unknown_or_bottom.t =
    ...
    then Ok (Array_kind.of_kind (T.kind init_value_ty))
    else
  ...
  let type_of_returned_array =
    T.mutable_array ~array_kind ~length:len_ty Alloc_mode.For_types.heap
  in
