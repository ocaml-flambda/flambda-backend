...
let simple_join typing_env uses ~params =
  ...
  let bottom_types =
    ListLabels.map params ~f:(fun param ->
        ...
        |> Flambda_kind.With_subkind.kind |> T.bottom)
  in
  let joined_types =
...
let compute_handler_env t ~env_at_fork_plus_params_and_consts
  ...
  (* Format.eprintf "%d uses for %a\n%!" (List.length t.uses) Continuation.print
     t.continuation; *)
  match t.uses with
  | [] -> No_uses
  | uses ->
    ...
    let need_to_meet_param_types =
      ...
      (* If there is information available from the subkinds of the parameters,
         we will need to meet the existing parameter types (e.g. "unknown boxed
      ...
      List.map
        (fun use ->
          (* Format.eprintf "Use: parameters: %a,@ arg types: %a,@ env:@ %a\n%!"
             Kinded_parameter.List.print params (Format.pp_print_list
             ~pp_sep:Format.pp_print_space T.print) (U.arg_types use) DE.print
             (U.env_at_use use); *)
          let use_env =
            DE.map_typing_env (U.env_at_use use) ~f:(fun typing_env ->
                if need_to_meet_param_types
                then
                  TE.meet_equations_on_params typing_env ~params
                    ~param_types:(U.arg_types use)
                else
                  TE.add_equations_on_params typing_env ~params
                    ~param_types:(U.arg_types use))
          in
          use_env, U.id use, U.use_kind use)
        uses
    in
    (* Format.eprintf "Unknown at or later than %a\n%!" Scope.print (Scope.next
    ...
    let handler_env, extra_params_and_args, is_single_inlinable_use, escapes =
      ...
      | [(use_env, _, Inlinable)] ->
        (* We need to make sure any lifted constants generated during the
           simplification of the body are in the environment. Otherwise we might
           share a constant based on information in [DA] but then find the
           definition of the corresponding constant isn't in [DE]. Note that
           some of the constants may already be defined. *)
        let use_env =
      ...
      | (_, _, (Inlinable | Non_inlinable _)) :: _ -> (
        (* The lifted constants are put into the fork environment now because it
        ...
        let handler_env, extra_params_and_args =
          ...
          then
            ...
            (* Format.eprintf "handler env extension for %a is:@ %a\n%!"
               Continuation.print t.continuation T.Typing_env_extension.print
               env_extension; Format.eprintf "The extra params and args are:@
