...
let simplify_phys_equal (op : P.equality_comparison) dacc ~original_term dbg
  ...
  then
    ...
    let proof1 = T.check_equals_tagged_immediates typing_env arg1_ty in
    let proof2 = T.check_equals_tagged_immediates typing_env arg2_ty in
    match proof1, proof2 with
    | Known_result _, Known_result _ ->
      Binary_int_eq_comp_tagged_immediate.simplify op dacc ~original_term dbg
        ~arg1 ~arg1_ty ~arg2 ~arg2_ty ~result_var
    | Known_result _, (Unknown | Invalid)
    | (Unknown | Invalid), Known_result _
