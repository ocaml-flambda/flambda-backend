...
let did_perform_mutable_unboxing (result : T.Flow_result.t) =
  (* This function is used to determine whether the mutable unboxing pass
     ...
     we would like not to trigger a second pass of Simplify. In such cases the
     continuation param unboxing will have already rewritten most uses, so the
     mutable unboxing will only emit [Remove_prim] rewrites. Such rewrites are
     actually superfluous because the upwards pass of Simplify would already
     remove these bindings. Therefore, when all the rewrites generated by the
     mutable unboxing pass are [Remove_prim] rewrites, we can consider that the
     mutable unboxing pass did not actually make any useful change, and thus we
     do not need to redo a pass of Simplify. *)
