...
(* TODO: add comment that stageX contains all info known at the beginning of
   stage X *)
...
type stage3_handler_to_rebuild =
  { params : Bound_parameters.t;
    ...
    (* TODO: add comment to describe this precisely *)
...
type stage4_handler_to_rebuild =
  { params : Bound_parameters.t;
    ...
    invariant_extra_params_and_args : EPA.t;
    (* Note: EPA.extra_params invariant_extra_params_and_args should always be equa
       to invariant_extra_params in stage4 *)
...
type stage4_handlers_group =
  | Recursive of
      { rebuild_continuation_handlers :
          stage4_handler_to_rebuild Continuation.Map.t
      }
  | Non_recursive of
      { cont : Continuation.t;
...
type stage5_handlers_group =
  | Recursive of { continuation_handlers : rebuilt_handler Continuation.Map.t; invariant_params : Bound_parameters.t }
  | Non_recursive of
      { cont : Continuation.t;
...
let simplify_let_cont_stage6 (stage6 : stage6) ~after_rebuild body uacc =
        ...
        UA.with_name_occurrences
          ~name_occurrences:
            (Name_occurrences.union name_occurrences_body
               stage6.name_occurrences_of_subsequent_exprs)
          uacc
      in
      let uacc =
        UA.with_cost_metrics
          (Cost_metrics.( + ) cost_metrics_of_body
             stage6.cost_metrics_of_subsequent_exprs)
          uacc
      in
  ...
  rebuild_groups body name_occurrences_body cost_metrics_of_body uacc
    stage6.handlers_from_the_inside_to_the_outside
...
let simplify_let_cont_stage5 (stage5 : stage5) uacc ~after_rebuild =
  ...
  let stage6 : stage6 =
    { name_occurrences_of_subsequent_exprs =
        stage5.name_occurrences_of_subsequent_exprs;
      ...
      handlers_from_the_inside_to_the_outside =
        stage5.handlers_from_the_inside_to_the_outside
    }
  in
  stage5.rebuild_body uacc
    ~after_rebuild:(simplify_let_cont_stage6 stage6 ~after_rebuild)
...
let rec rebuild_continuation_handlers_loop ~rebuild_body
    ...
    ~uenv_of_subsequent_exprs ~at_unit_toplevel ~original_invariant_params ~invariant_extra_params uacc ~after_rebuild
    (groups_to_rebuild : stage4_handlers_group list) rebuilt_groups =
  ...
  | Non_recursive { cont; handler; is_single_inlinable_use }
    ...
    rebuild_single_non_recursive_handler ~at_unit_toplevel
      ...
      ~is_single_inlinable_use cont handler uacc (fun rebuilt_handler uacc ->
        rebuild_continuation_handlers_loop ~rebuild_body
          ~name_occurrences_of_subsequent_exprs
          ~cost_metrics_of_subsequent_exprs ~uenv_of_subsequent_exprs
          ~at_unit_toplevel ~original_invariant_params ~invariant_extra_params uacc ~after_rebuild groups_to_rebuild
  ...
  | Recursive { rebuild_continuation_handlers } :: groups_to_rebuild ->
    ...
    let rec loop uacc invariant_params remaining_handlers rebuilt_handlers k =
      ...
      | None ->
        k (Option.get invariant_params) rebuilt_handlers uacc
      | Some (cont, handler) ->
        ...
        rebuild_single_recursive_handler cont handler uacc
          (fun cont_invariant_params rebuilt_handler uacc ->
             let invariant_params = match invariant_params with
               | None -> Some cont_invariant_params
               | Some invariant_params ->
                 if not (bound_parameters_equal invariant_params cont_invariant_params) then
                   Misc.fatal_errorf "TODO good error message"
    ...
    loop uacc None rebuild_continuation_handlers Continuation.Map.empty
      (fun invariant_params rebuilt_handlers uacc ->
        ...
        rebuild_continuation_handlers_loop ~rebuild_body
          ...
          ~cost_metrics_of_subsequent_exprs ~uenv_of_subsequent_exprs
          ~at_unit_toplevel ~original_invariant_params ~invariant_extra_params uacc ~after_rebuild groups_to_rebuild
          (Recursive { continuation_handlers = rebuilt_handlers; invariant_params }
          :: rebuilt_groups))
...
let simplify_let_cont_stage4 (stage4 : stage4) uacc ~after_rebuild =
  ...
  rebuild_continuation_handlers_loop ~rebuild_body:stage4.rebuild_body
    ~at_unit_toplevel:stage4.at_unit_toplevel ~original_invariant_params:stage4.original_invariant_params ~invariant_extra_params:stage4.invariant_extra_params
    ~name_occurrences_of_subsequent_exprs ~cost_metrics_of_subsequent_exprs
    ~uenv_of_subsequent_exprs uacc ~after_rebuild
    stage4.handlers_from_the_outside_to_the_inside []
...
let simplify_let_cont_stage3 (stage3 : stage3) ~down_to_up dacc =
  ...
  let get_uses cont =
    ...
    | None ->
      Misc.fatal_errorf "Uses of %a not found in [simplify_let_cont_stage3]@."
        Continuation.print cont
    | Some cont -> cont
  in
  let handlers =
    Continuation.Map.mapi
      (fun cont (handler : stage3_handler_to_rebuild) ->
        let uses = get_uses cont in
        let use_ids = Continuation_uses.get_use_ids uses in
        let invariant_extra_args =
          Apply_cont_rewrite_id.Map.of_set (fun rewrite_id ->
              match Apply_cont_rewrite_id.Map.find rewrite_id (EPA.extra_args stage3.invariant_extra_params_and_args) with
              | extra_args -> extra_args
              | exception Not_found ->
                List.map (fun param ->
                    EPA.Extra_arg.Already_in_scope (Simple.var (Bound_parameter.var param))
                  )
                  (Bound_parameters.to_list (EPA.extra_params stage3.invariant_extra_params_and_args))
            ) use_ids
        in
        let invariant_epa =
          EPA.replace_extra_args stage3.invariant_extra_params_and_args invariant_extra_args
        in
        let extra_params_and_args =
          ...
          let _, arg_types_by_use_id =
            Misc.Stdlib.List.split_at
              (List.length (Bound_parameters.to_list stage3.invariant_params))
              arg_types_by_use_id
          in
          Unbox_continuation_params.compute_extra_params_and_args handler.unbox_decisions
            ~arg_types_by_use_id handler.extra_params_and_args_for_cse
        in
        ...
        Continuation.Map.fold
          (fun cont handler flow_acc ->
            Flow.Acc.add_extra_params_and_args cont
              (EPA.concat ~inner:handler.invariant_extra_params_and_args ~outer:handler.extra_params_and_args) flow_acc)
          handlers flow_acc)
  in
  let handlers_graph =
    Continuation.Map.map
  ...
  let handlers_from_the_outside_to_the_inside =
    Array.fold_left
      (fun inner group ->
        let group : stage4_handlers_group =
  ...
  let stage4 : stage4 =
    { rebuild_body = stage3.rebuild_body;
      ...
      original_invariant_params = stage3.invariant_params;
      invariant_extra_params = EPA.extra_params stage3.invariant_extra_params_and_args;
    }
  in
...
let simplify_handler ~simplify_expr ~is_recursive ~is_exn_handler ~invariant_params ~params cont
    dacc handler k =
    ...
    DA.map_flow_acc
      ~f:
        (Flow.Acc.enter_continuation cont ~recursive:is_recursive
           ~is_exn_handler (Bound_parameters.append invariant_params params))
      dacc
  in
...
let simplify_single_recursive_handler ~simplify_expr cont_uses_env_so_far ~invariant_params
  ...
  (* Here we perform the downwards traversal on a single handler. As an
     ...
     use the uses from [cont_uses_env_so_far] to compute the environment for the
     handler.
  ...
  simplify_handler ~simplify_expr ~is_recursive:true ~is_exn_handler:false ~params ~invariant_params cont
...
let simplify_let_cont_stage2 ~simplify_expr (stage2 : stage2) ~down_to_up dacc
  ...
  let dacc =
    DA.add_to_lifted_constant_accumulator dacc stage2.prior_lifted_constants
  in
  ...
  | Recursive { continuation_handlers; invariant_params } ->
    ...
    let rec simplify_handlers cont_uses_env_so_far used_handlers
      ...
      (* CR ncourant: this makes the order in which continuations are processed
         dependant on things like the name of the compilation unit. However,
         recursive continuations are specified using a [Continuation.Map.t],
         which already depends on the name of the compilation unit, so we leave
         this as-is for now. *)
      (* CR gbury: explain why we use used_handlers instead of processing
         everything from remaining_handlers. *)
      ...
      | None ->
        ...
        in
        let dacc = DA.with_continuation_uses_env dacc ~cont_uses_env in
        (* CR ncourant: we could possibly mark all remaning handlers as unused,
           however I'm not sure it is useful: they correspond to completely
           unreachable code, and any symbol defined within them wouldn't be in
           scope of the other code, so I think we can ignore them safely. *)
        let (stage3 : stage3) =
          { rebuild_body;
            cont_uses_env = cont_uses_env_so_far;
            invariant_params;
      ...
      | Some cont ->
        ...
        simplify_single_recursive_handler ~simplify_expr ~invariant_params
          ...
          dacc cont handler (fun dacc rebuild cont_uses_env_so_far ->
            ...
            in
            let used_handlers =
              Continuation.Set.union used_handlers
...
let simplify_let_cont_stage1 ~simplify_expr dacc (stage1 : stage1) ~down_to_up =
  ...
  in
  let stage2 =
    { denv_before_body; prior_lifted_constants; recinfo = stage1.recinfo }
  in
  simplify_expr dacc stage1.body
    ~down_to_up:(simplify_let_cont_stage2 ~simplify_expr stage2 ~down_to_up)
...
let simplify_let_cont ~simplify_expr dacc (let_cont : Let_cont.t) ~down_to_up =
  ...
  let stage1 =
    ...
    | Non_recursive { handler; _ } ->
      ...
      in
      { body;
        recinfo = Non_recursive { cont; params; handler; is_exn_handler }
      }
    | Recursive handlers ->
...
let simplify_as_recursive_let_cont ~simplify_expr dacc (body, handlers)
  ...
  let stage1 : stage1 =
    { body;
      recinfo =
        Recursive
          { invariant_params = Bound_parameters.empty (* FIXME *);
            continuation_handlers
          }
    }
  in
