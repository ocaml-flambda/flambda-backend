...
let as_extension_without_bindings ({ defined_vars; binding_times; equations; symbol_projections } as t) =
  if Flambda_features.check_invariants () then begin
    if Variable.Map.is_empty defined_vars
    && Binding_time.Map.is_empty binding_times
    && Variable.Map.is_empty symbol_projections
    then ()
    else Misc.fatal_errorf "Typing_env_level.as_extension_without_bindings:@ level %a has bindings"
        print t
  end;
