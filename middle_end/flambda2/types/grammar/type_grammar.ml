...
type t =
  ...
  | Naked_vec128 of Vector_types.Vec128.t * head_of_kind_naked_vec128 TD.t
...
let rec free_names0 ~follow_value_slots t =
  ...
  | Naked_vec128 (vty, ty) ->
    type_descr_free_names
      ~free_names_head:(free_names_head_of_kind_naked_vec128 vty)
      ty
  | Rec_info ty ->
...
and free_names_head_of_kind_value0 ~follow_value_slots head =
  ...
  | Boxed_vector (_, ty, _alloc_mode) -> free_names0 ~follow_value_slots ty
...
and free_names_head_of_kind_naked_vec128 _ _ = Name_occurrences.empty
    ...
    | Naked_vec128 (vty, ty) ->
      let ty' =
        TD.apply_renaming
          ...
          ty renaming
      in
      if ty == ty' then t else Naked_vec128 (vty, ty')
    | Rec_info ty ->
      let ty' =
        TD.apply_renaming
...
and apply_renaming_head_of_kind_value head renaming =
  ...
  | Boxed_vector (vty, ty, alloc_mode) ->
    let ty' = apply_renaming ty renaming in
    if ty == ty' then head else Boxed_vector (vty, ty', alloc_mode)
...
let rec print ppf t =
  ...
  | Naked_nativeint ty ->
    Format.fprintf ppf "@[<hov 1>(Naked_nativeint@ %a)@]"
      ...
      ty
  | Naked_vec128 (vty, ty) ->
    Format.fprintf ppf "@[<hov 1>(Naked_vector[%s]@ %a)@]"
      (Vector_types.Vec128.name vty)
      (TD.print ~print_head:(print_head_of_kind_naked_vec128 vty))
      ty
  | Rec_info ty ->
...
and print_head_of_kind_value ppf head =
  ...
  | Boxed_nativeint (ty, alloc_mode) ->
    Format.fprintf ppf "@[<hov 1>(Boxed_nativeint@ %a@ %a)@]"
      Alloc_mode.For_types.print alloc_mode print ty
  | Boxed_vector (vty, ty, alloc_mode) ->
    Format.fprintf ppf "@[<hov 1>(Boxed_vector[%s]@ %a@ %a)@]"
      (Vector_types.name vty) Alloc_mode.For_types.print alloc_mode print ty
  | Closures { by_function_slot; alloc_mode } ->
...
and print_head_of_kind_naked_vec128 ty ppf head =
  Format.fprintf ppf "@[(Naked_vec128[%s]@ (%a))@]"
    (Vector_types.Vec128.name ty)
    Vec128.Set.print head
...
let rec ids_for_export t =
  ...
  | Naked_vec128 (vty, ty) ->
    TD.ids_for_export
      ~ids_for_export_head:(ids_for_export_head_of_kind_naked_vec128 vty)
      ty
  | Rec_info ty ->
...
and ids_for_export_head_of_kind_value head =
  ...
  | Boxed_vector (_, t, _alloc_mode) -> ids_for_export t
  | Closures { by_function_slot; alloc_mode = _ } ->
...
and ids_for_export_head_of_kind_naked_vec128 _ _ = Ids_for_export.empty
...
let rec apply_coercion t coercion : t Or_bottom.t =
  ...
  then Ok t
    ...
    | Naked_vec128 (vty, ty) ->
      let<+ ty' =
        TD.apply_coercion
          ~apply_coercion_head:(apply_coercion_head_of_kind_naked_vec128 vty)
          coercion ty
      in
      if ty == ty' then t else Naked_vec128 (vty, ty')
    | Rec_info ty ->
...
and apply_coercion_head_of_kind_value head coercion : _ Or_bottom.t =
  ...
  | Boxed_float _ ->
    (* Even if we had coercions that would act on float constants, we would want
       ...
       value coercion. *)
    if Coercion.is_id coercion then Ok head else Bottom
  | Boxed_int32 _ | Boxed_int64 _ | Boxed_nativeint _ | Boxed_vector _
  | String _ ->
...
and apply_coercion_head_of_kind_naked_vec128 _ head coercion : _ Or_bottom.t =
  if Coercion.is_id coercion then Ok head else Bottom
...
and remove_unused_value_slots_and_shortcut_aliases_head_of_kind_naked_vec128 _
    head ~used_value_slots:_ ~canonicalise:_ =
  head
  ...
  | Naked_vec128 (vty, ty) ->
    ...
    let ty' =
      TD.project_variables_out
        ...
        ~to_project ~expand:expand_with_coercion
        ~project_head:
          (project_head_of_kind_naked_vector vty ~to_project ~expand)
        ty
    in
    if ty == ty' then t else Naked_vec128 (vty, ty')
  | Rec_info ty ->
...
and project_head_of_kind_naked_vector _ ~to_project:_ ~expand:_ head = head
...
let kind t =
  ...
  | Naked_vec128 (ty, _) -> K.naked_vector (Vec128 ty)
...
let get_alias_exn t =
  ...
  | Naked_vec128 (_, ty) -> TD.get_alias_exn ty
...
let is_obviously_bottom t =
  ...
  | Naked_vec128 (_, ty) -> TD.is_obviously_bottom ty
...
let is_obviously_unknown t =
  ...
  | Naked_vec128 (_, ty) -> TD.is_obviously_unknown ty
...
let alias_type_of (kind : K.t) name : t =
  ...
  | Naked_number (Naked_vector (Vec128 ty)) ->
    Naked_vec128 (ty, TD.create_equals name)
...
let bottom_naked_vector : Vector_types.t -> flambda_type = function
  | Vec128 ty -> Naked_vec128 (ty, TD.bottom)
...
let any_naked_vector : Vector_types.t -> flambda_type = function
  | Vec128 ty -> Naked_vec128 (ty, TD.unknown)
...
let this_naked_vec128 ty i : t =
  Naked_vec128 (ty, TD.create_equals (Simple.const (RWC.naked_vec128 ty i)))
...
let these_naked_vec128s ty vs =
  match Vector_types.Vec128.Bit_pattern.Set.get_singleton vs with
  | Some v -> this_naked_vec128 ty v
  | _ ->
    if Vector_types.Vec128.Bit_pattern.Set.is_empty vs
    then bottom_naked_vector (Vec128 ty)
    else Naked_vec128 (ty, TD.create vs)
...
let box_vec128 ty (t : t) alloc_mode : t =
  match t with
  | Naked_vec128 (vty, _) ->
    Value (TD.create (Boxed_vector (Vec128 vty, t, alloc_mode)))
  | Value _ | Naked_immediate _ | Naked_float _ | Naked_int32 _ | Naked_int64 _
  | Naked_nativeint _ | Rec_info _ | Region _ ->
...
let box_vector (ty : Vector_types.t) (t : t) alloc_mode : t =
  match ty with Vec128 vty -> box_vec128 vty t alloc_mode
...
let boxed_vec128_alias_to ty ~naked_vec128 =
  box_vec128 ty (Naked_vec128 (ty, TD.create_equals (Simple.var naked_vec128)))
...
let descr t : Descr.t =
  ...
  | Naked_vec128 (vty, ty) -> Naked_vec128 (vty, TD.descr ty)
...
let create_from_head_naked_vec128 ty head = Naked_vec128 (ty, TD.create head)
...
module Head_of_kind_value = struct
  ...
  let create_boxed_vector vty ty alloc_mode = Boxed_vector (vty, ty, alloc_mode)
...
let rec recover_some_aliases t =
  ...
  | Value ty -> (
    ...
    | Ok
        (No_alias
          ...
          | Boxed_vector _ | Boxed_nativeint _ | String _ | Closures _ | Array _
            )) ->
      t
  ...
  | Naked_vec128 (vty, ty) -> (
    ...
    | Ok (No_alias is) -> (
      ...
      | Some f -> this_naked_vec128 vty f
      | None -> t))
