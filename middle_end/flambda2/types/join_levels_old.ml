...
let meet_type = TE.New Meet_new_and_join_old.meet_type

let join = Meet_new_and_join_old.join
    ...
    ~f:(fun joined_types (env_at_use, _, _, t) ->
      ...
      let join_types name joined_ty use_ty =
        ...
        then None
          ...
          in
          match join ~bound_name:name join_env joined_ty use_ty with
...
let cut_and_n_way_join definition_typing_env ts_and_use_ids ~params ~cut_after
  ...
  in
  let result_env =
    TE.add_env_extension_from_level definition_typing_env level ~meet_type
  in
