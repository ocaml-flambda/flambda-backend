...
struct
  ...
  type case =
    { maps_to : Maps_to.t;
          (** kinds different than value are only allowed in cases with known
              tags. Currently tag 254 must have fields of kind float and all
              other must have fields of kind value. *)
      index : index;
      env_extension : TEE.t
    }
  ...
  let meet (meet_env : Meet_env.t) t1 t2 :
    ...
    let need_join =
      (* The returned env_extension is the join of the env_extension produced by
         each non bottom cases. Therefore there is some loss of precision in
         that case and we need to store the one produced for each tag. But when
         only one tag is kept it would be wasteful (but correct) to store it.

         We consider that the result of the meet between t1 and t2 will have
         only one tag when t1 (or t2) has exactly one tag (one that and no
         'other' cases).
      ...
      match
        ( other1,
          Tag.Map.get_singleton known1,
          other2,
          Tag.Map.get_singleton known2 )
      with
