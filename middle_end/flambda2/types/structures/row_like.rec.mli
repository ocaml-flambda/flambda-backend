...
(* It would be nicer to have the following elsewhere other than with the generic
   definition of the row-like structure, but unfortunately restrictions on the
   evaluation of recursive modules prevent this, as the functor applications
   produce "undefined recursive module" errors. *)

(* CR mshinwell: tidy up! *)
(* CR gbury: Consider using Tag.Scannable.t instead of Tag.t in For_blocks, and
   use another module to specifically handle record of floats *)
