...
let rec declare_const acc (const : Lambda.structured_constant) :
  ...
  | Const_base (Const_unboxed_int32 c) -> acc, Unboxed_number (Unboxed_int32 c), "unboxed_int32"
  | Const_base (Const_unboxed_int64 c) -> acc, Unboxed_number (Unboxed_int64 c), "unboxed_int64"
  ...
  | Const_mixed_block (tag, shape, consts) ->
      ...
      in
      let const : SC.t =
        SC.mixed_block (Tag.Scannable.create_exn tag) Immutable shape field_of_blocks
      in
      register_const acc const "const_mixed-block"
