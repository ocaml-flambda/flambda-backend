...
let close_functions acc external_env ~current_region function_declarations =
  ...
  in
  let acc = Acc.with_free_names Name_occurrences.empty acc in
  let funs = function_code_ids_in_order |> List.rev |> Function_slot.Lmap.of_list |> Function_slot.Lmap.map (fun code_id -> ({ code_id ; is_required_at_runtime = true } : Function_declarations.code_id_in_function_declaration)) in
