...
let close_one_function acc ~code_id ~external_env ~by_function_slot
  ...
  let acc, body, return_continuation, my_closure =
    ...
    | Unboxed_float_record (_, unboxed_function_slot) ->
      ...
      let handler acc =
        ...
        let (acc, expr), _ =
          List.fold_left
            (fun ((acc, expr), i) field_var ->
              ( Let_with_acc.create acc
                  (Bound_pattern.singleton
                     (Bound_var.create field_var Name_mode.normal))
  ...
  let result_arity_main_code, main_code_id =
    ...
    | Unboxed_float_record (size, _) ->
      ( Flambda_arity.With_subkinds.create
          (List.init size (fun _ -> K.With_subkind.naked_float)),
        Code_id.rename code_id )
  in
  let main_code =
  ...
  let code, by_function_slot, function_code_ids, acc =
    ...
    | Unboxed_float_record (_, unboxed_function_slot) ->
      (* The outside caller gave us the function slot and code ID meant for the
         ...
         starting with 'main_' refers to the version with unboxed return. *)
              ...
              Named.create_prim
                (Flambda_primitive.Variadic
                   ( Make_block
                       ( Values
                           ( Tag.Scannable.zero,
                             Flambda_arity.With_subkinds.to_list
        ...
        let handler, free_names_of_handler =
          ...
          let handler, free_names_of_handler =
            ...
            ( Expr.create_let
                (Let_expr.create
                   (Bound_pattern.singleton
                      (Bound_var.create block Name_mode.normal))
                   box_result_named
        ...
        let _, _, acc, approxs_and_code_ids =
          Acc.measure_cost_metrics acc ~f:(fun acc ->
              close_one_function acc ~code_id ~external_env ~by_function_slot
                ~function_code_ids function_decl
...
let wrap_partial_application acc env apply_continuation (apply : IR.apply)
  ...
  let function_declarations =
    [ Function_decl.create ~let_rec_ident:(Some wrapper_id) ~function_slot
        ...
        ~params ~return:(Normal_return apply.return_arity) ~return_continuation
