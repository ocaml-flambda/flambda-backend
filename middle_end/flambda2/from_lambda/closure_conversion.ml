...
let close_one_function acc ~code_id ~external_env ~by_function_slot decl
  ...
  let param_modes =
    List.map
      (fun (_, _, mode) -> Alloc_mode.For_types.from_lambda mode)
      raw_params
  in
    ...
    List.fold_left
      (fun approx_map decl ->
        (* The only fields of metadata which are used for this pass are
           params_arity, param_modes, is_tupled, num_trailing_local_closures,
        ...
        let params_arity =
          List.map
            (fun (_, kind, _) -> Flambda_kind.With_subkind.from_lambda kind)
            params
...
let wrap_partial_application acc env apply_continuation (apply : IR.apply)
  ...
  let function_declarations =
    ...
    [ Function_decl.create ~let_rec_ident:(Some wrapper_id) ~function_slot
        ...
        ~free_idents_of_body ~closure_alloc_mode ~num_trailing_local_closures
