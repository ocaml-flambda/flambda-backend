...
let layout_aux j slot (startenv, acc_slots) =
  ...
  | Env_var _ ->
    ...
    startenv, acc_slots
  | Infix_header | Dummy_closure_info ->
...
let layout env closures env_vars =
  ...
  let startenv =
    ...
    | None, (_, Env_var _) :: _ ->
      ...
      assert false
    | None, (_, Dummy_closure_info) :: _ ->
      (* Cannot happend because layout_aux does not generate
         `Dummy_closure_info`; and even if it did, this case means we have a
         truly empty set of closures: no closures *and* no env vars. *)
      assert false
  in
  ...
  (* The Gc assumes that a set of closures block actually starts with a closure
     ...
     second field (i.e. field at offset 1) of a set of closures to find out the
     start of environment. Thus if needed we insert a dummy closure info with
     the correct start of env. *)
  match slots with
  | (n, Closure _) :: _ ->
    if n = 0
    then { startenv; slots }
      ...
      Misc.fatal_error
        "Closure_offsets: closure slot not at offset 0 and not preceded by a \
         infix header."
  | (n, Infix_header) :: _ ->
    if n >= 2
    then
      let slots = (1, Dummy_closure_info) :: slots in
      { startenv; slots }
    else
      Misc.fatal_error
        "Closure_offsets: no closure slot at offset 0 and no space to insert a \
         dummy closure info"
  | _ -> Misc.fatal_error "Sets of closures must start with a closure slot"
...
let print_layout_slot fmt = function
  ...
  | Dummy_closure_info -> Format.fprintf fmt "dummy_closure_info"
