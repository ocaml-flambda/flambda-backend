...
module Make_list (Bindable : Bindable.S) (Term : Term) = struct
  ...
  let create names term =
    ...
    if List.length names <> Bindable.Set.cardinal names_set
    then
      Misc.fatal_errorf
        "Cannot create generalised abstraction value with non-disjoint names \
         in binding position: %a"
        (Format.pp_print_list ~pp_sep:Format.pp_print_space Bindable.print)
        names;
    names, term
  ...
  let[@inline always] pattern_match (names, term) ~f =
    ...
    | _ ->
      let fresh_names_rev, perm =
        List.fold_left
          (fun (fresh_names_rev, perm) stale_name ->
  ...
  let[@inline always] pattern_match_pair ((names0, term0) as t1)
      ((names1, term1) as t2) ~f =
    (if List.compare_lengths names0 names1 <> 0
    then
      let print ppf t : unit = print ppf t in
      Misc.fatal_errorf
        "Cannot concrete a pair of generalised abstractions unless they have \
         the same number of names in binding position:@ %a@ and@ %a"
        print t1 print t2);
...
module Make_map (Bindable : Bindable.S) (Term : Term) = struct
  ...
  let[@inline always] pattern_match t ~f =
    match t with
    | E (names, term) ->
      if Bindable.Map.is_empty names
      then f term
      else
        let perm =
          Bindable.Map.fold
            (fun stale_name _ perm ->
              let fresh_name = Bindable.rename stale_name in
              Bindable.add_to_name_permutation stale_name
                ~guaranteed_fresh:fresh_name perm)
            names Renaming.empty
        in
    ...
    | E (names, term) ->
      ...
      then f [] term
        ...
        let fresh_names, perm =
          Bindable.Map.fold
            (fun stale_name _ (fresh_names, perm) ->
              let fresh_name = Bindable.rename stale_name in
        ...
        in
        f (Bindable.Set.elements fresh_names) (Term.apply_renaming term perm)
  ...
  let apply_renaming t perm =
    ...
    | E (names, term) ->
      let names =
        Bindable.Map.fold
          (fun name datum names ->
            let name = Bindable.apply_renaming name perm in
            Bindable.Map.add name datum names)
          names Bindable.Map.empty
      in
      let term = Term.apply_renaming term perm in
      E (names, term)
  ...
  let free_names t =
    ...
    | E (names, term) ->
      let in_binding_position =
        Bindable.Map.fold
          (fun name _ in_binding_position ->
            Bindable.add_occurrence_in_terms name in_binding_position)
          names Name_occurrences.empty
      in
      let free_in_term = Term.free_names term in
      Name_occurrences.diff free_in_term in_binding_position
end
