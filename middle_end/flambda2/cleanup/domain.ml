...
let get_or_create_node (g : graph) (id : Code_id_or_name.t) : node =
  ...
  | None ->
    let node =
      { id; is_top = false; is_bot = true; edges = Hashtbl.create 0 }
    in
...
let propagate (uses : graph) (elt : elt) (dep : dep) : updated =
  ...
  let add_alias src src_node target acc =
    ...
    then acc
      ...
      let acc =
        if Hashtbl.mem target.edges (Alias, src)
        then acc
        else begin
  ...
  let find_fields_from_aliases field node =
    ...
    while !work_stack <> [] do
      ...
      in
      Hashtbl.iter
        (fun (edge, id) target_node ->
          match edge with
          | Field f ->
            if Global_flow_graph.Field.equal f field
            then targets := target_node :: !targets
          | Alias ->
            if not (Code_id_or_name.Set.mem id !aliases)
            then begin
  ...
  then Not_updated
    ...
    | Field (f, n) ->
      let target = get_or_create_node uses (Code_id_or_name.name n) in
      let acc =
        if Hashtbl.mem target.edges (Field f, elt)
        then Not_updated
        else begin
    ...
    | Alias_if_def (n, c) ->
      ...
      let target = get_or_create_node uses (Code_id_or_name.name n) in
      if not control.is_bot
      then add_alias elt node target Not_updated
      else Not_updated
    | Propagate (n1, n2) ->
      let t1 = get_or_create_node uses (Code_id_or_name.name n1) in
      let t2 = get_or_create_node uses (Code_id_or_name.name n2) in
      if not t2.is_bot
      then add_alias (Code_id_or_name.name n2) t2 t1 Not_updated
      else Not_updated
  end
