...
module Make (Rec_info_expr : Rec_info_expr0.S) :
  S
    with type variable = Rec_info_expr.variable
     and type rec_info_expr = Rec_info_expr.t = struct
  ...
  let change_depth ~from ~to_ =
    (* The special case here is actually very common; it appears, for instance,
       when something gets composed when its own inverse, which happens often in
       the back-and-forth in [Typing_env.add_equation] for instance *)
    if from == to_ then Id else Change_depth { from; to_ }

  let is_id = function Id -> true | Change_depth _ -> false
  ...
  let compose t1 ~then_:t2 =
    ...
    | ( Change_depth { from = from1; to_ = to_1 },
      ...
      (* CR lmaurer: We would like to check that [to_1] equals [from_2], but we
         ...
         function purely syntactic (and thus total) before; that would solve the
  ...
  let equal t1 t2 =
    ...
    | ( Change_depth { from = from1; to_ = to_1 },
        Change_depth { from = from2; to_ = to_2 } ) ->
      Rec_info_expr.equal from1 from2 && Rec_info_expr.equal to_1 to_2
    | (Id | Change_depth _), _ -> false

  let hash = function
    | Id -> Hashtbl.hash 0
    | Change_depth { from; to_ } ->
      Hashtbl.hash (1, Rec_info_expr.hash from, Rec_info_expr.hash to_)
  ...
  let map_depth_variables t ~f =
    ...
    | Change_depth { from = old_from; to_ = old_to } ->
      ...
      then t
      else change_depth ~from:new_from ~to_:new_to
end
