...
and apply_expr env res apply =
  ...
  (* With respect to flushing the environment we have three cases:
     ...
     thing that jumps to that point.

     In cases 1 and 2 we can't affect the code generation at the jump target
     (either because it wasn't generated by us, or is in another function) so
