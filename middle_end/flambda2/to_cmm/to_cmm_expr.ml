...
let translate_apply0 env res apply =
  ...
  let callee, callee_free_vars, env, res, _ =
  ...
  | Call_kind.Method { kind; obj; alloc_mode } ->
    fail_if_probe apply;
    let obj, obj_free_vars, env, res, _ = C.simple ~dbg env res obj in
...
let translate_apply env res apply =
  ...
  then
    ...
    let aux (call, env, res) (arg, _k) v =
      let arg, _arg_free_vars, env, res, _ = C.simple ~dbg env res arg in
...
let translate_raise env res apply exn_handler args =
  ...
  | exn :: extra ->
    ...
    let exn, exn_free_vars, env, res, _ = C.simple ~dbg env res exn in
...
let translate_jump_to_return_continuation env res apply return_cont args =
  ...
  | [return_value] -> (
    let dbg = Apply_cont.debuginfo apply in
    let return_value, free_vars, env, res, _ =
...
and let_expr0 env res let_expr (bound_pattern : Bound_pattern.t)
  ...
  | Static bound_static, Static_consts consts -> (
    let env, res, update_opt, update_free_vars =
    ...
    | None ->
      if not (Backend_var.Set.is_empty update_free_vars)
      then Misc.fatal_errorf "Non_empty free_vars for an empty update";
      expr env res body
    | Some update ->
      let wrap, env, res =
      ...
      let free_vars = Backend_var.Set.union update_free_vars body_free_vars in
      let cmm, free_vars = wrap (C.sequence update body) free_vars in
      cmm, free_vars, res)
...
and switch env res switch =
  ...
  let untagged_scrutinee_cmm, scrutinee_free_vars, env, res, _ =
