...
let eq ?(dbg = Debuginfo.none) x y =
  ...
  | Cmm.Cop (Csubi, [Cconst_int (m, _); c], _), Cmm.Cconst_int (n, _)
    ...
    (* [n = m - c] <=> [c = m - n]
       ...
       There is some subtlety here involving sign extension and corner cases in
       2's-complement arithmetic, but z3 says that the side conditions [n >= 0]
       and [m >= 0] are sufficient: *)
    (*
