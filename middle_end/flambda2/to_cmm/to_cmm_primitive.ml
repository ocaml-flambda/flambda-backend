...
let nullary_primitive _env res dbg prim =
  ...
  | Probe_is_enabled { name } ->
    ...
    None, res, expr
  | Begin_region { definitely_unused = true } -> None, res, C.unit ~dbg
  | Begin_region { definitely_unused = false } -> None, res, C.beginregion ~dbg
...
let unary_primitive env res dbg f arg =
  ...
  | Begin_try_region { definitely_unused = true } -> None, res, C.unit ~dbg
  | Begin_try_region { definitely_unused = false } ->
    None, res, C.beginregion ~dbg
  | End_region { definitely_unused = true } -> None, res, C.unit ~dbg
