...
let rec value_kind0 ppf kind =
  ...
  | Pboxedvectorval (Pvec128 ty) ->
    Format.pp_print_string ppf (":" ^ (Primitive.vec128_name ty))
...
let layout (layout : Lambda.layout) =
  ...
  | Punboxed_vector (Pvec128 ty) ->
    ":unboxed_" ^ (Primitive.vec128_name ty)
...
let rec structured_constant ppf = function
  ...
  | Uconst_vec128 {ty; high; low} -> fprintf ppf "[%s]%016Lx:%016Lx" (Primitive.vec128_name ty) high low
  | Uconst_block (tag, l) ->
