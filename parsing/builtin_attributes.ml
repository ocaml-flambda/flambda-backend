...
let int_of_cst = function
  | Pconst_integer {naked = Value; value=i; suffix=""} -> Some (int_of_string i)
  | _ -> None
...
let get_int_from_exp =
  ...
  function
  | { pexp_desc =
        Pexp_constant (Pconst_integer { naked=Value; value=s; suffix="" })
    } ->
...
let get_id_or_constant_from_exp =
  ...
  | { pexp_desc = Pexp_constant (Pconst_integer {naked=Value; value=s; suffix=""}) } -> Result.Ok (Const_int, s)
