...
let type_int = newgenty (Tconstr(path_int, AppArgs.unapp, ref Mnil))
and type_char = newgenty (Tconstr(path_char, AppArgs.unapp, ref Mnil))
and type_bytes = newgenty (Tconstr(path_bytes, AppArgs.unapp, ref Mnil))
and type_float = newgenty (Tconstr(path_float, AppArgs.unapp, ref Mnil))
and type_float32 = newgenty (Tconstr(path_float32, AppArgs.unapp, ref Mnil))
and type_bool = newgenty (Tconstr(path_bool, AppArgs.unapp, ref Mnil))
and type_unit = newgenty (Tconstr(path_unit, AppArgs.unapp, ref Mnil))
and type_exn = newgenty (Tconstr(path_exn, AppArgs.unapp, ref Mnil))
...
and type_nativeint = newgenty (Tconstr(path_nativeint, AppArgs.unapp, ref Mnil))
and type_int32 = newgenty (Tconstr(path_int32, AppArgs.unapp, ref Mnil))
and type_int64 = newgenty (Tconstr(path_int64, AppArgs.unapp, ref Mnil))
and type_lazy_t t = newgenty (Tconstr(path_lazy_t, AppArgs.one t, ref Mnil))
and type_string = newgenty (Tconstr(path_string, AppArgs.unapp, ref Mnil))
and type_extension_constructor =
      newgenty (Tconstr(path_extension_constructor, AppArgs.unapp, ref Mnil))
and type_floatarray = newgenty (Tconstr(path_floatarray, AppArgs.unapp, ref Mnil))
and type_lexing_position = newgenty (Tconstr(path_lexing_position, AppArgs.unapp, ref Mnil))

and type_unboxed_float = newgenty (Tconstr(path_unboxed_float, AppArgs.unapp, ref Mnil))
and type_unboxed_float32 = newgenty (Tconstr(path_unboxed_float32, AppArgs.unapp, ref Mnil))
and type_unboxed_nativeint =
      newgenty (Tconstr(path_unboxed_nativeint, AppArgs.unapp, ref Mnil))
and type_unboxed_int32 = newgenty (Tconstr(path_unboxed_int32, AppArgs.unapp, ref Mnil))
and type_unboxed_int64 = newgenty (Tconstr(path_unboxed_int64, AppArgs.unapp, ref Mnil))
and type_or_null t = newgenty (Tconstr(path_or_null, AppArgs.one t, ref Mnil))

and type_int8x16 = newgenty (Tconstr(path_int8x16, AppArgs.unapp, ref Mnil))
and type_int16x8 = newgenty (Tconstr(path_int16x8, AppArgs.unapp, ref Mnil))
and type_int32x4 = newgenty (Tconstr(path_int32x4, AppArgs.unapp, ref Mnil))
and type_int64x2 = newgenty (Tconstr(path_int64x2, AppArgs.unapp, ref Mnil))
and type_float32x4 = newgenty (Tconstr(path_float32x4, AppArgs.unapp, ref Mnil))
and type_float64x2 = newgenty (Tconstr(path_float64x2, AppArgs.unapp, ref Mnil))
...
let mk_add_type1 add_type type_ident
      ...
      *)
      ?jkind_annotation
      ?(param_jkind=Jkind.Type.Primitive.value ~why:(
        Type_argument {
  ...
  let param = newgenvar (Jkind.of_type_jkind param_jkind) in
  let decl =
    {type_params = [param];
      type_arity = 1;
...
let mk_add_extension add_extension id args jkinds =
  Array.iter (fun jkind ->
      ...
      | Const const ->
          begin
            match Jkind.Type.Const.get_layout const with
            | Sort Value -> ()
...
let build_initial_env add_type add_extension empty_env =
  ...
  |> add_type ident_lexing_position
       ~kind:(
         ...
         in
         let immediate = Jkind.Type.Primitive.value ~why:(Primitive ident_int) in
         let labels = List.map lbl [
           ("pos_fname", type_string, Jkind.Type.Primitive.value ~why:(Primitive ident_string));
       ...
       ~jkind:(Jkind.Type.Primitive.value ~why:Boxed_record)
  ...
  |> add_extension ident_assert_failure
       ...
       [| Jkind.Type.Primitive.value ~why:Tuple |]
  |> add_extension ident_division_by_zero [] [||]
  |> add_extension ident_end_of_file [] [||]
  |> add_extension ident_failure [type_string]
       [| Jkind.Type.Primitive.value ~why:(Primitive ident_string) |]
