        ...
        let decl_row =
          ...
          { type_params =
              (* jkind any is fine on the params because they get thrown away
                 below *)
              List.map
                (fun _ -> Btype.newgenvar (Jkind.Type.Primitive.any ~why:Dummy_jkind |> Jkind.of_type_jkind))
                sdecl.ptype_params;
            type_arity = arity;
  ...
  List.iter
    (fun (n, ty) ->
      try Ctype.unify env ty
            (Ctype.newvar (Jkind.Type.Primitive.any ~why:Dummy_jkind |> Jkind.of_type_jkind))
      with Ctype.Unify _ ->
