    ...
    | Sig_type(id, decl, rs, priv), [s],
      ...
      when Ident.name id = s && Typedecl.is_fixed_type sdecl ->
        let decl_row =
          ...
          { type_params =
              (* jkind top is fine on the params because they get thrown away below *)
              List.map
             ...
             | path -> begin
                 ...
                 | decl ->
                     if decl.type_arity > 0 then begin
                       ...
                       let t = Btype.newgenty (Tconstr (path,AppArgs.unapp,ref Mnil)) in
                       (lid, t) :: fl
                     end
               end)
          fl []
