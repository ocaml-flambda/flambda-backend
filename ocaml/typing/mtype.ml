      ...
      let newdecl =
        ...
        | _ ->
            ...
            if Btype.type_kind_is_abstract decl then
              { decl with type_private = Type_public; type_manifest = manif }
            else
        ...
        if decl.type_arity <> orig_decl.type_arity then
          ...
          let orig_ty =
            Ctype.reify_univars env
              (Btype.newgenty(Tconstr(p, AppArgs.of_list orig_decl.type_params, ref Mnil)))
          in
          let new_ty =
            Ctype.reify_univars env
              (Btype.newgenty(Tconstr(Pident id, AppArgs.of_list decl.type_params, ref Mnil)))
          in
          ...
          | () ->
              let orig_ty =
                Btype.newgenty(Tconstr(p, AppArgs.of_list decl.type_params, ref Mnil))
              in
...
and contains_type_item env = function
    Sig_type (_,({type_manifest = None} |
                 {type_kind = Type_abstract _; type_private = Type_private}),_, _)
  | Sig_modtype _
