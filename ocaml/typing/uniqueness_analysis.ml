...
module Paths : sig
  ...
  val modal_child : Modality.Vector.t -> Projection.t -> t -> t
  ...
  val record_field : Modality.Vector.t -> string -> t -> t

  (** [construct_field gf s i t] is
      [modal_child gf (Projection.Construct_field(s, i)) t]. *)
  val construct_field : Modality.Vector.t -> string -> int -> t -> t
  ...
  let modal_child gf proj t =
    (* CR zqian: Should try to detect [Shared] and [Many] instead. They could be
       generated when interpreting user-written [global]. Moreover, instead of
       just ignoring such children, we should add modality to [Projection.t] and
       add corresponding logic in [UsageTree]. *)
    let gf = Modality.Vector.to_list gf in
    if List.exists
         (function Exist Global -> true
           : Modality.not_id Modality.Exist.t -> _)
         gf
    then untracked
    else child proj t
...
module Value : sig
  ...
  val implicit_record_field :
    Modality.Vector.t -> string -> t -> unique_use -> t
