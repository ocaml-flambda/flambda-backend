...
module Paths : sig
  ...
  val modal_child : Modality.Value.user -> Projection.t -> t -> t
  ...
  val record_field : Modality.Value.user -> string -> t -> t

  (** [construct_field gf s i t] is
      [modal_child gf (Projection.Construct_field(s, i)) t]. *)
  val construct_field : Modality.Value.user -> string -> int -> t -> t
...
module Value : sig
  ...
  val implicit_record_field :
    Modality.Value.user -> string -> t -> unique_use -> t
