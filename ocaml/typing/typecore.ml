...
type expected_mode =
  { position : position_in_region;
    ...
    (** No invariant between this and [mode]. It is UNSOUND to ignore this field.
        If this is [Some [m0; m1; ..]]:

        - If we are type checking [Pexp_tuple [e0; e1; ..]], then [e0] will
        be checked at [meet [mode; m0]], [e1] at [meet [mode; m1]], and so on.

        - If we are type checking "non-concrete" [Pexp_constraint] etc, this
        field must be preserved.

        - If we are type checking other expressions, then it will be checked
        at [meet [mode; l]] *)
  }
      ...
      let exp_desc =
        ...
        | Val_self (_, _, _, cl_num) ->
            ...
            Texp_ident(path, lid, desc, kind,
              unique_use ~loc ~env actual_mode.mode (get_single_mode_exn expected_mode))
        | _ ->
            Texp_ident(path, lid, desc, kind,
              unique_use ~loc ~env actual_mode.mode (as_single_mode expected_mode))
      in
      let exp = rue {
  ...
  | Pexp_field(srecord, lid) ->
      ...
      let boxing : texp_field_boxing =
        ...
        | true ->
          ...
          submode ~loc ~env mode argument_mode;
          let uu = unique_use ~loc ~env mode (get_single_mode_exn argument_mode) in
...
and type_tuple ~loc ~env ~(expected_mode : expected_mode) ~ty_expected
  ...
  let argument_modes =
    ...
    | Some tuple_modes ->
        if List.compare_length_with tuple_modes arity = 0 then
          List.map (fun mode -> Value.meet [mode; argument_mode])
            tuple_modes
        else Misc.fatal_error "tuple_modes length mismatch"
    | None ->
