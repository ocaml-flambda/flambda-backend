    ...
    with_local_level_if_principal begin fun () ->
      ...
      if is_optional l then
        unify_pat (ref val_env) pat
          (type_option (newvar (Type Predef.option_argument_jkind)));
...
let rec approx_type env sty =
  ...
  | Ptyp_arrow (p, ({ ptyp_desc = Ptyp_poly _ } as arg_sty), sty) ->
      ...
      if is_optional p then newvar (Type Predef.option_argument_jkind)
      else begin
  ...
  | Ptyp_arrow (p, arg_sty, sty) ->
      ...
      let arg =
        if is_optional p
        then type_option (newvar (Type Predef.option_argument_jkind))
...
and type_comprehension_expr
  ...
  let element_ty =
    with_local_level_if_principal begin fun () ->
      let element_ty = newvar (Type jkind) in
      unify_exp_types
        loc
        env
