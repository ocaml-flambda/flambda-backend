      ...
      | ({pv_id = x1; pv_type = t1; pv_mode = m1; _} as pv1)::rem1,
        ...
        when Ident.equal x1 x2 ->
          if x1==x2 then
            ...
            begin try
              unify_var env (new_type_var (Jkind.Type.Primitive.any ~why:Dummy_jkind)) t1;
              unify env t1 t2
            with
            | Unify err ->
...
and build_as_type_aux ~refine ~mode (env : Env.t ref) p =
  ...
  | Tpat_record (lpl,_) ->
      ...
      let ty = new_type_var (Jkind.Type.Primitive.any ~why:Dummy_jkind) in
  ...
  let ann =
    ...
    List.map2
      (fun (label, p) mode ->
        ( label,
          p,
          newgenvar (Jkind.Type.Primitive.value_or_null ~why:Tuple_element |> Jkind.of_type_jkind),
...
let solve_Ppat_variant ~refine loc env tag no_arg expected_ty =
  ...
  let arg_type =
    if no_arg
    then []
    else [newgenvar (Jkind.Type.Primitive.value_or_null ~why:Polymorphic_variant_field |> Jkind.of_type_jkind)]
  in
...
(* CR layouts v2: RAE thinks this any is fine in perpetuity. Before changing
   this, let's talk. *)
let approx_type_default () = new_type_var (Jkind.Type.Primitive.any ~why:Dummy_jkind)
...
let rec approx_type env sty =
  ...
  | Ptyp_constr (lid, ctl) ->
      ...
      then new_type_var (Jkind.Type.Primitive.any ~why:Dummy_jkind)
      else begin
...
and type_tuple_approx (env: Env.t) loc ty_expected l =
  let labeled_tys = List.map
    (fun (label, _) -> label, new_type_var (Jkind.Type.Primitive.value_or_null ~why:Tuple_element)) l
  in
...
let split_function_ty
  ...
  if not has_poly && not (tpoly_is_mono ty_arg) && !Clflags.principal
    ...
    let really_poly =
      try
        unify env (newmono (new_type_var (Jkind.Type.Primitive.any ~why:Dummy_jkind))) ty_arg;
        false
      with Unify _ -> true
    in
    Btype.backtrack snap;
...
let rec type_exp ?recarg env expected_mode sexp =
  ...
  type_expect ?recarg env expected_mode sexp
    (mk_expected (new_type_var (Jkind.Type.Primitive.any ~why:Dummy_jkind)))
...
and type_expect_
      ...
      let (pat_exp_list, body, _new_env) =
        ...
        *)
          ...
          if rec_flag = Recursive && may_contain_modules then begin
            List.iter
              (fun vb ->
                ...
                let outer_var =
                  new_type_var2 outer_level (Jkind.Type.Primitive.any ~why:Dummy_jkind)
                in
        ...
        ~post:(fun (_pat_exp_list, body, new_env) ->
          ...
          unify_exp new_env body (new_type_var (Jkind.Type.Primitive.any ~why:Dummy_jkind)))
      in
      re {
  ...
  | Pexp_apply(sfunct, sargs) ->
      ...
      let rec ret_tvar seen ty_fun =
        ...
        if TypeSet.mem ty seen then false else
          ...
          | Tvar _ ->
              let v = new_type_var (Jkind.Type.Primitive.any ~why:Dummy_jkind) in
  ...
  | Pexp_variant(l, sarg) ->
      ...
      with Exit ->
        ...
        | Some sarg ->
            let ty_expected =
              new_type_var (Jkind.Type.Primitive.value_or_null ~why:Polymorphic_variant_field)
            in
            let alloc_mode, argument_mode = register_allocation expected_mode in
            let arg =
  ...
  | Pexp_pack m ->
      ...
      rue {
        ...
        exp_env = env }
  | Pexp_open (od, e) ->
      let tv = new_type_var (Jkind.Type.Primitive.any ~why:Dummy_jkind) in
  ...
  | Pexp_letop{ let_ = slet; ands = sands; body = sbody } ->
      ...
      let rec loop spat_acc ty_acc ty_acc_sort sands =
        ...
        | { pbop_pat = spat; _} :: rest ->
            ...
            let ty = new_type_var (Jkind.Type.Primitive.value_or_null ~why:Tuple_element) in
        ...
        with_local_level_iter_if_principal
          ...
          let spat_params, ty_params, param_sort =
            let initial_jkind, initial_sort = match sands with
              ...
              | _ -> Jkind.Type.Primitive.value_or_null ~why:Tuple_element |> Jkind.of_type_jkind, Jkind.Type.Sort.value
            in
          ...
          let { function_ = exp_type, params, body;
            ...
            *)
            type_function env expected_mode
              (new_type_var (Jkind.Type.Primitive.any ~why:Dummy_jkind))
      ...
      let arg =
        if vars = [] then begin
          ...
          if !Clflags.principal
            ...
            let really_poly =
              try
                unify env (newmono (new_type_var (Jkind.Type.Primitive.any ~why:Dummy_jkind)))
                  ty_arg;
                false
              with Unify _ -> true
            in
  ...
  let half_typed_cases, ty_res, do_copy_types, ty_arg' =
   (* propagation of the argument *)
    with_local_level begin fun () ->
      ...
      let ty_arg' = new_type_var (Jkind.Type.Primitive.any ~why:Dummy_jkind) in
      let unify_pats ty =
  ...
  with_local_level begin fun () ->
    ...
    let rec replace t =
      if Hashtbl.mem seen (get_id t) then ()
        ...
        | Tconstr (Path.Pident id', Unapplied, _) when id == id' -> link_type t ty
        | Tconstr (Path.Pident id', (Applied _ as args), _) when id == id' ->
