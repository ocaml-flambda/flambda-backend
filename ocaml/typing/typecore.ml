      ...
      | ({pv_id = x1; pv_type = t1; pv_mode = m1; _} as pv1)::rem1,
        ...
        when Ident.equal x1 x2 ->
          if x1==x2 then
            ...
            begin try
              unify_var env (newvar (Jkind.Type.Primitive.any ~why:Dummy_jkind)) t1;
              unify env t1 t2
            with
            | Unify err ->
...
and build_as_type_aux ~refine ~mode (env : Env.t ref) p =
  ...
  | Tpat_record (lpl,_) ->
      ...
      let ty = newvar (Jkind.Type.Primitive.any ~why:Dummy_jkind) in
...
let solve_Ppat_variant ~refine loc env tag no_arg expected_ty =
  ...
  (arg_type, make_row (newvar (Jkind.Type.Primitive.value ~why:Row_variable)),
...
let build_or_pat env loc lid =
  ...
  let tyl = List.mapi (fun i _ ->
    newvar (Jkind.Type.Primitive.value ~why:(Type_argument {parent_path = path; position = i+1; arity}))
  ) decl.type_params in
  let row0 =
  ...
  let row' = ref (make_row (newvar (Jkind.Type.Primitive.value ~why:Row_variable))) in
  let pats =
    List.map
      (fun (l,p) ->
...
and type_pat_aux
  ...
  | Ppat_record(lid_sp_list, closed) ->
      ...
      let expected_type, record_ty =
        ...
        | Maybe_a_record_type ->
          None, newvar (Jkind.Type.Primitive.value ~why:Boxed_record)
        | Not_a_record_type ->
...
let type_class_arg_pattern cl_num val_env met_env l spat =
  let pvs, pat =
    with_local_level_if_principal begin fun () ->
      ...
      let nv = newvar (Jkind.Type.Primitive.value ~why:Class_term_argument) in
      let alloc_mode = simple_pat_mode Value.legacy in
      let pat =
...
let type_self_pattern env spat =
  ...
  let tps = create_type_pat_state Modules_rejected in
  let nv = newvar (Jkind.Type.Primitive.value ~why:Object) in
  let alloc_mode = simple_pat_mode Value.legacy in
  let pat =
    ...
    | (lbl, sarg) :: rest ->
        let (sort_arg, mode_arg, ty_arg_mono, mode_ret, ty_res) =
          ...
          | Tvar _ ->
              ...
              let ty_res =
                newvar (Jkind.Type.of_new_sort ~why:Function_result)
              in
              if ret_tvar &&
...
(* CR layouts v2: RAE thinks this any is fine in perpetuity. Before changing
   this, let's talk. *)
let approx_type_default () = newvar (Jkind.Type.Primitive.any ~why:Dummy_jkind)
...
let rec approx_type env sty =
  ...
  | Ptyp_arrow (p, arg_sty, sty) ->
      ...
      let arg =
        ...
        else newvar (Jkind.Type.Primitive.any ~why:Inside_of_Tarrow)
      in
      let ret = approx_type env sty in
      let marg = Alloc.of_const arg_mode in
      let mret = Alloc.newvar () in
  ...
  | Ptyp_constr (lid, ctl) ->
      ...
      then newvar (Jkind.Type.Primitive.any ~why:Dummy_jkind)
      else begin
...
and type_tuple_approx (env: Env.t) loc ty_expected l =
  let labeled_tys = List.map
    (fun (label, _) -> label, newvar (Jkind.Type.Primitive.value ~why:Tuple_element)) l
  in
...
let split_function_ty
  ...
  if not has_poly && not (tpoly_is_mono ty_arg) && !Clflags.principal
    ...
    let really_poly =
      try
        unify env (newmono (newvar (Jkind.Type.Primitive.any ~why:Dummy_jkind))) ty_arg;
        false
      with Unify _ -> true
    in
    Btype.backtrack snap;
...
let rec type_exp ?recarg env expected_mode sexp =
  ...
  type_expect ?recarg env expected_mode sexp
    (mk_expected (newvar (Jkind.Type.Primitive.any ~why:Dummy_jkind)))
...
and type_expect_
      ...
      let (pat_exp_list, body, _new_env) =
        ...
        *)
          ...
          if rec_flag = Recursive && may_contain_modules then begin
            List.iter
              (fun vb ->
                ...
                let outer_var =
                  newvar2 outer_level (Jkind.Type.Primitive.any ~why:Dummy_jkind)
                in
        ...
        ~post:(fun (_pat_exp_list, body, new_env) ->
          ...
          unify_exp new_env body (newvar (Jkind.Type.Primitive.any ~why:Dummy_jkind)))
      in
      re {
  ...
  | Pexp_apply(sfunct, sargs) ->
      ...
      let rec ret_tvar seen ty_fun =
        ...
        if TypeSet.mem ty seen then false else
          ...
          | Tvar _ ->
              let v = newvar (Jkind.Type.Primitive.any ~why:Dummy_jkind) in
  ...
  | Pexp_variant(l, sarg) ->
      ...
      with Exit ->
        ...
        | Some sarg ->
            let ty_expected =
              newvar (Jkind.Type.Primitive.value ~why:Polymorphic_variant_field)
            in
            let alloc_mode, argument_mode = register_allocation expected_mode in
            let arg =
  ...
  | Pexp_record(lid_sexp_list, opt_sexp) ->
      ...
      let ty_record, expected_type =
        ...
        | None, None ->
          newvar (Jkind.Type.of_new_sort ~why:Record_projection), None
  ...
  | Pexp_setfield(srecord, lid, snewval) ->
      ...
      let ty_record =
        ...
        then newvar (Jkind.Type.of_new_sort ~why:Record_assignment)
  ...
  | Pexp_send (e, {txt=met}) ->
      ...
      let typ =
        ...
        | Tvar _ ->
            let ty' = newvar (Jkind.Type.Primitive.value ~why:Object_field) in
  ...
  | Pexp_pack m ->
      ...
      rue {
        ...
        exp_env = env }
  | Pexp_open (od, e) ->
      let tv = newvar (Jkind.Type.Primitive.any ~why:Dummy_jkind) in
  ...
  | Pexp_letop{ let_ = slet; ands = sands; body = sbody } ->
      ...
      let rec loop spat_acc ty_acc ty_acc_sort sands =
        ...
        | { pbop_pat = spat; _} :: rest ->
            ...
            let ty = newvar (Jkind.Type.Primitive.value ~why:Tuple_element) in
          ...
          let { function_ = exp_type, params, body;
            ...
            *)
            type_function env expected_mode
              (newvar (Jkind.Type.Primitive.any ~why:Dummy_jkind))
      ...
      let arg =
        if vars = [] then begin
          ...
          if !Clflags.principal
            ...
            let really_poly =
              try
                unify env (newmono (newvar (Jkind.Type.Primitive.any ~why:Dummy_jkind)))
                  ty_arg;
                false
              with Unify _ -> true
            in
  ...
  let half_typed_cases, ty_res, do_copy_types, ty_arg' =
   (* propagation of the argument *)
    with_local_level begin fun () ->
      ...
      let ty_arg' = newvar (Jkind.Type.Primitive.any ~why:Dummy_jkind) in
      let unify_pats ty =
        ...
        let filter_ty_ret_exn ty arg_label ~force_tpoly =
          ...
          | exception (Filter_arrow_failed error) ->
              let trace =
                ...
                | Not_a_function ->
                    let tarrow =
                      let new_ty_var why = newvar (Jkind.Type.of_new_sort ~why) in
...
and type_comprehension_expr
  ...
  let comprehension_type, container_type, make_texp,
    ...
    | Cexp_array_comprehension (amut, comp) ->
        ...
        Jkind.Type.Primitive.value ~why:Jkind.Type.History.Array_comprehension_element
  in
  let element_ty =
    with_local_level_if_principal begin fun () ->
...
and type_comprehension_iterator
  ...
  | In seq ->
      let item_ty = newvar (Jkind.Type.Primitive.any ~why:Dummy_jkind) in
...
let type_expression env sexp =
  type_expression env (Jkind.Type.Primitive.any ~why:Type_expression_call) sexp
