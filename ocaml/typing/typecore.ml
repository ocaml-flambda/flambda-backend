...
let solve_Ppat_array ~refine loc env mutability expected_ty =
  ...
  let arg_jkind = Jkind.of_new_sort ~why:Array_element in
  let ty_elt = newgenvar arg_jkind in
  let expected_ty = generic_instance expected_ty in
...
and type_pat_aux
  ...
  let type_pat_array mutability spl pat_attributes =
    ...
    let ty_elt, arg_jkind = solve_Ppat_array ~refine loc env mutability expected_ty in
      map_fold_cont (fun p ->
        let alloc_mode =
        ...
        type_pat ~alloc_mode tps Value p ty_elt) spl (fun pl ->
          rvp k {
          pat_desc = Tpat_array (mutability, arg_jkind, pl);
