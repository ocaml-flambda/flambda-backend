...
let transl_declaration env sdecl (id, uid) =
  ...
  let unbox, unboxed_default =
    ...
    | Ptype_record_flat _ -> lazy (assert false), true
  ...
  let (tkind, kind, jkind_default) =
    match sdecl.ptype_kind with
      ...
      *)
      ...
      | (Ptype_variant _ | Ptype_record _ | Ptype_record_flat _ | Ptype_open) when
        Builtin_attributes.has_or_null_reexport sdecl_attributes ->
      ...
      | Ptype_record_flat lbls ->
          ...
          let lbls, lbls' =
            ...
            env None true lbls (Record_flat_sort)
          in
...
let check_kind_coherence env loc dpath decl =
  ...
  (* CR rtjoa:  *)
  | (Type_variant _ | Type_record _ | Type_record_flat _ | Type_open), Some ty ->
...
let report_error ppf = function
  ...
  | Unbound_type_var (ty, decl) ->
      ...
      | Type_record_flat (tl, _), _ ->
          explain_unbound ppf ty tl (fun l -> l.Types.ld_type)
            "field_flat" (fun l -> Ident.name l.Types.ld_id ^ ": ")
