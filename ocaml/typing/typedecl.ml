...
let update_type temp_env env id loc =
  ...
  match get_type_manifest decl with None -> assert false
  | Some ty ->
      try
...
let set_private_row env loc p decl =
  ...
  let rv =
    ...
    | _ -> assert false
  in
  set_type_desc rv (Tconstr (p, get_type_param_exprs decl |> AppArgs.of_list, ref Mnil))
              ...
              (fun annot ->
                 let const =
                    Jkind.Const.of_user_written_annotation
                      ...
                      annot
                 in
                 Jkind_types.Higher_const.Type const, annot)
              l)
          vars_jkinds
  in
...
let get_path_manifest ~loc env params ty =
  ...
  | Tconstr(path, Applied args, _) ->
    ...
    then raise (Error (loc, Definition_mismatch (ty, env, Some Includecore.Arity)))
    else if not (List.for_all2 eq_type args params)
    then raise (Error (loc, Datatype_manifest_mismatch { use = args; def = params }))
    else path
...
let transl_declaration env sdecl (id, uid) =
  ...
  let jkind_from_annotation, jkind_annotation, sdecl_attributes =
    match Higher_jkind.of_type_decl ~context:(Type_declaration path) sdecl with
      ...
      | Ptype_abstract when
        Builtin_attributes.has_or_null_reexport sdecl_attributes ->
          let param =
            ...
            | Some (Tconstr(path, Applied [param], _))
...
let narrow_to_manifest_jkind env loc decl =
  ...
  | Some ty ->
    ...
    match Higher_jkind.sub_with_history jkind' (get_type_jkind decl) with
...
let check_kind_coherence env loc dpath decl =
  ...
  | Datatype { ret_jkind; manifest = Some path } ->
      let ty = Btype.newgenty (Tconstr (path, get_type_param_exprs decl |> AppArgs.of_list, ref Mnil)) in
      if !Clflags.allow_illegal_crossing then begin
        let jkind' = Ctype.type_jkind_purely env ty in
        begin match Higher_jkind.sub_with_history jkind' (Higher_jkind.wrap ret_jkind) with
        | Ok _ -> ()
        | Error v ->
          raise (Error (loc, Jkind_mismatch_of_type (ty,v)))
        end
...
let update_decl_jkind env dpath decl =
  ...
  let new_decl, new_jkind = match decl.type_noun with
    ...
    *)
    | Datatype { ret_jkind; noun = Datatype_variant _ } when
      Builtin_attributes.has_or_null_reexport decl.type_attributes ->
      decl, Higher_jkind.wrap ret_jkind
  ...
  if new_jkind != get_type_jkind decl then
    begin match Higher_jkind.sub_or_error new_jkind (get_type_jkind decl) with
    | Ok () -> ()
    | Error err ->
...
let update_decls_jkind_reason decls =
  List.map
    (fun (id, decl) ->
       ...
       let new_decl =
         set_type_jkind (Higher_jkind.History.update_reason (get_type_jkind decl) reason) decl
       in
       (id, new_decl)
    )
    decls
...
let check_well_founded_manifest ~abs_env env loc path decl =
  if get_type_manifest decl = None then () else
  let args =
...
let name_recursion sdecl id decl =
  ...
  | Equation { eq = Type_abbrev { priv = Private; expansion = ty } } when is_fixed_type sdecl ->
    ...
    if Ctype.deep_occur ty ty' then
      let td = Tconstr(Path.Pident id, get_type_param_exprs decl |> AppArgs.of_list, ref Mnil) in
    ...
    | Pext_rebind lid ->
        ...
        let args =
          ...
          | Some decl ->
              ...
              AppArgs.iter_with_list (Ctype.unify env) (get_type_param_exprs decl) tl;
              let lbls =
                match decl.type_noun with
  ...
  let sort_or_poly =
    ...
    *)
    | Tvar {jkind} when is_layout_poly
                      && Higher_jkind.has_layout_any jkind
...
let transl_value_decl env loc valdecl =
  ...
  begin match Ctype.constrain_type_jkind env cty.ctyp_type
                (Higher_jkind.Builtin.value_or_null ~why:Structure_element) with
  | Ok () -> ()
  | Error err ->
...
let abstract_type_decl ~injective ~jkind ~jkind_annotation ~params =
  Ctype.with_local_level ~post:generalize_decl begin fun () ->
    ...
    { type_noun = create_higher_kinded_type_equation_noun type_params jkind Public None;
      type_jkind_annotation = jkind_annotation;
      type_is_newtype = false;
      type_expansion_scope = Btype.lowest_level;
  ...
  List.map
    (fun sdecl ->
       ...
       let jkind, jkind_annotation, _sdecl_attributes =
         Higher_jkind.of_type_decl_default
           ~context:(Type_declaration path)
           ~default:(Higher_jkind.Builtin.value ~why:Default_type_jkind)
           sdecl
       in
       let params =
...
let report_error ppf = function
  ...
  | Definition_mismatch (ty, env, Some err) ->
      fprintf ppf "@[<v>@[<hov>%s@ %s@;<1 2>%a@]%a@]"
        ...
        err
  | Datatype_manifest_mismatch { def; use } ->
      fprintf ppf "@[<v>@[<hov>%s@ %s@;<1 2>defined (%a), manifest has (%a)@]@]"
        "This variant or record definition"
        "uses a manifest with a mismatched parameter list to the one defined"
        (pp_print_list ~pp_sep:(fun ppf () -> fprintf ppf ", ") Printtyp.type_expr) def
        (pp_print_list ~pp_sep:(fun ppf () -> fprintf ppf ", ") Printtyp.type_expr) use
