...
let classify_expression : Typedtree.expression -> sd =
  ...
  *)
  let rec classify_expression env e : sd =
    ...
    | Texp_tuple (exprs, _) ->
        if List.for_all (fun (_, expr) -> is_constant expr) exprs
        then Constant
        else Static
...
let is_valid_recursive_expression idlist expr : sd option =
  ...
  | _ ->
     ...
     | (Static | Constant) as recursive ->
        let ty = expression expr Return in
        if Env.unguarded ty idlist = [] then Some recursive else None
     | Not_recursive ->
        let ty = expression expr Return in
