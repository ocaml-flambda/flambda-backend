...
let dummy_label_flat =
  { lbl_name = ""; lbl_res = none; lbl_arg = none;
    lbl_mut = Immutable; lbl_modalities = Mode.Modality.Value.Const.id;
    lbl_jkind = Jkind.Builtin.any ~why:Dummy_jkind;
    lbl_num = -1; lbl_pos = -1; lbl_all = [||];
    lbl_repres = Record_flat [||];
    lbl_private = Public;
    lbl_loc = Location.none;
    lbl_attributes = [];
    lbl_uid = Uid.internal_not_actually_unique;
...
let label_flat_descrs ty_res lbls repres priv =
  let all_labels = Array.make (List.length lbls) dummy_label_flat in
  let rec describe_labels num pos = function
      [] -> []
    | l :: rest ->
        let is_void = Jkind.is_void_defaulting l.ld_jkind  in
...
let label_flats_of_type ty_path decl =
  match decl.type_kind with
  | Type_record_flat(labels, rep) ->
      label_flat_descrs (newgenconstr ty_path decl.type_params)
        labels rep decl.type_private
  | Type_record _
  | Type_variant _ | Type_abstract _ | Type_open -> []
