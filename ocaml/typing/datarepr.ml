...
let constructor_existentials cd_args cd_res =
  ...
  let existentials =
    ...
    | None -> []
    | Some type_ret ->
        let arg_vars_set = free_vars (newgenty (Ttuple (List.map (fun ty -> None, ty) tyl))) in
...
let constructor_args ~current_unit priv cd_args cd_res path rep =
  ...
  | Cstr_record lbls ->
      let arg_vars_set = free_vars ~param:true (newgenty (Ttuple (List.map (fun ty -> None, ty) tyl))) in
