...
and 'k pattern_desc =
  ...
  | Tpat_tuple : (string option * value general_pattern) list * tuple_shape ->
      value pattern_desc
  | Tpat_construct :
...
and expression_desc =
  ...
  | Texp_tuple of (string option * expression) list * Mode.Alloc.r * tuple_shape
  | Texp_construct of
...
let shallow_map_pattern_desc
  ...
  | Tpat_tuple (pats, shape) ->
      Tpat_tuple (List.map (fun (label, pat) -> label, f.f pat) pats, shape)
...
let iter_pattern_full ~both_sides_of_or f sort pat =
  let rec loop :
    ...
    fun f sort pat ->
      ...
      | Tpat_tuple (patl, shape) ->
        List.iteri (fun i (_, pat) ->
