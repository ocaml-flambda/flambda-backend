...
let format_history ~intro ppf t =
  Format_history.format_history ~intro ~layout_or_kind:"kind" ppf
    (wrap_higher_jkind t)
...
let combine_histories reason lhs rhs =
  if flattened_histories
  then
    match Desc.sub (Jkind_desc.get lhs.desc) (Jkind_desc.get rhs.desc) with
    | Less -> lhs.history
    | Not_le ->
      rhs.history
...
let intersection_or_error ~reason t1 t2 =
  ...
  | None ->
    Error
      (Violation.of_
         (No_intersection
            (Jkind_types.wrap_higher_jkind t1, Jkind_types.wrap_higher_jkind t2)))
  | Some desc ->
    Ok
      { desc;
...
let sub_with_history sub super =
  ...
  | Not_le ->
    Error
      (Violation.of_
         (Not_a_subjkind
            ( Jkind_types.wrap_higher_jkind sub,
              Jkind_types.wrap_higher_jkind super )))
