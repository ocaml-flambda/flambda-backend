...
module Type = struct
  ...
  module Nullability = struct
    type t = Jkind_types.Type.Nullability.t =
      | Non_null
      | Maybe_null
  ...
  module Const = struct
    ...
    module Primitive = struct
      ...
      let void =
        { jkind =
            of_layout Layout.Const.void ~mode_crossing:false
              ~nullability:Non_null;
          name = "void"
        }
      ...
      let immediate =
        { jkind =
            of_layout Layout.Const.value ~mode_crossing:true
              ~nullability:Non_null;
          name = "immediate"
        }
      ...
      *)
      ...
      let word =
        { jkind =
            of_layout Layout.Const.word ~mode_crossing:false
              ~nullability:Non_null;
          name = "word"
        }
    ...
    let format ppf jkind =
      to_out_jkind_const jkind |> !Oprint.out_jkind_const ppf
  ...
  module Jkind_desc = struct
    ...
    let add_portability_and_contention_crossing ~from t =
      ...
      let added_crossings =
        (not
           (Portability.Const.le t.modes_upper_bounds.portability
              new_portability))
        || not
  ...
  *)
  ...
  let intersection ~reason t t' =
    let ( let* ) = Option.bind in
    let* jkind = Jkind_desc.intersection t.jkind t'.jkind in
    Some
      { jkind;
  ...
  let union ~reason t t' =
    { jkind = Jkind_desc.union t.jkind t'.jkind;
      history = combine_histories reason t t';
      has_warned = t.has_warned || t'.has_warned
    }
  ...
  module Debug_printers = struct
    ...
    let creation_reason ppf : History.creation_reason -> unit = function
      ...
      | Concrete_legacy_creation concrete ->
        fprintf ppf "Concrete_legacy_creation %a"
          concrete_legacy_creation_reason concrete
...
module Const = struct
  ...
  let rec format_gen ?(hiding = true) ppf (t : t) =
    match t with
    | Type t ->
      if hiding
      then Type.Const.format ppf t
      else Type.Const.format_no_hiding ppf t
    | Arrow { args; result } ->
  ...
  let format_no_hiding = format_gen ~hiding:false
...
let format_history_gen ?(layout_or_kind = "kind") ~intro ppf (t : t) =
  match t.jkind with
  | Type ty ->
    Type.format_history_gen ~layout_or_kind ~intro ppf
      { jkind = ty;
        history = History.to_type_jkind t.history;
    ...
    let k1, k2, fmt_k1, fmt_k2, missing_cmi_option =
      ...
      | { violation = No_intersection (k1, k2); missing_cmi } ->
        ...
        ( k1,
          ...
          None )
      | { violation = No_union (k1, k2); missing_cmi } ->
        assert (Option.is_none missing_cmi);
        ( k1,
          k2,
          dprintf "%s %a" layout_or_kind format_layout_or_kind k1,
...
let rec intersection_or_error ~reason (t1 : t) (t2 : t) =
  ...
  | Type ty1, Type ty2 ->
    Type.intersection ~reason ty1 ty2
    |> Option.map of_type_jkind
...
and union_or_error ~reason (t1 : t) (t2 : t) =
  ...
  | Type ty1, Type ty2 ->
    ...
    Ok (Type.union ~reason ty1 ty2 |> of_type_jkind)
  | ( Arrow { args = args1; result = result1 },
      Arrow { args = args2; result = result2 } ) ->
