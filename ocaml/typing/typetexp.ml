...
module TyVarEnv : sig
  ...
  *)
  ...
  let new_jkind ~is_named { jkind_initialization } jkind_check =
    ...
    let gen_sort () = Jkind.of_new_sort ~why:(if is_named then Unification_var else Wildcard) in
      ...
      fprintf ppf
        ...
        (Jkind.format_history ~intro:(
          dprintf "But it was inferred to have %t"
            ...
            | Some c -> fprintf ppf "layout %a" Jkind.Const.format c
            | None -> fprintf ppf "a representable layout")))
        inferred_jkind
