        ...
        | (l, arg_mode, arg) :: rest ->
          ...
          let l, arg_cty =
            ...
            | (Labelled l | Position l), Ptyp_extension ({txt="src_pos"; _}, _payload) ->
              ...
              (* TODO vding: This will change once we have typedtree labels st Position contains no type *)
              let path = Predef.path_lexing_position in
              let lid = Longident.Lident "[%src_pos]" in
              let constr = newconstr path [] in
              Position l, ctyp (Ttyp_constr (path, {txt=lid; loc=Location.none}, [])) constr
