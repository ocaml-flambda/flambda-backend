...
let get_type_param_jkind path styp =
  match Jane_syntax.Core_type.of_ast styp with
  | None -> Jkind.Type.of_new_legacy_sort ~why:(Unannotated_type_parameter path) |> Jkind.of_type_jkind
            ...
            List.iter (fun {ctyp_type; ctyp_loc} ->
              ...
              match
                constrain_type_jkind env ctyp_type
                  (Jkind.Type.Primitive.value_or_null ~why:Polymorphic_variant_field |> Jkind.of_type_jkind)
              with
              | Ok _ -> ()
              | Error e ->
...
and transl_type_aux_tuple env ~policy ~row_context stl =
  ...
  List.iter (fun (_, {ctyp_type; ctyp_loc}) ->
    ...
    match
      constrain_type_jkind env ctyp_type (Jkind.Type.Primitive.value_or_null ~why:Tuple_element |> Jkind.of_type_jkind)
    with
    | Ok _ -> ()
    | Error e ->
      raise (Error(ctyp_loc, env,
