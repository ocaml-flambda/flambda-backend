...
module History = struct
  ...
  let is_imported (t : t) =
    match t.hhistory with Creation Imported -> true | _ -> false

  let rec update_reason ({ hdesc; hhistory = _ } : t) reason : t =
    let go t = update_reason t reason in
    { hdesc =
        (match hdesc with
        | Type ty -> Type (Jkind.History.update_reason ty reason)
        | Arrow (args, result) -> Arrow (List.map go args, go result)
        | Top -> Top);
      hhistory = Creation reason
    }
end
...
let format_history ~intro ppf t = Jkind.format_history_higher ~intro ppf t
...
let combine_histories = Jkind.combine_histories_higher
