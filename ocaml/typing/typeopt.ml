...
let rec value_kind env ~loc ~visited ~depth ~num_nodes_visited ty
  ...
  begin
    (* CR layouts: We want to avoid correcting levels twice, and scrape_ty will
       ...
       sufficient.  *)
    match Ctype.check_type_jkind env scty (Jkind.Type.Primitive.value_or_null ~why:V1_safety_check |> Jkind.of_type_jkind)
    with
    | Ok _ -> ()
    | Error _ ->
      match
        Ctype.(check_type_jkind env
                 (correct_levels ty) (Jkind.Type.Primitive.value_or_null ~why:V1_safety_check |> Jkind.of_type_jkind))
      with
      | Ok _ -> ()
      | Error violation ->
