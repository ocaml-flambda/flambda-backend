...
module Simple = struct
  type view = [
    ...
    | `Array of mutable_flag * Jkind.t * pattern list
    | `Lazy of pattern
  ]
...
module General = struct
  ...
  let view_desc = function
    ...
    | Tpat_array (am, arg_jkind, ps) -> `Array (am, arg_jkind, ps)
  ...
  let erase_desc = function
    ...
    | `Record (fields, closed) ->
       Tpat_record (fields, closed)
    | `Array (am, arg_jkind, ps) -> Tpat_array (am, arg_jkind, ps)
...
module Head : sig
  type desc =
    ...
    | Array of mutable_flag * Jkind.t * int
    | Lazy
  ...
  let deconstruct (q : Simple.pattern) =
    let deconstruct_desc = function
      ...
      | `Array (am, arg_jkind, args) ->
          Array (am, arg_jkind, List.length args), args
      | `Record (largs, _) ->
  ...
  let to_omega_pattern t =
    let pat_desc =
      ...
      | Array (am, arg_jkind, n) -> Tpat_array (am, arg_jkind, omegas n)
      | Construct c ->
