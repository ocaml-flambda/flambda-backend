  ...
  let rec compute_variance_rec vari ty =
    ...
    | Tconstr (path, Applied tl, _) ->
        let open Variance in begin
          try
            let var = Env.find_type path env |> Ctype.app_variance_of_decl in
            List.iter2
...
let compute_variance_gadt env ~check (required, loc as rloc) decl
  ...
  | None ->
      compute_variance_type env ~check rloc {decl with type_private = Type_private}
        (for_constr tl)
  | Some ret_type ->
      ...
      | Tconstr (_, tyl, _) ->
          ...
          compute_variance_type env ~check rloc
            {decl with type_params = tyl; type_private = Type_private}
            (for_constr tl)
      | _ -> assert false
          ...
          if List.for_all (fun c -> c.Types.cd_res = None) tll then
            ...
            let vari =
              List.map
                (fun ty ->
                   compute_variance_type env ~check rloc
                     {decl with type_private = Type_private}
                     (add_false [ ty ])
                )
...
let update_class_decls env cldecls =
  ...
  List.map2
    (fun (_,decl) (_, _, clty, cltydef, _) ->
      let variance = Ctype.app_variance_of_decl decl in
