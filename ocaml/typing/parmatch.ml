...
(* More standard stuff *)
  ...
  | Tpat_record (l1,_),Tpat_record (l2,_) ->
      ...
      compats ps qs
  | Tpat_array (am1, _arg_jkind1, ps), Tpat_array (am2, _arg_jkind2, qs) ->
      am1 = am2 &&
...
| {pat_desc = Tpat_array (am, arg_jkind, omegas)} ->
    let args,rest = read_args omegas r in
    make_pat
      (Tpat_array (am, arg_jkind, args)) q.pat_type q.pat_env::
    rest
...
let build_other ext env =
  ...
  | (d, _) :: _ ->
      ...
      | Array (am, arg_jkind, _) ->
          let all_lengths =
            List.map
              (fun (p,_) -> match p.pat_desc with
                 | Array (am', arg_jkind', len) when
                     am = am' && Jkind.equal arg_jkind arg_jkind' -> len
              | _ -> assert false)
              env in
          let rec try_arrays l =
            if List.mem l all_lengths then try_arrays (l+1)
            else
              make_pat (Tpat_array (am, arg_jkind, omegas l))
...
let rec le_pat p q =
  ...
  | Tpat_record (l1,_), Tpat_record (l2,_) ->
      let ps,qs = records_args l1 l2 in
      le_pats ps qs
  | Tpat_array(am1, arg_jkind1, ps), Tpat_array(am2, arg_jkind2, qs) ->
    am1 = am2 && Jkind.equal arg_jkind1 arg_jkind2
...
| Tpat_record (l1,closed),Tpat_record (l2,_) ->
    ...
    make_pat (Tpat_record (rs, closed))
      p.pat_type p.pat_env
| Tpat_array (am1, arg_jkind1, ps), Tpat_array (am2, arg_jkind2, qs)
  when am1 = am2 && Jkind.equal arg_jkind1 arg_jkind2
       && List.length ps = List.length qs ->
        let rs = lubs ps qs in
        make_pat (Tpat_array (am1, arg_jkind1, rs))
          p.pat_type p.pat_env
| _,_  ->
    raise Empty
