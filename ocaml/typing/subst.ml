...
let record_flat_representation ~prepare_jkind loc = function
  | Record_flat lays -> Record_flat (Array.map (prepare_jkind loc) lays)
    ...
    type_kind =
      ...
      | Type_record_flat(lbls, rep) ->
          let rep =
            ...
            | No_action | Duplicate_variables -> rep
            | Prepare_for_saving { prepare_jkind } ->
                record_flat_representation ~prepare_jkind decl.type_loc rep
          in
          Type_record_flat (List.map (label_declaration copy_scope s) lbls, rep)
      | Type_open -> Type_open
      end;
    type_manifest =
      begin
