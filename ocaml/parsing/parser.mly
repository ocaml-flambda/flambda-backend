...
let papp_extension loc =
  Exp.mk ~loc:(ghost_loc loc) (Pexp_extension(papp_ext_loc (make_loc loc), PStr []))
...
labeled_simple_expr:
  ...
  | OPTLABEL simple_expr %prec below_HASH
      { (Optional $1, $2) }
  | UNDERSCORE
      { (Nolabel, papp_extension $loc($1)) }
  | LABEL UNDERSCORE
      { (Labelled $1, papp_extension $loc($2)) }
  | OPTLABEL UNDERSCORE
      { (Optional $1, papp_extension $loc($2)) }
;
%inline lident_list:
  xs = mkrhs(LIDENT)+
    { xs }
