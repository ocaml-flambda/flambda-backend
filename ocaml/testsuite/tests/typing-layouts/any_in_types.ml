...
let unbox = Stdlib_upstream_compatible.Float_u.of_float
...
module M1 : S with type t = float# = struct
  ...
  let add x y = Stdlib_upstream_compatible.Float_u.add x y
  let one () = unbox 1.
...
let () = Printf.printf "%f\n" (Stdlib_upstream_compatible.Float_u.to_float (M1.add (unbox 10.) (unbox 10.)))
let () = Printf.printf "%d\n" (M2.add 10 10)
...
let () = Printf.printf "%f\n" (Stdlib_upstream_compatible.Float_u.to_float (M3.add (unbox 10.) (unbox 10.)))
let () = Printf.printf "%d\n" (M4.add 10 10)
    ...
    let r =
      g
        ...
        (Printf.printf "a\n";
         unbox 10.)
    in
    Printf.printf "%f\n" (Stdlib_upstream_compatible.Float_u.to_float r)
  with
  | _ -> Printf.printf "b\n"
;;
