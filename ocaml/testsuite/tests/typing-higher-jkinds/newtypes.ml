...
(* CR jbachurski: I ended up making these not parse. *)
(*
type 'a t = new { foo : 'a }
[%%expect {|
...
(* CR jbachurski: I think this should fail *)
type t = new < foo : t; .. >
[%%expect {|
type t = new < foo : t; .. >
|}]
...
Error: Signature mismatch:
       ...
       is not included in
         type t = new int
       The first is abstract, but the second is a datatype new-abbreviation.
|}]
...
Error: Signature mismatch:
       Modules do not match:
         sig type s = new int type t = s end
       is not included in
         sig type t = new int end
       Type declarations do not match:
         type t = s
...
Error: Signature mismatch:
       ...
       is not included in
         type t = new int
       The first is an abstract datatype, but the second is a datatype new-abbreviation.
|}]
...
(* CR jbachurski: This should work, but doesn't - it gets the wrong kind... *)
type 'a two = new 'a * 'a
