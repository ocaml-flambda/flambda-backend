...
[%%expect{|
val ul : 'a@unique local -> 'a@local = <fun>
|}]
...
let gc_soundness_nobug (local_ unique_ p) (local_ f) =
  local_ { p with x = f }
[%%expect{|
val gc_soundness_nobug : point@unique local -> float@local -> point@local =
  <fun>
|}]
...
let rec foo =
  ...
  | None -> ()
[%%expect{|
val foo : unit option@unique local -> unit = <fun>
|}]
...
[%%expect{|
val bar : unit option@unique local -> unit = <fun>
|}]

let foo : local_ unique_ string -> unit = fun (local_ s) -> ()
[%%expect{|
val foo : string@unique local -> unit = <fun>
|}]

let bar : local_ unique_ string -> unit = fun (unique_ s) -> ()
[%%expect{|
val bar : string@unique local -> unit = <fun>
|}]
...
[%%expect{|
val curry : box@unique -> box@unique -> box = <fun>
|}]

let curry : unique_ box -> unique_ box -> unique_ box = fun b1 b2 -> b1
[%%expect{|
val curry : box@unique -> box@unique -> box@unique = <fun>
|}]
...
3 |     ((fun y z -> z) : int -> unique_ (int -> int)) in
        ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error: This expression has type int -> (int -> int)@unique
