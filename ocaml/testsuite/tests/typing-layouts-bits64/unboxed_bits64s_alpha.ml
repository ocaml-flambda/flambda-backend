...
let to_binary_string x =
  String.init Int64.size (fun i ->
    if Int64.(equal (logand x (shift_left 1n (Int64.size - i - 1))) 0n)
    then '0'
    else '1')
...
let test1 () =
  ...
  let three = of_int64 3n in
  print_int64u "Test 1, three" three;

  let twice_three = three + (of_int64 3n) in
  print_int64u "Test 1, twice_three" twice_three;

  let thrice_three = (of_int64 3n) * three in
  ...
  let three_again = twice_three_again / (of_int64 2n) in
  print_int64u "Test 1, three_again" three_again;

  let three_again_unsigned = twice_three_again // (of_int64 2n) in
  ...
  let minus_five = of_int64 (-5n) in
  print_int64u "Test 1, minus_five" minus_five;

  let twice_minus_five = minus_five + (of_int64 (-5n)) in
  print_int64u "Test 1, twice_minus_five" twice_minus_five;

  let thrice_minus_five = (of_int64 (3n)) * minus_five in
  ...
  let minus_five_again = twice_minus_five_again / (of_int64 2n) in
  print_int64u "Test 1, minus_five_again" minus_five_again;

  let minus_five_again_unsigned = twice_minus_five_again // (of_int64 2n) in
...
let[@inline never] twice_on_three f =
  let pi = Int64_u.of_int64 3n in
  twice f pi

let times_four = twice Int64_u.(fun x -> x * (of_int64 2n))

let _ =
  let open Int64_u in
  print_int64u "Test 2, add three twice"
    (twice (fun x -> x + (of_int64 3n)) (of_int64 0n));
  print_int64u "Test 2, add three four times"
    (twice (twice (fun x -> x + (of_int64 3n))) (of_int64 0n));
  print_int64u "Test 2, increment three twice"
    (twice_on_three (fun x -> (of_int64 1n) + x));
  print_int64u "Test 2, increment three four times"
    (twice_on_three (twice (fun x -> (of_int64 1n) + x)));
  print_int64u "Test 2, two times four"
    (times_four (of_int64 2n));
  print_int64u "Test 2, three times sixteen"
    (twice_on_three times_four);
  print_int64u "Test 2, three times sixteen again"
    (compose times_four times_four (of_int64 3n));
  print_int64u "Test 2, three minus four"
    (let two = twice (fun x -> x + (of_int64 1n)) (of_int64 0n) in
     let add_two = Int64_u.(+) two in
     let add_two_after = compose add_two in
     let minus_four = add_two_after (twice (fun x -> x - (of_int64 3n))) in
     minus_four (of_int64 3n))
...
let test3 () =
  ...
  let five_times_three = f3 5 (Int64_u.of_int64 3n) steps in
  ...
  *)
  let steps = Array.init 10 (fun _ -> 0n) in
  let x1 = Int64_u.of_int64 1n in
  let x3 = Int64_u.of_int64 2n in
  let x5 = Int64_u.of_int64 3n in
  let x7 = Int64_u.of_int64 5n in
  let x9 = Int64_u.of_int64 8n in
...
let[@inline never] test4 () =
  ...
  let[@inline never] go f =
    Int64_u.to_int64 (f (Int64_u.of_int64 1n) (Int64_u.of_int64 2n))
  in
  ...
  let steps = Array.init 10 (fun _ -> 0n) in
  let f = Sys.opaque_identity (f3 5 (Int64_u.of_int64 3n)) in
  ...
  let six_times_three = f (Int64_u.of_int64 3n) steps in
