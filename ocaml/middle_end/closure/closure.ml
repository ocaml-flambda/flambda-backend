  ...
  | Lletrec(defs, body) ->
      ...
      then begin
        ...
        | { id; rkind; def } :: rem ->
            ...
            let (ulam, approx) = close_named { env with kinds } id def in
...
and close_functions { backend; fenv; cenv; mutable_vars; kinds; catch_env } fun_defs =
  let fun_defs =
    List.flatten
      (List.map
         (function
           | { id; rkind=_;
               def = Lfunction{kind; params; return; body; attr;
  ...
  let uncurried_defs =
    List.map
      (function
          { id; rkind=_;
            def = Lfunction {kind; params; return; body; loc; mode; region; attr}) ->
            let label =
