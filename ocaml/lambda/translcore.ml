...
let declare_probe_handlers lam =
  List.fold_left (fun acc (funcid, func) ->
      Llet(Strict, Pvalue Pgenval, funcid, func, acc))
    lam
    !probe_handlers
...
and transl_exp0 ~in_new_scope ~scopes e =
  ...
  | Texp_override(path_self, modifs) ->
      ...
      Llet(Strict, Pvalue Pgenval, cpy,
           Lapply{
             ap_loc=Loc_unknown;
  ...
  | Texp_letmodule(Some id, loc, Mp_present, modl, body) ->
      ...
      in
      Llet(Strict, Pvalue Pgenval, id, defining_expr, transl_exp ~scopes body)
  | Texp_letmodule(_, _, Mp_absent, _, body) ->
      transl_exp ~scopes body
  | Texp_letexception(cd, body) ->
      Llet(Strict, Pvalue Pgenval,
  ...
  | Texp_lazy e ->
      ...
      | `Other ->
         ...
         let fn = lfunction ~kind:(Curried {nlocal=0})
                            ~params:[Ident.create_local "param", Pvalue Pintval (* unit *)]
      ...
      | _ ->
          ...
          let body, _ =
            List.fold_left (fun (body, pos) id ->
              Llet(Alias, Pvalue Pgenval (* for now, module fields can only be values *), id,
  ...
  | Texp_probe {name; handler=exp} ->
    if !Clflags.native_code && !Clflags.probes then begin
      ...
      (* TODO extract the kinds of the arguments & return *)
      let handler =
        let scopes = enter_value_definition ~scopes funcid in
        lfunction
          ~kind:(Curried {nlocal=0})
          ~params:(List.map (fun v -> v, Pvalue Pgenval) param_idents)
          ~return:(Pvalue Pgenval)
          ~body
      ...
      | true ->
          Llet(Strict, Pvalue Pgenval, funcid, handler, Lapply app)
      | false ->
...
and transl_list_with_shape ~scopes expr_list =
  let transl_with_shape e =
    let shape = must_be_value (Typeopt.layout e.exp_env e.exp_type) in
  ...
  let rec build_apply lam args loc pos ap_mode = function
    | Omitted { mode_closure; mode_arg; mode_ret } :: l ->
        ...
        let body =
          ...
          in
          (* TODO get kinds *)
          lfunction ~kind:(Curried {nlocal}) ~params:[id_arg, Pvalue Pgenval]
                    ~return:(Pvalue Pgenval) ~body ~mode ~region
                    ~attr:default_stub_attribute ~loc
        in
        (* TODO kinds *)
        List.fold_right
          (fun (id, lam) body -> Llet(Strict, Pvalue Pgenval, id, lam, body))
          !defs body
...
and transl_record ~scopes loc env mode fields repres opt_init_expr =
  ...
  then begin
    ...
    begin match opt_init_expr with
      None -> lam
    | Some init_expr -> Llet(Strict, Pvalue Pgenval, init_id,
    ...
    | Some init_expr ->
        ...
        Llet(Strict, Pvalue Pgenval, copy_id,
...
and transl_letop ~scopes loc env let_ ands param case partial warnings =
  let rec loop prev_layout prev_lam = function
    ...
    | and_ :: rest ->
        ...
        in
        let result_layout = Pvalue Pgenval (* TODO *) in
  ...
  let exp =
    loop (layout let_.bop_exp.exp_env let_.bop_exp.exp_type)
      (transl_exp ~scopes let_.bop_exp) ands
  in
  let func =
