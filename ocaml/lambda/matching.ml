  ...
  let rec do_cases = function
    ...
    | ((p, patl), action) :: rem -> (
        ...
        | `Or _ ->
            ...
            let arg_sort = args.first.arg_sort in
...
let get_expr_args_tuple ~scopes head { arg; mut; _ } rem =
  ...
  let rec make_args pos =
    if pos >= arity then
      ...
      {
        arg = Lprim (Pfield (pos, Pointer, Reads_agree), [ arg ], loc);
        binding_kind = Alias;
...
let bind_check kind v arg_layout arg lam =
  match (kind, arg) with
  | _, Lvar _ -> bind_with_layout kind (v, arg_layout) arg lam
  | Alias, _ -> lower_bind v arg_layout arg lam
  | _, _ -> bind_with_layout kind (v, arg_layout) arg lam
...
and bind_match_arg kind v arg arg_layout (lam, jumps) =
  let jumps =
  ...
  | Pm pm -> (
      ...
      | Construct cstr ->
          compile_test
            ...
            (combine_constructor value_kind ploc arg ph.pat_env cstr arg_partial)
            ctx pm
      | Array (_, elt_sort, _) ->
      ...
      | Variant { cstr_row = row } ->
          compile_test
            ...
            (combine_variant value_kind ploc !row arg arg_partial)
            ctx pm
    )
  | PmVar { inside = pmh } ->
      let lam, total =
