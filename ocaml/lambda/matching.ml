...
module Simple : sig
  ...
  let alpha env (p : pattern) : pattern =
    ...
    let pat_desc =
      ...
      | `Array (am, arg_jkind, ps) -> `Array (am, arg_jkind, List.map (alpha_pat env) ps)
...
let get_expr_args_array ~scopes kind head (arg, _mut, _sort, _layout) rem =
  let am, arg_jkind, len =
    let open Patterns.Head in
    match head.pat_desc with
    | Array (am, arg_jkind, len) -> am, arg_jkind, len
    | _ -> assert false
  in
  let loc = head_loc ~scopes head in
  let rec make_args pos =
