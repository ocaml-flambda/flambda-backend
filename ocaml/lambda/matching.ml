...
module Default_environment : sig
  ...
  let cons matrix raise_num default =
    ...
    | [] -> default
    | _ ->
        { default with env = (matrix, raise_num) :: default.env }
  ...
  let flatten size def =
    { def with
      env = List.map (fun (pss, i) -> (flatten_matrix size pss, i)) def.env;
    }
end
...
type args = (lambda * let_kind * Jkind.sort * layout) list
...
type split_args = {
  head : (pure_head * let_kind * Jkind.sort * layout);
  rest : args;
}
...
let pretty_pm pm =
  pretty_pm_ ~print_default:false Format.err_formatter pm
...
and precompile_var args cls def k =
  ...
  | (Lvar v, str, arg_sort, arg_layout) :: rargs -> (
      ...
      | _ -> (
          (* Precompile *)
          let var_args = { head = (Var v, str, arg_sort, arg_layout); rest = rargs } in
          let var_cls =
            List.map
              (fun ((p, ps), act) ->
  ...
  let rec do_cases = function
    ...
    | ((p, patl), action) :: rem -> (
        ...
        | `Or _ ->
            ...
            let _, _, arg_sort, _ = args.head in
            ...
            in
            let new_cases =
              let arg, _, arg_sort, _ = args.head in
              let arg = arg_of_pure_head arg in
...
let make_matching get_expr_args head def ctx { head = (arg, _, sort, layout); rest } =
...
let make_line_matching get_expr_args head def { head = (arg, _, sort, layout); rest } =
  { cases = [];
...
let rec compile_match ~scopes value_kind repr partial ctx
  ...
  | ([], action) :: rem ->
      ...
      in
      if dbg then Format.eprintf "empty matrix%t"
...
and compile_match_nonempty ~scopes value_kind repr partial ctx
  ...
  | { args = (arg, str, arg_sort, layout) :: rest } ->
      ...
      bind_match_arg str v arg layout (
...
and bind_match_arg str v arg arg_layout (lam, jumps) =
  let jumps =
  ...
  (bind_check str v arg_layout arg lam,
   jumps)
...
and do_compile_matching_pr ~scopes value_kind repr partial ctx x =
  ...
  if dbg then Format.eprintf "@,@[<v 2>CTX:@,%a@]"
    Context.pp ctx;
  if dbg then Format.eprintf "@,@[<v 2>COMPILE:@,";
  let ((_, jumps) as r) =
    try do_compile_matching ~scopes value_kind repr partial ctx x with
    | exn ->
        if dbg then Format.eprintf "EXN (%s)@]@]" (Printexc.to_string exn);
        raise exn
  in
  if dbg then Format.eprintf "@]";
  if dbg then Format.eprintf "%a" Jumps.pp_section jumps;
  if dbg then Format.eprintf "@]";
  r
...
let do_for_multiple_match ~scopes ~return_layout loc paraml mode pat_act_list partial =
  ...
  let input_args = { head = (Tuple arg, Strict, Jkind.Sort.for_tuple, layout_block); rest = [] } in
  let handler =
    let partial = check_partial pat_act_list partial in
