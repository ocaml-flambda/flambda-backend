...
let transl_arr_clause ~transl_exp ~scopes ~loc clause body =
  ...
  let bindings, for_ =
    ...
    | In (pat , e2) ->
        ...
        let in_binding =
          binding Strict (Pvalue (Parrayval in_kind)) in_var (transl_exp ~scopes e2)
        in
        let len_binding =
          let init = Lprim( (Parraylength(in_kind)), [Lvar(in_var)], loc) in
          binding Alias (Pvalue Pintval) len_var init
        in
    ...
    | From_to(id, _, e2, e3, dir) ->
        ...
        let from_binding =
          binding Strict (Pvalue Pintval) from_var (transl_exp ~scopes e2)
        in
        let to_var = Ident.create_local "to" in
        let to_binding =
          binding Strict (Pvalue Pintval) to_var (transl_exp ~scopes e3)
        in
        let low, high =
          match dir with
...
let iterate_arr_block ~transl_exp ~loc ~scopes
  ...
  let body =
    ...
    | None -> body
    | Some guard ->
      Lifthenelse(transl_exp ~scopes guard, body, lambda_unit, Pvalue Pintval)
  in
  ...
  let length_binding = binding Alias (Pvalue Pintval) length_var length in
...
let make_array ~loc ~kind ~size ~array =
  ...
  | Pgenarray ->
      ...
      binding_mut (Pvalue (Parrayval kind)) array init
  | Pintarray | Paddrarray ->
      let init = make_array_prim ~loc size (int 0) in
      binding Strict (Pvalue (Parrayval kind)) array init
  | Pfloatarray ->
...
let init_array_elem ~loc ~kind ~size ~array ~index ~value =
  ...
  | Pgenarray ->
      ...
      in
      Lifthenelse(is_first_iteration, make_array, set_elem, Pvalue Pintval)
  ...
  | Pgenarray ->
      ...
      Lsequence(
        Lifthenelse(is_first_iteration,
          Lifthenelse(is_not_empty, make_array, lambda_unit, Pvalue Pintval),
...
let make_counter counter =
  binding_mut (Pvalue Pintval) counter (int 0)
...
let transl_arr_block ~transl_exp ~loc ~scopes
  ...
  let set_result =
    ...
    | With_size {body; raise_count} ->
        ...
        in
        Lstaticcatch(body,
          (raise_count, [(elem_var, layout); (elem_len_var, Pvalue Pintval)]),
          Lsequence(init_elem, Lsequence(set_len,
             increment_counter ~loc counter_var (int 2))), Pvalue Pintval)
  in
  let bindings, loops =
...
let transl_single_arr_block ~transl_exp ~loc ~scopes
  ...
  | With_size { body; raise_count } ->
      ...
      Lstaticcatch(body,
          (raise_count, [(array_var, Pvalue (Parrayval array_kind)); (len_var, Pvalue Pintval)]),
          sub_array ~loc (Lvar array_var) (int 0) (Lvar len_var), Pvalue Pintval)
...
let concat_arrays ~loc arr kind shape global_count_var =
  ...
  let rec loop shape arr_var len_var =
    ...
    let len_var, bindings =
      ...
      | None ->
        ...
        let binding = binding Alias (Pvalue Pintval) var init in
        var, [binding]
    in
    match shape with
    | Array_of_elements ->
    ...
    | Array_of_arrays shape ->
        ...
        gen_bindings bindings (Lfor {
          ...
          for_to = last_index;
          for_dir = Upto;
          for_body = Llet(Strict, Pvalue (Parrayval kind), sub_arr_var, sub_arr,
          ...
          (gen_binding index_binding
            (Lwhile
               {wh_cond = Lprim(Pintcomp Clt, [Lvar index_var; Lvar len_var],
                ...
                wh_body =
                  Lsequence(
                    Llet(Strict, Pvalue (Parrayval kind), sub_arr_var, sub_arr,
                         Llet(Strict, Pvalue Pintval, sub_arr_len_var, sub_arr_len,
  ...
  | Without_size { body } ->
      let array_var = Ident.create_local "array" in
      Llet(Strict, Pvalue (Parrayval kind), array_var, body,
  ...
  | With_size { body; raise_count } ->
      ...
      let len_var = Ident.create_local "len" in
      Lstaticcatch(body,
          (raise_count, [(array_var, Pvalue (Parrayval kind)); (len_var, Pvalue Pintval)]),
           gen_binding res_binding
             (gen_binding counter_binding
                ((Lsequence
                    (loop shape array_var (Some len_var),
                     res_var)))), Pvalue (Parrayval kind))
      ...
      let shape, body =
        List.fold_left
          (fun (shape, body) block ->
             ...
             let body =
               transl_arr_block ~transl_exp ~loc ~scopes None
                 body Paddrarray (Pvalue (Parrayval Paddrarray)) block
             in
             shape, body)
...
let transl_list_comp type_comp body acc_var mats ~transl_exp ~scopes ~loc =
  ...
  let fn =
    lfunction
      ...
      ~params:[param, pval; acc_var, Pvalue Pgenval]
    ...
    (fun (body, acc_var)  block ->
      ...
      let body, acc_var, materialize =
        ...
        let body = List.fold_right (fun (id, arr) body ->
          Llet(Strict, Pvalue Pgenval, id, arr, body))
          materialize body
        in
        body, acc_var)
    (bdy, acc_var) blocks
  in
  Llet(Alias, Pvalue Pintval, res_var, int 0, (*Empty list.*)
    Lapply{
        ap_loc=loc;
        ap_func=comp_rev ();
        ap_args=[res_list];
