...
let to_bytecode i Typedtree.{structure; coercion; secondary_iface; _} =
  ...
  |> Profile.(record ~accumulate:true generate)
    (fun { Lambda.code = lambda; required_globals; coercion_field } ->
       lambda
       ...
       |> fun bytecode -> bytecode, required_globals, coercion_field
    )

let emit_bytecode i (bytecode, required_globals, coercion_field) =
