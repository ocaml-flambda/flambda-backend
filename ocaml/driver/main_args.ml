...
let mk_inlining_report f =
  "-inlining-report", Arg.Unit f, " Emit `.<round>.inlining.org' file(s) (one \
      per round) showing the inliner's decisions, in a human readable format"
;;

let mk_inlining_report_bin f =
  "-inlining-report-bin", Arg.Unit f, " Emit `.<round>.inlining' file(s) (one per \
      round) showing the inliner's decisions, in a binary format"
;;
