...
val ok : 'a -> ('a, 'e) result @@ portable
(** [ok v] is [Ok v]. *)

val error : 'e -> ('a, 'e) result @@ portable
(** [error e] is [Error e]. *)

val value : ('a, 'e) result -> default:'a -> 'a @@ portable
(** [value r ~default] is [v] if [r] is [Ok v] and [default] otherwise. *)

val get_ok : ('a, 'e) result -> 'a @@ portable
(** [get_ok r] is [v] if [r] is [Ok v] and raise otherwise.

    @raise Invalid_argument if [r] is [Error _]. *)

val get_error : ('a, 'e) result -> 'e @@ portable
(** [get_error r] is [e] if [r] is [Error e] and raise otherwise.

    @raise Invalid_argument if [r] is [Ok _]. *)

val bind : ('a, 'e) result -> ('a -> ('b, 'e) result) -> ('b, 'e) result @@ portable
(** [bind r f] is [f v] if [r] is [Ok v] and [r] if [r] is [Error _]. *)

val join : (('a, 'e) result, 'e) result -> ('a, 'e) result @@ portable
(** [join rr] is [r] if [rr] is [Ok r] and [rr] if [rr] is [Error _]. *)

val map : ('a -> 'b) -> ('a, 'e) result -> ('b, 'e) result @@ portable
(** [map f r] is [Ok (f v)] if [r] is [Ok v] and [r] if [r] is [Error _]. *)

val map_error : ('e -> 'f) -> ('a, 'e) result -> ('a, 'f) result @@ portable
(** [map_error f r] is [Error (f e)] if [r] is [Error e] and [r] if
    [r] is [Ok _]. *)

val fold : ok:('a -> 'c) -> error:('e -> 'c) -> ('a, 'e) result -> 'c @@ portable
(** [fold ~ok ~error r] is [ok v] if [r] is [Ok v] and [error e] if [r]
    is [Error e]. *)

val iter : ('a -> unit) -> ('a, 'e) result -> unit @@ portable
(** [iter f r] is [f v] if [r] is [Ok v] and [()] otherwise. *)

val iter_error : ('e -> unit) -> ('a, 'e) result -> unit @@ portable
(** [iter_error f r] is [f e] if [r] is [Error e] and [()] otherwise. *)

(** {1:preds Predicates and comparisons} *)

val is_ok : ('a, 'e) result -> bool @@ portable
(** [is_ok r] is [true] if and only if [r] is [Ok _]. *)

val is_error : ('a, 'e) result -> bool @@ portable
(** [is_error r] is [true] if and only if [r] is [Error _]. *)

val equal :
  ok:('a -> 'a -> bool) -> error:('e -> 'e -> bool) -> ('a, 'e) result ->
  ('a, 'e) result -> bool @@ portable
...
val to_option : ('a, 'e) result -> 'a option @@ portable
(** [to_option r] is [r] as an option, mapping [Ok v] to [Some v] and
    [Error _] to [None]. *)

val to_list : ('a, 'e) result -> 'a list @@ portable
(** [to_list r] is [[v]] if [r] is [Ok v] and [[]] otherwise. *)

val to_seq : ('a, 'e) result -> 'a Seq.t @@ portable
