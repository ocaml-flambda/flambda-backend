...
val init : int -> f:(int -> char) -> string @@ portable
...
val concat : sep:string -> string list -> string @@ portable
...
val sub : string -> pos:int -> len:int -> string @@ portable
...
val split_on_char : sep:char -> string -> string list @@ portable
...
val map : f:(char -> char) -> string -> string @@ portable
(** [map f s] is the string resulting from applying [f] to all the
    characters of [s] in increasing order.

    @since 4.00 *)

val mapi : f:(int -> char -> char) -> string -> string @@ portable
(** [mapi ~f s] is like {!map} but the index of the character is also
    passed to [f].

    @since 4.02 *)

val fold_left : f:('acc -> char -> 'acc) -> init:'acc -> string -> 'acc @@ portable
(** [fold_left f x s] computes [f (... (f (f x s.[0]) s.[1]) ...) s.[n-1]],
    where [n] is the length of the string [s].
    @since 4.13 *)

val fold_right : f:(char -> 'acc -> 'acc) -> string -> init:'acc -> 'acc @@ portable
(** [fold_right f s x] computes [f s.[0] (f s.[1] ( ... (f s.[n-1] x) ...))],
    where [n] is the length of the string [s].
    @since 4.13 *)

val for_all : f:(char -> bool) -> string -> bool @@ portable
(** [for_all p s] checks if all characters in [s] satisfy the predicate [p].
    @since 4.13 *)

val exists : f:(char -> bool) -> string -> bool @@ portable
