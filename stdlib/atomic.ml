...
module Safe = struct
  external make : ('a : value_or_null)
    . 'a @ portable contended -> 'a t @@ portable = "%makemutable"
  external make_contended : ('a : value_or_null)
    . 'a @ portable contended -> 'a t @@ portable = "caml_atomic_make_contended"
  external get : ('a : value_or_null)
    . 'a t -> 'a @ portable contended @@ portable = "%atomic_load"
  external set : ('a : value_or_null)
    . 'a t -> 'a @ portable contended -> unit @@ portable = "%atomic_set"
  external exchange : ('a : value_or_null)
    . 'a t -> 'a @ portable contended
    -> 'a @ portable contended @@ portable = "%atomic_exchange"
  external compare_and_set : ('a : value_or_null)
    . 'a t -> 'a @ portable contended -> 'a @ portable contended
    -> bool @@ portable = "%atomic_cas"
  external compare_exchange : ('a : value_or_null)
    . 'a t -> 'a @ portable contended -> 'a @ portable contended
    -> 'a @ portable contended @@ portable = "%atomic_compare_exchange"
end
