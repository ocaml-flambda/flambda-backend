  ...
  let rec conv = function
    | hd :: tl ->
        (match hd x with
         ...
         | Some s -> Some s)
    | [] -> None in
  conv (Atomic.get_safe printers).portable
...
let errors = Obj.magic_portable_with [| "";
  (* FILE_NOT_FOUND *)
...
let default_uncaught_exception_handler exn raw_backtrace =
  ...
  if status < 0 then
    prerr_endline (Obj.magic_uncontended_borrow errors).(abs status);
  flush stderr
...
let empty_backtrace : raw_backtrace = Obj.magic_portable_with [| |]

let try_get_raw_backtrace () =
  try
    get_raw_backtrace ()
  with _ (* Out_of_memory? *) ->
    Obj.magic_uncontended_cocontended empty_backtrace
...
let handle_uncaught_exception' exn debugger_in_use =
  try
    ...
    try
      (Atomic.get_safe uncaught_exception_handler).portable exn raw_backtrace
    with exn' ->
...
(* CR tdelvecchio: This seems to be called from signal handlers and finalisers;
