...
external ( |> ) : ('a : value_or_null) ('b : value_or_null) . 'a -> ('a -> 'b) -> 'b @@ portable = "%revapply"
external ( @@ ) : ('a : value_or_null) ('b : value_or_null) . ('a -> 'b) -> 'a -> 'b @@ portable = "%apply"
...
let (at_exit @ nonportable) f = at_exit_safe (magic_portable f)

let do_domain_local_at_exit = ref (fun () -> ())

let (do_at_exit @ nonportable) () =
