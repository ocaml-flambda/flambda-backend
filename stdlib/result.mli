...
val bind : ('a : value_or_null) ('b : value_or_null) ('e : value_or_null). ('a, 'e) result -> ('a -> ('b, 'e) result) -> ('b, 'e) result
(** [bind r f] is [f v] if [r] is [Ok v] and [r] if [r] is [Error _]. *)

val join : ('a : value_or_null) ('e : value_or_null). (('a, 'e) result, 'e) result -> ('a, 'e) result
(** [join rr] is [r] if [rr] is [Ok r] and [rr] if [rr] is [Error _]. *)

val map : ('a : value_or_null) ('b : value_or_null) ('e : value_or_null). ('a -> 'b) -> ('a, 'e) result -> ('b, 'e) result
(** [map f r] is [Ok (f v)] if [r] is [Ok v] and [r] if [r] is [Error _]. *)

val map_error : ('a : value_or_null) ('e : value_or_null) ('f : value_or_null). ('e -> 'f) -> ('a, 'e) result -> ('a, 'f) result
(** [map_error f r] is [Error (f e)] if [r] is [Error e] and [r] if
    [r] is [Ok _]. *)

val fold : ('a : value_or_null) ('c : value_or_null) ('e : value_or_null). ok:('a -> 'c) -> error:('e -> 'c) -> ('a, 'e) result -> 'c
(** [fold ~ok ~error r] is [ok v] if [r] is [Ok v] and [error e] if [r]
    is [Error e]. *)

val iter : ('a : value_or_null) ('e : value_or_null). ('a -> unit) -> ('a, 'e) result -> unit
(** [iter f r] is [f v] if [r] is [Ok v] and [()] otherwise. *)

val iter_error : ('a : value_or_null) ('e : value_or_null). ('e -> unit) -> ('a, 'e) result -> unit
(** [iter_error f r] is [f e] if [r] is [Error e] and [()] otherwise. *)

(** {1:preds Predicates and comparisons} *)

val is_ok : ('a : value_or_null) ('e : value_or_null). ('a, 'e) result -> bool
(** [is_ok r] is [true] if and only if [r] is [Ok _]. *)

val is_error : ('a : value_or_null) ('e : value_or_null). ('a, 'e) result -> bool
...
val to_option : ('a : value_or_null) ('e : value_or_null). ('a, 'e) result -> 'a option
(** [to_option r] is [r] as an option, mapping [Ok v] to [Some v] and
    [Error _] to [None]. *)

val to_list : ('a : value_or_null) ('e : value_or_null). ('a, 'e) result -> 'a list
(** [to_list r] is [[v]] if [r] is [Ok v] and [[]] otherwise. *)

val to_seq : ('a : value_or_null) ('e : value_or_null). ('a, 'e) result -> 'a Seq.t
