...
(** ['k Key.t @ unique] represent the ownership of a capsule ['k]. *)
module Key : sig
  (* CR layouts v5: this should have layout [void], but
     [void] can't be used for function argument and return types yet. *)
  ...
  (** [with_password k f] runs [f], providing it a password for
      ...
      On exception, the key is destroyed, leaking the contents of the capsule.
      Exception [Encapsulated (id, exn)] (see below) is unwrapped and
      reraised as the original [exn]. *)
  ...
  (** [with_password_local k f] runs [f], providing it a password for
      ...
      the local password still provides access to the capsule. *)
  ...
  (* CR dkalinichenko: implement borrowing, [with_password_shared]
     and [access_shared]. *)
...
module Mutex : sig
    ...
    val id : 'k t @ local -> 'k Password.Id.t @@ portable
    (** [id m] identifies the password that [m] is associated with. *)
