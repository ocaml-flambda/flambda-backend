...
let prng_key =
  Domain.DLS.new_key_safe
    (fun (_ : Domain.DLS.Password.t) -> Random.State.make_self_init ())
...
let current_temp_dir_name =
  Domain.DLS.new_key_safe
    ~split_from_parent:(fun (s : string) -> (fun (_ : Domain.DLS.Password.t) -> s))
    (fun (_ : Domain.DLS.Password.t) -> temp_dir_name)

let set_temp_dir_name (s : string) =
  Domain.DLS.with_password (fun pw -> Domain.DLS.set' pw current_temp_dir_name s)
let get_temp_dir_name () =
  Domain.DLS.with_password (fun pw : string -> Domain.DLS.get' pw current_temp_dir_name)
