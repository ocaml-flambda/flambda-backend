...
let random_key =
  Domain.DLS.new_key_safe
    ~split_from_parent:(fun st ->
      ...
      (fun _ -> Obj.magic_uncontended_move st))
    (fun _ -> mk_default ())
...
let full_init seed =
  let seed : int array = Obj.magic_portable_with seed in
  Domain.DLS.with_password (fun pw ->
    let seed = Obj.magic_uncontended_borrow seed in
...
let split () =
  Domain.DLS.with_password (fun pw ->
    State.split (Domain.DLS.get' pw random_key))
  |> Obj.magic_uncontended_move

(* Manipulating the current state. *)

let get_state () =
  Domain.DLS.with_password (fun pw ->
    State.copy (Domain.DLS.get' pw random_key))
  |> Obj.magic_uncontended_move
let set_state s =
  Domain.DLS.with_password (fun pw ->
    State.assign (Domain.DLS.get' pw random_key) (Obj.magic_uncontended_borrow s))
