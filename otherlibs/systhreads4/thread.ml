...
let set_uncaught_exception_handler (fn @ portable) = Atomic.Safe.set uncaught_exception_handler fn
  ...
  thread_new
    (fun () ->
      ...
      | exn ->
        ...
        try
          (Atomic.Safe.get uncaught_exception_handler) exn
        with
        | Exit -> ()
        | exn' ->
          Printf.eprintf
