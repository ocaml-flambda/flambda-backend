...
module Test_error : sig
  (* The payload "assume_zero_alloc" is not allowed on signatures, just like
     "assume". We get a warning, then the check of the implementation fails in
     "opt" mode, because the unsupported payload is removed from the attribute,
     and the signature annotation becomes [@@zero_alloc], and the check of the
     caller passes. *)
