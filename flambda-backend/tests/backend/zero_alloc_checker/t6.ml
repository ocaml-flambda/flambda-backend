...
(* assume still works *)
let[@zero_alloc assume] [@inline never] [@specialise never] fail_loud x =
  [x; x + 1]

let call_loud x = fail_loud (x * 2)

(* assume strict still works *)
let[@inline never] [@specialise never] [@zero_alloc strict assume] fail_loud2 x
    =
  raise (Exn (x, x))
...
let[@inline never]
   [@specialise never]
   [@zero_alloc strict assume]
   [@zero_alloc ignore] fail_loud1 x =
  raise (Exn (x, x))
