...
module A4 = struct
  ...
  let[@zero_alloc] foo _y = (opaque_identity [@zero_alloc assume]) (fun x -> x)

  (* overapply, still succeeds *)
  let[@zero_alloc] bar y = (opaque_identity [@zero_alloc assume]) (fun x -> x) y
...
module A5 = struct
  ...
  let[@zero_alloc] foo a i = (get [@zero_alloc assume]) a (i + 1)
end
