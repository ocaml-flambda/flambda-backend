let[@zero_alloc strict assume] [@inline never] [@specialise never] test x =
  [x; x + 1]

(* The test below to make sure "noalloc" on external is still handled
   correctly. *)
...
let[@zero_alloc strict] test5 x = external_test_noalloc ()
