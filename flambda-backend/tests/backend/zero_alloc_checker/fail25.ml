(* all functions fail the check when -disable-zero-zero-alloc-checker is
   passed. *)
module Sexp = struct
  type t =
    | Atom of string
...
(* pass the check when -disable-precise-zero-alloc-checker is passed, because
   conservative summary of ppx_sexp_message is on used on a path to raise_s. *)
let raise_invalid_index_FAIL ~unknown_index ~name =
  raise_s
    (let ppx_sexp_message () =
       Sexp.List
         [ Sexp.Atom "Unknown";
           Sexp.List [Sexp.Atom "unknown_index"; Sexp.of_int unknown_index];
...
let[@zero_alloc] rec g x =
  if Sys.opaque_identity true then try g x with _ -> () else raise (Failure x)

(* Functions below fail the check when -disable-precise-zero-alloc-checker is
   passed. *)
let[@zero_alloc] rec foo n = bar (n - 1)

and[@zero_alloc] bar n = foo (n - 1)

let[@zero_alloc] rec f1 x = if x > Sys.opaque_identity 10 then x else f2 (x + 1)
