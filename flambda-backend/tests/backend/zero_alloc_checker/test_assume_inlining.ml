...
module A1 = struct
  ...
  let[@inline never] g x n =
    let y = x + n in
    if y > 10 then raise (Exn y) else y

  let[@zero_alloc assume] [@inline never] bar x = try f x with Exn n -> g x n

  let[@zero_alloc] foo x = bar x
end
...
module A2 = struct
  ...
  let[@zero_alloc assume] [@inline always] bar x = try f x with Exn n -> g x n
...
module A3 = struct
  let[@inline never] [@zero_alloc assume strict] f x =
    if x = 0 then raise (Exn x) else x * x
...
module A4 = struct
  let[@inline always] [@zero_alloc assume strict] f x =
    if x = 0 then raise (Exn x) else x * x
