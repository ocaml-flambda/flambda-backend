...
let () =
  ...
  CF32.check_float32s (fun f _ ->
      ...
      if CF32.is_nan f
      then assert (Float.is_nan (F32.to_float f))
      else assert (F32.to_float f = CF32.to_float f));
  for _ = 0 to 100_000 do
    let i =
      if Random.bool ()
      then Random.full_int Int.max_int
      else Int.neg (Random.full_int Int.max_int)
    in
    let f =
      if Random.bool ()
      then Random.float Float.max_float
      else Float.neg (Random.float Float.max_float)
    in
    let i64 =
      if Random.bool ()
...
let () =
  (* Cast & sqrt-with-memory intrinsics (see selection.ml) *)
  CF32.check_float32s (fun f _ ->
