...
let eq l r = if l <> r then Printf.printf "%d <> %d\n" l r

let eqf l r =
  let open Float in
  if is_nan l && is_nan r
  then ()
  else if l <> r
  ...
  let check_float32s f =
    ...
    for _ = 0 to 100_000 do
      ...
      f
        ...
        |> Int32.float_of_bits |> Float32.of_float)
...
let () =
  ...
  let run compare =
    CFloat32.check_float32s (fun f1 f2 ->
        ...
        then (
          ...
          if CFloat32.is_nan f1 && CFloat32.is_nan f2
          then assert (compare f1 f2 = 0);
          if CFloat32.is_nan f1 && not (CFloat32.is_nan f2)
          then (
  ...
  (run [@inlined never]) (fun l r ->
      Poly.compare (Sys.opaque_identity l) (Sys.opaque_identity r))
...
let () =
  ...
  CFloat32.check_float32s (fun f _ ->
      ...
      let bits =
        if f = Float32.of_float 0.0
        then 0l
        else if CFloat32.is_nan f
        then 0x7F800001l
...
let check f32 f64 =
  assert (CFloat32.is_boxed_float32 f32 && f32 = Float32.of_float f64)
